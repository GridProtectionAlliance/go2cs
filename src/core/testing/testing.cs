...
[GoType] partial struct chattyFlag {
    public bool on; // -v is set in some form
    public bool json; // -v=test2json is set, to make output better for test2json
}

[GoRecv] internal static bool IsBoolFlag(this ref chattyFlag ) {
    return true;
}
...
[GoType] partial struct chattyPrinter {
    ...
    public sync_package.Mutex lastNameMu; // guards lastName
    public @string lastName;    // last printed test name in chatty mode
    public bool json;       // -v=json output mode
}
...
internal const nint maxStackLen = 50;
// common holds the elements common between T and B and
// captures common methods such as Errorf.
[GoType] partial struct common {
    public sync_package.RWMutex mu;         // guards this group of fields
    public slice<byte> output;          // Output generated by test or benchmark.
    public io_package.Writer w;            // For flushToParent.
    ...
    public map<uintptr, struct{}> helperPCs; // functions to be skipped when writing file/line info
    public map<@string, struct{}> helperNames; // helperPCs converted to function names
    ...
    public slice<uintptr> cleanupPc;       // The stack trace at the point where Cleanup was called.
    public bool finished;                 // Test function has completed.
    public bool inFuzzFn;                 // Whether the fuzz target, if this is one, is running.
    public ж<chattyPrinter> chatty; // A copy of chattyPrinter, if the chatty flag is set.
    public bool bench;           // Whether the current test is a benchmark.
    public atomic_package.Bool hasSub;    // whether there are sub-benchmarks.
    public atomic_package.Bool cleanupStarted;    // Registered cleanup callbacks have started to execute
    public @string runner;        // Function name of tRunner running the test.
    public bool isParallel;           // Whether the test is parallel.
    public ж<common> parent;
    public nint level;              // Nesting depth of test or benchmark.
    public slice<uintptr> creator;    // If level > 0, the stack trace at the point where the parent called t.Run.
    public @string name;           // Name of test or benchmark.
    public highPrecisionTime start; // Time test or benchmark started
    public time_package.Duration duration;
    public channel<bool> barrier; // To signal parallel subtests they may start. Nil when T.Parallel is not present (B) or not usable (when fuzzing).
    public channel<bool> signal; // To signal a test is done.
    public slice<ж<T>> sub; // Queue of subtests to be run in parallel.
    public atomic_package.Int64 lastRaceErrors; // Max value of race.Errors seen during the test or its subtests.
    public atomic_package.Bool raceErrorLogged;
    public sync_package.Mutex tempDirMu;
    public @string tempDir;
    public error tempDirErr;
    public int32 tempDirSeq;
}
...
[GoRecv] internal static void checkFuzzFn(this ref common c, @string name) {
    if (c.inFuzzFn) {
        panic(fmt.Sprintf("testing: f.%s was called inside the fuzz target, use t.%s instead"u8, name, name));
    }
}

[GoType] partial struct frameSkip_type {
}
    ...
    for (var more = true; more; prevFrame = frame) {
        ...
        if (c.helperNames == default!) {
            ...
            foreach (var (pcΔ1, _) in c.helperPCs) {
                c.helperNames[pcToName(pcΔ1)] = new frameSkip_type();
            }
        }
        {
...
[GoRecv] internal static @string decorate(this ref common c, @string s, nint skip) {
 ...
 @"/\"u8); if (index >= 0) {
    ...
    {
        nint l = len(lines); if (l > 1 && lines[l - 1] == ""u8) {
            lines = lines[..(int)(l - 1)];
        }
    }
...
[GoRecv] internal static void flushToParent(this ref common c, @string testName, @string format, params ꓸꓸꓸany argsʗp) => func((defer, _) => {
    ...
    if (c.chatty != default! && (AreEqual((~p).w, (~c.chatty).w) || (~c.chatty).json)){
...
internal static (nint n, error err) Write(this indenter w, slice<byte> b) {
    ...
    while (len(b) > 0) {
        ...
        var line = b[..(int)(end)];
        if (line[0] == marker) {
            w.c.val.output = append((~w.c).output, marker);
            line = line[1..];
        }
        @string indent = "    "u8;
        w.c.val.output = append((~w.c).output, indent.ꓸꓸꓸ);
        w.c.val.output = append((~w.c).output, line.ꓸꓸꓸ);
        b = b[(int)(end)..];
    }
    return (n, err);
}
...
[GoType] partial struct T {
    ...
    public ж<testContext> context; // For running tests and subtests.
}
...
[GoRecv] internal static void Fail(this ref common c) => func((defer, _) => {
    ...
    if (c.done) {
        panic("Fail in goroutine after "u8 + c.name + " has completed"u8);
    }
    c.failed = true;
});
...
[GoRecv] internal static void logDepth(this ref common c, @string s, nint depth) => func((defer, _) => {
    ...
    if (c.done){
        ...
        for (var parent = c.parent; parent != default!; parent = parent.val.parent) {
            (~parent).mu.Lock();
            var parentʗ1 = parent;
        ...
        panic("Log in goroutine after "u8 + c.name + " has completed: "u8 + s);
    } else {
        if (c.chatty != default!) {
            if (c.bench){
...
[GoType] partial struct Helper_type {
}
...
[GoRecv] internal static void Helper(this ref common c) => func((defer, _) => {
    ...
    }
    {
        var (_, found) = c.helperPCs[pc[0]]; if (!found) {
            c.helperPCs[pc[0]] = new Helper_type();
...
internal static error removeAll(@string path) {
    const time.Duration arbitraryTimeout = /* 2 * time.Second */ 2000000000;
...
internal const panicHandling normalPanic = /* iota */ 0;
internal const panicHandling recoverAndReturnPanic = 1;
...
[GoRecv] internal static void Parallel(this ref T t) {
    if (t.isParallel) {
    ...
    t.parent.val.sub = append((~t.parent).sub, t);
...
// Because Setenv affects the whole process, it cannot be used
// in parallel tests or tests with parallel ancestors.
[GoRecv] internal static void Setenv(this ref T t, @string key, @string value) {
    ...
    for (var c = Ꮡ(t.common); c != default!; c = c.val.parent) {
        if ((~c).isParallel) {
            isParallel = true;
            break;
        }
...
internal static void tRunner(ж<T> Ꮡt, Action<ж<T>> fn) => func((defer, recover) => {
    ...
    defer(() => {
        ...
        if (!finished && err == default!) {
            ...
            for (var p = t.parent; p != default!; p = p.val.parent) {
                (~p).mu.RLock();
        ...
        (any err) => {
            ...
            for (var root = Ꮡ(t.common); (~root).parent != default!; root = root.val.parent) {
                (~root).mu.Lock();
        ...
        t.done = true;
        if (t.parent != default! && !t.hasSub.Load()) {
            t.setRan();
        }
    });
    defer(() => {
...
[GoRecv] internal static bool Run(this ref T t, @string name, Action<ж<T>> f) {
...
// continue walking the stack into the parent test.
    ...
    if (t.chatty != default! && (~t.chatty).json) {
...
[GoRecv] internal static (time.Time deadline, bool ok) Deadline(this ref T t) {
    time.Time deadline = default!;
    bool ok = default!;

    deadline = t.context.val.deadline;
...
// does not match).
    ...
    public channel<bool> startParallel;
    // running is the number of tests currently running in parallel.
// This does not include tests that are waiting for subtests to complete.
    public nint running;
    // numWaiting is the number tests waiting to be run in parallel.
    public nint numWaiting;
...
internal static error WriteProfileTo(this matchStringOnly f, @string , io.Writer , nint ) {
    return errMain;
}
...
internal static void StartTestLog(this matchStringOnly f, io.Writer ) {
}
...
internal static void SetPanicOnExit0(this matchStringOnly f, bool ) {
}

internal static error CoordinateFuzzing(this matchStringOnly f, time.Duration , int64 , time.Duration , int64 , nint , slice<corpusEntry> , slice<reflect.Type> , @string , @string ) {
    return errMain;
}

internal static error RunFuzzWorker(this matchStringOnly f, Func<corpusEntry, error> ) {
    return errMain;
}

internal static (slice<corpusEntry>, error) ReadCorpus(this matchStringOnly f, @string , slice<reflect.Type> ) {
    return (default!, errMain);
}

internal static error CheckCorpus(this matchStringOnly f, slice<any> , slice<reflect.Type> ) {
    return default!;
}
...
[GoType] partial struct M {
    ...
    public slice<InternalFuzzTarget> fuzzTargets;
    public slice<InternalExample> examples;
    public ж<time.Timer> timer;
    public sync_package.Once afterOnce;
    public nint numRun;
...
[GoType] partial interface testDeps {
    ...
    void SetPanicOnExit0(bool );
    error StartCPUProfile(io.Writer );
    void StopCPUProfile();
    void StartTestLog(io.Writer );
    error StopTestLog();
    error WriteProfileTo(@string , io.Writer , nint );
    error CoordinateFuzzing(time.Duration , int64 , time.Duration , int64 , nint , slice<corpusEntry> , slice<reflect.Type> , @string , @string );
    error RunFuzzWorker(Func<corpusEntry, error> );
    (slice<corpusEntry>, error) ReadCorpus(@string , slice<reflect.Type> );
    error CheckCorpus(slice<any> , slice<reflect.Type> );
    void ResetCoverage();
...
[GoRecv] internal static nint /*code*/ Run(this ref M m) => func((defer, _) => {
    nint code = default!;
    ...
    if (shuffle.val != "off"u8) {
        ...
        rng.Shuffle(len(m.benchmarks), (nint i, nint j) => {
            m.benchmarks[i] = m.benchmarks[j];
            m.benchmarks[j] = m.benchmarks[i];
        });
    }
    parseCpuList();
    m.before();
...
[GoRecv] internal static time.Time startAlarm(this ref M m) {
    ...
    m.timer = time.AfterFunc(timeout.val, () => {
        ...
        panic(fmt.Sprintf("test timed out after %v%s"u8, timeout.val, extra));
    });
    return deadline;
}
