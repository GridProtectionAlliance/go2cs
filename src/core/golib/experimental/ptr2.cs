...
public class ж<T> : IPointer<T>, IEquatable<ж<T>>
{
    ...
    private readonly nuint? m_pointerRef;
    private T m_val;
    ...
    // Create a new pointer from an existing pointer reference.
    internal unsafe ж(T* value)
    {
        m_pointerRef = (nuint)value;
        m_val = default!;
    }
    ...
    public unsafe ref T val
    {
        get
        {
            ...
            if (m_pointerRef is not null)
                return ref *(T*)m_pointerRef;

            throw new InvalidOperationException("Cannot get reference to value, source is not a valid struct field, array or slice reference, or pointer reference.");
        }
    }
    ...
    public static unsafe implicit operator ж<T>(T* value)
    {
