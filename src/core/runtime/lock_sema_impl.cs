...
internal static void lock2(ж<mutex> Ꮡl) {
    ...
    if (Interlocked.CompareExchange(ref l.key, locked, 0) == 0) {
        return;
    }

    SpinWait spinner = default;

    while (Volatile.Read(ref l.key) != 0 || Interlocked.CompareExchange(ref l.key, locked, 0) != 0) {
        spinner.SpinOnce();
    }
}
...
internal static void unlock2(ж<mutex> Ꮡl) {
    ...
    Interlocked.Exchange(ref l.key, (uintptr)0);
}
...
internal static void notewakeup(ж<note> Ꮡn) {
    ...
    uintptr v = Interlocked.Exchange(ref n.key, locked);
...
internal static void notesleep(ж<note> Ꮡn) {
    ...
    while (ᐧ) {
        uintptr v = Volatile.Read(ref n.key);
