...
// is implicitly dereferenced to a value local (`ref var buf = ref Ꮡbuf.val`), so `buf` already
// denotes the array; the range conversion must not add a second `.val` (`buf.val` on the value
...
func swap(buf *[4]uint64) {
	for i, x := range buf { // pointer param -> deref'd value; no extra `.val`
		buf[i] = x>>32 | x<<32
	}
}
