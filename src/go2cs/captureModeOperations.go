...
// (interned per method across files). Generic receivers are excluded — the captured field
// is a static ThreadLocal that cannot hold the receiver's type parameter.
...
func scanFileForCaptureModeMethods(file *ast.File, info *types.Info) {
	ast.Inspect(file, func(node ast.Node) bool {
		...
		// Only non-generic named receivers (see packageCaptureModeMethods).
		if named, ok := pointer.Elem().(*types.Named); !ok || named.TypeParams().Len() > 0 {
			return true
		}

		if bodyTakesReceiverFieldAddress(funcDecl.Body, recvName) {
			packageCaptureModeMethods[funcObj] = true
		}
...
func (v *Visitor) isCaptureModeMethod(selectorExpr *ast.SelectorExpr) bool {
	...
	return funcObj != nil && packageCaptureModeMethods[funcObj]
}
