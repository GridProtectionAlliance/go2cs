		...
		for i := range params.Len() {
			...
			if needsInterfaceCast, isEmpty := isInterface(paramType); needsInterfaceCast && !v.instantiatedParamIsPointer(callExpr, paramType, i) && !signatureErasedParamPointerOk(funcSignature, paramType) {
				...
				if !isEmpty {
					...
					// trailing argument. Since the empty interface leaves `interfaceTypes` unset, the
					// argument takes the identical convExpr path a `*T`-parameter argument does.
					lastArg := i
					...
					for j := i; j <= lastArg && j < len(callExpr.Args); j++ {
