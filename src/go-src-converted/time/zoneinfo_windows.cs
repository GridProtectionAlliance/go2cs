...
// package time -- go2cs converted at 2022 March 06 22:30:21 UTC
// import "time" ==> using time = go.time_package
// Original source: C:\Program Files\Go\src\time\zoneinfo_windows.go
...
private static (bool, error) matchZoneKey(registry.Key zones, @string kname, @string stdname, @string dstname) => func((defer, _, _) => {
    ...
    if (err != null) { // Fallback to Std and Dlt
...
private static (@string, error) toEnglishName(@string stdname, @string dstname) => func((defer, _, _) => {
    @string _p0 = default;
    error _p0 = default!;
...
private static long pseudoUnix(nint year, ptr<syscall.Systemtime> _addr_d) {
    ...
    nint day = 1;
    var t = Date(year, Month(d.Month), day, int(d.Hour), int(d.Minute), int(d.Second), 0, UTC);
...
private static void initLocalFromTZI(ptr<syscall.Timezoneinformation> _addr_i) {
    ...
    if (d0.Month > d1.Month) {
        (d0, d1) = (d1, d0);        (i0, i1) = (i1, i0);
    }
    l.tx = make_slice<zoneTrans>(400);
