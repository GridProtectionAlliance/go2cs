...
private static void addValueFuncs(map<@string, reflect.Value> @out, FuncMap @in) => func((_, panic, _) => {
...
private static bool goodName(@string name) {
    ...
    foreach (var (i, r) in name) {
        ...
        if (r == '_')         else if (i == 0 && !unicode.IsLetter(r)) 
            return false;
...
private static (reflect.Value, bool) findFunction(@string name, ptr<Template> _addr_tmpl) => func((defer, _, _) => {
...
private static (nint, error) indexArg(reflect.Value index, nint cap) {
    nint _p0 = default;
    error _p0 = default!;
    ...
    if (index.Kind() == reflect.Int || index.Kind() == reflect.Int8 || index.Kind() == reflect.Int16 || index.Kind() == reflect.Int32 || index.Kind() == reflect.Int64) 
        ...
        return (0, error.As(fmt.Errorf("cannot index slice/array with nil"))!);
    else 
        return (0, error.As(fmt.Errorf("cannot index slice/array with type %s", index.Type()))!);
...
private static (reflect.Value, error) index(reflect.Value item, params reflect.Value[] indexes) => func((_, panic, _) => {
    reflect.Value _p0 = default;
    error _p0 = default!;
...
private static (reflect.Value, error) slice(reflect.Value item, params reflect.Value[] indexes) {
    ...
    array<nint> idx = new array<nint>(new nint[] { 0, item.Len() });
    ...
    if (len(indexes) < 3) {
        return (item.Slice(idx[0], idx[1]), error.As(null!)!);
    }
    if (idx[1] > idx[2]) {
...
private static (nint, error) length(reflect.Value item) {
    nint _p0 = default;
    error _p0 = default!;

    var (item, isNil) = indirect(item);
    if (isNil) {
        return (0, error.As(fmt.Errorf("len of nil pointer"))!);
    }
...
private static (reflect.Value, error) safeCall(reflect.Value fun, slice<reflect.Value> args) => func((defer, _, _) => {
...
private partial struct kind { // : nint
}
...
private static (bool, error) lt(reflect.Value arg1, reflect.Value arg2) => func((_, panic, _) => {
    bool _p0 = default;
    error _p0 = default!;
