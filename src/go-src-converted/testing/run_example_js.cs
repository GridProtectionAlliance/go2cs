...
// package testing -- go2cs converted at 2022 March 06 23:19:17 UTC
// import "testing" ==> using testing = go.testing_package
// Original source: C:\Program Files\Go\src\testing\run_example_js.go
...
private static bool runExample(InternalExample eg) => func((defer, _, _) => {
    ...
    defer(() => {
        ...
        var (_, seekErr) = f.Seek(0, os.SEEK_SET);
...
private static ptr<os.File> createTempFile(@string exampleName) {
    for (nint i = 0; >>MARKER:FOREXPRESSION_LEVEL_1<<; i++) {
        ...
        var (f, err) = os.OpenFile(name, os.O_RDWR | os.O_CREATE | os.O_EXCL, 0600);
        if (err != null) {
            if (os.IsExist(err)) {
                continue;
            }
