...
private static ptr<nint> defaultMaxCountflag.Int("quickchecks", 100, "The default number of iterations for each check");

// A Generator can generate random values of its own type.
public partial interface Generator {
    reflect.Value Generate(ptr<rand.Rand> rand, nint size);
}
...
// values.
private static readonly nint complexSize = 50;
...
private static (reflect.Value, bool) sizedValue(reflect.Type t, ptr<rand.Rand> _addr_rand, nint size) {
    reflect.Value value = default;
    bool ok = default;
    ref rand.Rand rand = ref _addr_rand.val;
...
public partial struct Config {
    public nint MaxCount; // MaxCountScale is a non-negative scale factor applied to the
// default maximum.
...
// Config.
private static nint getMaxCount(this ptr<Config> _addr_c) {
