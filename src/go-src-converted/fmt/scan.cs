...
// for more information.
    (nint, error) SkipSpace(); // Token skips space in the input if skipSpace is true, then returns the
...
// as input, or when the calling Scan method returns.
    (nint, error) Token(bool skipSpace, Func<int, bool> f); // Width returns the value of the width option and whether it has been set.
// The unit is Unicode code points.
    (nint, error) Width(); // Because ReadRune is implemented by the interface, Read should never be
// called by the scanning routines and a valid implementation of
// ScanState may choose always to return an error from Read.
    (nint, error) Read(slice<byte> buf);
}
...
public static (nint, error) Scan(params object[] a) {
    nint n = default;
    error err = default!;
    a = a.Clone();
...
public static (nint, error) Scanln(params object[] a) {
    nint n = default;
    error err = default!;
    a = a.Clone();
...
public static (nint, error) Scanf(@string format, params object[] a) {
    nint n = default;
    error err = default!;
    a = a.Clone();
...
private static (nint, error) Read(this ptr<stringReader> _addr_r, slice<byte> b) {
    nint n = default;
    error err = default!;
...
public static (nint, error) Sscan(@string str, params object[] a) {
    nint n = default;
    error err = default!;
    a = a.Clone();
...
// after the final item there must be a newline or EOF.
public static (nint, error) Sscanln(@string str, params object[] a) {
    nint n = default;
    error err = default!;
    a = a.Clone();
...
// Newlines in the input must match newlines in the format.
public static (nint, error) Sscanf(@string str, @string format, params object[] a) {
    nint n = default;
    error err = default!;
    a = a.Clone();
...
// returns the number of items successfully scanned. If that is less
// than the number of arguments, err will report why.
public static (nint, error) Fscan(io.Reader r, params object[] a) {
    nint n = default;
    error err = default!;
    a = a.Clone();
...
// after the final item there must be a newline or EOF.
public static (nint, error) Fscanln(io.Reader r, params object[] a) {
    nint n = default;
    error err = default!;
    a = a.Clone();
...
// returns the number of items successfully parsed.
// Newlines in the input must match newlines in the format.
public static (nint, error) Fscanf(io.Reader r, @string format, params object[] a) {
    nint n = default;
    error err = default!;
    a = a.Clone();
...
private partial struct ss {
    ...
    public nint count; // runes consumed so far.
...
private partial struct ssave {
    ...
    public nint argLimit; // max value of ss.count for this arg; argLimit <= limit
    public nint limit; // max value of ss.count.
    public nint maxWid; // width of this arg.
}

// The Read method is only in ScanState so that ScanState
// satisfies io.Reader. It will never be called when used as
// intended, so there is no need to make it actually work.
private static (nint, error) Read(this ptr<ss> _addr_s, slice<byte> buf) {
    nint n = default;
    error err = default!;
    ref ss s = ref _addr_s.val;

    return (0, error.As(errors.New("ScanState's Read should not be called. Use ReadRune"))!);
}

private static (int, nint, error) ReadRune(this ptr<ss> _addr_s) {
    int r = default;
    nint size = default;
    error err = default!;
    ref ss s = ref _addr_s.val;
...
private static void error(this ptr<ss> _addr_s, error err) => func((_, panic, _) => {
    ref ss s = ref _addr_s.val;

    panic(new scanError(err));
});

private static void errorString(this ptr<ss> _addr_s, @string err) => func((_, panic, _) => {
    ref ss s = ref _addr_s.val;
...
private partial struct readRune {
    ...
    public nint pending; // number of bytes in pendBuf; only >0 for bad UTF-8
...
private static (byte, error) readByte(this ptr<readRune> _addr_r) {
    ...
    if (r.pending > 0) {
        b = r.pendBuf[0];
        copy(r.pendBuf[(int)0..], r.pendBuf[(int)1..]);
        r.pending--;
        return ;
    }
    var (n, err) = io.ReadFull(r.reader, r.pendBuf[..(int)1]);
    if (n != 1) {
...
// io.Reader inside r.
private static (int, nint, error) ReadRune(this ptr<readRune> _addr_r) {
    int rr = default;
    nint size = default;
    error err = default!;
    ref readRune r = ref _addr_r.val;
...
private static nint indexRune(@string s, int r) {
...
private static long scanRune(this ptr<ss> _addr_s, nint bitSize) {
    ref ss s = ref _addr_s.val;
...
private static (nint, @string, bool) scanBasePrefix(this ptr<ss> _addr_s) {
    nint @base = default;
    @string digits = default;
...
private static long scanInt(this ptr<ss> _addr_s, int verb, nint bitSize) {
    ref ss s = ref _addr_s.val;
...
private static ulong scanUint(this ptr<ss> _addr_s, int verb, nint bitSize) {
    ref ss s = ref _addr_s.val;
...
private static double convertFloat(this ptr<ss> _addr_s, @string str, nint n) {
    ref ss s = ref _addr_s.val;
    ...
    // decimal+binary exponent mix (1.2p4) ourselves.
    {
        ...
        if (p >= 0 && !hasX(str)) { 
            ...
            var (f, err) = strconv.ParseFloat(str[..(int)p], n);
            if (err != null) { 
...
private static System.Numerics.Complex128 scanComplex(this ptr<ss> _addr_s, int verb, nint n) {
    ref ss s = ref _addr_s.val;
...
private static (nint, bool) hexDigit(int d) {
    nint _p0 = default;
    bool _p0 = default;
...
private static readonly nint hugeWid = 1 << 30;
...
private static void scanOne(this ptr<ss> _addr_s, int verb, object arg) {
    ...
    switch (arg.type()) {
        ...
        case ptr<complex64> v:
            v.val = complex64(s.scanComplex(verb, 64));
            break;
        case ptr<System.Numerics.Complex128> v:
            v.val = s.scanComplex(verb, 128);
            break;
        case ptr<nint> v:
            v.val = int(s.scanInt(verb, intBits));
            break;
        case ptr<sbyte> v:
            v.val = int8(s.scanInt(verb, 8));
            break;
        case ptr<short> v:
            v.val = int16(s.scanInt(verb, 16));
            break;
        case ptr<int> v:
            v.val = int32(s.scanInt(verb, 32));
            break;
        case ptr<long> v:
            v.val = s.scanInt(verb, 64);
            break;
        case ptr<nuint> v:
            v.val = uint(s.scanUint(verb, intBits));
            break;
        case ptr<byte> v:
            v.val = uint8(s.scanUint(verb, 8));
            break;
        case ptr<ushort> v:
            v.val = uint16(s.scanUint(verb, 16));
            break;
        case ptr<uint> v:
            v.val = uint32(s.scanUint(verb, 32));
            break;
        case ptr<ulong> v:
            v.val = s.scanUint(verb, 64);
            break;
        ...
        case ptr<float> v:
            if (s.okVerb(verb, floatVerbs, "float32")) {
                s.SkipSpace();
                s.notEOF();
                v.val = float32(s.convertFloat(s.floatToken(), 32));
            }
            break;
        case ptr<double> v:
            if (s.okVerb(verb, floatVerbs, "float64")) {
                s.SkipSpace();
                s.notEOF();
                v.val = s.convertFloat(s.floatToken(), 64);
            }
            break;
        case ptr<@string> v:
...
private static void errorHandler(ptr<error> _addr_errp) => func((_, panic, _) => {
...
private static (nint, error) doScan(this ptr<ss> _addr_s, slice<object> a) => func((defer, _, _) => {
...
private static nint advance(this ptr<ss> _addr_s, @string format) {
    nint i = default;
    ref ss s = ref _addr_s.val;
...
private static (nint, error) doScanf(this ptr<ss> _addr_s, @string format, slice<object> a) => func((defer, _, _) => {
    nint numProcessed = default;
    error err = default!;
    ref ss s = ref _addr_s.val;
