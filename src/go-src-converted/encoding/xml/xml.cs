            ...
            switch (b)
            {
                ...
                case '!': 
                    ...
                    HandleB:
                                        ...
                                        if (b == inquote) 
                                            inquote = 0L;
                                        else if (inquote != 0L)                     else if (b == '\'' || b == '"') 
                                            inquote = b;
                                        else if (b == '>' && inquote == 0L) 
                                            depth--;
                                        else if (b == '<' && inquote == 0L) 
...
Input:
            while (true)
            {
                ...
                { 
                    ...
                    if (b == '#')
                    {
                        ...
                        {
                            var s = string(d.buf.Bytes()[start..]);
                            d.buf.WriteByte(';');
                        ...
                        {
                            var name = d.buf.Bytes()[before + 1L..];
                            d.buf.WriteByte(';');
                            if (isName(name))
                            {
                                s = string(name);
                ...
                while (i < len(s))
                {
                    ...
                    switch (r)
                    {
                        ...
                        case '\r': 
                            ...
                            break;
                        default: 
                            if (!isInCharacterRange(r) || (r == 0xFFFDUL && width == 1L))
                            {
                                esc = escFFFD;
                                break;
                            }
                    ...
                    {
                        var (_, err) = w.Write(s[last..i - width]);
        ...
        private static void EscapeString(this ptr<printer> _addr_p, @string s)
        {
            ...
            long last = 0L;
            {
                ...
                while (i < len(s))
                {
                    var (r, width) = utf8.DecodeRuneInString(s[i..]);
                    i += width;
                    switch (r)
                    {
                        case '"': 
