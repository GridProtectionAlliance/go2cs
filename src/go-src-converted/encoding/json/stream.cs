...
namespace encoding
{
    public static partial class json_package
    {
        ...
        public partial struct Decoder
        {
            ...
            public long scanp; // start of unread data in buf
            public long scanned; // amount of data already scanned
            public scanner scan;
            public error err;
            public long tokenState;
            public slice<long> tokenStack;
        }
...
Input:
        ...
        private static error refill(this ptr<Decoder> _addr_dec)
        {
            ...
            if (dec.scanp > 0L)
            {
                ...
                var n = copy(dec.buf, dec.buf[dec.scanp..]);
                dec.buf = dec.buf[..n];
                dec.scanp = 0L;
            } 
            ...
            {
                var newBuf = make_slice<byte>(len(dec.buf), 2L * cap(dec.buf) + minRead);
                copy(newBuf, dec.buf);
                dec.buf = newBuf;
            } 
