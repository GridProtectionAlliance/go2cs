...
// package ssa -- go2cs converted at 2022 March 06 23:33:18 UTC
...
// there is none.
private static nint predIndex(this ptr<BasicBlock> _addr_b, ptr<BasicBlock> _addr_c) => func((_, panic, _) => {
    ref BasicBlock b = ref _addr_b.val;
...
private static bool hasPhi(this ptr<BasicBlock> _addr_b) {
    ...
    ptr<Phi> (_, ok) = b.Instrs[0]._<ptr<Phi>>();
    return ok;
}
...
//
private static Value lookup(this ptr<Function> _addr_f, types.Object obj, bool escaping) => func((_, panic, _) => {
    ref Function f = ref _addr_f.val;
...
public static void WriteFunction(ptr<bytes.Buffer> _addr_buf, ptr<Function> _addr_f) {
    ...
    const nint punchcard = 80; // for old time's sake.
 // for old time's sake.
    const nint tabwidth = 8;
