...
// package cgo -- go2cs converted at 2022 March 06 23:33:50 UTC
...
public static partial class cgo_package {
    ...
    //
public static (slice<ptr<ast.File>>, error) ProcessFiles(ptr<build.Package> _addr_bp, ptr<token.FileSet> _addr_fset, Func<@string, @string> DisplayPath, parser.Mode mode) => func((defer, _, _) => {
    slice<ptr<ast.File>> _p0 = default;
    error _p0 = default!;
    ref build.Package bp = ref _addr_bp.val;
    ref token.FileSet fset = ref _addr_fset.val;

    var (tmpdir, err) = ioutil.TempDir("", strings.Replace(bp.ImportPath, "/", "_", -1) + "_C");
    if (err != null) {
...
public static (slice<@string>, slice<@string>, error) Run(ptr<build.Package> _addr_bp, @string pkgdir, @string tmpdir, bool useabs) {
    ...
    foreach (var (_, fn) in bp.CgoFiles) { 
        ...
        var f = cgoRe.ReplaceAllString(fn[..(int)len(fn) - len("go")], "_");
...
private static slice<@string> stringList(params object[] args) => func((_, panic, _) => {
    args = args.Clone();
