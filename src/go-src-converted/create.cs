...
namespace go
{
    public static partial class ssa_package
    {
        ...
        // phase.
        //
        private static void memberFromObject(ptr<Package> _addr_pkg, types.Object obj, ast.Node syntax) => func((_, panic, __) =>
        {
        ...
        private static ptr<Package> CreatePackage(this ptr<Program> _addr_prog, ptr<types.Package> _addr_pkg, slice<ptr<ast.File>> files, ptr<types.Info> _addr_info, bool importable)
        {
            ...
            if (prog.mode & GlobalDebug != 0L)
            {
                p.SetDebugMode(true);
            }

            if (prog.mode & PrintPackages != 0L)
            {
                printMu.Lock();
                p.WriteTo(os.Stdout);
                printMu.Unlock();
        ...
        private static slice<ptr<Package>> AllPackages(this ptr<Program> _addr_prog)
        {
            ...
            var pkgs = make_slice<ptr<Package>>(0L, len(prog.packages));
