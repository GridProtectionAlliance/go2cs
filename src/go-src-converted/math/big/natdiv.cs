...
private static (nat, Word) divW(this nat z, nat x, Word y) => func((_, panic, _) => {
    nat q = default;
    Word r = default;
...
private static Word divWVW(slice<Word> z, Word xn, slice<Word> x, Word y) {
    ...
    r = xn;
    if (len(x) == 1) {
        var (qq, rr) = bits.Div(uint(r), uint(x[0]), uint(y));
        z[0] = Word(qq);
        return Word(rr);
    }
    var rec = reciprocalWord(y);
...
private static (nat, nat) divLarge(this nat z, nat u, nat uIn, nat vIn) {
    ...
    // uIn is copied to u.
    var shift = nlz(vIn[n - 1]);
    var vp = getNat(n);
    var v = vp.val;
    shlVU(v, vIn, shift);
    u = u.make(len(uIn) + 1);
    u[len(uIn)] = shlVU(u[(int)0..(int)len(uIn)], uIn, shift); 
...
private static void divBasic(this nat q, nat u, nat v) {
    ...
    for (var j = m; j >= 0; j--) { 
        ...
        if (ujn != vn1) {
            ...
            var ujn2 = u[j + n - 2];
            while (greaterThan(x1, x2, rhat, ujn2)) { // x1x2 > r̂ u[j+n-2]
                qhat--;
                var prevRhat = rhat;
                rhat += vn1; 
        ...
        var c = subVV(u[(int)j..(int)j + qhl], u[(int)j..], qhatv);
        if (c != 0) {
            c = addVV(u[(int)j..(int)j + n], u[(int)j..], v); 
...
private static readonly nint divRecursiveThreshold = 100;
...
private static void divRecursive(this nat z, nat u, nat v) { 
    ...
    nint recDepth = 2 * bits.Len(uint(len(v)));
...
private static void divRecursiveStep(this nat z, nat u, nat v, nint depth, ptr<nat> _addr_tmp, slice<ptr<nat>> temps) => func((_, panic, _) => {
 ...
 {
        temps[depth].val = temps[depth].make(B + 1);
    }
    var j = m;
    while (j > B) { 
        ...
        qhat.clear();
        qhat.divRecursiveStep(uu[(int)s..(int)B + n], v[(int)s..], depth + 1, tmp, temps);
        qhat = qhat.norm(); 
        ...
        qhatv = qhatv.mul(qhat, v[..(int)s]);
        {
            ...
            for (nint i = 0; i < 2; i++) {
                ...
                subVW(qhat, qhat, 1);
                var c = subVV(qhatv[..(int)s], qhatv[..(int)s], v[..(int)s]);
                if (len(qhatv) > s) {
                    subVW(qhatv[(int)s..], qhatv[(int)s..], c);
                }
                addAt(uu[(int)s..], v[(int)s..], 0);
            }
        ...
        c = subVV(uu[..(int)len(qhatv)], uu[..(int)len(qhatv)], qhatv);
        if (c > 0) {
            subVW(uu[(int)len(qhatv)..], uu[(int)len(qhatv)..], c);
        }
        addAt(z, qhat, j - B);
        j -= B;
    ...
    qhat.clear();
    qhat.divRecursiveStep(u[(int)s..].norm(), v[(int)s..], depth + 1, tmp, temps);
    qhat = qhat.norm();
    qhatv = tmp.make(3 * n);
    qhatv.clear();
    qhatv = qhatv.mul(qhat, v[..(int)s]); 
        ...
        for (i = 0; i < 2; i++) {
            {
                ...
                if (e > 0) {
                    subVW(qhat, qhat, 1);
                    c = subVV(qhatv[..(int)s], qhatv[..(int)s], v[..(int)s]);
                    if (len(qhatv) > s) {
                        subVW(qhatv[(int)s..], qhatv[(int)s..], c);
                    }
    ...
    c = subVV(u[(int)0..(int)len(qhatv)], u[(int)0..(int)len(qhatv)], qhatv);
    if (c > 0) {
