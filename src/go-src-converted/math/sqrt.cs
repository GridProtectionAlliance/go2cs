...
// Original source: C:\Program Files\Go\src\math\sqrt.go
namespace go;
...
private static double sqrt(double x) { 
 ...
 // q = sqrt(x)
    var r = uint64(1 << (int)((shift + 1))); // r = moving bit from MSB to LSB
    while (r != 0) {
        var t = s + r;
        if (t <= ix) {
            s = t + r;
    ...
    ix = q >> 1 + uint64(exp - 1 + bias) << (int)(shift); // significand + biased exponent
