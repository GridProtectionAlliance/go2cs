...
// package strconv -- go2cs converted at 2020 August 29 08:42:53 UTC
...
namespace go
{
    public static partial class strconv_package
    {
        ...
        private static readonly long firstPowerOfTen = -348L;
        private static readonly long stepPowerOfTen = 8L;
        ...
        private static (ulong, bool) floatBits(this ref extFloat f, ref floatInfo flt)
        {
            f.Normalize();
        ...
        private static (extFloat, extFloat) AssignComputeBounds(this ref extFloat f, ulong mant, long exp, bool neg, ref floatInfo flt)
        {
            f.mant = mant;
        ...
        private static ulong Normalize(this ref extFloat f)
        {
            var mant = f.mant;
            var exp = f.exp;
            if (mant == 0L)
            ...
            if (mant >> (int)((64L - 32L)) == 0L)
            {
                mant <<= 32L;
                exp -= 32L;
            }
            if (mant >> (int)((64L - 16L)) == 0L)
            {
                mant <<= 16L;
                exp -= 16L;
            }
            if (mant >> (int)((64L - 8L)) == 0L)
            {
                mant <<= 8L;
                exp -= 8L;
            }
            if (mant >> (int)((64L - 4L)) == 0L)
            {
                mant <<= 4L;
                exp -= 4L;
            }
            if (mant >> (int)((64L - 2L)) == 0L)
            {
                mant <<= 2L;
                exp -= 2L;
            }
            if (mant >> (int)((64L - 1L)) == 0L)
            {
                mant <<= 1L;
                exp -= 1L;
            }
        ...
        // but not normalized.
        private static void Multiply(this ref extFloat f, extFloat g)
        {
            var fhi = f.mant >> (int)(32L);
            var flo = uint64(uint32(f.mant));
            var ghi = g.mant >> (int)(32L);
            var glo = uint64(uint32(g.mant)); 
            ...
            f.mant = fhi * ghi + (cross1 >> (int)(32L)) + (cross2 >> (int)(32L));
            var rem = uint64(uint32(cross1)) + uint64(uint32(cross2)) + ((flo * glo) >> (int)(32L)); 
            // Round up.
            rem += (1L << (int)(31L));
        ...
        private static bool AssignDecimal(this ref extFloat f, ulong mantissa, long exp10, bool neg, bool trunc, ref floatInfo flt)
        {
            const long uint64digits = 19L;

            const long errorscale = 8L;

            long errors = 0L; // An upper bound for error, computed in errorscale*ulp.
            if (trunc)
            { 
        ...
        private static (long, long) frexp10(this ref extFloat f)
        { 
            ...
            // by 10.
            const long expMin = -60L;

            const long expMax = -32L; 
...
Loop: 
        ...
        private static long frexp10Many(ref extFloat a, ref extFloat b, ref extFloat c)
        {
        ...
        private static bool FixedDecimal(this ref extFloat _f, ref decimalSlice _d, long n) => func(_f, _d, (ref extFloat f, ref decimalSlice d, Defer _, Panic panic, Recover __) =>
        {
            if (f.mant == 0L)
            {
                d.nd = 0L;
            ...
            var ok = adjustLastDigitFixed(d, uint64(rest) << (int)(shift) | fraction, pow10, shift, ε);
            if (!ok)
            {
                return false;
            } 
        ...
        private static bool adjustLastDigitFixed(ref decimalSlice _d, ulong num, ulong den, ulong shift, ulong ε) => func(_d, (ref decimalSlice d, Defer _, Panic panic, Recover __) =>
        {
        ...
        private static bool ShortestDecimal(this ref extFloat f, ref decimalSlice d, ref extFloat lower, ref extFloat upper)
        {
            if (f.mant == 0L)
            {
                d.nd = 0L;
            ...
            if (f.exp == 0L && lower == f && lower == upper.Value.Value.Value.Value)
            { 
                // an exact integer.
                ...
                for (i = 0L; i < integerDigits; i++)
                {
                    ...
                    // evaluate whether we should stop.
                    {
                        ...
                        if (currentDiff < allowance)
                        {
                            ...
                            return adjustLastDigit(d, currentDiff, targetDiff, allowance, pow << (int)(shift), 2L);
                        }
            ...
            while (true)
            {
                ...
                { 
                    ...
                    return adjustLastDigit(d, fraction, targetDiff * multiplier, allowance * multiplier, 1L << (int)(shift), multiplier * 2L);
                }
            }
        ...
        private static bool adjustLastDigit(ref decimalSlice d, ulong currentDiff, ulong targetDiff, ulong maxDiff, ulong ulpDecimal, ulong ulpBinary)
        {
