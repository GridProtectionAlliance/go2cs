...
// package ssa -- go2cs converted at 2022 March 06 23:33:20 UTC
...
private static void lift(ptr<Function> _addr_fn) {
    ...
    ref nint fresh = ref heap(1000, out ptr<nint> _addr_fresh); 
    ...
    // Renaming.
    rename(_addr_fn.Blocks[0], renaming, newPhis); 
...
private static nint take(this ptr<blockSet> _addr_s) {
    ref blockSet s = ref _addr_s.val;

    var l = s.BitLen();
    for (nint i = 0; i < l; i++) {
        if (s.Bit(i) == 1) {
            s.SetBit(_addr_s.Int, i, 0);
            return i;
        }
    }
    return -1;
...
//
private static bool liftAlloc(domFrontier df, ptr<Alloc> _addr_alloc, newPhiMap newPhis, ptr<nint> _addr_fresh) => func((_, panic, _) => {
    ref Alloc alloc = ref _addr_alloc.val;
    ref nint fresh = ref _addr_fresh.val;
...
private static void replaceAll(Value x, Value y) {
    ...
    foreach (var (_, instr) in pxrefs.val) {
        rands = instr.Operands(rands[..(int)0]); // recycle storage
                ...
                case ptr<Store> instr:
                    {
                        ...
                        if (ok && alloc.index >= 0) { // store to Alloc cell
                    ...
                    if (instr.Op == token.MUL) {
                        {
                            ...
                            if (ok && alloc.index >= 0) { // load of Alloc cell
                ...
                case ptr<DebugRef> instr:
                    {
                        ...
                        if (ok && alloc.index >= 0) { // ref of Alloc cell
                            if (instr.IsAddr) {
