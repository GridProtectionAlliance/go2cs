...
// package atomicalign -- go2cs converted at 2022 March 06 23:32:56 UTC
...
public static readonly @string Doc = "check for non-64-bits-aligned arguments to sync/atomic functions";
    ...
    inspect.Preorder(nodeFilter, node => {
        ...
        switch (sel.Sel.Name) {
            ...
            case "CompareAndSwapUint64": 
                ...
                // For all the listed functions, the expression to check is always the first function argument.
                check64BitAlignment(_addr_pass, sel.Sel.Name, call.Args[0]);
                break;
        }
...
private static void check64BitAlignment(ptr<analysis.Pass> _addr_pass, @string funcName, ast.Expr arg) {
    ...
    for (nint i = 0; i < stype.NumFields(); i++) {
        var f = stype.Field(i);
