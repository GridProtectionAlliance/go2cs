...
// package diff -- go2cs converted at 2022 March 06 23:31:17 UTC
...
public static @string ApplyEdits(@string before, slice<TextEdit> edits) { 
    ...
    foreach (var (_, edit) in edits) {
        ...
        if (start > last) {
            after.WriteString(before[(int)last..(int)start]);
            last = start;
        }
        ...
        after.WriteString(before[(int)last..]);
    }
    return after.String();
...
private static (ptr<span.TokenConverter>, slice<TextEdit>, bool) prepareEdits(@string before, slice<TextEdit> edits) {
    ...
    foreach (var (i, edit) in edits) {
        ...
        copied[i] = edit;
        partial = partial || edit.Span.Start().Offset() >= len(before) || edit.Span.Start().Column() > 1 || edit.Span.End().Column() > 1;
...
private static slice<TextEdit> lineEdits(@string before, ptr<span.TokenConverter> _addr_c, slice<TextEdit> edits) {
    ...
    var adjusted = make_slice<TextEdit>(0, len(edits));
    ...
    foreach (var (_, edit) in edits) {
        if (current.Span.IsValid() && edit.Span.Start().Line() <= current.Span.End().Line()) { 
            ...
            var gap = before[(int)current.Span.End().Offset()..(int)edit.Span.Start().Offset()]; 
...
private static slice<TextEdit> addEdit(@string before, slice<TextEdit> edits, TextEdit edit) {
    ...
    if (start.Column() > 1) { 
        ...
        var delta = start.Column() - 1;
        start = span.NewPoint(start.Line(), 1, start.Offset() - delta);
        edit.Span = span.New(edit.Span.URI(), start, end);
        edit.NewText = before[(int)start.Offset()..(int)start.Offset() + delta] + edit.NewText;
    ...
    if (end.Column() > 1) {
        var remains = before[(int)end.Offset()..];
        ...
        else
 {
        ...
        edit.Span = span.New(edit.Span.URI(), start, end);
        edit.NewText = edit.NewText + remains[..(int)eol];
