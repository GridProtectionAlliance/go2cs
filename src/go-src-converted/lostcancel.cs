...
// package lostcancel -- go2cs converted at 2022 March 06 23:34:02 UTC
// import "golang.org/x/tools/go/analysis/passes/lostcancel" ==> using lostcancel = go.golang.org.x.tools.go.analysis.passes.lostcancel_package
// Original source: C:\Users\ritchie\go\src\golang.org\x\tools\go\analysis\passes\lostcancel\lostcancel.go
...
public static readonly @string Doc = @"check cancel func returned by context.WithCancel is called
...
private static void runFunc(ptr<analysis.Pass> _addr_pass, ast.Node node) {
    ...
    var stack = make_slice<ast.Node>(0, 32);
    ast.Inspect(node, n => {
        switch (n.type()) {
        ...
        //
        if (!isContextWithCancel(_addr_pass.TypesInfo, n) || !isCall(stack[len(stack) - 2])) {
            return true;
        }
        ptr<ast.Ident> id; // id of cancel var
        var stmt = stack[len(stack) - 3];
        switch (stmt.type()) {
            ...
            case ptr<ast.AssignStmt> stmt:
                if (len(stmt.Lhs) > 1) {
                    id, _ = stmt.Lhs[1]._<ptr<ast.Ident>>();
                }
                break;
        }
        if (id != null) {
...
private static ptr<ast.ReturnStmt> lostCancelPath(ptr<analysis.Pass> _addr_pass, ptr<cfg.CFG> _addr_g, ptr<types.Var> _addr_v, ast.Node stmt, ptr<types.Signature> _addr_sig) => func((_, panic, _) => {
    ref analysis.Pass pass = ref _addr_pass.val;
