...
// GOPROXY="file://<dir>".
// package proxydir -- go2cs converted at 2022 March 06 23:32:26 UTC
...
public static error WriteModuleVersion(@string rootDir, @string module, @string ver, map<@string, slice<byte>> files) => func((defer, _, _) => {
    error rerr = default!;
    ...
    var (f, err) = os.OpenFile(filepath.Join(dir, "list"), os.O_APPEND | os.O_CREATE | os.O_WRONLY, 0644);
    if (err != null) {
        return error.As(err)!;
    }
    ...
    }
    {
        ...
        err = ioutil.WriteFile(filepath.Join(dir, ver + ".mod"), modContents, 0644);
    ...
    slice<byte> infoContents = (slice<byte>)fmt.Sprintf("{\"Version\": \"%v\", \"Time\":\"2017-12-14T13:08:43Z\"}", ver);
    {
        ...
        err = ioutil.WriteFile(filepath.Join(dir, ver + ".info"), infoContents, 0644);
