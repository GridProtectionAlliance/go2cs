...
// package packages -- go2cs converted at 2022 March 06 23:31:51 UTC
...
public partial struct LoadMode { // : nint
}
...
public static readonly LoadMode NeedName = 1 << (int)(iota); 
...
// type-checker.
public partial struct ErrorKind { // : nint
}

public static readonly ErrorKind UnknownError = iota;
...
private static (slice<ptr<Package>>, error) refine(this ptr<loader> _addr_ld, slice<@string> roots, params ptr<ptr<Package>>[] _addr_list) => func((_, panic, _) => {
    slice<ptr<Package>> _p0 = default;
    error _p0 = default!;
    list = list.Clone();
    ...
    var rootMap = make_map<@string, nint>(len(roots));
    {
        var i__prev1 = i;
    ...
    foreach (var (_, pkg) in list) {
        ...
        // Overlays can invalidate export data.
        // TODO(matloob): make this check fine-grained based on dependencies on overlaid files
        var exportDataInvalid = len(ld.Overlay) > 0 || pkg.ExportFile == "" && pkg.PkgPath != "unsafe"; 
        // This package needs type information if the caller requested types and the package is
        // either a root, or it's a non-root and the user requested dependencies ...
        var needtypes = (ld.Mode & NeedTypes | NeedTypesInfo != 0 && (rootIndex >= 0 || ld.Mode & NeedDeps != 0)); 
        // This package needs source if the call requested source (or types info, which implies source)
        // and the package is either a root, or itas a non- root and the user requested dependencies...
        var needsrc = ((ld.Mode & (NeedSyntax | NeedTypesInfo) != 0 && (rootIndex >= 0 || ld.Mode & NeedDeps != 0)) || (ld.Mode & NeedTypes | NeedTypesInfo != 0 && exportDataInvalid)) && pkg.PkgPath != "unsafe";
    ...
    const nint white = 0; // new
    const nint grey = 1; // in progress
    const nint black = 2; // complete 
...
// Precondition: ld.Mode & NeedTypes.
private static void loadPackage(this ptr<loader> _addr_ld, ptr<loaderPackage> _addr_lpkg) => func((_, panic, _) => {
    ref loader ld = ref _addr_ld.val;
    ref loaderPackage lpkg = ref _addr_lpkg.val;
...
outer:
    ...
    var illTyped = len(lpkg.Errors) > 0;
    if (!illTyped) {
        {
            var imp__prev1 = imp;
...
private static var ioLimit = make_channel<bool>(20);
...
private static (ptr<types.Package>, error) loadFromExportData(this ptr<loader> _addr_ld, ptr<loaderPackage> _addr_lpkg) => func((defer, _, _) => {
    ptr<types.Package> _p0 = default!;
    error _p0 = default!;
    ref loader ld = ref _addr_ld.val;
    ref loaderPackage lpkg = ref _addr_lpkg.val;
    ...
    var viewLen = len(view) + 1; // adding the self package
