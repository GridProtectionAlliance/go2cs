...
// package lzw -- go2cs converted at 2020 August 29 10:10:05 UTC
...
namespace compress
{
    public static partial class lzw_package
    {
        ...
        private static (long, error) Write(this ref errWriteCloser e, slice<byte> _p0)
        {
            return (0L, e.err);
        }

        private static error Close(this ref errWriteCloser e)
        {
        ...
        private static readonly long maxCode = 1L << (int)(12L) - 1L;
        private static readonly long invalidCode = 1L << (int)(32L) - 1L; 
        // There are 1<<12 possible codes, which is an upper bound on the number of
        // valid hash table entries at any given point in time. tableSize is 4x that.
        private static readonly long tableSize = 4L * 1L << (int)(12L);
        private static readonly var tableMask = tableSize - 1L; 
        // A hash table entry is a uint32. Zero is an invalid entry since the
        // lower 12 bits of a valid entry must be a non-literal code.
        private static readonly long invalidEntry = 0L;
...
// converting a code stream into a byte stream.
            public Order order;
            public Func<ref encoder, uint, error> write;
            public uint bits;
            public ulong nBits;
...
// The values are a 12-bit code.
        ...
        private static error writeLSB(this ref encoder e, uint c)
        {
        ...
        private static error writeMSB(this ref encoder e, uint c)
        {
        ...
        private static error incHi(this ref encoder e)
        {
            e.hi++;
            if (e.hi == e.overflow)
            {
...
loop:
        ...
        {
            Func<ref encoder, uint, error> write = default;
            ...
            if (order == LSB) 
                ...
                return ref new errWriteCloser(errors.New("lzw: unknown order"));
                        if (litWidth < 2L || 8L < litWidth)
            {
                return ref new errWriteCloser(fmt.Errorf("lzw: litWidth %d out of range",litWidth));
            }
            writer (bw, ok) = w._<writer>();
            if (!ok)
            {
                bw = bufio.NewWriter(w);
            }
            var lw = uint(litWidth);
            return ref new encoder(w:bw,order:order,write:write,width:1+lw,litWidth:lw,hi:1<<lw+1,overflow:1<<(lw+1),savedCode:invalidCode,);
        }
    }
}}
