...
// package expect -- go2cs converted at 2022 March 06 23:32:24 UTC
...
private static (@string, nint) getAdjustedNote(@string text) {
    @string _p0 = default;
    nint _p0 = default;
    ...
    text = text[(int)2..]; // remove "//" or "/*" prefix
    ...
    text = text[(int)commentStartLen..];
    return (text, commentStartLen + adjust + 1);

}

private static readonly int invalidToken = 0;
...
private static nint Skip(this ptr<tokens> _addr_t, int r) {
    ref tokens t = ref _addr_t.val;
...
private static void parseArgument(ptr<tokens> _addr_t) {
    ...
    if (t.Token() == scanner.Ident) 
        ...
        var s = t.Consume();
        var (v, err) = strconv.ParseInt(s, 0, 0);
        if (err != null) {
