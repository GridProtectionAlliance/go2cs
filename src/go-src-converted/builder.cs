...
namespace go
{
    public static partial class ssa_package
    {
        ...
        private static Value logicalBinop(this ptr<builder> _addr_b, ptr<Function> _addr_fn, ptr<ast.BinaryExpr> _addr_e)
        {
            ...
            foreach (>>MARKER:FORRANGEEXPRESSIONS_LEVEL_1<< in done.Preds)
        ...
        // is token.ARROW).
        //
        private static Value exprN(this ptr<builder> _addr_b, ptr<Function> _addr_fn, ast.Expr e) => func((_, panic, __) =>
        {
            ref builder b = ref _addr_b.val;
            ref Function fn = ref _addr_fn.val;
            ...
            switch (e.type())
            {
                ...
                case ptr<ast.TypeAssertExpr> e:
                    return emitTypeTest(fn, b.expr(fn, e.X), typ.At(0L).Type(), e.Lparen);
                    break;
                case ptr<ast.UnaryExpr> e:
        ...
        private static Value builtin(this ptr<builder> _addr_b, ptr<Function> _addr_fn, ptr<types.Builtin> _addr_obj, slice<ast.Expr> args, types.Type typ, token.Pos pos)
        {
            ...
            switch (obj.Name())
            {
                ...
                case "cap": 
                    ...
                    // was side-effect free, the whole call would have
                    // been constant-folded.)
                    var t = deref(fn.Pkg.typeOf(args[0L])).Underlying();
                    {
                        var at__prev1 = at;
                        ...
                        if (ok)
                        {
                            b.expr(fn, args[0L]); // for effects only
        ...
        //
        private static lvalue addr(this ptr<builder> _addr_b, ptr<Function> _addr_fn, ast.Expr e, bool escaping) => func((_, panic, __) =>
        {
            ref builder b = ref _addr_b.val;
            ref Function fn = ref _addr_fn.val;
        ...
        private static Value expr0(this ptr<builder> _addr_b, ptr<Function> _addr_fn, ast.Expr e, types.TypeAndValue tv) => func((_, panic, __) =>
        {
            ref builder b = ref _addr_b.val;
            ref Function fn = ref _addr_fn.val;
        ...
        //
        private static void setCall(this ptr<builder> _addr_b, ptr<Function> _addr_fn, ptr<ast.CallExpr> _addr_e, ptr<CallCommon> _addr_c) => func((_, panic, __) =>
        {
            ref builder b = ref _addr_b.val;
            ref Function fn = ref _addr_fn.val;
            ref ast.CallExpr e = ref _addr_e.val;
        ...
        private static void localValueSpec(this ptr<builder> _addr_b, ptr<Function> _addr_fn, ptr<ast.ValueSpec> _addr_spec)
        {
            ...
            if (len(spec.Values) == len(spec.Names)) 
                ...
                // e.g. var x, y = pos()
                var tuple = b.exprN(fn, spec.Values[0L]);
                {
                    var i__prev1 = i;
                    var id__prev1 = id;
        ...
        private static void assignStmt(this ptr<builder> _addr_b, ptr<Function> _addr_fn, slice<ast.Expr> lhss, slice<ast.Expr> rhss, bool isDef)
        {
            ...
            { 
                ...
                // e.g. x, y = pos()
                var tuple = b.exprN(fn, rhss[0L]);
                emitDebugRef(fn, rhss[0L], tuple, false);
                {
                    var i__prev1 = i;
        ...
        //
        private static void compLit(this ptr<builder> _addr_b, ptr<Function> _addr_fn, Value addr, ptr<ast.CompositeLit> _addr_e, bool isZero, ptr<storebuf> _addr_sb) => func((_, panic, __) =>
        {
            ref builder b = ref _addr_b.val;
            ref Function fn = ref _addr_fn.val;
        ...
        private static void typeSwitchStmt(this ptr<builder> _addr_b, ptr<Function> _addr_fn, ptr<ast.TypeSwitchStmt> _addr_s, ptr<lblock> _addr_label)
        {
            ...
            switch (s.Assign.type())
            {
                ...
                case ptr<ast.AssignStmt> ass:
                    x = b.expr(fn, unparen(ass.Rhs[0L])._<ptr<ast.TypeAssertExpr>>().X);
                    break;
            ...
            foreach (var (_, clause) in s.Body.List)
            {
                ...
                foreach (var (_, cond) in cc.List)
                {
                    ...
                    else
                    {
                        ...
                        ti = emitExtract(fn, yok, 0L);
                        condv = emitExtract(fn, yok, 1L);
                    }
        ...
        private static void selectStmt(this ptr<builder> _addr_b, ptr<Function> _addr_fn, ptr<ast.SelectStmt> _addr_s, ptr<lblock> _addr_label)
        {
            ...
            if (len(s.Body.List) == 1L)
            {
                ptr<ast.CommClause> clause = s.Body.List[0L]._<ptr<ast.CommClause>>();
                if (clause.Comm != null)
                {
                ...
                foreach (var (_, __clause) in s.Body.List)
                {
                    ...
                    switch (clause._<ptr<ast.CommClause>>().Comm.type())
                    {
                        ...
                        case ptr<ast.AssignStmt> comm:
                            ptr<ast.UnaryExpr> recv = unparen(comm.Rhs[0L])._<ptr<ast.UnaryExpr>>();
            ...
            long state = 0L;
            long r = 2L; // index in 'sel' tuple of value; increments if st.Dir==RECV
            foreach (var (_, cc) in s.Body.List)
            {
                ...
                switch (clause.Comm.type())
                {
                    ...
                    case ptr<ast.AssignStmt> comm:
                        ...
                        {
                            fn.addLocalForIdent(comm.Lhs[0L]._<ptr<ast.Ident>>());
                        }

                        var x = b.addr(fn, comm.Lhs[0L], false); // non-escaping
                        ...
                        { // x, ok := ...
                            if (comm.Tok == token.DEFINE)
                            {
                                fn.addLocalForIdent(comm.Lhs[1L]._<ptr<ast.Ident>>());
                            }

                            var ok = b.addr(fn, comm.Lhs[1L], false); // non-escaping
                            ok.store(fn, emitExtract(fn, sel, 1L));
        ...
        //
        private static (Value, Value, ptr<BasicBlock>, ptr<BasicBlock>) rangeIndexed(this ptr<builder> _addr_b, ptr<Function> _addr_fn, Value x, types.Type tv, token.Pos pos) => func((_, panic, __) =>
        {
            Value k = default;
            Value v = default;
            ...
            emitStore(fn, index, intConst(-1L), pos);
        ...
        private static (Value, Value, ptr<BasicBlock>, ptr<BasicBlock>) rangeIter(this ptr<builder> _addr_b, ptr<Function> _addr_fn, Value x, types.Type tk, types.Type tv, token.Pos pos)
        {
            ...
            emitIf(fn, emitExtract(fn, okv, 0L), body, done);
            fn.currentBlock = body;

            if (tk != tInvalid)
            {
                k = emitExtract(fn, okv, 1L);
            }

            if (tv != tInvalid)
            {
                v = emitExtract(fn, okv, 2L);
            }
        ...
        private static (Value, ptr<BasicBlock>, ptr<BasicBlock>) rangeChan(this ptr<builder> _addr_b, ptr<Function> _addr_fn, Value x, types.Type tk, token.Pos pos)
        {
            ...
            emitIf(fn, emitExtract(fn, ko, 1L), body, done);
            fn.currentBlock = body;
            if (tk != null)
            {
                k = emitExtract(fn, ko, 0L);
            }
        ...
        // labelled by label.
        //
        private static void rangeStmt(this ptr<builder> _addr_b, ptr<Function> _addr_fn, ptr<ast.RangeStmt> _addr_s, ptr<lblock> _addr_label) => func((_, panic, __) =>
        {
            ref builder b = ref _addr_b.val;
            ref Function fn = ref _addr_fn.val;
        ...
        private static void stmt(this ptr<builder> _addr_b, ptr<Function> _addr_fn, ast.Stmt _s) => func((_, panic, __) =>
        {
            ref builder b = ref _addr_b.val;
            ref Function fn = ref _addr_fn.val;
...
start:
            switch (_s.type())
            {
                ...
                case ptr<ast.IncDecStmt> s:
                    ...
                    b.assignOp(fn, loc, NewConst(constant.MakeInt64(1L), loc.typ()), op, s.Pos());
                    break;
                case ptr<ast.AssignStmt> s:
                    ...
                    if (s.Tok == token.ASSIGN || s.Tok == token.DEFINE) 
                        ...
                        b.assignOp(fn, b.addr(fn, s.Lhs[0L], false), b.expr(fn, s.Rhs[0L]), op, s.Pos());
                                        break;
                ...
                case ptr<ast.ReturnStmt> s:
                    ...
                    if (len(s.Results) == 1L && fn.Signature.Results().Len() > 1L)
                    { 
                        // Return of one expression in a multi-valued function.
                        var tuple = b.exprN(fn, s.Results[0L]);
        ...
        private static void buildFunction(this ptr<builder> _addr_b, ptr<Function> _addr_fn) => func((defer, panic, _) =>
        {
            ...
            b.stmt(fn, body);
            {
                ...
                if (cb != null && (cb == fn.Blocks[0L] || cb == fn.Recover || cb.Preds != null))
                { 
        ...
        private static void build(this ptr<Package> _addr_p) => func((defer, panic, _) =>
        {
            ...
            {
                if (init.Prog.mode & LogSource != 0L)
                {
                    fmt.Fprintf(os.Stderr, "build global initializer %v @ %s\n", varinit.Lhs, p.Prog.Fset.Position(varinit.Rhs.Pos()));
                }

                if (len(varinit.Lhs) == 1L)
                { 
            ...
            if (p.Prog.mode & SanityCheckFunctions != 0L)
            {
        ...
        private static types.Object objectOf(this ptr<Package> _addr_p, ptr<ast.Ident> _addr_id) => func((_, panic, __) =>
        {
            ref Package p = ref _addr_p.val;
        ...
        // Only valid during p's create and build phases.
        private static types.Type typeOf(this ptr<Package> _addr_p, ast.Expr e) => func((_, panic, __) =>
        {
            ref Package p = ref _addr_p.val;
