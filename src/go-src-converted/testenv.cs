...
// package testenv -- go2cs converted at 2020 October 08 04:54:31 UTC
...
namespace @internal
{
    public static partial class testenv_package
    {
        ...
        private static error hasTool(@string tool) => func((defer, _, __) =>
        {
            if (tool == "cgo")
            {
        ...
        public static long Go1Point() => func((_, panic, __) =>
        {
            for (var i = len(build.Default.ReleaseTags) - 1L; i >= 0L; i--)
            {
                ref long version = ref heap(out ptr<long> _addr_version);
                {
        ...
        // 1.x.
        public static void NeedsGo1Point(Testing t, long x)
        {
            {
                helperer (t, ok) = helperer.As(t._<helperer>())!;
        ...
        // 1.x.
        public static void SkipAfterGo1Point(Testing t, long x)
        {
            {
                helperer (t, ok) = helperer.As(t._<helperer>())!;
