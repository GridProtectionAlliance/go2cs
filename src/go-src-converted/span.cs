...
// text files.
// package span -- go2cs converted at 2022 March 06 23:31:23 UTC
...
public partial interface Converter {
    (nint, error) ToPosition(nint offset); //ToOffset converts from a line:column pair to an offset.
    (nint, error) ToOffset(nint line, nint col);
}
...
public static Point NewPoint(nint line, nint col, nint offset) {
    Point p = new Point(v:point{Line:line,Column:col,Offset:offset});
    p.v.clean();
    return p;
}

public static nint Compare(Span a, Span b) {
    {
        var r__prev1 = r;
...
public static nint ComparePoint(Point a, Point b) {
    return comparePoint(a.v, b.v);
}

private static nint comparePoint(point a, point b) {
    if (!a.hasPosition()) {
...
public static nint Line(this Point p) => func((_, panic, _) => {
    if (!p.v.hasPosition()) {
...
public static nint Column(this Point p) => func((_, panic, _) => {
    if (!p.v.hasPosition()) {
        panic(fmt.Errorf("position not set in %v", p.v));
    }
    return p.v.Column;
...
public static nint Offset(this Point p) => func((_, panic, _) => {
    if (!p.v.hasOffset()) {
...
public static void Format(this Span s, fmt.State f, int c) {
    ...
    var printColumn = printLine && (fullForm || (s.v.Start.Column > 1 || s.v.End.Column > 1));
    fmt.Fprint(f, ":");
    if (printLine) {
