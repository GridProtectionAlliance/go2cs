...
// package ocagent -- go2cs converted at 2022 March 06 23:31:42 UTC
...
private static slice<ptr<wire.TimeSeries>> dataToTimeseries(metric.Data data, time.Time start) {
    ...
    var timeseries = make_slice<ptr<wire.TimeSeries>>(0, numRows);

    for (nint i = 0; i < numRows; i++) {
...
private static nint numRows(metric.Data data) {
    switch (data.type()) {
        case ptr<metric.Int64Data> d:
            return len(d.Rows);
            break;
...
// in data at index i.
private static slice<ptr<wire.Point>> dataToPoints(metric.Data data, nint i) {
    switch (data.type()) {
        case ptr<metric.Int64Data> d:
...
private static slice<ptr<wire.LabelKey>> infoKeysToLabelKeys(slice<label.Key> infoKeys) {
    var labelKeys = make_slice<ptr<wire.LabelKey>>(0, len(infoKeys));
