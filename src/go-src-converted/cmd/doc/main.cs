...
private static void Main() {
    ...
    log.SetPrefix("doc: ");
    dirsInit();
    var err = do(os.Stdout, _addr_flag.CommandLine, os.Args[(int)1..]);
    if (err != null) {
        log.Fatal(err);
    }
}
...
private static (ptr<build.Package>, @string, @string, bool) parseArgs(slice<@string> args) {
    ...
    switch (len(args)) {
        ...
        case 2: 
            ...
            var (pkg, err) = build.Import(args[0], wd, build.ImportComment);
            if (err == null) {
                return (_addr_pkg!, args[0], args[1], false);
            }
            while (true) {
                ...
                }
                {
                    ...
                    if (err == null) {
                        return (_addr_pkg!, arg, args[1], true);
                    }
    ...
    else
 {
    ...
    // start is the byte after the slash.
    {
        ...
        while (start < len(arg)) {
            period = strings.Index(arg[(int)start..], ".");
            @string symbol = "";
            if (period < 0) {
                period = len(arg);
            start = period + 1;
 ...
 {
                period += start;
                symbol = arg[(int)period + 1..];
            } 
            // Have we identified a package already?
            (pkg, err) = build.Import(arg[(int)0..(int)period], wd, build.ImportComment);
            if (err == null) {
                return (_addr_pkg!, arg[(int)0..(int)period], symbol, false);
            } 
            // See if we have the basename or tail of a package, as in json for encoding/json
            // or ivy/value for robpike.io/ivy/value.
            var pkgName = arg[..(int)period];
            while (true) {
    ...
    if (slash >= 0) { 
        ...
        if (strings.Contains(importErrStr, arg[..(int)period])) {
            log.Fatal(importErrStr);
        }
        else
 {
            log.Fatalf("no such package %s: %s", arg[..(int)period], importErrStr);
        }
    }
