...
// match any packages.
        ...
        private static void MatchDirs(this ptr<Match> _addr_m)
        {
            ...
            var (dir, _) = filepath.Split(cleanPattern[..i]); 
            ...
            var err = filepath.Walk(dir, (path, fi, err) =>
            {
                if (err != null)
                {
                    return err; // Likely a permission error, which could interfere with matching.
        ...
        { 
            ...
            const @string vendorChar = (@string)"\x00";
        ...
        public static void WarnUnmatched(slice<ptr<Match>> matches)
        {
            ...
            {
                if (len(m.Pkgs) == 0L && len(m.Errs) == 0L)
                {
        ...
        private static @string inDirLex(@string path, @string dir)
        {
            ...
            if (pv != dv) 
                ...
                {
                    if (len(path) == len(dir) + 1L)
                    {
                        return ".";
                    }

                    return path[len(dir) + 1L..];
