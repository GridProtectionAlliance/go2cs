...
// package load -- go2cs converted at 2020 October 09 05:45:59 UTC
// import "cmd/go/internal/load" ==> using load = go.cmd.go.@internal.load_package
// Original source: C:\Go\src\cmd\go\internal\load\test.go
...
namespace @internal
{
    public static partial class load_package
    {
        ...
        public static (ptr<Package>, ptr<Package>, ptr<Package>, error) TestPackagesFor(ptr<Package> _addr_p, ptr<TestCover> _addr_cover)
        {
            ...
            pmain, ptest, pxtest = TestPackagesAndErrors(_addr_p, _addr_cover);
            foreach (var (_, p1) in new slice<ptr<Package>>(new ptr<Package>[] { ptest, pxtest, pmain }))
            {
                ...
                {
                    var perr = p1.DepsErrors[0L];
                    err = perr;
                    break;
                }

            }
            if (pmain.Error != null || len(pmain.DepsErrors) > 0L)
            {
                pmain = null;
            }

            if (ptest.Error != null || len(ptest.DepsErrors) > 0L)
            {
                ptest = null;
            }

            if (pxtest != null && (pxtest.Error != null || len(pxtest.DepsErrors) > 0L))
            {
                pxtest = null;
            }
        ...
        public static (ptr<Package>, ptr<Package>, ptr<Package>) TestPackagesAndErrors(ptr<Package> _addr_p, ptr<TestCover> _addr_cover) => func((defer, _, __) =>
        {
            ptr<Package> pmain = default!;
            ptr<Package> ptest = default!;
            ptr<Package> pxtest = default!;
            ...
            var rawTestImports = str.StringList(p.TestImports);
            {
                ...
                foreach (var (__i, __path) in p.TestImports)
                {
                    i = __i;
                    path = __path;
                    var p1 = loadImport(pre, path, p.Dir, p, _addr_stk, p.Internal.Build.TestImportPos[path], ResolveImport);
                    if (str.Contains(p1.Deps, p.ImportPath) || p1.ImportPath == p.ImportPath)
                    { 
                        // Same error that loadPackage returns (via reusePackage) in pkg.go.
                        // Can't change that code, because that code is only for loading the
                        // non-test copy of a package.
                        ptestErr = addr(new PackageError(ImportStack:testImportStack(stk[0],p1,p.ImportPath),Err:errors.New("import cycle not allowed in test"),IsImportCycle:true,));
            ...
            var rawXTestImports = str.StringList(p.XTestImports);
            {
                ...
                foreach (var (__i, __path) in p.XTestImports)
                {
                    i = __i;
                    path = __path;
                    p1 = loadImport(pre, path, p.Dir, p, _addr_stk, p.Internal.Build.XTestImportPos[path], ResolveImport);
            ...
            // Test package.
            if (len(p.TestGoFiles) > 0L || p.Name == "main" || cover != null && cover.Local)
            {
                ptest = @new<Package>();
                ptest.val = p;
            ...
            {
                pxtest = addr(new Package(PackagePublic:PackagePublic{Name:p.Name+"_test",ImportPath:p.ImportPath+"_test",Root:p.Root,Dir:p.Dir,Goroot:p.Goroot,GoFiles:p.XTestGoFiles,Imports:p.XTestImports,ForTest:p.ImportPath,Error:pxtestErr,},Internal:PackageInternal{LocalPrefix:p.Internal.LocalPrefix,Build:&build.Package{ImportPos:p.Internal.Build.XTestImportPos,},Imports:ximports,RawImports:rawXTestImports,Asmflags:p.Internal.Asmflags,Gcflags:p.Internal.Gcflags,Ldflags:p.Internal.Ldflags,Gccgoflags:p.Internal.Gccgoflags,},));
                if (pxtestNeedsPtest)
                {
            ...
            // Build main package.
            pmain = addr(new Package(PackagePublic:PackagePublic{Name:"main",Dir:p.Dir,GoFiles:[]string{"_testmain.go"},ImportPath:p.ImportPath+".test",Root:p.Root,Imports:str.StringList(TestMainDeps),},Internal:PackageInternal{Build:&build.Package{Name:"main"},BuildInfo:p.Internal.BuildInfo,Asmflags:p.Internal.Asmflags,Gcflags:p.Internal.Gcflags,Ldflags:p.Internal.Ldflags,Gccgoflags:p.Internal.Gccgoflags,},)); 
            ...
            foreach (var (_, dep) in deps)
            {
                ...
                {
                    p1 = loadImport(pre, dep, "", null, _addr_stk, null, 0L);
            ...
            var allTestImports = make_slice<ptr<Package>>(0L, len(pmain.Internal.Imports) + len(imports) + len(ximports));
            ...
            long w = 0L;
            {
                ...
                {
                    path = __path;
                    if (w == 0L || path != pmain.Imports[w - 1L])
                    {
        ...
        private static slice<@string> testImportStack(@string top, ptr<Package> _addr_p, @string target)
        {
            ref Package p = ref _addr_p.val;
...
Search:
        ...
        private static void recompileForTest(ptr<Package> _addr_pmain, ptr<Package> _addr_preal, ptr<Package> _addr_ptest, ptr<Package> _addr_pxtest) => func((_, panic, __) =>
        {
...
; 
        ...
        private static bool isTestFunc(ptr<ast.FuncDecl> _addr_fn, @string arg)
        {
            ...
            ptr<ast.StarExpr> (ptr, ok) = fn.Type.Params.List[0L].Type._<ptr<ast.StarExpr>>();
            if (!ok)
            {
                return false;
            } 
        ...
        private static error load(this ptr<testFuncs> _addr_t, @string filename, @string pkg, ptr<bool> _addr_doImport, ptr<bool> _addr_seen)
        {
            ref testFuncs t = ref _addr_t.val;
