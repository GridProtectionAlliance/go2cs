...
namespace @internal
{
    public static partial class lockedfile_package
    {
        ...
        public static (ptr<File>, error) Open(@string name)
        {
            ...
            return _addr_OpenFile(name, os.O_RDONLY, 0L)!;
        }
        ...
        public static (ptr<File>, error) Create(@string name)
        {
            ...
            return _addr_OpenFile(name, os.O_RDWR | os.O_CREATE | os.O_TRUNC, 0666L)!;
        }
        ...
        public static (ptr<File>, error) Edit(@string name)
        {
            ...
            return _addr_OpenFile(name, os.O_RDWR | os.O_CREATE, 0666L)!;
        }
        ...
        private static error Close(this ptr<File> _addr_f)
        {
            ...
            if (f.closed)
            {
                return error.As(addr(new os.PathError(Op:"close",Path:f.Name(),Err:os.ErrClosed,))!)!;
            }
        ...
        public static (slice<byte>, error) Read(@string name) => func((defer, _, __) =>
        {
        ...
        public static error Write(@string name, io.Reader content, os.FileMode perm)
        {
            error err = default!;
        ...
        public static error Transform(@string name, Func<slice<byte>, (slice<byte>, error)> t) => func((defer, _, __) =>
        {
            error err = default!;
            ...
            { 
                ...
                {
                    var (_, err) = f.WriteAt(new[len(old)..], int64(len(old)));
            ...
            {
                {
                    (_, err) = f.WriteAt(new[..len(old)], 0L);
