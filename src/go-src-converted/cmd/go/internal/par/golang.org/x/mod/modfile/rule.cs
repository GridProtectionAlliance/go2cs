...
// written to a file.
// package modfile -- go2cs converted at 2022 March 06 23:16:52 UTC
...
private static (ptr<File>, error) parseToFile(@string file, slice<byte> data, VersionFixer fix, bool strict) {
    ...
    ref ErrorList errs = ref heap(out ptr<ErrorList> _addr_errs);
    {
        ...
        foreach (var (_, __x) in fs.Stmt) {
            ...
            switch (x.type()) {
                case ptr<Line> x:
                    f.add(_addr_errs, x, x.Token[0], x.Token[(int)1..], fix, strict);
                    break;
                    ...
                    switch (x.Token[0]) {
                        ...
                        case "replace": 
                            foreach (var (_, l) in x.Line) {
                                f.add(_addr_errs, l, x.Token[0], l.Token, fix, strict);
                            }
                            break;
                        default: 
                            if (strict) {
...
private static void setIndirect(ptr<Line> _addr_line, bool indirect) {
    ...
    var f = strings.Fields(line.Suffix[0].Token);
    if (len(f) == 2) { 
                        ...
                        foreach (var (_, line) in stmt.Line) {
                            {
                                ...
                                var (p, err) = parseString(_addr_line.Token[0]);
                    ...
                    if (len(stmt.Token) > 0 && stmt.Token[0] == "require") {
                        {
                            ...
                            (p, err) = parseString(_addr_stmt.Token[1]);
