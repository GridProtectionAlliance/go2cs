...
private static void initEnvCache() {
    ...
    while (len(data) > 0) { 
        ...
        if (i >= 0) {
            (line, data) = (line[..(int)i], data[(int)i + 1..]);
        }
        else
 {
            data = null;
...
public static @string Getenv(@string key) => func((_, panic, _) => {
    if (!CanGetenv(key)) {
        switch (key) {
            case "CGO_TEST_ALLOW": 
...
public static var GOROOT = BuildContext.GOROOT;public static var GOBIN = Getenv("GOBIN");public static var GOROOTbin = filepath.Join(GOROOT, "bin");public static var GOROOTpkg = filepath.Join(GOROOT, "pkg");public static var GOROOTsrc = filepath.Join(GOROOT, "src");public static var GOROOT_FINAL = findGOROOT_FINAL();public static var GOMODCACHE = envOr("GOMODCACHE", gopathDir("pkg/mod"));public static var GOARM = envOr("GOARM", fmt.Sprint(buildcfg.GOARM));public static var GO386 = envOr("GO386", buildcfg.GO386);public static var GOMIPS = envOr("GOMIPS", buildcfg.GOMIPS);public static var GOMIPS64 = envOr("GOMIPS64", buildcfg.GOMIPS64);public static var GOPPC64 = envOr("GOPPC64", fmt.Sprintf("%s%d", "power", buildcfg.GOPPC64));public static var GOWASM = envOr("GOWASM", fmt.Sprint(buildcfg.GOWASM));public static var GOPROXY = envOr("GOPROXY", "https://proxy.golang.org,direct");public static var GOSUMDB = envOr("GOSUMDB", "sum.golang.org");public static var GOPRIVATE = Getenv("GOPRIVATE");public static var GONOPROXY = envOr("GONOPROXY", GOPRIVATE);public static var GONOSUMDB = envOr("GONOSUMDB", GOPRIVATE);public static var GOINSECURE = Getenv("GOINSECURE");public static var GOVCS = Getenv("GOVCS");
