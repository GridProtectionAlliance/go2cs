...
private static void runList(context.Context ctx, ptr<base.Command> _addr_cmd, slice<@string> args) => func((defer, _, _) => {
    ref base.Command cmd = ref _addr_cmd.val;

    if (listFmt != "" && listJson == true.val) {
    ...
    if (listFmt == "".val) {
        if (listM.val) {
            listFmt.val = "{{.String}}";
            if (listVersions.val) {
                listFmt.val = "{{.Path}}{{range .Versions}} {{.}}{{end}}{{if .Deprecated}} (deprecated){{end}}";
            }
        }
        else
 {
...
;
        ...
        template.FuncMap fm = new template.FuncMap("join":strings.Join,"context":context,"module":func(pathstring)*modinfo.ModulePublic{returnmodload.ModuleInfo(ctx,path)},);
    ...
    if (listM.val) { 
        ...
        if (cfg.BuildMod == "vendor") {
            const @string actionDisabledFormat = "go list -m: can't %s using the vendor directory\n\t(Use -mod=mod or -mod=readonly to bypass.)";
        ...
        var (mods, err) = modload.ListModules(ctx, args, mode);
        if (!listE.val) {
            {
                var m__prev1 = m;
    ...
    load.PackageOpts pkgOpts = new load.PackageOpts(IgnoreImports:*listFind,ModResolveTests:*listTest,);
    var pkgs = load.PackagesAndErrors(ctx, pkgOpts, args);
    if (!listE.val) {
        nint w = 0;
        ...
        // Add test binaries to packages to be listed.
        {
            ...
            foreach (var (_, __p) in pkgs) {
                ...
                if (len(p.TestGoFiles) + len(p.XTestGoFiles) > 0) {
                    ...
                    err = default!;
                    if (listE.val) {
                        pmain, ptest, pxtest = load.TestPackagesAndErrors(ctx, pkgOpts, p, null);
                    }
                    else
 {
                        pmain, ptest, pxtest, err = load.TestPackagesFor(ctx, pkgOpts, p, null);
                        if (err != null) {
    ...
    if (listRetracted.val) { 
        ...
        // all instances.
        var modToArg = make_map<ptr<modinfo.ModulePublic>, @string>();
        var argToMods = make_map<@string, slice<ptr<modinfo.ModulePublic>>>();
        slice<@string> args = default;
        Action<ptr<modinfo.ModulePublic>> addModule = mod => {
            if (mod.Version == "") {
                return ;
            }
            var arg = fmt.Sprintf("%s@%s", mod.Path, mod.Version);
            if (argToMods[arg] == null) {
        ...
        if (len(args) > 0) {
            ...
            var (rmods, err) = modload.ListModules(ctx, args, mode);
            if (err != null && !listE.val) {
            ...
            }
            {
                ...
                foreach (var (__i, __arg) in args) {
                    ...
                    arg = __arg;
                    var rmod = rmods[i];
                    foreach (var (_, mod) in argToMods[arg]) {
                        mod.Retracted = rmod.Retracted;
                        if (rmod.Error != null && mod.Error == null) {
    ...
    }
    {
        ...
        foreach (var (_, __p) in pkgs) {
            p = __p;
            var nRaw = len(p.Internal.RawImports);
            {
                var i__prev2 = i;

                foreach (var (__i, __path) in p.Imports) {
                    i = __i;
                    path = __path;
...
// has errors.
private static slice<ptr<load.Package>> loadPackageList(slice<ptr<load.Package>> roots) {
    var pkgs = load.PackageList(roots);
...
private static (nint, error) Write(this ptr<TrackingWriter> _addr_t, slice<byte> p) {
    nint n = default;
    error err = default!;
