...
// package tlog -- go2cs converted at 2022 March 06 23:19:07 UTC
...
public partial struct Tile {
    ...
    public nint W; // width of tile (1 ≤ W ≤ 2**H; 2**H is complete tile)
}
...
public static Tile TileForIndex(nint h, long index) => func((_, panic, _) => {
    if (h <= 0) {
...
private static (Tile, nint, nint) tileForIndex(nint h, long index) {
    Tile t = default;
    nint start = default;
    nint end = default;
...
public static (Hash, error) HashFromTile(Tile t, slice<byte> data, long index) {
    ...
    if (t.H < 1 || t.H > 30 || t.L < 0 || t.L >= 64 || t.W < 1 || t.W > 1 << (int)(uint(t.H))) {
...
private static Hash tileHash(slice<byte> data) => func((_, panic, _) => {
    if (len(data) == 0) {
...
//
// If h ≤ 0, TileForIndex panics.
public static slice<Tile> NewTiles(nint h, long oldTreeSize, long newTreeSize) => func((_, panic, _) => {
    if (h <= 0) {
        panic(fmt.Sprintf("NewTiles: invalid height %d", h));
    }
    var H = uint(h);
    slice<Tile> tiles = default;
    for (var level = uint(0); newTreeSize >> (int)((H * level)) > 0; level++) {
...
public static (slice<byte>, error) ReadTileData(Tile t, HashReader r) {
    ...
    var tile = make_slice<byte>(size * HashSize);
    {
        ...
        for (i = 0; i < size; i++) {
            copy(tile[(int)i * HashSize..], hashes[i][..]);
        }
...
private static readonly nint pathBase = 1000;
...
public static (Tile, error) ParseTilePath(@string path) {
    ...
    var (h, err1) = strconv.Atoi(f[1]);
    var isData = false;
    if (f[2] == "data") {
        isData = true;
        f[2] = "0";
    }
    var (l, err2) = strconv.Atoi(f[2]);
    if (err1 != null || err2 != null || h < 1 || l < 0 || h > 30) {
        return (new Tile(), error.As(addr(new badPathError(path))!)!);
    }
    nint w = 1 << (int)(uint(h));
    {
        ...
        if (strings.HasSuffix(dotP, ".p")) {
            var (ww, err) = strconv.Atoi(f[len(f) - 1]);
            if (err != null || ww <= 0 || ww >= w) {
                return (new Tile(), error.As(addr(new badPathError(path))!)!);
            }
            w = ww;
            f[len(f) - 2] = dotP[..(int)len(dotP) - len(".p")];
            f = f[..(int)len(f) - 1];
        }
    }
    f = f[(int)3..];
...
private static Tile tileParent(Tile t, nint k, long n) {
    t.L += k;
    t.N>>=uint(k * t.H);
    t.W = 1 << (int)(uint(t.H));
    {
...
private static (slice<Hash>, error) ReadHashes(this ptr<tileHashReader> _addr_r, slice<long> indexes) {
    ...
    var tileOrder = make_map<Tile, nint>(); // tileOrder[tileKey(tiles[i])] = i
    slice<Tile> tiles = default; 

    // Plan to fetch tiles necessary to recompute tree hash.
    // If it matches, those tiles are authenticated.
    var stx = subTreeIndex(0, r.tree.N, null);
    var stxTileOrder = make_slice<nint>(len(stx));
    {
        var i__prev1 = i;
        var x__prev1 = x;
    ...
    var indexTileOrder = make_slice<nint>(len(indexes));
    {
        var i__prev1 = i;
        var x__prev1 = x;
    ...
    var (th, err) = HashFromTile(tiles[stxTileOrder[len(stx) - 1]], data[stxTileOrder[len(stx) - 1]], stx[len(stx) - 1]);
    if (err != null) {
        return (null, error.As(err)!);
    }
    {
