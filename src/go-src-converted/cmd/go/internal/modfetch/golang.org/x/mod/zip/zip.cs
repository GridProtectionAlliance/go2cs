...
// package zip -- go2cs converted at 2020 October 09 05:47:14 UTC
...
namespace mod
{
    public static partial class zip_package
    {
        ...
        public static readonly long MaxZipFile = (long)500L << (int)(20L); 

        // MaxGoMod is the maximum size in bytes of a go.mod file within a
        // module zip file.
        public static readonly long MaxGoMod = (long)16L << (int)(20L); 

        // MaxLICENSE is the maximum size in bytes of a LICENSE file within a
        // module zip file.
        public static readonly long MaxLICENSE = (long)16L << (int)(20L);
...
// an error if called on a directory or symbolic link.
        ...
        public static error Create(io.Writer w, module.Version m, slice<File> files) => func((defer, _, __) =>
        {
            error err = default!;
...
;
        ...
        public static error CreateFromDir(io.Writer w, module.Version m, @string dir) => func((defer, _, __) =>
        {
            error err = default!;
        ...
        public static error Unzip(@string dir, module.Version m, @string zipFile) => func((defer, _, __) =>
        {
            error err = default!;
            ...
            }
            {
                ...
                foreach (var (_, __zf) in z.File)
                {
                    ...
                    var dst = filepath.Join(dir, name);
                    {
                        ...
                        err = os.MkdirAll(filepath.Dir(dst), 0777L);
                    ...
                    var (w, err) = os.OpenFile(dst, os.O_WRONLY | os.O_CREATE | os.O_EXCL, 0444L);
                    if (err != null)
                    {
                        return error.As(err)!;
                    }
