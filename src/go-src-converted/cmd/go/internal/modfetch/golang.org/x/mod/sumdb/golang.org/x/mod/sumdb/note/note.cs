...
//
// package note -- go2cs converted at 2020 October 09 05:47:26 UTC
// import "golang.org/x/mod/sumdb/note" ==> using note = go.golang.org.x.mod.sumdb.note_package
// Original source: C:\Users\ritchie\go\src\golang.org\x\mod\sumdb\note\note.go
...
namespace sumdb
{
    public static partial class note_package
    {
        ...
        private static readonly long algEd25519 = (long)1L;

        // isValidName reports whether name is valid.
        // It must be non-empty and not have any Unicode spaces or pluses.
        private static bool isValidName(@string name)
        {
            return name != "" && utf8.ValidString(name) && strings.IndexFunc(name, unicode.IsSpace) < 0L && !strings.Contains(name, "+");
        }
        ...
        public static (Verifier, error) NewVerifier(@string vkey)
        {
            ...
            var (hash, err1) = strconv.ParseUint(hash16, 16L, 32L);
            var (key, err2) = base64.StdEncoding.DecodeString(key64);
            if (len(hash16) != 8L || err1 != null || err2 != null || !isValidName(name) || len(key) == 0L)
            {
        ...
        public static (Signer, error) NewSigner(@string skey)
        {
            ...
            var (hash, err1) = strconv.ParseUint(hash16, 16L, 32L);
            var (key, err2) = base64.StdEncoding.DecodeString(key64);
            if (priv1 != "PRIVATE" || priv2 != "KEY" || len(hash16) != 8L || err1 != null || err2 != null || !isValidName(name) || len(key) == 0L)
            {
            ...
            if (alg == algEd25519) 
                ...
                pubkey = append(new slice<byte>(new byte[] { algEd25519 }), key[32L..]);
        ...
        public static (ptr<Note>, error) Open(slice<byte> msg, Verifiers known)
        {
            ...
            // Must have valid UTF-8 with no non-newline ASCII control characters.
            {
                ...
                while (i < len(msg))
                {
                    var (r, size) = utf8.DecodeRune(msg[i..]);
                    if (r < 0x20UL && r != '\n' || r == utf8.RuneError && size == 1L)
                    {
            ...
            var text = msg[..split + 1L];
            var sigs = msg[split + 2L..];
            if (len(sigs) == 0L || sigs[len(sigs) - 1L] != '\n')
            {
                return (_addr_null!, error.As(errMalformedNote)!);
            }
            ...
            { 
                ...
                if (err != null || !isValidName(name) || b64 == "" || len(sig) < 5L)
                {
                    return (_addr_null!, error.As(errMalformedNote)!);
                }
                var hash = binary.BigEndian.Uint32(sig[0L..4L]);
                sig = sig[4L..];
        ...
        public static (slice<byte>, error) Sign(ptr<Note> _addr_n, params Signer[] signers)
        {
            ...
            foreach (var (_, s) in signers)
            {
                ...
                array<byte> hbuf = new array<byte>(4L);
            ...
            {
                {
                    ...
                    foreach (var (_, __sig) in list)
                    {
                        ...
                        if (err != null || len(raw) < 4L || binary.BigEndian.Uint32(raw) != hash)
                        {
                            return (null, error.As(errMalformedNote)!);
                        }
                        buf.WriteString("— ");
