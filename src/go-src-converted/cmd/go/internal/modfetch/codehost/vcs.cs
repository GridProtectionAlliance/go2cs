...
namespace modfetch
{
    public static partial class codehost_package
    {
        ...
        private static (Repo, error) newVCSRepo(@string vcs, @string remote) => func((defer, _, __) =>
        {
            Repo _p0 = default;
            error _p0 = default!;
        ...
        private static readonly @string vcsWorkDirType = (@string)"vcs1.";
        ...
        private static void loadTags(this ptr<vcsRepo> _addr_r)
        {
            ...
            foreach (var (_, tag) in r.cmd.tagRE.FindAllString(string(out), -1L))
            {
        ...
        private static void loadBranches(this ptr<vcsRepo> _addr_r)
        {
            ...
            foreach (var (_, branch) in r.cmd.branchRE.FindAllString(string(out), -1L))
            {
        ...
        private static (slice<@string>, error) Tags(this ptr<vcsRepo> _addr_r, @string prefix) => func((defer, _, __) =>
        {
        ...
        private static (ptr<RevInfo>, error) Stat(this ptr<vcsRepo> _addr_r, @string rev) => func((defer, _, __) =>
        {
        ...
        private static (slice<byte>, error) ReadFile(this ptr<vcsRepo> _addr_r, @string rev, @string file, long maxSize) => func((defer, _, __) =>
        {
        ...
        private static (map<@string, ptr<FileRev>>, error) ReadFileRevs(this ptr<vcsRepo> _addr_r, slice<@string> revs, @string file, long maxSize) => func((defer, _, __) =>
        {
        ...
        private static (@string, error) RecentTag(this ptr<vcsRepo> _addr_r, @string rev, @string prefix, @string major) => func((defer, _, __) =>
        {
            @string tag = default;
            error err = default!;
            ref vcsRepo r = ref _addr_r.val;
        ...
        private static (bool, error) DescendsFrom(this ptr<vcsRepo> _addr_r, @string rev, @string tag) => func((defer, _, __) =>
        {
            bool _p0 = default;
            error _p0 = default!;
            ref vcsRepo r = ref _addr_r.val;
        ...
        private static (io.ReadCloser, error) ReadZip(this ptr<vcsRepo> _addr_r, @string rev, @string subdir, long maxSize) => func((defer, _, __) =>
        {
            ...
            var (f, err) = ioutil.TempFile("", "go-readzip-*.zip");
            if (err != null)
            {
                return (null, error.As(err)!);
            }
        ...
        private static error Close(this ptr<deleteCloser> _addr_d) => func((defer, _, __) =>
        {
        ...
        private static (ptr<RevInfo>, error) bzrParseStat(@string rev, @string @out)
        {
            ...
            foreach (var (_, line) in strings.Split(out, "\n"))
            {
                ...
                var key = line[..i];
                var val = strings.TrimSpace(line[i + 1L..]);
                switch (key)
                {
                    case "revno": 
                        {
                        ...
                        var (i, err) = strconv.ParseInt(val, 10L, 64L);
                        if (err != null)
                        {
                    ...
                    case "timestamp": 
                        ...
                        var (t, err) = time.Parse("2006-01-02 15:04:05 -0700", val[j + 1L..]);
                        if (err != null)
                        {
                            return (_addr_null!, error.As(vcsErrorf("unexpected timestamp from bzr log: %q", line))!);
                        }
        ...
        private static (ptr<RevInfo>, error) fossilParseStat(@string rev, @string @out)
        {
            ...
            foreach (var (_, line) in strings.Split(out, "\n"))
            {
                if (strings.HasPrefix(line, "uuid:"))
                {
                    ...
                    if (len(f) != 5L || len(f[1L]) != 40L || f[4L] != "UTC")
                    {
                        return (_addr_null!, error.As(vcsErrorf("unexpected response from fossil info: %q", line))!);
                    }

                    var (t, err) = time.Parse("2006-01-02 15:04:05", f[2L] + " " + f[3L]);
                    if (err != null)
                    {
                        return (_addr_null!, error.As(vcsErrorf("unexpected response from fossil info: %q", line))!);
                    }
        ...
        private static (long, error) Write(this ptr<limitedWriter> _addr_l, slice<byte> p)
        {
            long n = default;
            error err = default!;
