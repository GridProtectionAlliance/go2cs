...
// but they are not recorded when talking about module versions.
        ...
        public static (Repo, error) Lookup(@string proxy, @string path) => func((defer, _, __) =>
        {
            Repo _p0 = default;
            error _p0 = default!;
            ...
            cached c = lookupCache.Do(new lookupCacheKey(proxy,path), () =>
            {
                ...
                return new cached(r,err);

            })._<cached>();

            return (c.r, error.As(c.err)!);
        ...
        private static (Repo, error) lookup(@string proxy, @string path)
        {
            ...
            {
                case "off": 
                    return (null, error.As(errProxyOff)!);
                    break;
                case "direct": 
                    return lookupDirect(path);
                    break;
        ...
        private static (Repo, error) lookupDirect(@string path)
        {
            ...
            var (rr, err) = get.RepoRootForImportPath(path, get.PreferMod, security);
            if (err != null)
            { 
        ...
        private static (codehost.Repo, error) lookupCodeRepo(ptr<get.RepoRoot> _addr_rr)
        {
            codehost.Repo _p0 = default;
            error _p0 = default!;
            ref get.RepoRoot rr = ref _addr_rr.val;

            var (code, err) = codehost.NewRepo(rr.VCS, rr.Repo);
            if (err != null)
            {
                {
        ...
        public static (Repo, ptr<RevInfo>, error) ImportRepoRev(@string path, @string rev)
        {
            Repo _p0 = default;
            ...
            {
                return (null, _addr_null!, error.As(fmt.Errorf("repo version lookup disabled by -mod=%s", cfg.BuildMod))!);
            } 
            ...
            var (rr, err) = get.RepoRootForImportPath(path, get.IgnoreMod, security);
            if (err != null)
            {
            ...
            var (repo, err) = newCodeRepo(code, rr.Root, rr.Root);
            if (err != null)
            {
                return (null, _addr_null!, error.As(err)!);
            }

            ptr<codeRepo> (info, err) = repo._<ptr<codeRepo>>().convert(revInfo, rev);
            if (err != null)
            {
                return (null, _addr_null!, error.As(err)!);
            }

            return (repo, _addr_info!, error.As(null!)!);

        }

        public static void SortVersions(slice<@string> list)
        {
            sort.Slice(list, (i, j) =>
            {
                var cmp = semver.Compare(list[i], list[j]);
                if (cmp != 0L)
                {
                    return cmp < 0L;
                }
        ...
        private static (slice<@string>, error) Versions(this ptr<loggingRepo> _addr_l, @string prefix) => func((defer, _, __) =>
        {
        ...
        private static (ptr<RevInfo>, error) Stat(this ptr<loggingRepo> _addr_l, @string rev) => func((defer, _, __) =>
        {
            ptr<RevInfo> _p0 = default!;
            error _p0 = default!;
            ref loggingRepo l = ref _addr_l.val;
        ...
        private static (ptr<RevInfo>, error) Latest(this ptr<loggingRepo> _addr_l) => func((defer, _, __) =>
        {
            ptr<RevInfo> _p0 = default!;
            error _p0 = default!;
            ref loggingRepo l = ref _addr_l.val;
        ...
        private static (slice<byte>, error) GoMod(this ptr<loggingRepo> _addr_l, @string version) => func((defer, _, __) =>
        {
        ...
        private static error Zip(this ptr<loggingRepo> _addr_l, io.Writer dst, @string version) => func((defer, _, __) =>
        {
            ref loggingRepo l = ref _addr_l.val;
