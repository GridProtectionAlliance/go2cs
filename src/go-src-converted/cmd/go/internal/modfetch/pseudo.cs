...
namespace @internal
{
    ...
    "9A-Za-z-]+)*)?$");
        ...
        private static readonly @string pseudoVersionTimestampFormat = (@string)"20060102150405";
        ...
        { 
            ...
            if (i == 0L && digits[i] == '1' && len(digits) > 1L)
            {
                digits = digits[1L..];
            }
            else
        ...
        {
            return strings.Count(v, "-") >= 2L && semver.IsValid(v) && pseudoVersionRE.MatchString(v);
        }
        ...
        public static (time.Time, error) PseudoVersionTime(@string v)
        {
            ...
            if (err != null)
            {
                return (new time.Time(), error.As(addr(new module.InvalidVersionError(Version:v,Pseudo:true,Err:fmt.Errorf("malformed time %q",timestamp),))!)!);
            }
        ...
        public static (@string, error) PseudoVersionBase(@string v) => func((_, panic, __) =>
        {
            @string _p0 = default;
            error _p0 = default!;
                ...
                switch (pre)
                {
                    case "": 
                        ...
                        { 
                            ...
                            return ("", error.As(addr(new module.InvalidVersionError(Version:v,Pseudo:true,Err:fmt.Errorf("lacks base version, but has build metadata %q",build),))!)!);
                    ...
                    case "-0": 
                        ...
                        var patch = decDecimal(base[i + 1L..]);
                        if (patch == "")
                        { 
                            ...
                            // reject them.
                            return ("", error.As(addr(new module.InvalidVersionError(Version:v,Pseudo:true,Err:fmt.Errorf("version before %s would have negative patch number",base),))!)!);

                        }

                        return (base[..i + 1L] + patch + build, error.As(null!)!);
                        break;
                    default: 
        ...
        private static (@string, @string, @string, @string, error) parsePseudoVersion(@string v)
        {
            ...
            {
                return ("", "", "", "", error.As(addr(new module.InvalidVersionError(Version:v,Pseudo:true,Err:errPseudoSyntax,))!)!);
            }
