...
private static (slice<@string>, error) appendIncompatibleVersions(this ptr<codeRepo> _addr_r, slice<@string> list, slice<@string> incompatible) {
    ...
    if (len(list) > 0) {
        var (ok, err) = versionHasGoMod(list[len(list) - 1]);
        if (err != null) {
...
private static (ptr<RevInfo>, error) convert(this ptr<codeRepo> _addr_r, ptr<codehost.RevInfo> _addr_info, @string statVers) {
                ...
                else
 {
    ...
    var (isRetracted, err) = r.retractedVersions();
    if (err != null) {
        isRetracted = _p0 => _addr_false!;
    }
    Func<@string, (@string, bool)> tagToVersion = tag => {
        if (!strings.HasPrefix(tag, tagPrefix)) {
            return (_addr_""!, error.As(false)!);
        }
        var trimmed = tag[(int)len(tagPrefix)..]; 
...
// have trivial bases!
private static error validatePseudoVersion(this ptr<codeRepo> _addr_r, ptr<codehost.RevInfo> _addr_info, @string version) => func((defer, _, _) => {
    error err = default!;
    ref codeRepo r = ref _addr_r.val;
    ref codehost.RevInfo info = ref _addr_info.val;
    ...
    if (!t.Equal(info.Time.Truncate(time.Second))) {
        return error.As(fmt.Errorf("does not match version-control timestamp (expected %s)", info.Time.UTC().Format(module.PseudoVersionTimestampFormat)))!;
    }
    @string tagPrefix = "";
    if (r.codeDir != "") {
        tagPrefix = r.codeDir + "/";
    }
    var (base, err) = module.PseudoVersionBase(strings.TrimSuffix(version, "+incompatible"));
    if (err != null) {
        return error.As(err)!;
    }
    if (base == "") {
...
private static @string revToRev(this ptr<codeRepo> _addr_r, @string rev) {
    ...
    if (semver.IsValid(rev)) {
        if (module.IsPseudoVersion(rev)) {
            var (r, _) = module.PseudoVersionRev(rev);
            return r;
        }
        if (semver.Build(rev) == "+incompatible") {
            rev = rev[..(int)len(rev) - len("+incompatible")];
        }
        if (r.codeDir == "") {
            return rev;
        }
...
private static (@string, @string, slice<byte>, error) findDir(this ptr<codeRepo> _addr_r, @string version) {
    ...
    if (r.pathMajor != "" && r.codeRoot != r.modPath && !strings.HasPrefix(r.pathMajor, ".")) { 
        ...
        // a replace directive.
        var dir2 = path.Join(r.codeDir, r.pathMajor[(int)1..]);
...
public static slice<byte> LegacyGoMod(@string modPath) {
...
private static (Func<@string, bool>, error) retractedVersions(this ptr<codeRepo> _addr_r) {
    Func<@string, bool> _p0 = default;
    error _p0 = default!;
    ref codeRepo r = ref _addr_r.val;

    var (versions, err) = r.Versions("");
    if (err != null) {
        return (null, error.As(err)!);
    }
    {
        ...
        foreach (var (__i, __v) in versions) {
            i = __i;
            v = __v;
            if (strings.HasSuffix(v, "+incompatible")) {
                versions = versions[..(int)i];
                break;
            }
        }
        i = i__prev1;
    ...
    if (len(versions) == 0) {
        return (_p0 => false, error.As(null!)!);
    }
    @string highest = default;
    {
        var i__prev1 = i;

        for (var i = len(versions) - 1; i >= 0; i--) {
            var v = versions[i];
    ...
    var (data, err) = r.GoMod(highest);
    if (err != null) {
        return (null, error.As(err)!);
    }
    var (f, err) = modfile.ParseLax("go.mod", data, null);
    if (err != null) {
        return (null, error.As(err)!);
    }
    var retractions = make_slice<modfile.VersionInterval>(len(f.Retract));
    {
        var r__prev1 = r;
    ...
    return (v => {
        {
            ...
            foreach (var (_, __r) in retractions) {
                r = __r;
...
private static error Zip(this ptr<codeRepo> _addr_r, io.Writer dst, @string version) => func((defer, _, _) => {
    ...
    // Spool to local file.
    var (f, err) = os.CreateTemp("", "go-codehost-");
    if (err != null) {
        dl.Close();
        return error.As(err)!;
    }
...
private static (fs.FileInfo, error) Lstat(this zipFile f) {
...
private static (fs.FileInfo, error) Lstat(this dataFile f) {
    fs.FileInfo _p0 = default;
    error _p0 = default!;
...
private static (io.ReadCloser, error) Open(this dataFile f) {
    ...
    return (io.NopCloser(bytes.NewReader(f.data)), error.As(null!)!);
}
...
private static fs.FileMode Mode(this dataFileInfo fi) {
    return 0644;
}
