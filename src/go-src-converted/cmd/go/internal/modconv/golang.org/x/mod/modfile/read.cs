...
// package modfile -- go2cs converted at 2020 October 09 05:46:43 UTC
// import "golang.org/x/mod/modfile" ==> using modfile = go.golang.org.x.mod.modfile_package
// Original source: C:\Users\ritchie\go\src\golang.org\x\mod\modfile\read.go
...
namespace mod
{
    public static partial class modfile_package
    {
        ...
        {
            public long Line; // line in input (starting at 1)
            public long LineRune; // rune in line (starting at 1)
            public long Byte; // byte in input (starting at 0)
        }
...
// comments following the expression.
        ...
        private static (Position, Position) Span(this ptr<FileSyntax> _addr_x)
        {
            ...
            start, _ = x.Stmt[0L].Span();
            _, end = x.Stmt[len(x.Stmt) - 1L].Span();
            return (start, end);
        }
...
Loop:
                {
                    ...
                    for (var i = len(x.Stmt) - 1L; i >= 0L; i--)
                    {
                        var stmt = x.Stmt[i];
                        switch (stmt.type())
                        {
                            case ptr<Line> stmt:
                                if (stmt.Token != null && stmt.Token[0L] == tokens[0L])
                                {
                                    hint = stmt;
                                    _breakLoop = true;
                                    break;
            ...
            Func<long, ptr<Line>> newLineAfter = i =>
            {
...
;
                            ...
                            case ptr<LineBlock> stmt:
                                ...
                                foreach (var (j, line) in stmt.Line)
                                {
                                    if (line == hint)
                                    {
                                        ...
                                        copy(stmt.Line[j + 2L..], stmt.Line[j + 1L..]);
                                        @new = addr(new Line(Token:tokens[1:],InBlock:true));
                                        stmt.Line[j + 1L] = new;
                                        return _addr_new!;
                                    }
        ...
        private static void removeLine(this ptr<FileSyntax> _addr_x, ptr<Line> _addr_line)
        {
            ref FileSyntax x = ref _addr_x.val;
            ref Line line = ref _addr_line.val;
...
// Comment assignment state.
        ...
        private static (ptr<FileSyntax>, error) parse(@string file, slice<byte> data) => func((defer, _, __) =>
        {
            ...
            defer(() =>
            {
                ...
                if (err == null && len(@in.parseErrors) > 0L)
                {
                    err = @in.parseErrors;
                }
            }()); 
        ...
        private static void Error(this ptr<input> _addr_@in, @string s) => func((_, panic, __) =>
        {
        ...
        private static bool eof(this ptr<input> _addr_@in)
        {
            ...
            return len(@in.remaining) == 0L;
        }

        // peekRune returns the next rune in the input without consuming it.
        private static long peekRune(this ptr<input> _addr_@in)
        {
            ref input @in = ref _addr_@in.val;

            if (len(@in.remaining) == 0L)
            {
                return 0L;
            }
        ...
        private static long readRune(this ptr<input> _addr_@in)
        {
            ref input @in = ref _addr_@in.val;
        ...
        private partial struct tokenKind // : long
        {
        }
        ...
        private static void endToken(this ptr<input> _addr_@in, tokenKind kind)
        {
            ...
            @in.token.kind = kind;
            var text = string(@in.tokenStart[..len(@in.tokenStart) - len(@in.remaining)]);
            @in.token.text = text;
        ...
        private static void readToken(this ptr<input> _addr_@in)
        {
            ...
            while (!@in.eof())
            {
                ...
                if (@in.peekPrefix("//"))
                {
                    ...
                    var i = bytes.LastIndex(@in.complete[..@in.pos.Byte], (slice<byte>)"\n");
                    var suffix = len(bytes.TrimSpace(@in.complete[i + 1L..@in.pos.Byte])) > 0L;
                    @in.readRune();
                    @in.readRune(); 

                    // Consume comment.
                    while (len(@in.remaining) > 0L && @in.readRune() != '\n')
                    {
                    } 
        ...
        private static bool isIdent(long c)
        {
            {
                var r = rune(c);
        ...
        private static void order(this ptr<input> _addr_@in, Expr x) => func((_, panic, __) =>
        {
            ref input @in = ref _addr_@in.val;
        ...
        private static void assignComments(this ptr<input> _addr_@in)
        {
            ...
            // Assign line comments to syntax immediately following.
            {
                ...
                foreach (var (_, __x) in @in.pre)
                {
                    ...
                    var xcom = x.Comment();
                    while (len(line) > 0L && start.Byte >= line[0L].Start.Byte)
                    {
                        if (debug)
                        {
                            fmt.Fprintf(os.Stderr, "ASSIGN LINE %q #%d\n", line[0L].Token, line[0L].Start.Byte);
                        }
                        xcom.Before = append(xcom.Before, line[0L]);
                        line = line[1L..];
                    }
                } 
            ...
            for (var i = len(@in.post) - 1L; i >= 0L; i--)
            {
                var x = @in.post[i];
                ...
                xcom = x.Comment();
                while (len(suffix) > 0L && end.Byte <= suffix[len(suffix) - 1L].Start.Byte)
                {
                    if (debug)
                    {
                        fmt.Fprintf(os.Stderr, "ASSIGN SUFFIX %q #%d\n", suffix[len(suffix) - 1L].Token, suffix[len(suffix) - 1L].Start.Byte);
                    }
                    xcom.Suffix = append(xcom.Suffix, suffix[len(suffix) - 1L]);
                    suffix = suffix[..len(suffix) - 1L];
                }
            } 
        ...
        private static void parseFile(this ptr<input> _addr_@in)
        {
            ...
            while (true)
            {
                ...
                if (@in.peek() == '\n') 
                    ...
                    if (cb != null)
                    {
                        @in.file.Stmt[len(@in.file.Stmt) - 1L].Comment().Before = cb.Before;
                        cb = null;
                    }
                            }
        }
        ...
        private static ptr<LineBlock> parseLineBlock(this ptr<input> _addr_@in, Position start, slice<@string> token, token lparen)
        {
            ...
            while (true)
            {
                ...
                if (@in.peek() == _EOLCOMMENT) 
                    ...
                    @in.lex();
                    if (len(comments) == 0L && len(x.Line) > 0L || len(comments) > 0L && comments[len(comments) - 1L].Token != "")
                    {
