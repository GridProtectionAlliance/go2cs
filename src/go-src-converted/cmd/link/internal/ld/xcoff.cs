...
namespace @internal
{
    public static partial class ld_package
    {
        ...
        // in the symbol table.
        private static void xcoffUpdateOuterSize2(ptr<Link> _addr_ctxt, long size, sym.SymKind stype)
        {
            ref Link ctxt = ref _addr_ctxt.val;
            ...
            if (stype == sym.STYPERELRO)
            {
                ...
                { 
                    ...
                    var tsize = ldr.SymSize(ldr.Lookup("runtime.types", 0L));
            ...
            if (fallthrough || stype == sym.STYPE)
            {
                ...
                { 
                    // runtime.types size must be removed, as it's a real symbol.
                    tsize = ldr.SymSize(ldr.Lookup("runtime.types", 0L));
        ...
        private static void putaixsym(ptr<Link> _addr_ctxt, ptr<sym.Symbol> _addr_x, @string str, SymbolType t, long addr)
        {
            ref Link ctxt = ref _addr_ctxt.val;
            ref sym.Symbol x = ref _addr_x.val;
        ...
        private static void adddynimpsym(this ptr<xcoffFile> _addr_f, ptr<Link> _addr_ctxt, loader.Sym s)
        {
            ...
            var extsym = ldr.CreateSymForUpdate(ldr.SymExtname(s), 0L);
            ...
            sb.AddBytes(make_slice<byte>(8L));
            sb.AddReloc(new loader.Reloc(Off:0,Size:uint8(ctxt.Arch.PtrSize),Type:objabi.R_ADDR,Sym:extsym.Sym())); 
        ...
        public static bool Xcoffadddynrel2(ptr<Target> _addr_target, ptr<loader.Loader> _addr_ldr, ptr<ArchSyms> _addr_syms, loader.Sym s, loader.Reloc2 r, long rIdx)
        {
            ...
            ptr<xcoffLoaderReloc> xldr = addr(new xcoffLoaderReloc(sym2:s,roff:r.Off(),));
            var targ = r.Sym();
        ...
        private static void doxcoff(this ptr<Link> _addr_ctxt) => func((_, panic, __) =>
        {
            ref Link ctxt = ref _addr_ctxt.val;
            ...
            { 
                ...
                var rt0 = ldr.Lookup("runtime.rt0_go", 0L);
                ...
                var nsym = loader.Sym(ldr.NSym());
                {
                    ...
                    for (s = loader.Sym(1L); s < nsym; s++)
                    {
                        ...
                        { 
                            ...
                            var desc = ldr.MakeSymbolUpdater(ldr.CreateExtSym(name, 0L));
        ...
        private static void writeLdrScn(this ptr<xcoffFile> _addr_f, ptr<Link> _addr_ctxt, ulong globalOff)
        {
            ...
            ptr<XcoffLdRel64> xldr = addr(new XcoffLdRel64(Lvaddr:uint64(ep.Value),Lrtype:0x3F00,Lrsecnm:f.getXCOFFscnum(ep.Sect),Lsymndx:0,));
            off += 16L;
            ...
            off += uint64(16L * len(f.loaderReloc));
            {
                ...
                foreach (var (_, __r) in f.loaderReloc)
                {
                    ...
                    xldr = addr(new XcoffLdRel64(Lvaddr:uint64(symp.Value+int64(r.roff)),Lrtype:r.rtype,Lsymndx:r.symndx,));
        ...
        private static void emitRelocations(this ptr<xcoffFile> _addr_f, ptr<Link> _addr_ctxt, long fileoff) => func((_, panic, __) =>
        {
            ref xcoffFile f = ref _addr_f.val;
            ref Link ctxt = ref _addr_ctxt.val;
                    ...
                    {
                        {
                            ...
                            foreach (var (_, __sect) in seg.Sections)
                            {
                                ...
                                {
                                    n += relocsect(sect, ctxt.datap, 0L);
                                }
