...
// package lex -- go2cs converted at 2020 August 29 08:48:51 UTC
...
namespace @internal
{
    public static partial class lex_package
    {
        ...
        public partial struct Input
        {
            ...
            public map<@string, ref Macro> macros;
        ...
        public static ref Input NewInput(@string name)
        {
            return ref new Input(includes:append([]string{filepath.Dir(name)},flags.I...),beginningOfLine:true,macros:predefine(flags.D),);
        }

        // predefine installs the macros set by the -D flag on the command line.
        private static map<@string, ref Macro> predefine(flags.MultiFlag defines)
        {
            var macros = make_map<@string, ref Macro>();
            foreach (var (_, name) in defines)
            {
                ...
                macros[name] = ref new Macro(name:name,args:nil,tokens:Tokenize(value),);
            }
            return macros;
        }
        ...
        private static void Error(this ref Input _@in, params object[] args) => func(_@in, (ref Input @in, Defer _, Panic panic, Recover __) =>
        {
            if (panicOnError)
            {
        ...
        private static void expectText(this ref Input @in, params object[] args)
        {
            @in.Error(append(args, "; got", strconv.Quote(@in.Stack.Text())));
        }

        // enabled reports whether the input is enabled by an ifdef, or is at the top level.
        private static bool enabled(this ref Input @in)
        {
            return len(@in.ifdefStack) == 0L || @in.ifdefStack[len(@in.ifdefStack) - 1L];
        }

        private static void expectNewline(this ref Input @in, @string directive)
        {
        ...
        private static ScanToken Next(this ref Input @in)
        {
            if (@in.peek)
            {
                @in.peek = false;
        ...
        private static @string Text(this ref Input @in)
        {
            return @in.text;
        }

        // hash processes a # preprocessor directive. It returns true iff it completes.
        private static bool hash(this ref Input @in)
        { 
        ...
        private static @string macroName(this ref Input @in)
        { 
        ...
        // #define processing.
        private static void define(this ref Input @in)
        {
        ...
        private static void defineMacro(this ref Input @in, @string name, slice<@string> args, slice<Token> tokens)
        {
            if (@in.macros[name] != null)
            {
                @in.Error("redefinition of macro:", name);
            }
            @in.macros[name] = ref new Macro(name:name,args:args,tokens:tokens,);
        }
        ...
        private static (slice<@string>, slice<Token>) macroDefinition(this ref Input @in, @string name)
        {
...
Loop:
        ...
        private static void invokeMacro(this ref Input @in, ref Macro macro)
        { 
        ...
        private static map<@string, slice<Token>> argsFor(this ref Input @in, ref Macro macro)
        {
        ...
        private static (slice<Token>, ScanToken) collectArgument(this ref Input @in, ref Macro macro)
        {
            long nesting = 0L;
            slice<Token> tokens = default;
            while (true)
        ...
        private static void ifdef(this ref Input @in, bool truth)
        {
            var name = @in.macroName();
        ...
        // #else processing
        private static void else_(this ref Input @in)
        {
        ...
        // #endif processing.
        private static void endif(this ref Input @in)
        {
        ...
        // #include processing.
        private static void include(this ref Input @in)
        { 
        ...
        // #line processing.
        private static void line(this ref Input @in)
        { 
            ...
            var pos = src.MakePos(@in.Base(), uint(@in.Line()), uint(@in.Col()));
            @in.Stack.SetBase(src.NewLinePragmaBase(pos, file, objabi.AbsFile(objabi.WorkingDir(), file, flags.TrimPath.Value), uint(line)));
        }

        // #undef processing
        private static void undef(this ref Input @in)
        {
            var name = @in.macroName();
            if (@in.macros[name] == null)
            {
        ...
        private static void Push(this ref Input @in, TokenReader r)
        {
            if (len(@in.tr) > 100L)
            {
        ...
        private static void Close(this ref Input @in)
        {
        }
    }
}}}}
