...
// Copy itself returns either a copy or nil.
    ...
    @string GoString();
    @string goString(nint indent, @string field);
}
...
private static @string goString(this ptr<Name> _addr_n, nint indent, @string field) {
    ref Name n = ref _addr_n.val;
...
private static void print(this ptr<Typed> _addr_t, ptr<printState> _addr_ps) => func((defer, _, _) => {
...
private static @string goString(this ptr<Typed> _addr_t, nint indent, @string field) {
    ref Typed t = ref _addr_t.val;

    return fmt.Sprintf("%*s%sTyped:\n%s\n%s", indent, "", field, t.Name.goString(indent + 2, "Name: "), t.Type.goString(indent + 2, "Type: "));
}
...
private static @string goString(this ptr<Qualified> _addr_q, nint indent, @string field) {
    ref Qualified q = ref _addr_q.val;
...
private static void print(this ptr<Template> _addr_t, ptr<printState> _addr_ps) => func((defer, _, _) => {
...
private static @string goString(this ptr<Template> _addr_t, nint indent, @string field) {
    ref Template t = ref _addr_t.val;
...
private static void print(this ptr<TemplateParam> _addr_tp, ptr<printState> _addr_ps) => func((_, panic, _) => {
...
private static @string goString(this ptr<TemplateParam> _addr_tp, nint indent, @string field) {
    ref TemplateParam tp = ref _addr_tp.val;
...
private static void print(this ptr<LambdaAuto> _addr_la, ptr<printState> _addr_ps) {
    ref LambdaAuto la = ref _addr_la.val;
    ref printState ps = ref _addr_ps.val;
...
private static AST Copy(this ptr<LambdaAuto> _addr_la, Func<AST, AST> fn, Func<AST, bool> skip) {
    ref LambdaAuto la = ref _addr_la.val;
...
private static @string goString(this ptr<LambdaAuto> _addr_la, nint indent, @string field) {
    ref LambdaAuto la = ref _addr_la.val;
...
public partial struct Qualifiers {
    public slice<AST> Qualifiers;
}

private static void print(this ptr<Qualifiers> _addr_qs, ptr<printState> _addr_ps) {
    ref Qualifiers qs = ref _addr_qs.val;
    ref printState ps = ref _addr_ps.val;

    var first = true;
    foreach (var (_, q) in qs.Qualifiers) {
        if (!first) {
            ps.writeByte(' ');
        }
        q.print(ps);
...
private static bool Traverse(this ptr<Qualifiers> _addr_qs, Func<AST, bool> fn) {
    ref Qualifiers qs = ref _addr_qs.val;
...
private static AST Copy(this ptr<Qualifiers> _addr_qs, Func<AST, AST> fn, Func<AST, bool> skip) {
    ref Qualifiers qs = ref _addr_qs.val;
    ...
    var changed = false;
    var qualifiers = make_slice<AST>(len(qs.Qualifiers));
    foreach (var (i, q) in qs.Qualifiers) {
        var qc = q.Copy(fn, skip);
        if (qc == null) {
            qualifiers[i] = q;
        }
        else
 {
    ...
    qs = addr(new Qualifiers(Qualifiers:qualifiers));
    {
        var r = fn(qs);
...
private static @string goString(this ptr<Qualifiers> _addr_qs, nint indent, @string field) {
    ref Qualifiers qs = ref _addr_qs.val;

    var quals = fmt.Sprintf("%*s%s", indent, "", field);
    foreach (var (_, q) in qs.Qualifiers) {
        quals += "\n";
...
public partial struct Qualifier {
    public @string Name; // qualifier name: const, volatile, etc.
    public slice<AST> Exprs; // can be non-nil for noexcept and throw
}

private static void print(this ptr<Qualifier> _addr_q, ptr<printState> _addr_ps) {
    ref Qualifier q = ref _addr_q.val;
    ref printState ps = ref _addr_ps.val;
...
private static bool Traverse(this ptr<Qualifier> _addr_q, Func<AST, bool> fn) {
    ref Qualifier q = ref _addr_q.val;
...
private static AST Copy(this ptr<Qualifier> _addr_q, Func<AST, AST> fn, Func<AST, bool> skip) {
    ref Qualifier q = ref _addr_q.val;

    if (skip(q)) {
        return null;
    }
    var exprs = make_slice<AST>(len(q.Exprs));
    var changed = false;
    foreach (var (i, e) in q.Exprs) {
        ...
        else
 {
    ...
    q = addr(new Qualifier(Name:q.Name,Exprs:exprs));
    {
        var r = fn(q);
...
private static @string goString(this ptr<Qualifier> _addr_q, nint indent, @string field) {
    ref Qualifier q = ref _addr_q.val;

    var qs = fmt.Sprintf("%*s%s%s", indent, "", field, q.Name);
    if (len(q.Exprs) > 0) {
        foreach (var (i, e) in q.Exprs) {
            qs += "\n";
...
private static AST Copy(this ptr<TypeWithQualifiers> _addr_twq, Func<AST, AST> fn, Func<AST, bool> skip) {
    ...
    var quals = twq.Qualifiers.Copy(fn, skip);
    if (base == null && quals == null) {
        return fn(twq);
    }
    if (base == null) {
        base = twq.Base;
    ...
    twq = addr(new TypeWithQualifiers(Base:base,Qualifiers:quals));
    {
        var r = fn(twq);
...
private static @string goString(this ptr<TypeWithQualifiers> _addr_twq, nint indent, @string field) {
    ref TypeWithQualifiers twq = ref _addr_twq.val;

    return fmt.Sprintf("%*s%sTypeWithQualifiers:\n%s\n%s", indent, "", field, twq.Qualifiers.goString(indent + 2, "Qualifiers: "), twq.Base.goString(indent + 2, "Base: "));
}
...
private static AST Copy(this ptr<MethodWithQualifiers> _addr_mwq, Func<AST, AST> fn, Func<AST, bool> skip) {
    ...
    AST quals = default!;
    if (mwq.Qualifiers != null) {
        quals = AST.As(mwq.Qualifiers.Copy(fn, skip))!;
    }
    if (method == null && quals == null) {
        return fn(mwq);
    }
    if (method == null) {
        method = mwq.Method;
    ...
    if (quals == null) {
        quals = AST.As(mwq.Qualifiers)!;
    }
    mwq = addr(new MethodWithQualifiers(Method:method,Qualifiers:quals,RefQualifier:mwq.RefQualifier));
    {
        var r = fn(mwq);
...
private static @string goString(this ptr<MethodWithQualifiers> _addr_mwq, nint indent, @string field) {
    ref MethodWithQualifiers mwq = ref _addr_mwq.val;
...
private static @string goString(this ptr<BuiltinType> _addr_bt, nint indent, @string field) {
    ref BuiltinType bt = ref _addr_bt.val;
...
private static @string goString(this ptr<PointerType> _addr_pt, nint indent, @string field) {
    ref PointerType pt = ref _addr_pt.val;

    return fmt.Sprintf("%*s%sPointerType:\n%s", indent, "", field, pt.Base.goString(indent + 2, ""));
}
...
private static @string goString(this ptr<ReferenceType> _addr_rt, nint indent, @string field) {
    ref ReferenceType rt = ref _addr_rt.val;

    return fmt.Sprintf("%*s%sReferenceType:\n%s", indent, "", field, rt.Base.goString(indent + 2, ""));
}
...
private static @string goString(this ptr<RvalueReferenceType> _addr_rt, nint indent, @string field) {
    ref RvalueReferenceType rt = ref _addr_rt.val;
...
private static @string goString(this ptr<ComplexType> _addr_ct, nint indent, @string field) {
    ref ComplexType ct = ref _addr_ct.val;

    return fmt.Sprintf("%*s%sComplexType:\n%s", indent, "", field, ct.Base.goString(indent + 2, ""));
}
...
private static @string goString(this ptr<ImaginaryType> _addr_it, nint indent, @string field) {
    ref ImaginaryType it = ref _addr_it.val;

    return fmt.Sprintf("%*s%sImaginaryType:\n%s", indent, "", field, it.Base.goString(indent + 2, ""));
}
...
private static @string goString(this ptr<VendorQualifier> _addr_vq, nint indent, @string field) {
    ref VendorQualifier vq = ref _addr_vq.val;

    return fmt.Sprintf("%*s%sVendorQualifier:\n%s\n%s", indent, "", field, vq.Qualifier.goString(indent + 2, "Qualifier: "), vq.Type.goString(indent + 2, "Type: "));
}
...
private static void printDimension(this ptr<ArrayType> _addr_at, ptr<printState> _addr_ps) {
    ...
    while (len(ps.inner) > 0) { 
        ...
        // array
        var @in = ps.inner[len(ps.inner) - 1];
        {
...
private static @string goString(this ptr<ArrayType> _addr_at, nint indent, @string field) {
    ref ArrayType at = ref _addr_at.val;

    return fmt.Sprintf("%*s%sArrayType:\n%s\n%s", indent, "", field, at.Dimension.goString(indent + 2, "Dimension: "), at.Element.goString(indent + 2, "Element: "));
}
...
private static void printArgs(this ptr<FunctionType> _addr_ft, ptr<printState> _addr_ps) {
    ...
    var paren = false;
    var space = false;
    for (var i = len(ps.inner) - 1; i >= 0; i--) {
...
private static @string goString(this ptr<FunctionType> _addr_ft, nint indent, @string field) {
    ref FunctionType ft = ref _addr_ft.val;
...
private static @string goString(this ptr<FunctionParam> _addr_fp, nint indent, @string field) {
    ref FunctionParam fp = ref _addr_fp.val;
...
private static @string goString(this ptr<PtrMem> _addr_pm, nint indent, @string field) {
    ref PtrMem pm = ref _addr_pm.val;

    return fmt.Sprintf("%*s%sPtrMem:\n%s\n%s", indent, "", field, pm.Class.goString(indent + 2, "Class: "), pm.Member.goString(indent + 2, "Member: "));
}
...
private static @string goString(this ptr<FixedType> _addr_ft, nint indent, @string field) {
    ref FixedType ft = ref _addr_ft.val;

    return fmt.Sprintf("%*s%sFixedType: Accum: %t; Sat: %t\n%s", indent, "", field, ft.Accum, ft.Sat, ft.Base.goString(indent + 2, "Base: "));
}
...
private static @string goString(this ptr<VectorType> _addr_vt, nint indent, @string field) {
    ref VectorType vt = ref _addr_vt.val;

    return fmt.Sprintf("%*s%sVectorType:\n%s\n%s", indent, "", field, vt.Dimension.goString(indent + 2, "Dimension: "), vt.Base.goString(indent + 2, "Base: "));
}
...
private static @string goString(this ptr<Decltype> _addr_dt, nint indent, @string field) {
    ref Decltype dt = ref _addr_dt.val;

    return fmt.Sprintf("%*s%sDecltype:\n%s", indent, "", field, dt.Expr.goString(indent + 2, "Expr: "));
}
...
private static @string goString(this ptr<Operator> _addr_op, nint indent, @string field) {
    ref Operator op = ref _addr_op.val;
...
private static @string goString(this ptr<Constructor> _addr_c, nint indent, @string field) {
    ref Constructor c = ref _addr_c.val;
...
private static @string goString(this ptr<Destructor> _addr_d, nint indent, @string field) {
    ref Destructor d = ref _addr_d.val;
...
private static @string goString(this ptr<GlobalCDtor> _addr_gcd, nint indent, @string field) {
    ref GlobalCDtor gcd = ref _addr_gcd.val;

    return fmt.Sprintf("%*s%sGlobalCDtor: Ctor: %t\n%s", indent, "", field, gcd.Ctor, gcd.Key.goString(indent + 2, "Key: "));
}
...
private static @string goString(this ptr<TaggedName> _addr_t, nint indent, @string field) {
    ref TaggedName t = ref _addr_t.val;

    return fmt.Sprintf("%*s%sTaggedName:\n%s\n%s", indent, "", field, t.Name.goString(indent + 2, "Name: "), t.Tag.goString(indent + 2, "Tag: "));
}
...
private static @string goString(this ptr<PackExpansion> _addr_pe, nint indent, @string field) {
    ref PackExpansion pe = ref _addr_pe.val;

    return fmt.Sprintf("%*s%sPackExpansion: Pack: %p\n%s", indent, "", field, pe.Pack, pe.Base.goString(indent + 2, "Base: "));
}
...
private static @string goString(this ptr<ArgumentPack> _addr_ap, nint indent, @string field) {
    ref ArgumentPack ap = ref _addr_ap.val;
...
private static @string goString(this ptr<SizeofPack> _addr_sp, nint indent, @string field) {
    ref SizeofPack sp = ref _addr_sp.val;
...
private static @string goString(this ptr<SizeofArgs> _addr_sa, nint indent, @string field) {
    ref SizeofArgs sa = ref _addr_sa.val;
...
private static @string goString(this ptr<Cast> _addr_c, nint indent, @string field) {
    ref Cast c = ref _addr_c.val;

    return fmt.Sprintf("%*s%sCast\n%s", indent, "", field, c.To.goString(indent + 2, "To: "));
}
...
private static @string goString(this ptr<Nullary> _addr_n, nint indent, @string field) {
    ref Nullary n = ref _addr_n.val;

    return fmt.Sprintf("%*s%sNullary:\n%s", indent, "", field, n.Op.goString(indent + 2, "Op: "));
}
...
private static @string goString(this ptr<Unary> _addr_u, nint indent, @string field) {
    ref Unary u = ref _addr_u.val;
...
// initializer.
private static bool isDesignatedInitializer(AST x) {
    switch (x.type()) {
        case ptr<Binary> x:
            {
                ptr<Operator> op__prev1 = op;

                ptr<Operator> (op, ok) = x.Op._<ptr<Operator>>();
    ...
    if (op != null && op.Name == "()") {
        {
            ...
            if (ok) {
                {
                    ptr<FunctionType> (ft, ok) = ty.Type._<ptr<FunctionType>>();
...
private static @string goString(this ptr<Binary> _addr_b, nint indent, @string field) {
    ref Binary b = ref _addr_b.val;

    return fmt.Sprintf("%*s%sBinary:\n%s\n%s\n%s", indent, "", field, b.Op.goString(indent + 2, "Op: "), b.Left.goString(indent + 2, "Left: "), b.Right.goString(indent + 2, "Right: "));
}
...
private static @string goString(this ptr<Trinary> _addr_t, nint indent, @string field) {
    ref Trinary t = ref _addr_t.val;

    return fmt.Sprintf("%*s%sTrinary:\n%s\n%s\n%s\n%s", indent, "", field, t.Op.goString(indent + 2, "Op: "), t.First.goString(indent + 2, "First: "), t.Second.goString(indent + 2, "Second: "), t.Third.goString(indent + 2, "Third: "));
}
...
private static @string goString(this ptr<Fold> _addr_f, nint indent, @string field) {
    ref Fold f = ref _addr_f.val;

    if (f.Arg2 == null) {
        return fmt.Sprintf("%*s%sFold: Left: %t\n%s\n%s", indent, "", field, f.Left, f.Op.goString(indent + 2, "Op: "), f.Arg1.goString(indent + 2, "Arg1: "));
    }
    else
 {
        return fmt.Sprintf("%*s%sFold: Left: %t\n%s\n%s\n%s", indent, "", field, f.Left, f.Op.goString(indent + 2, "Op: "), f.Arg1.goString(indent + 2, "Arg1: "), f.Arg2.goString(indent + 2, "Arg2: "));
    }
}
...
private static @string goString(this ptr<New> _addr_n, nint indent, @string field) {
    ref New n = ref _addr_n.val;
...
private static @string goString(this ptr<Literal> _addr_l, nint indent, @string field) {
    ref Literal l = ref _addr_l.val;
...
private static @string goString(this ptr<ExprList> _addr_el, nint indent, @string field) {
    ref ExprList el = ref _addr_el.val;
...
private static @string goString(this ptr<InitializerList> _addr_il, nint indent, @string field) {
    ref InitializerList il = ref _addr_il.val;
...
private static void print(this ptr<DefaultArg> _addr_da, ptr<printState> _addr_ps) {
    ...
    fmt.Fprintf(_addr_ps.buf, "{default arg#%d}::", da.Num + 1);
    ps.print(da.Arg);
}
...
private static @string goString(this ptr<DefaultArg> _addr_da, nint indent, @string field) {
    ref DefaultArg da = ref _addr_da.val;

    return fmt.Sprintf("%*s%sDefaultArg: Num: %d\n%s", indent, "", field, da.Num, da.Arg.goString(indent + 2, "Arg: "));
}
...
private static @string goString(this ptr<Closure> _addr_cl, nint indent, @string field) {
    ref Closure cl = ref _addr_cl.val;
...
private static @string goString(this ptr<UnnamedType> _addr_ut, nint indent, @string field) {
    ref UnnamedType ut = ref _addr_ut.val;
...
private static @string goString(this ptr<Clone> _addr_c, nint indent, @string field) {
    ref Clone c = ref _addr_c.val;

    return fmt.Sprintf("%*s%sClone: Suffix: %s\n%s", indent, "", field, c.Suffix, c.Base.goString(indent + 2, "Base: "));
}
...
private static @string goString(this ptr<Special> _addr_s, nint indent, @string field) {
    ref Special s = ref _addr_s.val;

    return fmt.Sprintf("%*s%sSpecial: Prefix: %s\n%s", indent, "", field, s.Prefix, s.Val.goString(indent + 2, "Val: "));
}
...
private static @string goString(this ptr<Special2> _addr_s, nint indent, @string field) {
    ref Special2 s = ref _addr_s.val;

    return fmt.Sprintf("%*s%sSpecial2: Prefix: %s\n%s\n%*sMiddle: %s\n%s", indent, "", field, s.Prefix, s.Val1.goString(indent + 2, "Val1: "), indent + 2, "", s.Middle, s.Val2.goString(indent + 2, "Val2: "));
}
...
private static void print(this ptr<EnableIf> _addr_ei, ptr<printState> _addr_ps) {
    ref EnableIf ei = ref _addr_ei.val;
    ref printState ps = ref _addr_ps.val;

    ps.print(ei.Type);
    ps.writeString(" [enable_if:");
    var first = true;
    foreach (var (_, a) in ei.Args) {
        if (!first) {
            ps.writeString(", ");
        }
        ps.print(a);
...
private static AST Copy(this ptr<EnableIf> _addr_ei, Func<AST, AST> fn, Func<AST, bool> skip) {
    ref EnableIf ei = ref _addr_ei.val;

    if (skip(ei)) {
        return null;
    }
    var typ = ei.Type.Copy(fn, skip);
    var argsChanged = false;
    var args = make_slice<AST>(len(ei.Args));
    foreach (var (i, a) in ei.Args) {
        var ac = a.Copy(fn, skip);
        if (ac == null) {
            args[i] = a;
        }
        else
 {
    ...
    }    if (typ == null && !argsChanged) {
        return fn(ei);
    }
    if (typ == null) {
        typ = ei.Type;
    }
    ei = addr(new EnableIf(Type:typ,Args:args));
    {
        var r = fn(ei);
...
private static @string goString(this ptr<EnableIf> _addr_ei, nint indent, @string field) {
    ref EnableIf ei = ref _addr_ei.val;
...
// prefixes.
private static void printOneInner(this ptr<printState> _addr_ps, ptr<slice<AST>> _addr_save) => func((_, panic, _) => {
    ref printState ps = ref _addr_ps.val;
