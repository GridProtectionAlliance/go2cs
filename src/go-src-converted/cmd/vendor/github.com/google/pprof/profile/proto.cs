...
private partial struct buffer {
    public nint field; // field tag
    public nint typ; // proto wire type code for field
    public ulong u64;
...
private static error decodeInt64s(ptr<buffer> _addr_b, ptr<slice<long>> _addr_x) {
    ...
    if (b.typ == 2) { 
        // Packed encoding
        var data = b.data;
        var tmp = make_slice<long>(0, len(data)); // Maximally sized
        while (len(data) > 0) {
            ulong u = default;
            error err = default!;
...
private static error decodeUint64s(ptr<buffer> _addr_b, ptr<slice<ulong>> _addr_x) {
    ...
    if (b.typ == 2) {
        var data = b.data; 
        // Packed encoding
        var tmp = make_slice<ulong>(0, len(data)); // Maximally sized
        while (len(data) > 0) {
