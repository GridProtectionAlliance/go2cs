...
// package profile -- go2cs converted at 2022 March 13 06:37:08 UTC
// import "cmd/vendor/github.com/google/pprof/profile" ==> using profile = go.cmd.vendor.github.com.google.pprof.profile_package
// Original source: C:\Program Files\Go\src\cmd\vendor\github.com\google\pprof\profile\merge.go
...
private static sampleKey key(this ptr<Sample> _addr_sample) {
    ...
    }    var labels = make_slice<@string>(0, len(sample.Label));
    {
        var k__prev1 = k;
        var v__prev1 = v;
    ...
    var numlabels = make_slice<@string>(0, len(sample.NumLabel));
    {
        var k__prev1 = k;
        var v__prev1 = v;
...
private static locationKey key(this ptr<Location> _addr_l) {
    ...
    var lines = make_slice<@string>(len(l.Line) * 2);
...
private static mappingKey key(this ptr<Mapping> _addr_m) {
    ...
    const nuint mapsizeRounding = 0x1000;
...
private static (ptr<Profile>, error) combineHeaders(slice<ptr<Profile>> srcs) {
...

    {
        ...
        foreach (var (_, __s) in srcs[(int)1..]) {
            s = __s;
            {
                var err = srcs[0].compatible(s);
