...
private static void preEncode(this ptr<Profile> _addr_p) {
    ...
    var strings = make_map<@string, nint>();
    addString(strings, "");
...
private static void encode(this ptr<Profile> _addr_p, ptr<buffer> _addr_b) {
    ...
    encodeInt64Opt(b, 10, p.DurationNanos);
    {
        ...
        if (pt != null && (pt.typeX != 0 || pt.unitX != 0)) {
...
private static error postDecode(this ptr<Profile> _addr_p) {
    ...
    var mappingIds = make_slice<ptr<Mapping>>(len(p.Mapping) + 1);
    foreach (var (_, m) in p.Mapping) {
        ...
        else
 {
    ...
    var functionIds = make_slice<ptr<Function>>(len(p.Function) + 1);
    foreach (var (_, f) in p.Function) {
        ...
        else
 {
    ...
    var locationIds = make_slice<ptr<Location>>(len(p.Location) + 1);
    {
        var l__prev1 = l;
...
private static slice<@string> padStringArray(slice<@string> arr, nint l) {
    if (l <= len(arr)) {
        return arr;
    }
...
private static void encode(this ptr<Location> _addr_p, ptr<buffer> _addr_b) {
    ...
    foreach (var (i) in p.Line) {
        encodeMessage(b, 4, _addr_p.Line[i]);
...
private static long addString(map<@string, nint> strings, @string s) {
    var (i, ok) = strings[s];
    if (!ok) {
        i = len(strings);
        strings[s] = i;
