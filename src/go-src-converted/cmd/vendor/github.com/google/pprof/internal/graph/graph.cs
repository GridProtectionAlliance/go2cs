...
// package graph -- go2cs converted at 2022 March 06 23:23:39 UTC
...
// Original source: C:\Program Files\Go\src\cmd\vendor\github.com\google\pprof\internal\graph\graph.go
...
private static var javaRegExp = regexp.MustCompile("^(?:[a-z]\\w*\\.)*([A-Z][\\w\\$]*\\.(?:<init>|[a-z][\\w\\$]*(?:\\$\\d+)?))(?:(?:\\()|$)");private static var goRegExp = regexp.MustCompile("^(?:[\\w\\-\\.]+\\/)+(.+)");private static var goVerRegExp = regexp.MustCompile("^(.*?)/v(?:[2-9]|[1-9][0-9]+)([./].*)$");private static var cppRegExp = regexp.MustCompile("^(?:[_a-zA-Z]\\w*::)+(_*[A-Z]\\w*::~?[_a-zA-Z]\\w*(?:<.*>)?)");private static var cppAnonymousPrefixRegExp = regexp.MustCompile("^\\(anonymous namespace\\)::");
...
// there isn't such an edge one is created.
private static void AddToEdgeDiv(this ptr<Node> _addr_n, ptr<Node> _addr_to, long dv, long v, bool residual, bool inline) => func((_, panic, _) => {
    ref Node n = ref _addr_n.val;
    ref Node to = ref _addr_to.val;
...
private static (ptr<Graph>, map<ulong, Nodes>) newGraph(ptr<profile.Profile> _addr_prof, ptr<Options> _addr_o) {
    ...
    var seenNode = make_map<ptr<Node>, bool>();
    var seenEdge = make_map<nodePair, bool>();
    foreach (var (_, sample) in prof.Sample) {
        ...
        for (var i = len(sample.Location) - 1; i >= 0; i--) {
            ...
            for (var ni = len(locNodes) - 1; ni >= 0; ni--) {
                var n = locNodes[ni];
                if (n == null) {
                    residual = true;
                    continue;
...
private static ptr<Graph> selectNodesForGraph(Nodes nodes, bool dropNegative) { 
    ...
    var gNodes = make(Nodes, 0, len(nodes));
...
private static ptr<Graph> newTree(ptr<profile.Profile> _addr_prof, ptr<Options> _addr_o) {
    ...
    foreach (var (_, sample) in prof.Sample) {
        ...
        for (var i = len(sample.Location) - 1; i >= 0; i--) {
            ...
            for (var lidx = len(lines) - 1; lidx >= 0; lidx--) {
    ...
    foreach (var (_, re) in new slice<ptr<regexp.Regexp>>(new ptr<regexp.Regexp>[] { goRegExp, javaRegExp, cppRegExp })) {
        {
            ...
            if (len(matches) >= 2) {
                return strings.Join(matches[(int)1..], "");
            }
...
private static void TrimTree(this ptr<Graph> _addr_g, NodePtrSet kept) => func((_, panic, _) => {
...
private static nint Len(this tags t) {
    return len(t.t);
}
private static void Swap(this tags t, nint i, nint j) {
    (t.t[i], t.t[j]) = (t.t[j], t.t[i]);
}
private static bool Less(this tags t, nint i, nint j) {
    if (!t.flat) {
...
private static @string addSample(this ptr<Node> _addr_n, long dw, long w, @string labels, map<@string, slice<long>> numLabel, map<@string, slice<@string>> numUnit, Func<long, @string, @string> format, bool flat) {
    ...
    else
 {
    ...
    if (labels != "") {
        var t = n.LabelTags.findOrAddTag(labels, "", 0);
        if (flat) {
            t.FlatDiv += dw;
            t.Flat += w;
        }
...
private static @string String(this ptr<Graph> _addr_g) {
    ...
    var nodeIndex = make_map<ptr<Node>, nint>(len(g.Nodes));
...

    {
        ...
        foreach (var (__i, __n) in g.Nodes) {
            ...
            slice<nint> @in = default;            slice<nint> @out = default;
...
private static Nodes getNodesAboveCumCutoff(Nodes nodes, long nodeCutoff) {
    var cutoffNodes = make(Nodes, 0, len(nodes));
    foreach (var (_, n) in nodes) {
...
private static nint TrimLowFrequencyEdges(this ptr<Graph> _addr_g, long edgeCutoff) {
    ref Graph g = ref _addr_g.val;
...
private static NodePtrSet SelectTopNodePtrs(this ptr<Graph> _addr_g, nint maxNodes, bool visualMode) {
    ref Graph g = ref _addr_g.val;
...
private static NodeSet SelectTopNodes(this ptr<Graph> _addr_g, nint maxNodes, bool visualMode) {
    ref Graph g = ref _addr_g.val;
...
private static Nodes selectTopNodes(this ptr<Graph> _addr_g, nint maxNodes, bool visualMode) {
    ref Graph g = ref _addr_g.val;
...
private static nint countTags(ptr<Node> _addr_n) {
    ref Node n = ref _addr_n.val;
...
private static void RemoveRedundantEdges(this ptr<Graph> _addr_g) {
    ...
    for (var i = len(g.Nodes); i > 0; i--) {
        var n = g.Nodes[i - 1];
        var @in = n.In.Sort();
        for (var j = len(in); j > 0; j--) {
            var e = in[j - 1];
            if (!e.Residual) { 
...
private static nint Len(this nodeSorter s) {
    return len(s.rs);
}
private static void Swap(this nodeSorter s, nint i, nint j) {
    (s.rs[i], s.rs[j]) = (s.rs[j], s.rs[i]);
}
private static bool Less(this nodeSorter s, nint i, nint j) {
...
private static long entropyScore(ptr<Node> _addr_n) {
 ...
 {
        score += edgeEntropyScore(_addr_n, n.In, 0);
    }
    if (len(n.Out) == 0) {
...
public partial struct NodeOrder { // : nint
}
...
private static nint Len(this edgeList el) {
    return len(el);
}

private static bool Less(this edgeList el, nint i, nint j) {
...
private static void Swap(this edgeList el, nint i, nint j) {
    (el[i], el[j]) = (el[j], el[i]);
}
