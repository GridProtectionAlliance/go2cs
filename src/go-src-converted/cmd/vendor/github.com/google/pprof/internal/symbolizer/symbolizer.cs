...
private static (slice<byte>, error) postURL(@string source, @string post, http.RoundTripper tr) => func((defer, _, _) => {
...
// symbolization.
private static error doLocalSymbolize(ptr<profile.Profile> _addr_prof, bool fast, bool force, plugin.ObjTool obj, plugin.UI ui) => func((defer, _, _) => {
...
private static @string removeMatching(@string name, byte start, byte end) {
    ...
    nint nesting = default;    nint first = default;    nint current = default;

    {
        var index = strings.IndexAny(name[(int)current..], s);
        ...
        while (index != -1) {
            ...
            index = strings.IndexAny(name[(int)current..], s);
                ...
                if (nesting < 0) 
                    ...
                    name = name[..(int)first] + name[(int)current + 1..];
                    current = first - 1;
