 ...
 {
        {
            ...
            if (i >= 0) { 
                ...
                // part of the filename.
                {
                    var (line, err) = strconv.Atoi(fileline[(int)i + 1..]);

                    if (err == null) {
                        linenumber = line;
                        fileline = fileline[..(int)i];
                    }
...
private static (slice<plugin.Frame>, error) addrInfo(this ptr<addr2Liner> _addr_d, ulong addr) {
    ...
    if (len(stack) > 0 && d.nm != null) {
        var (nm, err) = d.nm.addrInfo(addr);
        if (err == null && len(nm) > 0) { 
            ...
            // names).
            var nmName = nm[len(nm) - 1].Func;
            var a2lName = stack[len(stack) - 1].Func;
