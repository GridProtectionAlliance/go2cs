...
// Original source: C:\Go\src\cmd\vendor\golang.org\x\tools\go\ast\inspector\inspector.go
...
namespace ast
{
    public static partial class inspector_package
    {
        ...
        private partial struct @event
        {
            ...
            public long index; // 1 + index of corresponding pop event, or 0 if this is a pop
        }
        ...
        { 
            ...
            var capacity = extent * 33L / 100L;
            if (capacity > 1e6F)
            {
                capacity = 1e6F; // impose some reasonable maximum
            }

            var events = make_slice<event>(0L, capacity);
            ...
            slice<event> stack = default;
            {
                ...
                foreach (var (_, __f) in files)
                {
                    ...
                    ast.Inspect(f, n =>
                    {
                        ...
                        { 
                            // pop
                            ev = stack[len(stack) - 1L];
                            stack = stack[..len(stack) - 1L];

                            events[ev.index].index = len(events) + 1L; // make push refer to pop

                            ev.index = 0L; // turn ev into a pop event
                            events = append(events, ev);
