...
namespace passes
{
    public static partial class printf_package
    {
        ...
        private static bool matchArgTypeInternal(ptr<analysis.Pass> _addr_pass, printfArgType t, types.Type typ, ast.Expr arg, map<types.Type, bool> inProgress) => func((_, panic, __) =>
        {
            ref analysis.Pass pass = ref _addr_pass.val;
            ...
            if (t & argString != 0L && isConvertibleToString(_addr_pass, typ))
            {
                return true;
            }
                ...
                {
                    {
                        ...
                        if (ok)
                        {
                            ...
                            if (sig.Params().Len() == 0L && sig.Results().Len() == 1L && sig.Results().At(0L).Type() == types.Typ[types.String])
                            {
                                return true;
                            }
        ...
        private static bool matchStructArgType(ptr<analysis.Pass> _addr_pass, printfArgType t, ptr<types.Struct> _addr_typ, ast.Expr arg, map<types.Type, bool> inProgress)
        {
            ...
            for (long i = 0L; i < typ.NumFields(); i++)
            {
                ...
                if (t & argString != 0L && !typf.Exported() && isConvertibleToString(_addr_pass, typf.Type()))
                { 
