...
public static readonly Reg W0 = iota;
...
public static readonly WSP V31 = WZR; // These are different registers with the same encoding.
...
private static readonly ExtShift _ = iota;
...
public partial struct MemExtend {
    ...
    public byte Amount; // Refer to ARM reference manual, for byte load/store(register), the index
...
public static @string String(this Cond c) {
    var cond31 = c.Value >> 1;
    var invert = bool((c.Value & 1) == 1);
...
public static @string String(this Imm_prfop i) {
    var prf_type = (i >> 3) & (1 << 2 - 1);
    var prf_target = (i >> 1) & (1 << 2 - 1);
    var prf_policy = i & 1;
...
public static @string String(this Imm_fp i) {
 ...
 {
        numerator = pre;
        denominator = (16 << (int)(uint8(-1 * i.exp)));
    }
...
private static readonly Arrangement _ = iota;
...
public static @string String(this RegisterWithArrangement r) {
    ...
    if (r.cnt > 0) {
        result = "{" + result;
        if (r.cnt == 2) {
            var r1 = V0 + Reg((uint16(r.r) - uint16(V0) + 1) & 31);
            result += ", " + r1.String() + r.a.String();
        }
        else if (r.cnt > 2) {
            if ((uint16(r.cnt) + ((uint16(r.r) - uint16(V0)) & 31)) > 32) {
                for (nint i = 1; i < int(r.cnt); i++) {
                    var cur = V0 + Reg((uint16(r.r) - uint16(V0) + uint16(i)) & 31);
