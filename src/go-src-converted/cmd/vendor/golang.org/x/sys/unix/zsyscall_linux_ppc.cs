...
// package unix -- go2cs converted at 2022 March 06 23:29:17 UTC
// import "cmd/vendor/golang.org/x/sys/unix" ==> using unix = go.cmd.vendor.golang.org.x.sys.unix_package
// Original source: C:\Program Files\Go\src\cmd\vendor\golang.org\x\sys\unix\zsyscall_linux_ppc.go
...
public static error Ftruncate(nint fd, long length) {
    ...
    var (_, _, e1) = Syscall(SYS_FTRUNCATE64, uintptr(fd), uintptr(length >> 32), uintptr(length));
    if (e1 != 0) {
        err = errnoErr(e1);
    }
    return ;
...
public static (nint, error) Pread(nint fd, slice<byte> p, long offset) {
    ...
    else
 {
    ...
    var (r0, _, e1) = Syscall6(SYS_PREAD64, uintptr(fd), uintptr(_p0), uintptr(len(p)), uintptr(offset >> 32), uintptr(offset), 0);
    n = int(r0);
    if (e1 != 0) {
        err = errnoErr(e1);
    }
...
public static (nint, error) Pwrite(nint fd, slice<byte> p, long offset) {
    ...
    else
 {
    ...
    var (r0, _, e1) = Syscall6(SYS_PWRITE64, uintptr(fd), uintptr(_p0), uintptr(len(p)), uintptr(offset >> 32), uintptr(offset), 0);
    n = int(r0);
    if (e1 != 0) {
        err = errnoErr(e1);
    }
...
public static error Truncate(@string path, long length) {
    ...
    var (_, _, e1) = Syscall(SYS_TRUNCATE64, uintptr(@unsafe.Pointer(_p0)), uintptr(length >> 32), uintptr(length));
    if (e1 != 0) {
        err = errnoErr(e1);
    }
    return ;
...
private static error syncFileRange2(nint fd, nint flags, long off, long n) {
    ...
    var (_, _, e1) = Syscall6(SYS_SYNC_FILE_RANGE2, uintptr(fd), uintptr(flags), uintptr(off >> 32), uintptr(off), uintptr(n >> 32), uintptr(n));
    if (e1 != 0) {
        err = errnoErr(e1);
    }
    return ;
