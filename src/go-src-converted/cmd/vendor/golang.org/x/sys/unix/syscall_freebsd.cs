...
// package unix -- go2cs converted at 2020 October 09 05:56:34 UTC
// import "cmd/vendor/golang.org/x/sys/unix" ==> using unix = go.cmd.vendor.golang.org.x.sys.unix_package
// Original source: C:\Go\src\cmd\vendor\golang.org\x\sys\unix\syscall_freebsd.go
...
namespace sys
{
    ...
    {
        public static readonly long SYS_FSTAT_FREEBSD12 = (long)551L; // { int fstat(int fd, _Out_ struct stat *sb); }
        public static readonly long SYS_FSTATAT_FREEBSD12 = (long)552L; // { int fstatat(int fd, _In_z_ char *path, \
        public static readonly long SYS_GETDIRENTRIES_FREEBSD12 = (long)554L; // { ssize_t getdirentries(int fd, \
        public static readonly long SYS_STATFS_FREEBSD12 = (long)555L; // { int statfs(_In_z_ char *path, \
        public static readonly long SYS_FSTATFS_FREEBSD12 = (long)556L; // { int fstatfs(int fd, \
        public static readonly long SYS_GETFSSTAT_FREEBSD12 = (long)557L; // { int getfsstat( \
        public static readonly long SYS_MKNODAT_FREEBSD12 = (long)559L; // { int mknodat(int fd, _In_z_ char *path, \
        ...
        public static error Stat(@string path, ptr<Stat_t> _addr_st)
        {
            ...
            if (supportsABI(_ino64First))
            {
                return error.As(fstatat_freebsd12(AT_FDCWD, path, st, 0L))!;
            }
        ...
        public static error Mknodat(long fd, @string path, uint mode, ulong dev)
        {
            error err = default!;
        ...
        public static error PtraceCont(long pid, long signal)
        {
            ...
            return error.As(ptrace(PTRACE_CONT, pid, 1L, signal))!;
        }
        ...
        public static error PtraceDetach(long pid)
        {
            ...
            return error.As(ptrace(PTRACE_DETACH, pid, 1L, 0L))!;
        }

        public static error PtraceGetFpRegs(long pid, ptr<FpReg> _addr_fpregsout)
        {
            error err = default!;
            ref FpReg fpregsout = ref _addr_fpregsout.val;

            return error.As(ptrace(PTRACE_GETFPREGS, pid, uintptr(@unsafe.Pointer(fpregsout)), 0L))!;
        }

        public static error PtraceGetRegs(long pid, ptr<Reg> _addr_regsout)
        {
            error err = default!;
            ref Reg regsout = ref _addr_regsout.val;

            return error.As(ptrace(PTRACE_GETREGS, pid, uintptr(@unsafe.Pointer(regsout)), 0L))!;
        }

        public static error PtraceLwpEvents(long pid, long enable)
        {
            error err = default!;

            return error.As(ptrace(PTRACE_LWPEVENTS, pid, 0L, enable))!;
        }

        public static error PtraceLwpInfo(long pid, System.UIntPtr info)
        {
            error err = default!;
        ...
        public static error PtraceSetRegs(long pid, ptr<Reg> _addr_regs)
        {
            error err = default!;
            ref Reg regs = ref _addr_regs.val;

            return error.As(ptrace(PTRACE_SETREGS, pid, uintptr(@unsafe.Pointer(regs)), 0L))!;
        }
        ...
        public static error PtraceSingleStep(long pid)
        {
            ...
            return error.As(ptrace(PTRACE_SINGLESTEP, pid, 1L, 0L))!;
        }
