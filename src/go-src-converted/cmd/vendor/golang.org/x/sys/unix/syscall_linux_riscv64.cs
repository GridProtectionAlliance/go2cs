...
// package unix -- go2cs converted at 2022 March 06 23:27:11 UTC
// import "cmd/vendor/golang.org/x/sys/unix" ==> using unix = go.cmd.vendor.golang.org.x.sys.unix_package
// Original source: C:\Program Files\Go\src\cmd\vendor\golang.org\x\sys\unix\syscall_linux_riscv64.go
...
public static error Stat(@string path, ptr<Stat_t> _addr_stat) {
    ...
    return error.As(Fstatat(AT_FDCWD, path, stat, 0))!;
}
...
public static (nint, error) Poll(slice<PollFd> fds, nint timeout) {
    ...
    if (len(fds) == 0) {
        return ppoll(null, 0, ts, null);
    }
    return ppoll(_addr_fds[0], len(fds), ts, null);
...
public static error Renameat(nint olddirfd, @string oldpath, nint newdirfd, @string newpath) {
    ...
    return error.As(Renameat2(olddirfd, oldpath, newdirfd, newpath, 0))!;
}
