...
// package unix -- go2cs converted at 2020 October 08 04:47:31 UTC
// import "cmd/vendor/golang.org/x/sys/unix" ==> using unix = go.cmd.vendor.golang.org.x.sys.unix_package
// Original source: C:\Go\src\cmd\vendor\golang.org\x\sys\unix\syscall_linux_arm64.go
...
namespace sys
{
    public static partial class unix_package
    {
        ...
        public static error Getrlimit(long resource, ptr<Rlimit> _addr_rlim)
        {
            ...
            var err = prlimit(0L, resource, null, rlim);
            if (err != ENOSYS)
            {
                return error.As(err)!;
            }
        ...
        public static error Setrlimit(long resource, ptr<Rlimit> _addr_rlim)
        {
            ...
            var err = prlimit(0L, resource, rlim, null);
            if (err != ENOSYS)
            {
                return error.As(err)!;
            }
...
;
        ...
        public static (long, error) Poll(slice<PollFd> fds, long timeout)
        {
            ...
            ptr<Timespec> ts;
            if (timeout >= 0L)
            {>>MARKER:FUNCTION_dup2_BLOCK_PREFIX<<
                ts = @new<Timespec>();
