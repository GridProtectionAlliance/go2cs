...
// package unix -- go2cs converted at 2022 March 06 23:28:36 UTC
// import "cmd/vendor/golang.org/x/sys/unix" ==> using unix = go.cmd.vendor.golang.org.x.sys.unix_package
// Original source: C:\Program Files\Go\src\cmd\vendor\golang.org\x\sys\unix\zsyscall_freebsd_amd64.go
...
public static error Fadvise(nint fd, long offset, long length, nint advice) {
    ...
    var (_, _, e1) = Syscall6(SYS_POSIX_FADVISE, uintptr(fd), uintptr(offset), uintptr(length), uintptr(advice), 0, 0);
    if (e1 != 0) {
        err = errnoErr(e1);
    }
    return ;
...
public static (nint, error) Pread(nint fd, slice<byte> p, long offset) {
    ...
    else
 {
    ...
    var (r0, _, e1) = Syscall6(SYS_PREAD, uintptr(fd), uintptr(_p0), uintptr(len(p)), uintptr(offset), 0, 0);
    n = int(r0);
    if (e1 != 0) {
        err = errnoErr(e1);
    }
...
public static (nint, error) Pwrite(nint fd, slice<byte> p, long offset) {
    ...
    else
 {
    ...
    var (r0, _, e1) = Syscall6(SYS_PWRITE, uintptr(fd), uintptr(_p0), uintptr(len(p)), uintptr(offset), 0, 0);
    n = int(r0);
    if (e1 != 0) {
        err = errnoErr(e1);
    }
