...
// package unix -- go2cs converted at 2022 March 06 23:29:15 UTC
// import "cmd/vendor/golang.org/x/sys/unix" ==> using unix = go.cmd.vendor.golang.org.x.sys.unix_package
// Original source: C:\Program Files\Go\src\cmd\vendor\golang.org\x\sys\unix\zsyscall_linux_mipsle.go
...
public static error Fadvise(nint fd, long offset, long length, nint advice) {
    ...
    var (_, _, e1) = Syscall9(SYS_FADVISE64, uintptr(fd), 0, uintptr(offset), uintptr(offset >> 32), uintptr(length), uintptr(length >> 32), uintptr(advice), 0, 0);
    if (e1 != 0) {
        err = errnoErr(e1);
    }
    return ;
...
public static error SyncFileRange(nint fd, long off, long n, nint flags) {
    ...
    var (_, _, e1) = Syscall9(SYS_SYNC_FILE_RANGE, uintptr(fd), 0, uintptr(off), uintptr(off >> 32), uintptr(n), uintptr(n >> 32), uintptr(flags), 0, 0);
    if (e1 != 0) {
        err = errnoErr(e1);
    }
    return ;
