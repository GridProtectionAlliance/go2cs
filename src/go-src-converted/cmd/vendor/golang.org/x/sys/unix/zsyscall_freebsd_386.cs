...
// package unix -- go2cs converted at 2022 March 06 23:28:30 UTC
// import "cmd/vendor/golang.org/x/sys/unix" ==> using unix = go.cmd.vendor.golang.org.x.sys.unix_package
// Original source: C:\Program Files\Go\src\cmd\vendor\golang.org\x\sys\unix\zsyscall_freebsd_386.go
...
public static error Fadvise(nint fd, long offset, long length, nint advice) {
    ...
    var (_, _, e1) = Syscall6(SYS_POSIX_FADVISE, uintptr(fd), uintptr(offset), uintptr(offset >> 32), uintptr(length), uintptr(length >> 32), uintptr(advice));
    if (e1 != 0) {
        err = errnoErr(e1);
    }
    return ;
...
public static error Ftruncate(nint fd, long length) {
    ...
    var (_, _, e1) = Syscall(SYS_FTRUNCATE, uintptr(fd), uintptr(length), uintptr(length >> 32));
    if (e1 != 0) {
        err = errnoErr(e1);
    }
    return ;
...
private static error mknodat_freebsd12(nint fd, @string path, uint mode, ulong dev) {
    ...
    var (_, _, e1) = Syscall6(SYS_MKNODAT_FREEBSD12, uintptr(fd), uintptr(@unsafe.Pointer(_p0)), uintptr(mode), uintptr(dev), uintptr(dev >> 32), 0);
    if (e1 != 0) {
        err = errnoErr(e1);
    }
    return ;
...
public static (long, error) Seek(nint fd, long offset, nint whence) {
    ...
    var (r0, r1, e1) = Syscall6(SYS_LSEEK, uintptr(fd), uintptr(offset), uintptr(offset >> 32), uintptr(whence), 0, 0);
...
public static error Truncate(@string path, long length) {
    ...
    var (_, _, e1) = Syscall(SYS_TRUNCATE, uintptr(@unsafe.Pointer(_p0)), uintptr(length), uintptr(length >> 32));
    if (e1 != 0) {
        err = errnoErr(e1);
    }
    return ;
...
private static (System.UIntPtr, error) mmap(System.UIntPtr addr, System.UIntPtr length, nint prot, nint flag, nint fd, long pos) {
    ...
    var (r0, _, e1) = Syscall9(SYS_MMAP, uintptr(addr), uintptr(length), uintptr(prot), uintptr(flag), uintptr(fd), uintptr(pos), uintptr(pos >> 32), 0, 0);
    ret = uintptr(r0);
    if (e1 != 0) {
        err = errnoErr(e1);
    }
