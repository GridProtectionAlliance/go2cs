...
// package unix -- go2cs converted at 2022 March 06 23:29:06 UTC
// import "cmd/vendor/golang.org/x/sys/unix" ==> using unix = go.cmd.vendor.golang.org.x.sys.unix_package
// Original source: C:\Program Files\Go\src\cmd\vendor\golang.org\x\sys\unix\zsyscall_linux_mips.go
...
private static error fanotifyMark(nint fd, nuint flags, ulong mask, nint dirFd, ptr<byte> _addr_pathname) {
    ...
    var (_, _, e1) = Syscall6(SYS_FANOTIFY_MARK, uintptr(fd), uintptr(flags), uintptr(mask >> 32), uintptr(mask), uintptr(dirFd), uintptr(@unsafe.Pointer(pathname)));
    if (e1 != 0) {
        err = errnoErr(e1);
    }
    return ;
...
public static error Fallocate(nint fd, uint mode, long off, long len) {
    ...
    var (_, _, e1) = Syscall6(SYS_FALLOCATE, uintptr(fd), uintptr(mode), uintptr(off >> 32), uintptr(off), uintptr(len >> 32), uintptr(len));
    if (e1 != 0) {
        err = errnoErr(e1);
    }
    return ;
...
public static error Fadvise(nint fd, long offset, long length, nint advice) {
    ...
    var (_, _, e1) = Syscall9(SYS_FADVISE64, uintptr(fd), 0, uintptr(offset >> 32), uintptr(offset), uintptr(length >> 32), uintptr(length), uintptr(advice), 0, 0);
    if (e1 != 0) {
        err = errnoErr(e1);
    }
    return ;
...
public static error Ftruncate(nint fd, long length) {
    ...
    var (_, _, e1) = Syscall6(SYS_FTRUNCATE64, uintptr(fd), 0, uintptr(length >> 32), uintptr(length), 0, 0);
    if (e1 != 0) {
        err = errnoErr(e1);
    }
    return ;
...
public static (nint, error) Pread(nint fd, slice<byte> p, long offset) {
    ...
    else
 {
    ...
    var (r0, _, e1) = Syscall6(SYS_PREAD64, uintptr(fd), uintptr(_p0), uintptr(len(p)), 0, uintptr(offset >> 32), uintptr(offset));
    n = int(r0);
    if (e1 != 0) {
        err = errnoErr(e1);
    }
...
public static (nint, error) Pwrite(nint fd, slice<byte> p, long offset) {
    ...
    else
 {
    ...
    var (r0, _, e1) = Syscall6(SYS_PWRITE64, uintptr(fd), uintptr(_p0), uintptr(len(p)), 0, uintptr(offset >> 32), uintptr(offset));
    n = int(r0);
    if (e1 != 0) {
        err = errnoErr(e1);
    }
...
public static error SyncFileRange(nint fd, long off, long n, nint flags) {
    ...
    var (_, _, e1) = Syscall9(SYS_SYNC_FILE_RANGE, uintptr(fd), 0, uintptr(off >> 32), uintptr(off), uintptr(n >> 32), uintptr(n), uintptr(flags), 0, 0);
    if (e1 != 0) {
        err = errnoErr(e1);
    }
    return ;
...
public static error Truncate(@string path, long length) {
    ...
    var (_, _, e1) = Syscall6(SYS_TRUNCATE64, uintptr(@unsafe.Pointer(_p0)), 0, uintptr(length >> 32), uintptr(length), 0, 0);
    if (e1 != 0) {
        err = errnoErr(e1);
    }
    return ;
