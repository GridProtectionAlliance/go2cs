...
// Original source: C:\Program Files\Go\src\cmd\vendor\golang.org\x\sys\unix\syscall_freebsd_386.go
...
private static (nint, error) sendfile(nint outfd, nint infd, ptr<long> _addr_offset, nint count) {
    ...
    var (_, _, e1) = Syscall9(SYS_SENDFILE, uintptr(infd), uintptr(outfd), uintptr(offset), uintptr((offset) >> 32), uintptr(count), 0, uintptr(@unsafe.Pointer(_addr_writtenOut)), 0, 0);
...
public static error PtraceGetFsBase(nint pid, ptr<long> _addr_fsbase) {
    error err = default!;
    ref long fsbase = ref _addr_fsbase.val;

    return error.As(ptrace(PTRACE_GETFSBASE, pid, uintptr(@unsafe.Pointer(fsbase)), 0))!;
}

public static (nint, error) PtraceIO(nint req, nint pid, System.UIntPtr addr, slice<byte> @out, nint countin) {
    nint count = default;
    error err = default!;

    ref PtraceIoDesc ioDesc = ref heap(new PtraceIoDesc(Op:int32(req),Offs:(*byte)(unsafe.Pointer(addr)),Addr:(*byte)(unsafe.Pointer(&out[0])),Len:uint32(countin)), out ptr<PtraceIoDesc> _addr_ioDesc);
    err = ptrace(PTRACE_IO, pid, uintptr(@unsafe.Pointer(_addr_ioDesc)), 0);
