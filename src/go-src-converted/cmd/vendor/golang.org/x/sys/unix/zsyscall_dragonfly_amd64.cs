...
// package unix -- go2cs converted at 2022 March 06 23:28:24 UTC
// import "cmd/vendor/golang.org/x/sys/unix" ==> using unix = go.cmd.vendor.golang.org.x.sys.unix_package
// Original source: C:\Program Files\Go\src\cmd\vendor\golang.org\x\sys\unix\zsyscall_dragonfly_amd64.go
...
private static (nint, nint, error) pipe2(ptr<array<_C_int>> _addr_p, nint flags) {
    nint r = default;
    nint w = default;
    error err = default!;
    ref array<_C_int> p = ref _addr_p.val;

    var (r0, r1, e1) = RawSyscall(SYS_PIPE2, uintptr(@unsafe.Pointer(p)), uintptr(flags), 0);
    r = int(r0);
    w = int(r1);
    if (e1 != 0) {
        err = errnoErr(e1);
...
private static (nint, error) extpread(nint fd, slice<byte> p, nint flags, long offset) {
    nint n = default;
    error err = default!;
    ...
    else
 {
    ...
    var (r0, _, e1) = Syscall6(SYS_EXTPREAD, uintptr(fd), uintptr(_p0), uintptr(len(p)), uintptr(flags), uintptr(offset), 0);
    n = int(r0);
    if (e1 != 0) {
        err = errnoErr(e1);
    }
...
private static (nint, error) extpwrite(nint fd, slice<byte> p, nint flags, long offset) {
    nint n = default;
    error err = default!;
    ...
    else
 {
    ...
    var (r0, _, e1) = Syscall6(SYS_EXTPWRITE, uintptr(fd), uintptr(_p0), uintptr(len(p)), uintptr(flags), uintptr(offset), 0);
    n = int(r0);
    if (e1 != 0) {
        err = errnoErr(e1);
    }
...
public static error Mknodat(nint fd, @string path, uint mode, nint dev) {
    error err = default!;
