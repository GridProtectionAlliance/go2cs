...
// package unix -- go2cs converted at 2022 March 06 23:29:56 UTC
// import "cmd/vendor/golang.org/x/sys/unix" ==> using unix = go.cmd.vendor.golang.org.x.sys.unix_package
// Original source: C:\Program Files\Go\src\cmd\vendor\golang.org\x\sys\unix\zsyscall_openbsd_386.go
...
public static (nint, error) Getrtable() {
    nint rtable = default;
    error err = default!;

    var (r0, _, e1) = RawSyscall(SYS_GETRTABLE, 0, 0, 0);
    rtable = int(r0);
    if (e1 != 0) {
        err = errnoErr(e1);
    }
...
public static error Setrtable(nint rtable) {
    error err = default!;

    var (_, _, e1) = RawSyscall(SYS_SETRTABLE, uintptr(rtable), 0, 0);
    if (e1 != 0) {
        err = errnoErr(e1);
    }
    return ;
