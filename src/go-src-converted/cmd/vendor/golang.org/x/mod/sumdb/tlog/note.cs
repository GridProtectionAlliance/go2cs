...
// package tlog -- go2cs converted at 2020 October 09 05:56:05 UTC
// import "cmd/vendor/golang.org/x/mod/sumdb/tlog" ==> using tlog = go.cmd.vendor.golang.org.x.mod.sumdb.tlog_package
// Original source: C:\Go\src\cmd\vendor\golang.org\x\mod\sumdb\tlog\note.go
...
namespace sumdb
{
    public static partial class tlog_package
    {
        ...
        public static (Tree, error) ParseTree(slice<byte> text)
        {
            ...
            var (n, err) = strconv.ParseInt(lines[1L], 10L, 64L);
            if (err != null || n < 0L || lines[1L] != strconv.FormatInt(n, 10L))
            {
                return (new Tree(), error.As(errMalformedTree)!);
            }

            var (h, err) = base64.StdEncoding.DecodeString(lines[2L]);
        ...
        private static bool isValidRecordText(slice<byte> text)
        {
            int last = default;
            {
                ...
                while (i < len(text))
                {
                    var (r, size) = utf8.DecodeRune(text[i..]);
                    if (r < 0x20UL && r != '\n' || r == utf8.RuneError && size == 1L || last == '\n' && r == '\n')
                    {
                        return false;
                    }
        ...
        public static (long, slice<byte>, slice<byte>, error) ParseRecord(slice<byte> msg)
        {
            ...
            if (i < 0L)
            {
                return (0L, null, null, error.As(errMalformedRecord)!);
            }

            id, err = strconv.ParseInt(string(msg[..i]), 10L, 64L);
            if (err != null)
            {
                return (0L, null, null, error.As(errMalformedRecord)!);
            }
