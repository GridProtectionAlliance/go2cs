...
// package module -- go2cs converted at 2020 October 08 04:45:54 UTC
// import "cmd/vendor/golang.org/x/mod/module" ==> using module = go.cmd.vendor.golang.org.x.mod.module_package
// Original source: C:\Go\src\cmd\vendor\golang.org\x\mod\module\module.go
// IMPORTANT NOTE
//
...
// represented by a path with an empty version.
        ...
        public static (@string, @string, bool) SplitPathVersion(@string path)
        {
            ...
            var i = len(path);
            var dot = false;
            while (i > 0L && ('0' <= path[i - 1L] && path[i - 1L] <= '9' || path[i - 1L] == '.'))
            {
                if (path[i - 1L] == '.')
                {
                    dot = true;
            ...
            if (i <= 1L || i == len(path) || path[i - 1L] != 'v' || path[i - 2L] != '/')
            {
                return (path, "", true);
            }
            ...
            prefix = path[..i - 2L];
            pathMajor = path[i - 2L..];
            if (dot || len(pathMajor) <= 2L || pathMajor[2L] == '0' || pathMajor == "/v1")
            {
                return (path, "", false);
            }
        ...
        private static (@string, @string, bool) splitGopkgIn(@string path)
        {
            ...
            while (i > 0L && ('0' <= path[i - 1L] && path[i - 1L] <= '9'))
            {
                i--;
            }

            if (i <= 1L || path[i - 1L] != 'v' || path[i - 2L] != '.')
            { 
            ...
            prefix = path[..i - 2L];
            pathMajor = path[i - 2L..];
            if (len(pathMajor) <= 2L || pathMajor[2L] == '0' && pathMajor != ".v0")
            {
                return (path, "", false);
            }
        ...
        { 
            ...
            else if (pathMajor[0L] == '/' || pathMajor[0L] == '.')
            {
                if (m == pathMajor[1L..])
                {
                    return error.As(null!)!;
        ...
        public static @string PathMajorPrefix(@string pathMajor) => func((_, panic, __) =>
        {
            if (pathMajor == "")
            {
                return "";
        ...
        public static void Sort(slice<Version> list)
        {
            sort.Slice(list, (i, j) =>
            {
                ...
                if (vi != vj)
                {
                    return semver.Compare(vi, vj) < 0L;
                }
