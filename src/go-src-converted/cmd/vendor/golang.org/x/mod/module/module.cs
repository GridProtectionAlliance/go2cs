...
public partial struct InvalidPathError {
    public @string Kind; // "module", "import", or "file"
    public @string Path;
    public error Err;
}

private static @string Error(this ptr<InvalidPathError> _addr_e) {
    ref InvalidPathError e = ref _addr_e.val;

    return fmt.Sprintf("malformed %s path %q: %v", e.Kind, e.Path, e.Err);
}

private static error Unwrap(this ptr<InvalidPathError> _addr_e) {
    ref InvalidPathError e = ref _addr_e.val;
...
private static bool modPathOK(int r) {
    if (r < utf8.RuneSelf) {
...
private static bool importPathOK(int r) {
...
public static error CheckPath(@string path) => func((defer, _, _) => {
    error err = default!;

    defer(() => {
        if (err != null) {
            err = addr(new InvalidPathError(Kind:"module",Path:path,Err:err));
        }
    }());

    {
        var err = checkPath(path, modulePath);
    ...
    if (i == 0) {
        return error.As(fmt.Errorf("leading slash"))!;
    }
    if (!strings.Contains(path[..(int)i], ".")) {
        return error.As(fmt.Errorf("missing dot in first path element"))!;
    }
    if (path[0] == '-') {
        return error.As(fmt.Errorf("leading dash in first path element"))!;
    }
    foreach (var (_, r) in path[..(int)i]) {
        if (!firstPathOK(r)) {
            return error.As(fmt.Errorf("invalid char %q in first path element", r))!;
        }
    }    {
...
public static error CheckImportPath(@string path) {
    {
        var err = checkPath(path, importPath);
...
private partial struct pathKind { // : nint
}
...
private static error checkPath(@string path, pathKind kind) {
    ...
    foreach (var (i, r) in path) {
        if (r == '/') {
            {
                ...
                var err = checkElem(path[(int)elemStart..(int)i], kind);
        ...
        err = checkElem(path[(int)elemStart..], kind);
...
private static error checkElem(@string elem, pathKind kind) => func((_, panic, _) => {
    if (elem == "") {
    ...
    }
    {
        ...
        if (tilde >= 0 && tilde < len(short) - 1) {
            var suffix = short[(int)tilde + 1..];
...
public static error CheckFilePath(@string path) {
    {
        var err = checkPath(path, filePath);
