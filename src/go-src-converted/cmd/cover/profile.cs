...
namespace go
{
    public static partial class main_package
    {
        ...
        private partial struct byFileName // : slice<ref Profile>
        {
        }
        ...
        public static (slice<ref Profile>, error) ParseProfiles(@string fileName) => func((defer, _, __) =>
        {
            ...
            var files = make_map<@string, ref Profile>();
            ...
            while (s.Scan())
            {
                ...
                if (mode == "")
                {
                    ...
                    {
                        return (null, fmt.Errorf("bad mode line: %v", line));
                    }
                    mode = line[len(p)..];
                    continue;
                }
                ...
                if (m == null)
                {
                    return (null, fmt.Errorf("line %q doesn't match expected format: %v", m, lineRe));
                }
                var fn = m[1L];
                var p = files[fn];
                if (p == null)
                {
                    p = ref new Profile(FileName:fn,Mode:mode,);
                    files[fn] = p;
                }
                ...
                foreach (var (_, __p) in files)
                {
                    ...
                    for (long i = 1L; i < len(p.Blocks); i++)
                    {
                        ...
                        if (b.StartLine == last.StartLine && b.StartCol == last.StartCol && b.EndLine == last.EndLine && b.EndCol == last.EndCol)
                        {
                            ...
                            {
                                return (null, fmt.Errorf("inconsistent NumStmt: changed from %d to %d", last.NumStmt, b.NumStmt));
                            }
                            if (mode == "set")
                            {
            ...
            var profiles = make_slice<ref Profile>(0L, len(files));
        ...
        private static slice<Boundary> Boundaries(this ref Profile p, slice<byte> src)
        { 
            // Find maximum count.
            long max = 0L;
            {
