...
public partial interface Object {
    ...
    syntax.Pos Pkg(); // package to which this object belongs; nil for labels and objects in the Universe scope
    syntax.Pos Name(); // package local object name
    syntax.Pos Type(); // object type
    syntax.Pos Exported(); // reports whether the name starts with a capital letter
    syntax.Pos Id(); // object name if exported, qualified name if not exported (see func Id)

// String returns a human-readable string of the object.
    syntax.Pos String(); // order reflects a package-level object's source order: if object
// a is before object b in the source, then a.order() < b.order().
// order returns a value > 0 for package-level objects; it returns
// 0 for all other objects (including objects in file scopes).
    syntax.Pos order(); // color returns the object's color.
    syntax.Pos color(); // setType sets the type of the object.
    syntax.Pos setType(Type _p0); // setOrder sets the order number of the object. It must be > 0.
    syntax.Pos setOrder(uint _p0); // setColor sets the object's color. It must not be white.
    syntax.Pos setColor(color color); // setParent sets the parent scope of the object.
    syntax.Pos setParent(ptr<Scope> _p0); // sameId reports whether obj.Id() and Id(pkg, name) are the same.
    syntax.Pos sameId(ptr<Package> pkg, @string name); // scopePos returns the start position of the scope of this Object
    syntax.Pos scopePos(); // setScopePos sets the start position of the scope for this Object.
    syntax.Pos setScopePos(syntax.Pos pos);
}
...
private static syntax.Pos Pos(this ptr<object> _addr_obj) {
    ref object obj = ref _addr_obj.val;
...
private static syntax.Pos scopePos(this ptr<object> _addr_obj) {
    ref object obj = ref _addr_obj.val;
...
private static void setScopePos(this ptr<object> _addr_obj, syntax.Pos pos) {
    ref object obj = ref _addr_obj.val;
...
public static ptr<PkgName> NewPkgName(syntax.Pos pos, ptr<Package> _addr_pkg, @string name, ptr<Package> _addr_imported) {
    ref Package pkg = ref _addr_pkg.val;
    ref Package imported = ref _addr_imported.val;

    return addr(new PkgName(object{nil,pos,pkg,name,Typ[Invalid],0,black,nopos},imported,false));
}
...
// The remaining arguments set the attributes found with all Objects.
public static ptr<Const> NewConst(syntax.Pos pos, ptr<Package> _addr_pkg, @string name, Type typ, constant.Value val) {
    ref Package pkg = ref _addr_pkg.val;

    return addr(new Const(object{nil,pos,pkg,name,typ,0,colorFor(typ),nopos},val));
}
...
// effect.
public static ptr<TypeName> NewTypeName(syntax.Pos pos, ptr<Package> _addr_pkg, @string name, Type typ) {
    ref Package pkg = ref _addr_pkg.val;

    return addr(new TypeName(object{nil,pos,pkg,name,typ,0,colorFor(typ),nopos}));
}
...
public static ptr<Var> NewVar(syntax.Pos pos, ptr<Package> _addr_pkg, @string name, Type typ) {
    ref Package pkg = ref _addr_pkg.val;

    return addr(new Var(object:object{nil,pos,pkg,name,typ,0,colorFor(typ),nopos}));
}

// NewParam returns a new variable representing a function parameter.
public static ptr<Var> NewParam(syntax.Pos pos, ptr<Package> _addr_pkg, @string name, Type typ) {
    ref Package pkg = ref _addr_pkg.val;

    return addr(new Var(object:object{nil,pos,pkg,name,typ,0,colorFor(typ),nopos},used:true)); // parameters are always 'used'
}

// NewField returns a new variable representing a struct field.
// For embedded fields, the name is the unqualified type name
/// under which the field is accessible.
public static ptr<Var> NewField(syntax.Pos pos, ptr<Package> _addr_pkg, @string name, Type typ, bool embedded) {
    ref Package pkg = ref _addr_pkg.val;

    return addr(new Var(object:object{nil,pos,pkg,name,typ,0,colorFor(typ),nopos},embedded:embedded,isField:true));
}
...
// the function's type.
public static ptr<Func> NewFunc(syntax.Pos pos, ptr<Package> _addr_pkg, @string name, ptr<Signature> _addr_sig) {
    ref Package pkg = ref _addr_pkg.val;
...
private static bool less(this ptr<Func> _addr_a, ptr<Func> _addr_b) {
    ref Func a = ref _addr_a.val;
    ref Func b = ref _addr_b.val;
...
public static ptr<Label> NewLabel(syntax.Pos pos, ptr<Package> _addr_pkg, @string name) {
    ref Package pkg = ref _addr_pkg.val;
