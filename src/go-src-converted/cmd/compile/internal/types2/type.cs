...
private static slice<ptr<TypeName>> RParams(this ptr<Signature> _addr_s) {
    ref Signature s = ref _addr_s.val;
...
// the result is nil.
public static Type NewSum(slice<Type> types) => func((_, panic, _) => {
    if (len(types) == 0) {
        return null;
    }
    foreach (var (_, t) in types) {
        {
            ptr<Sum> (_, ok) = t._<ptr<Sum>>();
...
private static bool @is(this ptr<Sum> _addr_s, Func<Type, bool> pred) {
    ref Sum s = ref _addr_s.val;
...
private static slice<Type> unpack(Type typ) {
    if (typ == null) {
        return null;
    }
    {
...
private static bool @is(this ptr<Interface> _addr_t, Func<Type, bool> pred) {
    ...
    foreach (var (_, t) in unpack(t.allTypes)) {
        if (!pred(t)) {
            return false;
        }
    }    return true;
...
public partial struct Named {
    public ptr<Checker> check; // for Named.under implementation
    public typeInfo info; // for cycle detection
    public ptr<TypeName> obj; // corresponding declared object
    public ptr<Named> orig; // original, uninstantiated type
    public Type fromRHS; // type (on RHS of declaration) this *Named type is derived from (for cycle reporting)
...
private static ptr<Named> newNamed(this ptr<Checker> _addr_check, ptr<TypeName> _addr_obj, ptr<Named> _addr_orig, Type underlying, slice<ptr<TypeName>> tparams, slice<ptr<Func>> methods) {
    ref Checker check = ref _addr_check.val;
    ref TypeName obj = ref _addr_obj.val;
    ref Named orig = ref _addr_orig.val;

    ptr<Named> typ = addr(new Named(check:check,obj:obj,orig:orig,fromRHS:underlying,underlying:underlying,tparams:tparams,methods:methods));
    if (typ.orig == null) {
        typ.orig = typ;
    }
    if (obj.typ == null) {
...
private static ptr<Named> Orig(this ptr<Named> _addr_t) {
    ref Named t = ref _addr_t.val;
...
private static void SetTParams(this ptr<Named> _addr_t, slice<ptr<TypeName>> tparams) {
    ref Named t = ref _addr_t.val;
...
public partial struct TypeParam {
    ...
    public ulong id; // unique id, for debugging only
    public ptr<TypeName> obj; // corresponding type name
    public nint index; // type parameter index in source order, starting at 0
    public Type bound; // *Named or *Interface; underlying type is always *Interface
}

// Obj returns the type name for the type parameter t.
private static ptr<TypeName> Obj(this ptr<TypeParam> _addr_t) {
    ref TypeParam t = ref _addr_t.val;

    return _addr_t.obj!;
}

// NewTypeParam returns a new TypeParam.
private static ptr<TypeParam> NewTypeParam(this ptr<Checker> _addr_check, ptr<TypeName> _addr_obj, nint index, Type bound) {
    ref Checker check = ref _addr_check.val;
    ref TypeName obj = ref _addr_obj.val;

    assert(bound != null);
    ptr<TypeParam> typ = addr(new TypeParam(check:check,id:nextId(),obj:obj,index:index,bound:bound));
    if (obj.typ == null) {
        obj.typ = typ;
    }
    return _addr_typ!;
...
private partial struct instance {
    ...
    public syntax.Pos pos; // position of type instantiation; for error reporting only
    public ptr<Named> @base; // parameterized type to be instantiated
    public slice<Type> targs; // type arguments
    public slice<syntax.Pos> poslist; // position of each targ; for error reporting only
...
private static Type Underlying(this ptr<Sum> _addr_t) {
    ref Sum t = ref _addr_t.val;
...
private static Type Underlying(this ptr<TypeParam> _addr_t) {
    ref TypeParam t = ref _addr_t.val;
...
private static @string String(this ptr<Sum> _addr_t) {
    ref Sum t = ref _addr_t.val;
...
private static @string String(this ptr<TypeParam> _addr_t) {
    ref TypeParam t = ref _addr_t.val;
...
private static ptr<Sum> asSum(Type t) {
    ptr<Sum> (op, _) = optype(t)._<ptr<Sum>>();
    return _addr_op!;
}
...
private static ptr<TypeParam> asTypeParam(Type t) {
    ptr<TypeParam> (u, _) = under(t)._<ptr<TypeParam>>();
    return _addr_u!;
}

// Exported for the compiler.

public static ptr<Pointer> AsPointer(Type t) {
    return _addr_asPointer(t)!;
}
public static ptr<Named> AsNamed(Type t) {
    return _addr_asNamed(t)!;
}
public static ptr<Signature> AsSignature(Type t) {
    return _addr_asSignature(t)!;
}
public static ptr<Interface> AsInterface(Type t) {
    return _addr_asInterface(t)!;
}
