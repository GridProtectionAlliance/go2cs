...
// package riscv64 -- go2cs converted at 2020 October 08 04:27:28 UTC
// import "cmd/compile/internal/riscv64" ==> using riscv64 = go.cmd.compile.@internal.riscv64_package
// Original source: C:\Go\src\cmd\compile\internal\riscv64\ssa.go
...
namespace @internal
{
    public static partial class riscv64_package
    {
        ...
        private static (obj.As, long) largestMove(long alignment)
        {
            ...
            if (alignment % 8L == 0L) 
                return (riscv.AMOV, 8L);
            else if (alignment % 4L == 0L) 
                return (riscv.AMOVW, 4L);
            else if (alignment % 2L == 0L) 
                return (riscv.AMOVH, 2L);
            else 
                return (riscv.AMOVB, 1L);
        ...
        private static void ssaGenValue(ptr<gc.SSAGenState> _addr_s, ptr<ssa.Value> _addr_v)
        {
            ...
            if (v.Op == ssa.OpInitMem)             else if (v.Op == ssa.OpArg)             else if (v.Op == ssa.OpPhi) 
            ...
            else if (v.Op == ssa.OpCopy || v.Op == ssa.OpRISCV64MOVconvert) 
                if (v.Type.IsMemory())
                {
                    return ;
                }

                var rs = v.Args[0L].Reg();
                var rd = v.Reg();
                if (rs == rd)
                {
                    return ;
                ...
                p.To.Type = obj.TYPE_REG;
                p.To.Reg = v.Reg();
            else if (v.Op == ssa.OpRISCV64MOVBconst || v.Op == ssa.OpRISCV64MOVHconst || v.Op == ssa.OpRISCV64MOVWconst || v.Op == ssa.OpRISCV64MOVDconst) 
                p = s.Prog(v.Op.Asm());
                p.From.Type = obj.TYPE_CONST;
                p.From.Offset = v.AuxInt;
                p.To.Type = obj.TYPE_REG;
                ...
                p3.Reg = v.Args[0L].Reg();
                p3.From.Type = obj.TYPE_REG;
                p3.From.Reg = v.Args[1L].Reg();
                gc.Patch(p3, p);
                ...
                p3.To.Reg = v.Args[0L].Reg();

                p4 = s.Prog(riscv.AADD);
                p4.From.Type = obj.TYPE_CONST;
                p4.From.Offset = sz;
                p4.To.Type = obj.TYPE_REG;
                p4.To.Reg = v.Args[1L].Reg();

                p5 = s.Prog(riscv.ABGEU);
                p5.To.Type = obj.TYPE_BRANCH;
                p5.Reg = v.Args[1L].Reg();
                p5.From.Type = obj.TYPE_REG;
                p5.From.Reg = v.Args[2L].Reg();
                gc.Patch(p5, p);
        ...
        private static void ssaGenBlock(ptr<gc.SSAGenState> _addr_s, ptr<ssa.Block> _addr_b, ptr<ssa.Block> _addr_next)
        {
            ...
            if (b.Kind == ssa.BlockDefer) 
                ...
                if (next == b.Succs[0L].Block()) 
                    p = s.Br(invAs, b.Succs[1L].Block());
                else if (next == b.Succs[1L].Block()) 
                    p = s.Br(as, b.Succs[0L].Block());
                else 
