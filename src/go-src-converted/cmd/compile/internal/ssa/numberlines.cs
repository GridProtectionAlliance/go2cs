...
// package ssa -- go2cs converted at 2020 October 08 04:10:49 UTC
...
namespace @internal
{
    public static partial class ssa_package
    {
        ...
        private static long nextGoodStatementIndex(ptr<Value> _addr_v, long i, ptr<Block> _addr_b)
        {
            ...
            for (var j = i + 1L; j < len(b.Values); j++)
            {
                var u = b.Values[j];
                if (u.Pos.IsStmt() == src.PosNotStmt)
        ...
        private static bool notStmtBoundary(Op op)
        {
            ...
            if (op == OpCopy || op == OpPhi || op == OpVarKill || op == OpVarDef || op == OpVarLive || op == OpUnknown || op == OpFwdRef || op == OpArg) 
                return true;
                        return false;
        ...
        private static long Len(this fileAndPairs fap)
        {
            return len(fap);
        }
        private static bool Less(this fileAndPairs fap, long i, long j)
        {
            return fap[i].f < fap[j].f;
        }
        private static void Swap(this fileAndPairs fap, long i, long j)
        {
            fap[i] = fap[j];
            fap[j] = fap[i];
        ...
        private static void numberLines(ptr<Func> _addr_f)
        {
            ...
            var ranges = make_map<long, lineRange>();
...
; 
            ...
            for (var j = len(po) - 1L; j >= 0L; j--)
            {
                ...
                if (firstPosIndex == -1L)
                { // Effectively empty block, check block's own Pos, consider preds.
                    line = src.NoXPos;
                    {
                        var p__prev2 = p;
                ...
                // iterate forward setting each new (interesting) position as a statement boundary.
                {
                    ...
                    for (i = firstPosIndex + 1L; i < len(b.Values); i++)
                    {
                        v = b.Values[i];
                        if (v.Pos.IsStmt() == src.PosNotStmt)
                        {
            ...
            { 
                ...
                sort.Sort(entries);
                var total = uint64(0L); // sum over files of maxline(file) - minline(file)
                var maxfile = int32(0L); // max(file indices)
                var minline = uint32(0xffffffffUL); // min over files of minline(file)
                var maxline = uint32(0L); // max over files of maxline(file)
                {
                    var v__prev1 = v;
