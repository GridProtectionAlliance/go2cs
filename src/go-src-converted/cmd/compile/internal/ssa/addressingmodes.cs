...
private static void addressingModes(ptr<Func> _addr_f) {
    ...
    var isInImmediateRange = is32Bit;
    ...
    foreach (var (_, b) in f.Blocks) {
        foreach (var (_, v) in b.Values) {
            ...
            var p = v.Args[ptrIndex];
            var (c, ok) = combine[new array<Op>(new Op[] { v.Op, p.Op })];
            if (!ok) {
                continue;
            }

            if (new array<auxType>(new auxType[] { opcodeTable[v.Op].auxType, opcodeTable[p.Op].auxType }) == new array<auxType>(new auxType[] { auxSymOff, auxInt32 })) 
                // TODO: introduce auxSymOff32
                if (!isInImmediateRange(v.AuxInt + p.AuxInt)) {
                    continue;
                }
                v.AuxInt += p.AuxInt;
            else if (new array<auxType>(new auxType[] { opcodeTable[v.Op].auxType, opcodeTable[p.Op].auxType }) == new array<auxType>(new auxType[] { auxSymOff, auxSymOff })) 
                if (v.Aux != null && p.Aux != null) {
                    continue;
                }
                if (!isInImmediateRange(v.AuxInt + p.AuxInt)) {
                    continue;
                ...
                v.AuxInt += p.AuxInt;
            else if (new array<auxType>(new auxType[] { opcodeTable[v.Op].auxType, opcodeTable[p.Op].auxType }) == new array<auxType>(new auxType[] { auxSymValAndOff, auxInt32 })) 
                var vo = ValAndOff(v.AuxInt);
                if (!vo.canAdd64(p.AuxInt)) {
                    continue;
                }
                v.AuxInt = int64(vo.addOffset64(p.AuxInt));
            else if (new array<auxType>(new auxType[] { opcodeTable[v.Op].auxType, opcodeTable[p.Op].auxType }) == new array<auxType>(new auxType[] { auxSymValAndOff, auxSymOff })) 
                ...
                v.AuxInt = int64(vo.addOffset64(p.AuxInt));
            else if (new array<auxType>(new auxType[] { opcodeTable[v.Op].auxType, opcodeTable[p.Op].auxType }) == new array<auxType>(new auxType[] { auxSymOff, auxNone }))             else if (new array<auxType>(new auxType[] { opcodeTable[v.Op].auxType, opcodeTable[p.Op].auxType }) == new array<auxType>(new auxType[] { auxSymValAndOff, auxNone }))             else 
                f.Fatalf("unknown aux combining for %s and %s\n", v.Op, p.Op);
            // Combine the operations.
            tmp = append(tmp[..(int)0], v.Args[..(int)ptrIndex]);
            tmp = append(tmp, p.Args);
            tmp = append(tmp, v.Args[(int)ptrIndex + 1..]);
            v.resetArgs();
            v.Op = c;
            v.AddArgs(tmp);
            if (needSplit[c]) { 
...
private static map combineFirst = /* TODO: Fix this in ScannerBase_Expression::ExitCompositeLit */ new map<Op, bool>{};
