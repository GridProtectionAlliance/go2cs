...
// package ssa -- go2cs converted at 2020 October 08 04:09:44 UTC
// import "cmd/compile/internal/ssa" ==> using ssa = go.cmd.compile.@internal.ssa_package
// Original source: C:\Go\src\cmd\compile\internal\ssa\addressingmodes.go
        ...
        private static void addressingModes(ptr<Func> _addr_f)
        {
            ...
            foreach (var (_, b) in f.Blocks)
            {
                foreach (var (_, v) in b.Values)
                {
                    ...
                    tmp = append(tmp[..0L], v.Args[..ptrIndex]);
                    tmp = append(tmp, p.Args);
                    tmp = append(tmp, v.Args[ptrIndex + 1L..]);
                    v.resetArgs();
                    v.Op = c;
                    v.AddArgs(tmp);
                    if (needSplit[c])
