    ...
    foreach (var (_, b) in po) {
        ...
        if (b.Kind == BlockExit) 
            ...
            if (len(b.Succs) == 1) {
                ...
                var b0 = b.Succs[0].b.ID;
                var b1 = b.Succs[1].b.ID;
...
private static ptr<loopnest> loopnestfor(ptr<Func> _addr_f) {
    ...
    var loops = make_slice<ptr<loop>>(0);
 ...
 { // No loop
...

    {
        ...
        foreach (var (_, __l) in loops) {
            ...
            while (len(tovisit) > 0) {
                var cur = tovisit[len(tovisit) - 1];
                tovisit = tovisit[..(int)len(tovisit) - 1];
    ...
    if (f.pass != null && f.pass.stats > 0 && len(loops) > 0) {
        ln.assembleChildren();
        ln.calculateDepths();
        ln.findExits(); 
    ...
    if (f.pass != null && f.pass.debug > 1 && len(loops) > 0) {
...
private static void findExits(this ptr<loopnest> _addr_ln) {
    ...
    foreach (var (_, b) in ln.po) {
        var l = b2l[b.ID];
        if (l != null && len(b.Succs) == 2) {
            var sl = b2l[b.Succs[0].b.ID];
            if (recordIfExit(_addr_l, _addr_sl, _addr_b.Succs[0].b)) {
                continue;
            }
            sl = b2l[b.Succs[1].b.ID];
            if (recordIfExit(_addr_l, _addr_sl, _addr_b.Succs[1].b)) {
                continue;
            }
        }
