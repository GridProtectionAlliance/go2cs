...
private static void nilcheckelim(ptr<Func> _addr_f) => func((defer, _, _) => {
    ...
    while (len(work) > 0) {
        ...
        if (node.op == Work) 
            ...
            for (nint j = 0; j < i; j++) { // is this an ordering problem?
                var v = b.Values[j];
                if (v.Pos.IsStmt() != src.PosNotStmt && !isPoorStatementOp(v.Op) && pendingLines.contains(v.Pos)) {
                    v.Pos = v.Pos.WithIsStmt();
                    pendingLines.remove(v.Pos);
...
private static readonly nint minZeroPage = 4096;

// faultOnLoad is true if a load to an address below minZeroPage will trigger a SIGSEGV.


// faultOnLoad is true if a load to an address below minZeroPage will trigger a SIGSEGV.
private static var faultOnLoad = buildcfg.GOOS != "aix";

// nilcheckelim2 eliminates unnecessary nil checks.
// Runs after lowering and scheduling.
private static void nilcheckelim2(ptr<Func> _addr_f) => func((defer, _, _) => {
    ref Func f = ref _addr_f.val;
        ...
        var firstToRemove = len(b.Values);
        {
            ...
            for (var i = len(b.Values) - 1; i >= 0; i--) {
                ...
                if (opcodeTable[v.Op].nilCheck && unnecessary.contains(v.Args[0].ID)) {
                    ...
                    var u = b.Values[unnecessary.get(v.Args[0].ID)];
                ...
                array<ptr<Value>> ptrstore = new array<ptr<Value>>(2);
                var ptrs = ptrstore[..(int)0];
                ...
                foreach (var (_, ptr) in ptrs) { 
                    ...
                    if (opcodeTable[v.Op].auxType == auxSym) 
                        ...
                        if (v.Aux != null || v.AuxInt < 0 || v.AuxInt >= minZeroPage) {
                            continue;
                        }
                    else if (opcodeTable[v.Op].auxType == auxSymValAndOff) 
                        var off = ValAndOff(v.AuxInt).Off();
                        if (v.Aux != null || off < 0 || off >= minZeroPage) {
                            continue;
                        }
