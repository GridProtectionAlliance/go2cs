...
private static void flagalloc(ptr<Func> _addr_f) => func((defer, panic, _) => {
    ...
    slice<ptr<Value>> oldSched = default;
    {
        ...
        foreach (var (_, __b) in f.Blocks) {
            b = __b;
            oldSched = append(oldSched[..(int)0], b.Values);
            ...
            if (len(b.Preds) > 0) {
                ...
                // Note: the following condition depends on the lack of critical edges.
                {
                    ...
                    foreach (var (_, __e) in b.Preds[(int)1..]) {
                        e = __e;
                        p = e.b;
                        if (end[p.ID] != flag) {
...

    {
        ...
        for (nint i = 0; i < len(remove); i++) {
            v = remove[i];
            if (v.Uses == 0) {
                v.reset(OpInvalid);
                continue;
            }
            var last = len(remove) - 1;
            remove[i] = remove[last];
            remove[last] = null;
            remove = remove[..(int)last];
...
private static ptr<Value> copyFlags(ptr<Value> _addr_v, ptr<Block> _addr_b) {
    ...
    var flagsArgs = make_map<nint, ptr<Value>>();
    {
        var i__prev1 = i;
        var a__prev1 = a;
