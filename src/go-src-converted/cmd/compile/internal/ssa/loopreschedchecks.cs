...
private static void insertLoopReschedChecks(ptr<Func> _addr_f) {
...

    {
        ...
        foreach (var (_, __emc) in tofixBackedges) {
            ...
            var mem1 = sched.NewValue1A(bb.Pos, OpStaticCall, types.TypeMem, StaticAuxCall(resched, null), mem0);
            sched.AddEdgeTo(h);
...
private static ptr<Value> newPhiFor(ptr<Block> _addr_b, ptr<Value> _addr_v) {
    ...
    foreach (>>MARKER:FORRANGEEXPRESSIONS_LEVEL_1<< in b.Preds) {>>MARKER:FORRANGEMUTABLEEXPRESSIONS_LEVEL_1<<
        phiV.AddArg(v);
    }    return _addr_phiV!;
}
...
private static void rewriteNewPhis(ptr<Block> _addr_h, ptr<Block> _addr_b, ptr<Func> _addr_f, slice<ptr<Value>> defsForUses, map<ptr<Block>, rewrite> newphis, map<rewriteTarget, bool> dfPhiTargets, SparseTree sdom) {
    ...
    if (x != null) { // don't waste time on the common case of no definition.
...
private static slice<ptr<Value>> findLastMems(ptr<Func> _addr_f) => func((defer, _, _) => {
    ref Func f = ref _addr_f.val;
...
private static readonly markKind notFound = iota; // block has not been discovered yet
