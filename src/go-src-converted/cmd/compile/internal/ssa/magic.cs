...
// package ssa -- go2cs converted at 2020 October 08 04:10:44 UTC
...
namespace @internal
{
    public static partial class ssa_package
    {
        ...
        private static bool udivisibleOK(ulong n, long c)
        { 
            // Convert from ConstX auxint values to the real uint64 constant they represent.
            var d = uint64(c) << (int)((64L - n)) >> (int)((64L - n)); 
        ...
        private static udivisibleData udivisible(ulong n, long c)
        { 
            // Convert from ConstX auxint values to the real uint64 constant they represent.
            var d = uint64(c) << (int)((64L - n)) >> (int)((64L - n));
        ...
        private static bool sdivisibleOK(ulong n, long c)
        {
            if (c < 0L)
            { 
                // Doesn't work for negative c.
        ...
        private static sdivisibleData sdivisible(ulong n, long c)
        {
            var d = uint64(c);
            var k = bits.TrailingZeros64(d);
            var d0 = d >> (int)(uint(k)); // the odd portion of the divisor
            ...
            var a = ((mask >> (int)(1L)) / d0) & -(1L << (int)(uint(k)));
            long max = (2L * a) >> (int)(uint(k));
