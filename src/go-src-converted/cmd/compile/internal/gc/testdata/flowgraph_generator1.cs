...
// package main -- go2cs converted at 2020 October 09 05:43:51 UTC
...
namespace go
{
    public static partial class main_package
    {
        ...
        private static @string fmtBlocks(slice<blo> bs)
        {
            ...
            {
                s += fmt.Sprintf("blo{inc:%d, cond:%v, succs:[2]int64{%d, %d}},", b.inc, b.cond, b.succs[0L], b.succs[1L]);
            }
            s += "}";
            return s;
    ...
    "in\nimport \"fmt\"\nvar glob bool\n");
            ...
            foreach (var (_, g) in graphs)
            {
                ...
                long nconfigs = 1L << (int)(uint(len(split) - 1L));

                for (long i = 0L; i < nconfigs; i++)
                {
