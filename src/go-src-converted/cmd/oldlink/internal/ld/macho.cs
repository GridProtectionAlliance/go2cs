...
// package ld -- go2cs converted at 2020 October 08 04:41:39 UTC
// import "cmd/oldlink/internal/ld" ==> using ld = go.cmd.oldlink.@internal.ld_package
// Original source: C:\Go\src\cmd\oldlink\internal\ld\macho.go
...
namespace @internal
{
    public static partial class ld_package
    {
        ...
        private static slice<ptr<sym.Symbol>> sortsym = default;
        ...
        private static long symkind(ptr<sym.Symbol> _addr_s)
        {
        ...
        private static void addsym(ptr<Link> _addr_ctxt, ptr<sym.Symbol> _addr_s, @string name, SymbolType type_, long addr, ptr<sym.Symbol> _addr_gotype)
        {
            ref Link ctxt = ref _addr_ctxt.val;
            ref sym.Symbol s = ref _addr_s.val;
        ...
        private partial struct machoscmp // : slice<ptr<sym.Symbol>>
        {
        }
        ...
        private static bool Less(this machoscmp x, long i, long j)
        {
            ...
            var k1 = symkind(_addr_s1);
            var k2 = symkind(_addr_s2);
            if (k1 != k2)
            {
                return k1 < k2;
            }
        ...
        private static void machogenasmsym(ptr<Link> _addr_ctxt)
        {
            ref Link ctxt = ref _addr_ctxt.val;
            ...
            { 
                ...
                if (s.Type == sym.SDYNIMPORT || s.Type == sym.SHOSTOBJ || s.Type == sym.SUNDEFEXT)
                {
                    if (s.Attr.Reachable())
                    {
                        addsym(_addr_ctxt, _addr_s, "", DataSym, 0L, _addr_null);
                    }
        ...
        private static void machosymorder(ptr<Link> _addr_ctxt)
        {
            ref Link ctxt = ref _addr_ctxt.val;
            ...
            machogenasmsym(_addr_ctxt);
            sortsym = make_slice<ptr<sym.Symbol>>(nsortsym);
            nsortsym = 0L;
            machogenasmsym(_addr_ctxt);
        ...
        private static void machosymtab(ptr<Link> _addr_ctxt)
        {
            ...
            for (long i = 0L; i < nsortsym; i++)
            {
                ...
                // See Issue #18190.
                var cexport = !isGoSymbol && (ctxt.BuildMode != BuildModePlugin || onlycsymbol(s));
        ...
        public static void Machoemitreloc(ptr<Link> _addr_ctxt)
        {
            ref Link ctxt = ref _addr_ctxt.val;
            ...
            machorelocsect(_addr_ctxt, _addr_Segtext.Sections[0L], ctxt.Textp);
            {
                ...
                {
                    sect = __sect;
                    machorelocsect(_addr_ctxt, _addr_sect, datap);
                }
...

            {
                ...
                {
                    sect = __sect;
                    machorelocsect(_addr_ctxt, _addr_sect, dwarfp);
                }
