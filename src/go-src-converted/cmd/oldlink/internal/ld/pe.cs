...
namespace @internal
{
    public static partial class ld_package
    {
        ...
        private static ptr<sym.Symbol> rsrcsym;        public static int PESECTHEADR = default;        public static int PEFILEHEADR = default;        private static long pe64 = default;        private static ptr<Dll> dr;        private static array<ptr<sym.Symbol>> dexport = new array<ptr<sym.Symbol>>(1024L);        private static long nexport = default;
        ...
        private static void emitRelocations(this ptr<peFile> _addr_f, ptr<Link> _addr_ctxt)
        {
            ref peFile f = ref _addr_f.val;
            ref Link ctxt = ref _addr_ctxt.val;
...
dwarfLoop:
        ...
        private static void writeSymbols(this ptr<peFile> _addr_f, ptr<Link> _addr_ctxt)
        {
            ...
            Action<ptr<Link>, ptr<sym.Symbol>, @string, SymbolType, long, ptr<sym.Symbol>> put = (ctxt, s, name, type_, addr, gotype) =>
            {
                if (s == null)
                {
                    return ;
...
;
                ...
                {
                    {
                        ...
                        while (m != null)
                        {
                            ...
                            strput(_addr_ctxt.Out, m.s.Extname());
                            m = m.next;
                        }
        ...
        private partial struct byExtname // : slice<ptr<sym.Symbol>>
        {
        }
...
// It is stored separately for ease of sorting.
 ...
 // 2 * sizeof(uint32)
        ...
        private static void setpersrc(ptr<Link> _addr_ctxt, ptr<sym.Symbol> _addr_sym)
        {
            ref Link ctxt = ref _addr_ctxt.val;
        ...
        private static void addpersrc(ptr<Link> _addr_ctxt)
        {
            ...
            // relocation
            foreach (var (ri) in rsrcsym.R)
            {
                var r = _addr_rsrcsym.R[ri];
                var p = rsrcsym.P[r.Off..];
