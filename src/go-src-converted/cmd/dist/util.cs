...
namespace go
{
    public static partial class main_package
    {
        ...
        private static slice<@string> uniq(slice<@string> list)
        {
            ...
            {
                if (len(keep) == 0L || keep[len(keep) - 1L] != x)
                {
                    keep = append(keep, x);
                }
        ...
        // not other callers.
        private static @string run(@string dir, long mode, params @string[] cmd)
        {
            cmd = cmd.Clone();
        ...
        private static void writefile(@string text, @string file, long flag)
        {
            slice<byte> @new = (slice<byte>)text;
            if (flag & writeSkipSame != 0L)
            {
        ...
        private static slice<@string> xreaddir(@string dir) => func((defer, _, __) =>
        {
        ...
        // The names are relative to dir; they are not full paths.
        private static slice<@string> xreaddirfiles(@string dir) => func((defer, _, __) =>
        {
            var (f, err) = os.Open(dir);
            if (err != null)
            {
        ...
        private static void xexit(long n)
        {
            for (var i = len(atexits) - 1L; i >= 0L; i--)
            {
                atexits[i]();
            }
        ...
        private static @string xgetgoarm()
        {
            ...
            var @out = run("", 0L, os.Args[0L], "-check-goarm");
        ...
        { 
            ...
            switch (hdr[5L])
            {
                ...
                case 2L: 
                    ...
                    break;
                default: 
                    fatalf("unknown ELF endianness of %s: EI_DATA = %d", fn, hdr[5L]);
                    break;
            }
            panic("unreachable");
