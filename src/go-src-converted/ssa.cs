...
// package ssa -- go2cs converted at 2022 March 06 23:33:27 UTC
...
public partial struct BasicBlock {
    public nint Index; // index of this block within Parent().Blocks
    ...
    public nint gaps; // number of nil Instrs (transient)
    public nint rundefers; // number of rundefers (transient)
}
...
public partial struct Alloc {
    ...
    public bool Heap;
    public nint index; // dense numbering; for lifting
}
...
public partial struct FieldAddr {
    ...
    public nint Field; // field is X.Type().Underlying().(*types.Pointer).Elem().Underlying().(*types.Struct).Field(Field)
}
...
public partial struct Field {
    public ref register register => ref register_val;
    public Value X; // struct
    public nint Field; // index into X.Type().(*types.Struct).Fields
}
...
private partial struct register {
    public ref anInstruction anInstruction => ref anInstruction_val;
    public nint num; // "name" of virtual register, e.g. "t0".  Not guaranteed unique.
...
private static void setNum(this ptr<register> _addr_v, nint num) {
    ref register v = ref _addr_v.val;
