...
// Original source: C:\Program Files\Go\src\crypto\aes\cbc_s390x.go
...
private static nint BlockSize(this ptr<cbc> _addr_x) {
...
//go:noescape
private static void cryptBlocksChain(code c, ptr<byte> iv, ptr<byte> key, ptr<byte> dst, ptr<byte> src, nint length);

private static void CryptBlocks(this ptr<cbc> _addr_x, slice<byte> dst, slice<byte> src) => func((_, panic, _) => {
    ref cbc x = ref _addr_x.val;

    if (len(src) % BlockSize != 0) {>>MARKER:FUNCTION_cryptBlocksChain_BLOCK_PREFIX<<
    ...
    if (len(src) > 0) {
        cryptBlocksChain(x.c, _addr_x.iv[0], _addr_x.b.key[0], _addr_dst[0], _addr_src[0], len(src));
    }
});

private static void SetIV(this ptr<cbc> _addr_x, slice<byte> iv) => func((_, panic, _) => {
    ref cbc x = ref _addr_x.val;
