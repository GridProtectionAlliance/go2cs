        ...
        private static void blockGeneric(ptr<digest> _addr_dig, slice<byte> p)
        {
            ...
            while (len(p) >= chunk)
            {
                ...
                }
                {
                    ...
                    {
                        var v1 = w[i - 2L];
                        var t1 = bits.RotateLeft64(v1, -19L) ^ bits.RotateLeft64(v1, -61L) ^ (v1 >> (int)(6L));
                        var v2 = w[i - 15L];
                        var t2 = bits.RotateLeft64(v2, -1L) ^ bits.RotateLeft64(v2, -8L) ^ (v2 >> (int)(7L));
...

                {
                    ...
                    for (i = 0L; i < 80L; i++)
                    {
                        t1 = h + (bits.RotateLeft64(e, -14L) ^ bits.RotateLeft64(e, -18L) ^ bits.RotateLeft64(e, -41L)) + ((e & f) ^ (~e & g)) + _K[i] + w[i];

                        t2 = (bits.RotateLeft64(a, -28L) ^ bits.RotateLeft64(a, -34L) ^ bits.RotateLeft64(a, -39L)) + ((a & b) ^ (a & c) ^ (b & c));
