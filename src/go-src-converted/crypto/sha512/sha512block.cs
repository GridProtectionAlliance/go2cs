...
private static void blockGeneric(ptr<digest> _addr_dig, slice<byte> p) {
    ...
    array<ulong> w = new array<ulong>(80);
    var h0 = dig.h[0];
    var h1 = dig.h[1];
    var h2 = dig.h[2];
    var h3 = dig.h[3];
    ...
    while (len(p) >= chunk) {
        {
            ...
            for (nint i = 0; i < 16; i++) {
                var j = i * 8;
                w[i] = uint64(p[j]) << 56 | uint64(p[j + 1]) << 48 | uint64(p[j + 2]) << 40 | uint64(p[j + 3]) << 32 | uint64(p[j + 4]) << 24 | uint64(p[j + 5]) << 16 | uint64(p[j + 6]) << 8 | uint64(p[j + 7]);
            }
        ...
        }
        {
            ...
            for (i = 16; i < 80; i++) {
                var v1 = w[i - 2];
                var t1 = bits.RotateLeft64(v1, -19) ^ bits.RotateLeft64(v1, -61) ^ (v1 >> 6);
                var v2 = w[i - 15];
                var t2 = bits.RotateLeft64(v2, -1) ^ bits.RotateLeft64(v2, -8) ^ (v2 >> 7);
