    ...
    public static partial class x509_package
    {
        ...
        private static (ptr<CertPool>, error) loadSystemRoots() => func((defer, _, __) =>
        {
            ...
            foreach (var (_, domain) in new slice<macOS.SecTrustSettingsDomain>(new macOS.SecTrustSettingsDomain[] { macOS.SecTrustSettingsDomainUser, macOS.SecTrustSettingsDomainAdmin, macOS.SecTrustSettingsDomainSystem }))
            {
                ...
                for (long i = 0L; i < macOS.CFArrayGetCount(certs); i++)
                {
        ...
        private static (ptr<Certificate>, error) exportCertificate(macOS.CFRef cert) => func((defer, _, __) =>
        {
        ...
        private static (macOS.SecTrustSettingsResult, error) sslTrustSettingsResult(macOS.CFRef cert) => func((defer, _, __) =>
        {
            ...
            if (macOS.CFArrayGetCount(trustSettings) == 0L)
            {
...
;
            ...
            for (long i = 0L; i < macOS.CFArrayGetCount(trustSettings); i++)
            {
