...
// package x509 -- go2cs converted at 2020 October 08 03:36:56 UTC
// import "crypto/x509" ==> using x509 = go.crypto.x509_package
// Original source: C:\Go\src\crypto\x509\root_cgo_darwin_amd64.go
...
*/
using C = go.C_package;// This cgo implementation exists only to support side-by-side testing by
// TestSystemRoots. It can be removed once we are confident in the no-cgo
// implementation.
...
namespace crypto
{
    public static partial class x509_package
    {
        ...
        private static (ptr<CertPool>, error) _loadSystemRootsWithCgo() => func((defer, _, __) =>
        {
            ptr<CertPool> _p0 = default!;
            error _p0 = default!;

            ref C.CFDataRef data = ref heap(out ptr<C.CFDataRef> _addr_data);            ref C.CFDataRef untrustedData = ref heap(out ptr<C.CFDataRef> _addr_untrustedData);

            var err = C.CopyPEMRoots(_addr_data, _addr_untrustedData, C.@bool(debugDarwinRoots));
            if (err == -1L)
            {
                return (_addr_null!, error.As(errors.New("crypto/x509: failed to load darwin system roots with cgo"))!);
            }

            defer(C.CFRelease(C.CFTypeRef(data)));
            defer(C.CFRelease(C.CFTypeRef(untrustedData)));

            var buf = C.GoBytes(@unsafe.Pointer(C.CFDataGetBytePtr(data)), C.@int(C.CFDataGetLength(data)));
            var roots = NewCertPool();
            roots.AppendCertsFromPEM(buf);

            if (C.CFDataGetLength(untrustedData) == 0L)
            {
            ...
            foreach (var (_, c) in roots.certs)
            {
            ...
            return (_addr_trustedRoots!, error.As(null!)!);
