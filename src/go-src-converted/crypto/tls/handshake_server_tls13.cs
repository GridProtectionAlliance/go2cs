...
namespace crypto
{
    public static partial class tls_package
    {
        ...
        private static readonly long maxClientPSKIdentities = (long)5L;
        ...
        private static error processClientHello(this ptr<serverHandshakeStateTLS13> _addr_hs)
        {
            ...
            if (len(hs.clientHello.supportedVersions) == 0L)
            {
            ...
            if (len(hs.clientHello.compressionMethods) != 1L || hs.clientHello.compressionMethods[0L] != compressionNone)
            {
                c.sendAlert(alertIllegalParameter);
...
GroupSelection:
            ...
            if (clientKeyShare == null)
            {
                ...
                clientKeyShare = _addr_hs.clientHello.keyShares[0L];
        ...
        private static error checkForResumption(this ptr<serverHandshakeStateTLS13> _addr_hs)
        {
            ...
            if (len(hs.clientHello.pskIdentities) == 0L)
            {
                return error.As(null!)!;
            }
            ...
            foreach (var (i, identity) in hs.clientHello.pskIdentities)
            {
                ...
                var createdAt = time.Unix(int64(sessionState.createdAt), 0L);
                ...
                var sessionHasClientCerts = len(sessionState.certificate.Certificate) != 0L;
        ...
        private static error pickCertificate(this ptr<serverHandshakeStateTLS13> _addr_hs)
        {
            ...
            if (len(hs.clientHello.supportedSignatureAlgorithms) == 0L)
            {
                return error.As(c.sendAlert(alertMissingExtension))!;
            }

            var (certificate, err) = c.config.getCertificate(clientHelloInfo(c, hs.clientHello));
            if (err != null)
            {
        ...
        private static error doHelloRetryRequest(this ptr<serverHandshakeStateTLS13> _addr_hs, CurveID selectedGroup)
        {
            ...
            if (len(clientHello.keyShares) != 1L || clientHello.keyShares[0L].group != selectedGroup)
            {
                c.sendAlert(alertIllegalParameter);
        ...
        private static error sendServerCertificate(this ptr<serverHandshakeStateTLS13> _addr_hs)
        {
            ...
            certMsg.scts = hs.clientHello.scts && len(hs.cert.SignedCertificateTimestamps) > 0L;
            certMsg.ocspStapling = hs.clientHello.ocspStapling && len(hs.cert.OCSPStaple) > 0L;
            ...
            if (err != null)
            {
                crypto.Signer @public = hs.cert.PrivateKey._<crypto.Signer>().Public();
                {
                    ...
                    if (ok && sigType == signatureRSAPSS && rsaKey.N.BitLen() / 8L < sigHash.Size() * 2L + 2L)
