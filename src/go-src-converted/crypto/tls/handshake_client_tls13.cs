...
private static error handshake(this ptr<clientHandshakeStateTLS13> _addr_hs) {
    ...
    if (hs.ecdheParams == null || len(hs.hello.keyShares) != 1) {
...
private static error checkServerHelloOrHRR(this ptr<clientHandshakeStateTLS13> _addr_hs) {
    ...
    if (hs.serverHello.ocspStapling || hs.serverHello.ticketSupported || hs.serverHello.secureRenegotiationSupported || len(hs.serverHello.secureRenegotiation) != 0 || len(hs.serverHello.alpnProtocol) != 0 || len(hs.serverHello.scts) != 0) {
...
private static error processHelloRetryRequest(this ptr<clientHandshakeStateTLS13> _addr_hs) {
    ...
    if (hs.serverHello.selectedGroup == 0 && hs.serverHello.cookie == null) {
        c.sendAlert(alertIllegalParameter);
...
private static error processServerHello(this ptr<clientHandshakeStateTLS13> _addr_hs) {
    ...
    if (len(hs.hello.pskIdentities) != 1 || hs.session == null) {
        return error.As(c.sendAlert(alertInternalError))!;
    }
    var pskSuite = cipherSuiteTLS13ByID(hs.session.cipherSuite);
    if (pskSuite == null) {
...
private static error readServerParameters(this ptr<clientHandshakeStateTLS13> _addr_hs) {
    ...
    {
        var err = checkALPN(hs.hello.alpnProtocols, encryptedExtensions.alpnProtocol);
...
private static error sendClientCertificate(this ptr<clientHandshakeStateTLS13> _addr_hs) {
    ...
    var (cert, err) = c.getClientCertificate(addr(new CertificateRequestInfo(AcceptableCAs:hs.certReq.certificateAuthorities,SignatureSchemes:hs.certReq.supportedSignatureAlgorithms,Version:c.vers,ctx:hs.ctx,)));
    if (err != null) {
        return error.As(err)!;
    }
