...
public static (net.Listener, error) Listen(@string network, @string laddr, ptr<Config> _addr_config) {
    ...
    if (config == null || len(config.Certificates) == 0 && config.GetCertificate == null && config.GetConfigForClient == null) {
...
private static (ptr<Conn>, error) dial(context.Context ctx, ptr<net.Dialer> _addr_netDialer, @string network, @string addr, ptr<Config> _addr_config) => func((defer, _, _) => {
    ptr<Conn> _p0 = default!;
    error _p0 = default!;
    ...
    var hostname = addr[..(int)colonPos];
...
public static (Certificate, error) X509KeyPair(slice<byte> certPEMBlock, slice<byte> keyPEMBlock) {
        ...
        else
 {
    ...
    var (x509Cert, err) = x509.ParseCertificate(cert.Certificate[0]);
    if (err != null) {
        return fail(err);
    }
