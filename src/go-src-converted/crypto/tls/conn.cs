...
namespace crypto
{
    public static partial class tls_package
    {
        ...
        public partial struct Conn
        {
            ...
            public Func<error> handshakeFn; // (*Conn).clientHandshake or serverHandshake
...
// renegotiation is not supported in that case.)
            ...
            public Func<@string, slice<byte>, long, (slice<byte>, error)> ekm; // resumptionSecret is the resumption_master_secret for handling
...
// channel-binding.
            ...
            public slice<byte> outBuf; // scratch buffer used by out.encrypt
...
// handshake, nor deliver application data. Protected by in.Mutex.
            public long retryCount; // activeCall is an atomic int32; the low bit is whether Close has
...
// in Conn.Write.
        ...
        private static @string Error(this ptr<permamentError> _addr_e)
        {
            ref permamentError e = ref _addr_e.val;

            return e.err.Error();
        }
        private static error Unwrap(this ptr<permamentError> _addr_e)
        {
            ref permamentError e = ref _addr_e.val;

            return error.As(e.err)!;
        }
        private static bool Timeout(this ptr<permamentError> _addr_e)
        {
            ref permamentError e = ref _addr_e.val;

            return e.err.Timeout();
        }
        private static bool Temporary(this ptr<permamentError> _addr_e)
        {
            ref permamentError e = ref _addr_e.val;
        ...
        private static error setErrorLocked(this ptr<halfConn> _addr_hc, error err)
        {
...

            {
                ...
                if (ok)
                {
                    hc.err = addr(new permamentError(err:e));
                }
                else
                {
                    hc.err = err;
        ...
        private static void prepareCipherSpec(this ptr<halfConn> _addr_hc, ushort version, object cipher, macFunction mac)
        {
            ref halfConn hc = ref _addr_hc.val;
        ...
        private static void incSeq(this ptr<halfConn> _addr_hc) => func((_, panic, __) =>
        {
            ref halfConn hc = ref _addr_hc.val;
        ...
        private static long explicitNonceLen(this ptr<halfConn> _addr_hc) => func((_, panic, __) =>
        {
            ref halfConn hc = ref _addr_hc.val;
        ...
        private static (slice<byte>, recordType, error) decrypt(this ptr<halfConn> _addr_hc, slice<byte> record) => func((_, panic, __) =>
        {
            ...
            var typ = recordType(record[0L]);
            var payload = record[recordHeaderLen..]; 
            ...
            if (hc.cipher != null)
            {
                switch (hc.cipher.type())
                {
                    ...
                    case aead c:
                        ...
                        {
                            return (null, 0L, error.As(alertBadRecordMAC)!);
                        }

                        var nonce = payload[..explicitNonceLen];
                        if (len(nonce) == 0L)
                        {
                            nonce = hc.seq[..];
                        }
                        ...
                        var additionalData = hc.additionalData[..];
                        ...
                        else
                        {
                            ...
                            copy(additionalData[8L..], record[..3L]);
                            var n = len(payload) - c.Overhead();
                            additionalData[11L] = byte(n >> (int)(8L));
                            additionalData[12L] = byte(n);
                        }

                        error err = default!;
                        plaintext, err = c.Open(payload[..0L], nonce, payload, additionalData);
                        if (err != null)
                        {
                            return (null, 0L, error.As(alertBadRecordMAC)!);
                        }
                    ...
                    case cbcMode c:
                        ...
                        var minPayload = explicitNonceLen + roundUp(hc.mac.Size() + 1L, blockSize);
                        if (len(payload) % blockSize != 0L || len(payload) < minPayload)
                        {
                            return (null, 0L, error.As(alertBadRecordMAC)!);
                        }

                        if (explicitNonceLen > 0L)
                        {
                ...
                if (hc.version == VersionTLS13)
                {
                    ...
                    {
                        return (null, 0L, error.As(alertUnexpectedMessage)!);
                    }

                    if (len(plaintext) > maxPlaintext + 1L)
                    {
                        return (null, 0L, error.As(alertRecordOverflow)!);
                    } 
                    // Remove padding and find the ContentType scanning from the end.
                    for (var i = len(plaintext) - 1L; i >= 0L; i--)
                    {
                        if (plaintext[i] != 0L)
                        {
            ...
            if (hc.mac != null)
            {
                ...
                record[3L] = byte(n >> (int)(8L));
                record[4L] = byte(n);
                var remoteMAC = payload[n..n + macSize];
                var localMAC = hc.mac.MAC(hc.seq[0L..], record[..recordHeaderLen], payload[..n], payload[n + macSize..]); 
        ...
        private static (slice<byte>, error) encrypt(this ptr<halfConn> _addr_hc, slice<byte> record, slice<byte> payload, io.Reader rand) => func((_, panic, __) =>
        {
            slice<byte> _p0 = default;
            error _p0 = default!;
            ref halfConn hc = ref _addr_hc.val;
            ...
            slice<byte> mac = default;
            if (hc.mac != null)
            {
                mac = hc.mac.MAC(hc.seq[..], record[..recordHeaderLen], payload, null);
            }
            ...
            switch (hc.cipher.type())
            {
                ...
                case aead c:
                    ...
                    if (hc.version == VersionTLS13)
                    {
                        ...
                        record = append(record, record[0L]);
                        record[0L] = byte(recordTypeApplicationData);

                        var n = len(payload) + 1L + c.Overhead();
                        record[3L] = byte(n >> (int)(8L));
                        record[4L] = byte(n);

                        record = c.Seal(record[..recordHeaderLen], nonce, record[recordHeaderLen..], record[..recordHeaderLen]);

                    }
                    else
                    {
                        copy(hc.additionalData[..], hc.seq[..]);
                        copy(hc.additionalData[8L..], record);
                ...
                case cbcMode c:
                    ...
                    {
                        dst[i] = byte(paddingLen - 1L);
                    }

                    if (len(explicitNonce) > 0L)
                    {
...
// been written to the connection.
        ...
        private static error readRecordOrCCS(this ptr<Conn> _addr_c, bool expectChangeCipherSpec)
        {
            ...
            // Read header, payload.
            {
                ...
                { 
                    ...
                    if (err == io.ErrUnexpectedEOF && c.rawInput.Len() == 0L)
                    {
                        err = io.EOF;
                    }
            ...
            var hdr = c.rawInput.Bytes()[..recordHeaderLen];
            var typ = recordType(hdr[0L]); 

            // No valid TLS record has a type of 0x80, however SSLv2 handshakes
            // start with a uint16 length where the MSB is set and the first record
            // is always < 256 bytes long. Therefore typ == 0x80 strongly suggests
            // an SSLv2 client.
            if (!handshakeComplete && typ == 0x80UL)
            {
                c.sendAlert(alertProtocolVersion);
                return error.As(c.@in.setErrorLocked(c.newRecordHeaderError(null, "unsupported SSLv2 handshake received")))!;
            }

            var vers = uint16(hdr[1L]) << (int)(8L) | uint16(hdr[2L]);
            var n = int(hdr[3L]) << (int)(8L) | int(hdr[4L]);
            ...
            { 
                ...
                if ((typ != recordTypeAlert && typ != recordTypeHandshake) || vers >= 0x1000UL)
                {
            ...
            if (typ != recordTypeAlert && typ != recordTypeChangeCipherSpec && len(data) > 0L)
            { 
            ...
            if (c.vers == VersionTLS13 && typ != recordTypeHandshake && c.hand.Len() > 0L)
            {
                return error.As(c.@in.setErrorLocked(c.sendAlert(alertUnexpectedMessage)))!;
            }
            ...
            if (typ == recordTypeAlert) 
                ...
                if (len(data) == 0L || expectChangeCipherSpec)
                {
                    return error.As(c.@in.setErrorLocked(c.sendAlert(alertUnexpectedMessage)))!;
                }
        ...
        private static (long, error) Read(this ptr<atLeastReader> _addr_r, slice<byte> p)
        {
            long _p0 = default;
            error _p0 = default!;
        ...
        private static error readFromUntil(this ptr<Conn> _addr_c, io.Reader r, long n)
        {
            ref Conn c = ref _addr_c.val;
        ...
        // sendAlert sends a TLS alert message.
        private static error sendAlert(this ptr<Conn> _addr_c, alert err) => func((defer, _, __) =>
        {
            ref Conn c = ref _addr_c.val;
        ...
        private static readonly long tcpMSSEstimate = (long)1208L; 

        // recordSizeBoostThreshold is the number of bytes of application data
        // sent after which the TLS record size will be increased to the
        // maximum.
        private static readonly long recordSizeBoostThreshold = (long)128L * 1024L;
        ...
        private static long maxPayloadSizeForWrite(this ptr<Conn> _addr_c, recordType typ) => func((_, panic, __) =>
        {
            ref Conn c = ref _addr_c.val;
        ...
        private static (long, error) write(this ptr<Conn> _addr_c, slice<byte> data)
        {
            long _p0 = default;
            error _p0 = default!;
            ref Conn c = ref _addr_c.val;
        ...
        private static (long, error) flush(this ptr<Conn> _addr_c)
        {
            long _p0 = default;
            error _p0 = default!;
            ref Conn c = ref _addr_c.val;
        ...
        private static (long, error) writeRecordLocked(this ptr<Conn> _addr_c, recordType typ, slice<byte> data)
        {
            long _p0 = default;
            error _p0 = default!;
            ref Conn c = ref _addr_c.val;
            ...
            while (len(data) > 0L)
            {
                ...
                _, c.outBuf = sliceForAppend(c.outBuf[..0L], recordHeaderLen);
                c.outBuf[0L] = byte(typ);
                var vers = c.vers;
                if (vers == 0L)
                { 
                ...
                c.outBuf[1L] = byte(vers >> (int)(8L));
                c.outBuf[2L] = byte(vers);
                c.outBuf[3L] = byte(m >> (int)(8L));
                c.outBuf[4L] = byte(m);
        ...
        // connection and updates the record layer state.
        private static (long, error) writeRecord(this ptr<Conn> _addr_c, recordType typ, slice<byte> data) => func((defer, _, __) =>
        {
            long _p0 = default;
            error _p0 = default!;
            ref Conn c = ref _addr_c.val;
        ...
        private static (object, error) readHandshake(this ptr<Conn> _addr_c)
        {
            ...
            if (data[0L] == typeHelloRequest) 
                ...
                m = @new<finishedMsg>();
            else if (data[0L] == typeEncryptedExtensions) 
                m = @new<encryptedExtensionsMsg>();
            else if (data[0L] == typeEndOfEarlyData) 
                m = @new<endOfEarlyDataMsg>();
            else if (data[0L] == typeKeyUpdate) 
                m = @new<keyUpdateMsg>();
            else 
        ...
        private static (long, error) Write(this ptr<Conn> _addr_c, slice<byte> b) => func((defer, _, __) =>
        {
            long _p0 = default;
            error _p0 = default!;
            ref Conn c = ref _addr_c.val;
            ...
            while (true)
            {
                ...
                if (x & 1L != 0L)
                {
                    return (0L, error.As(errClosed)!);
                }

                if (atomic.CompareAndSwapInt32(_addr_c.activeCall, x, x + 2L))
                {
                    break;
                }
            ...
            defer(atomic.AddInt32(_addr_c.activeCall, -2L));
            ...
            {
                return (0L, error.As(alertInternalError)!);
            }

            if (c.closeNotifySent)
            {
                return (0L, error.As(errShutdown)!);
            } 
        ...
        private static error handleRenegotiation(this ptr<Conn> _addr_c) => func((defer, _, __) =>
        {
            ref Conn c = ref _addr_c.val;
            ...
            atomic.StoreUint32(_addr_c.handshakeStatus, 0L);
        ...
        private static error handleKeyUpdate(this ptr<Conn> _addr_c, ptr<keyUpdateMsg> _addr_keyUpdate) => func((defer, _, __) =>
        {
            ref Conn c = ref _addr_c.val;
        ...
        private static (long, error) Read(this ptr<Conn> _addr_c, slice<byte> b) => func((defer, _, __) =>
        {
            long _p0 = default;
            error _p0 = default!;
            ref Conn c = ref _addr_c.val;
            ...
            if (n != 0L && c.input.Len() == 0L && c.rawInput.Len() > 0L && recordType(c.rawInput.Bytes()[0L]) == recordTypeAlert)
            {
                {
                    var err__prev2 = err;
        ...
        private static error Close(this ptr<Conn> _addr_c)
        {
            ...
            while (true)
            {
                ...
                if (atomic.CompareAndSwapInt32(_addr_c.activeCall, x, x | 1L))
                {
                    break;
                }
        ...
        private static error closeNotify(this ptr<Conn> _addr_c) => func((defer, _, __) =>
        {
            ref Conn c = ref _addr_c.val;
        ...
        private static error Handshake(this ptr<Conn> _addr_c) => func((defer, _, __) =>
        {
            ref Conn c = ref _addr_c.val;
        ...
        private static ConnectionState ConnectionState(this ptr<Conn> _addr_c) => func((defer, _, __) =>
        {
            ref Conn c = ref _addr_c.val;
        ...
        private static slice<byte> OCSPResponse(this ptr<Conn> _addr_c) => func((defer, _, __) =>
        {
            ref Conn c = ref _addr_c.val;
        ...
        private static error VerifyHostname(this ptr<Conn> _addr_c, @string host) => func((defer, _, __) =>
        {
            ref Conn c = ref _addr_c.val;
        ...
        private static bool handshakeComplete(this ptr<Conn> _addr_c)
        {
            ...
            return atomic.LoadUint32(_addr_c.handshakeStatus) == 1L;
        }
    }
}}
