...
// package tls -- go2cs converted at 2020 October 08 03:35:15 UTC
// import "crypto/tls" ==> using tls = go.crypto.tls_package
// Original source: C:\Go\src\crypto\tls\auth.go
...
namespace crypto
{
    public static partial class tls_package
    {
        ...
        private static readonly @string serverSignatureContext = (@string)"TLS 1.3, server CertificateVerify\x00";
        private static readonly @string clientSignatureContext = (@string)"TLS 1.3, client CertificateVerify\x00";
        ...
        private static (byte, crypto.Hash, error) typeAndHashFromSignatureScheme(SignatureScheme signatureAlgorithm)
        {
            ...
            if (signatureAlgorithm == PKCS1WithSHA1 || signatureAlgorithm == PKCS1WithSHA256 || signatureAlgorithm == PKCS1WithSHA384 || signatureAlgorithm == PKCS1WithSHA512) 
                ...
                return (0L, 0L, error.As(fmt.Errorf("unsupported signature algorithm: %v", signatureAlgorithm))!);
        ...
        private static (byte, crypto.Hash, error) legacyTypeAndHashFromPublicKey(crypto.PublicKey pub)
        {
            ...
            switch (pub.type())
            {
                ...
                case ed25519.PublicKey _:
                    return (0L, 0L, error.As(fmt.Errorf("tls: Ed25519 public keys are not supported before TLS 1.2"))!);
                    break;
                default:
                {
                    return (0L, 0L, error.As(fmt.Errorf("tls: unsupported public key: %T", pub))!);
                    break;
                }
            }
        ...
        private static slice<SignatureScheme> signatureSchemesForCertificate(ushort version, ptr<Certificate> _addr_cert)
        {
            ...
            switch (priv.Public().type())
            {
                ...
                case ptr<rsa.PublicKey> pub:
                    var size = pub.Size();
                    sigAlgs = make_slice<SignatureScheme>(0L, len(rsaSignatureSchemes));
        ...
        private static (SignatureScheme, error) selectSignatureScheme(ushort vers, ptr<Certificate> _addr_c, slice<SignatureScheme> peerAlgs)
        {
            ...
            if (len(supportedAlgs) == 0L)
            {
                return (0L, error.As(unsupportedCertificateError(_addr_c))!);
            }

            if (len(peerAlgs) == 0L && vers == VersionTLS12)
            { 
            ...
            return (0L, error.As(errors.New("tls: peer doesn't support any of the certificate's signature algorithms"))!);
