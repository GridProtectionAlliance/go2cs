...
// package elliptic -- go2cs converted at 2022 March 06 22:19:06 UTC
// import "crypto/elliptic" ==> using elliptic = go.crypto.elliptic_package
// Original source: C:\Program Files\Go\src\crypto\elliptic\p256_ppc64le.go
...
//
//go:noescape
private static void p256PointAddAffineAsm(ptr<p256Point> res, ptr<p256Point> in1, ptr<p256Point> in2, nint sign, nint sel, nint zero);

// Point add
//
//go:noescape
private static nint p256PointAddAsm(ptr<p256Point> res, ptr<p256Point> in1, ptr<p256Point> in2);
...
private static slice<byte> fromBig(ptr<big.Int> _addr_big) {
    ...
    if (len(res) < 32) {>>MARKER:FUNCTION_p256PointDoubleAsm_BLOCK_PREFIX<<
        copy(t[(int)32 - len(res)..], res);
    }
    else if (len(res) == 32) {>>MARKER:FUNCTION_p256PointAddAsm_BLOCK_PREFIX<<
        copy(t, res);
    }
    else
 {>>MARKER:FUNCTION_p256PointAddAffineAsm_BLOCK_PREFIX<<
        copy(t, res[(int)len(res) - 32..]);
    }
...
private static slice<byte> p256GetMultiplier(slice<byte> @in) {
    ...
    if (n.Cmp(p256Params.N) >= 0) {>>MARKER:FUNCTION_p256SelectBase_BLOCK_PREFIX<<
...
private static ptr<big.Int> maybeReduceModP(ptr<big.Int> _addr_@in) {
    ...
    if (@in.Cmp(p256Params.P) < 0) {>>MARKER:FUNCTION_p256Select_BLOCK_PREFIX<<
        return _addr_in!;
    }
...
private static void p256ReverseBytes(slice<byte> res, slice<byte> @in) { 
    ...
    var a = binary.BigEndian.Uint64(in[(int)0..]);
    var b = binary.BigEndian.Uint64(in[(int)8..]);
    var c = binary.BigEndian.Uint64(in[(int)16..]);
    var d = binary.BigEndian.Uint64(in[(int)24..]); 

    // Store in normal order
    binary.LittleEndian.PutUint64(res[(int)0..], d);
    binary.LittleEndian.PutUint64(res[(int)8..], c);
    binary.LittleEndian.PutUint64(res[(int)16..], b);
    binary.LittleEndian.PutUint64(res[(int)24..], a);
...
private static void p256Inverse(slice<byte> @out, slice<byte> @in) {
...

    {
        ...
        for (nint i = 0; i < 7; i++) {>>MARKER:FUNCTION_p256MovCond_BLOCK_PREFIX<<
            p256Sqr(out, out);
        }
    ...
    p256Sqr(out, p16);
    {
        ...
        for (i = 0; i < 15; i++) {>>MARKER:FUNCTION_p256NegCond_BLOCK_PREFIX<<
            p256Sqr(out, out);
        }
...

    {
        ...
        for (i = 0; i < 31; i++) {>>MARKER:FUNCTION_p256FromMont_BLOCK_PREFIX<<
            p256Sqr(out, out);
        }
...

    {
        ...
        for (i = 0; i < 32 * 4; i++) {>>MARKER:FUNCTION_p256MulAsm_BLOCK_PREFIX<<
            p256Sqr(out, out);
        }
...
private static void p256BaseMult(this ptr<p256Point> _addr_p, slice<byte> scalar) {
    ...
    var wvalue = (uint(scalar[0]) << 1) & 0xff;
    ...
    for (nint i = 1; i < 37; i++) { 
        ...
        if (index < 247) {
            wvalue = ((uint(scalar[index / 8]) >> (int)((index % 8))) + (uint(scalar[index / 8 + 1]) << (int)((8 - (index % 8))))) & 0xff;
        }
        else
 {
            wvalue = (uint(scalar[index / 8]) >> (int)((index % 8))) & 0xff;
        }
        index += 7;
...
private static void p256ScalarMult(this ptr<p256Point> _addr_p, slice<byte> scalar) {
    ...
    _addr_precomp[1].val = t0;
    _addr_precomp[3].val = t1;
    _addr_precomp[7].val = t2;
    _addr_precomp[15].val = t3;
    ...
    _addr_precomp[2].val = t0;
    _addr_precomp[4].val = t1;
    _addr_precomp[8].val = t2;

    p256PointDoubleAsm(_addr_t0, _addr_t0);
    p256PointDoubleAsm(_addr_t1, _addr_t1);
    _addr_precomp[5].val = t0;
    _addr_precomp[9].val = t1;

    p256PointAddAsm(_addr_t2, _addr_t0, _addr_p);
    p256PointAddAsm(_addr_t1, _addr_t1, _addr_p);
    _addr_precomp[6].val = t2;
    _addr_precomp[10].val = t1;

    p256PointDoubleAsm(_addr_t0, _addr_t0);
    p256PointDoubleAsm(_addr_t2, _addr_t2);
    _addr_precomp[11].val = t0;
    _addr_precomp[13].val = t2;

    p256PointAddAsm(_addr_t0, _addr_t0, _addr_p);
    p256PointAddAsm(_addr_t2, _addr_t2, _addr_p);
    _addr_precomp[12].val = t0;
    _addr_precomp[14].val = t2; 
    ...
    // TODO: For big endian, use index found in s390x implementation.
    var wvalue = (uint(scalar[index / 8]) >> (int)((index % 8))) & 0x3f;
    ...
    while (index > 4) {
        ...
        if (index < 247) {
            wvalue = ((uint(scalar[index / 8]) >> (int)((index % 8))) + (uint(scalar[index / 8 + 1]) << (int)((8 - (index % 8))))) & 0x3f;
        }
        else
 {
            wvalue = (uint(scalar[index / 8]) >> (int)((index % 8))) & 0x3f;
        }
