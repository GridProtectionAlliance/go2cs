...
namespace crypto
{
    public static partial class cipher_package
    {
        ...
        {
            long BlockSize(); // Encrypt encrypts the first block in src into dst.
// Dst and src must overlap entirely or not at all.
            long Encrypt(slice<byte> dst, slice<byte> src); // Decrypt decrypts the first block in src into dst.
// Dst and src must overlap entirely or not at all.
            long Decrypt(slice<byte> dst, slice<byte> src);
        }
        ...
        {
            long BlockSize(); // CryptBlocks encrypts or decrypts a number of blocks. The length of
...
// maintains state and does not reset at each CryptBlocks call.
            long CryptBlocks(slice<byte> dst, slice<byte> src);
        }
