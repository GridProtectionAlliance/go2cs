...
private static void check(this ptr<copyChecker> _addr_c) => func((_, panic, _) => {
    ref copyChecker c = ref _addr_c.val;

    if (uintptr(c.val) != uintptr(@unsafe.Pointer(c)) && !atomic.CompareAndSwapUintptr((uintptr.val)(c), 0, uintptr(@unsafe.Pointer(c))) && uintptr(c.val) != uintptr(@unsafe.Pointer(c))) {
