...
// package sync -- go2cs converted at 2020 October 09 05:01:10 UTC
...
// for the sema.
        ...
        private static (ptr<ulong>, ptr<uint>) state(this ptr<WaitGroup> _addr_wg)
        {
            ...
            if (uintptr(@unsafe.Pointer(_addr_wg.state1)) % 8L == 0L)
            {
                return (_addr_(uint64.val)(@unsafe.Pointer(_addr_wg.state1))!, _addr__addr_wg.state1[2L]!);
            }
            else
            {
                return (_addr_(uint64.val)(@unsafe.Pointer(_addr_wg.state1[1L]))!, _addr__addr_wg.state1[0L]!);
            }
        ...
        private static void Add(this ptr<WaitGroup> _addr_wg, long delta) => func((defer, panic, _) =>
        {
            ref WaitGroup wg = ref _addr_wg.val;
            ...
            while (w != 0L)
            {
                runtime_Semrelease(semap, false, 0L);
                w--;
            }
        ...
        private static void Wait(this ptr<WaitGroup> _addr_wg) => func((_, panic, __) =>
        {
            ref WaitGroup wg = ref _addr_wg.val;
