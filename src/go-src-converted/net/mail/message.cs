...
*/
// package mail -- go2cs converted at 2022 March 06 22:25:48 UTC
...
private static (ptr<Address>, error) parseSingleAddress(this ptr<addrParser> _addr_p) {
    ...
    return (_addr_addrs[0]!, error.As(null!)!);
...
private static (@string, error) consumeAddrSpec(this ptr<addrParser> _addr_p) => func((defer, _, _) => {
    @string spec = default;
    error err = default!;
    ref addrParser p = ref _addr_p.val;
...
Loop:
    ...
    while (true) {
        ...
        else if (size == 0 || !isAtext(r, dot, permissive)) 
            _breakLoop = true;
    ...
    (atom, p.s) = (p.s[..(int)i], p.s[(int)i..]);    if (!permissive) {
...
private static nint len(this ptr<addrParser> _addr_p) {
    ref addrParser p = ref _addr_p.val;
