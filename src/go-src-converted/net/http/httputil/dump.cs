...
private static (nint, error) Read(this neverEnding b, slice<byte> p) {
    nint n = default;
    error err = default!;
...
// User-Agent.
public static (slice<byte>, error) DumpRequestOut(ptr<http.Request> _addr_req, bool body) => func((defer, _, _) => {
...
private partial struct delegateReader {
    ...
    public error err; // only used if r is nil and c is closed.
    public io.Reader r; // nil until received from c
}

private static (nint, error) Read(this ptr<delegateReader> _addr_r, slice<byte> p) {
    nint _p0 = default;
    error _p0 = default!;
...
private static (nint, error) Read(this failureToReadBody _p0, slice<byte> _p0) {
    nint _p0 = default;
    error _p0 = default!;

    return (0, error.As(errNoBody)!);
}
