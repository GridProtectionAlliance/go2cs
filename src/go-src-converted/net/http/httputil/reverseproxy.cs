...
private static (@string, @string) joinURLPath(ptr<url.URL> _addr_a, ptr<url.URL> _addr_b) {
    ...
    if (aslash && bslash) 
        return (a.Path + b.Path[(int)1..], apath + bpath[(int)1..]);
    else if (!aslash && !bslash) 
...
private static void ServeHTTP(this ptr<ReverseProxy> _addr_p, http.ResponseWriter rw, ptr<http.Request> _addr_req) => func((defer, panic, _) => {
    ...
    if (announcedTrailers > 0) {
        var trailerKeys = make_slice<@string>(0, len(res.Trailer));
        {
            var k__prev1 = k;
...
private static time.Duration flushInterval(this ptr<ReverseProxy> _addr_p, ptr<http.Response> _addr_res) {
    ref ReverseProxy p = ref _addr_p.val;
    ref http.Response res = ref _addr_res.val;
...
private static (nint, error) Write(this ptr<maxLatencyWriter> _addr_m, slice<byte> p) => func((defer, _, _) => {
    nint n = default;
    error err = default!;
