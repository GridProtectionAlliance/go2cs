...
// package httpguts -- go2cs converted at 2022 March 06 22:22:47 UTC
...
namespace go.golang.org.x.net.http;
...
private static bool headerValueContainsToken(@string v, @string token) {
    v = trimOWS(v);
    {
        ...
        if (comma != -1) {
            return tokenEqual(trimOWS(v[..(int)comma]), token) || headerValueContainsToken(v[(int)comma + 1..], token);
        }
    }
