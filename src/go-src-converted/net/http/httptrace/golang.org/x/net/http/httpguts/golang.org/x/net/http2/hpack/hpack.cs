...
// package hpack -- go2cs converted at 2020 October 09 04:57:42 UTC
...
// to fully parse before. Unlike buf, we own this data.
        ...
        private static long maxTableIndex(this ptr<Decoder> _addr_d)
        {
            ref Decoder d = ref _addr_d.val;
        ...
        private static (slice<HeaderField>, error) DecodeFull(this ptr<Decoder> _addr_d, slice<byte> p) => func((defer, _, __) =>
        {
...
;
        ...
        private static (long, error) Write(this ptr<Decoder> _addr_d, slice<byte> p)
        {
            long n = default;
            error err = default!;
            ref Decoder d = ref _addr_d.val;
            ...
            while (len(d.buf) > 0L)
            {
                ...
                { 
                    ...
                    const long varIntOverhead = (long)8L; // conservative
 // conservative
                    if (d.maxStrLen != 0L && int64(len(d.buf)) > 2L * (int64(d.maxStrLen) + varIntOverhead))
                    {
                        return (0L, error.As(ErrStringLength)!);
                    }
        ...
        private static error parseDynamicTableSizeUpdate(this ptr<Decoder> _addr_d)
        {
            ...
            if (!d.firstField && d.dynTab.size > 0L)
            {
        ...
        private static (ulong, slice<byte>, error) readVarInt(byte n, slice<byte> p) => func((_, panic, __) =>
        {
            ...
            if (len(p) == 0L)
            {
                return (0L, p, error.As(errNeedMore)!);
            }
            ...
            while (len(p) > 0L)
            {
                ...
                { // TODO: proper overflow check. making this up.
                    return (0L, origP, error.As(errVarintOverflow)!);

                }

            }

            return (0L, origP, error.As(errNeedMore)!);
        ...
        private static (@string, slice<byte>, error) readString(this ptr<Decoder> _addr_d, slice<byte> p, bool wantStr) => func((defer, _, __) =>
        {
            @string s = default;
            slice<byte> remain = default;
            error err = default!;
            ...
            if (!isHuff)
            {
                ...
                return (s, p[strLen..], error.As(null!)!);
