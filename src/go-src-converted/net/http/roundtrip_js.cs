...
// package http -- go2cs converted at 2022 March 06 22:22:57 UTC
...
private static readonly @string jsFetchMode = "js.fetch:mode";
...
//
// Reference: https://developer.mozilla.org/en-US/docs/Web/API/WindowOrWorkerGlobalScope/fetch#Parameters
private static readonly @string jsFetchCreds = "js.fetch:credentials";
...
//
// Reference: https://developer.mozilla.org/en-US/docs/Web/API/WindowOrWorkerGlobalScope/fetch#Parameters
private static readonly @string jsFetchRedirect = "js.fetch:redirect";
...
private static (ptr<Response>, error) RoundTrip(this ptr<Transport> _addr_t, ptr<Request> _addr_req) {
    ...
    var respCh = make_channel<ptr<Response>>(1);    var errCh = make_channel<error>(1);    js.Func success = default;    js.Func failure = default;
    ...
    success = js.FuncOf((@this, args) => {
        ...
        while (true) {
            ...
            var key = pair.Index(0).String();
            var value = pair.Index(1).String();
        ...
        {
            var (cl, err) = strconv.ParseInt(header.Get("Content-Length"), 10, 64);
 ...
 { 
    ...
    failure = js.FuncOf((@this, args) => {
        success.Release();
        failure.Release();
        errCh.Send(fmt.Errorf("net/http: fetch() failed: %s", args[0].Get("message").String()));
        return _addr_null!;
    });
...
private static (nint, error) Read(this ptr<streamReader> _addr_r, slice<byte> p) => func((defer, _, _) => {
    nint n = default;
    error err = default!;
    ...
    if (len(r.pending) == 0) {
        var bCh = make_channel<slice<byte>>(1);        var errCh = make_channel<error>(1);
        ...
        var failure = js.FuncOf((@this, args) => { 
            ...
            // the read method.
            errCh.Send(errors.New(args[0].Get("message").String()));
            return null;
...
private static (nint, error) Read(this ptr<arrayReader> _addr_r, slice<byte> p) => func((defer, _, _) => {
    nint n = default;
    error err = default!;
    ...
    if (!r.read) {
        ...
        var success = js.FuncOf((@this, args) => { 
            ...
            var uint8arrayWrapper = uint8Array.New(args[0]);
