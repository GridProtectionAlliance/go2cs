...
private static (nint, error) Read(this ptr<netFD> _addr_fd, slice<byte> b) {
    nint n = default;
    error err = default!;
    ref netFD fd = ref _addr_fd.val;
...
private static (nint, error) Write(this ptr<netFD> _addr_fd, slice<byte> b) {
    nint n = default;
    error err = default!;
    ref netFD fd = ref _addr_fd.val;
...
private static (ptr<os.File>, error) file(this ptr<netFD> _addr_fd, ptr<os.File> _addr_f, @string s) {
    ...
    var (dfd, err) = syscall.Dup(int(f.Fd()), -1);
    if (err != null) {
