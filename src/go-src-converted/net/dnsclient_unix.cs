...
private static (ushort, slice<byte>, slice<byte>, error) newRequest(dnsmessage.Question q) {
    ...
    id = uint16(randInt());
    var b = dnsmessage.NewBuilder(make_slice<byte>(2, 514), new dnsmessage.Header(ID:id,RecursionDesired:true));
    b.EnableCompression();
    {
        var err__prev1 = err;
...
private static (dnsmessage.Parser, dnsmessage.Header, error) dnsStreamRoundTrip(Conn c, ushort id, dnsmessage.Question query, slice<byte> b) {
    ...
    var (n, err) = io.ReadFull(c, b[..(int)l]);
    if (err != null) {
        return (new dnsmessage.Parser(), new dnsmessage.Header(), error.As(err)!);
    }
    dnsmessage.Parser p = default;
...
private static (dnsmessage.Parser, dnsmessage.Header, error) exchange(this ptr<Resolver> _addr_r, context.Context ctx, @string server, dnsmessage.Question q, time.Duration timeout, bool useTCP) => func((defer, _, _) => {
    dnsmessage.Parser _p0 = default;
    dnsmessage.Header _p0 = default;
    error _p0 = default!;
...
// "/etc/resolv.conf".
private static void tryUpdate(this ptr<resolverConfig> _addr_conf, @string name) => func((defer, _, _) => {
    ref resolverConfig conf = ref _addr_conf.val;
...
private static slice<@string> nameList(this ptr<dnsConfig> _addr_conf, @string name) {
    ...
    var names = make_slice<@string>(0, 1 + len(conf.search)); 
...
private partial struct hostLookupOrder { // : nint
}
...
private static (slice<@string>, error) goLookupHostOrder(this ptr<Resolver> _addr_r, context.Context ctx, @string name, hostLookupOrder order) {
    ...
    var (ips, _, err) = r.goLookupIPCNAMEOrder(ctx, "ip", name, order);
    if (err != null) {
        return ;
    }
    addrs = make_slice<@string>(0, len(ips));
...
private static (slice<IPAddr>, error) goLookupIP(this ptr<Resolver> _addr_r, context.Context ctx, @string network, @string host) {
    slice<IPAddr> addrs = default;
    error err = default!;
    ref Resolver r = ref _addr_r.val;
...
private static (slice<IPAddr>, dnsmessage.Name, error) goLookupIPCNAMEOrder(this ptr<Resolver> _addr_r, context.Context ctx, @string network, @string name, hostLookupOrder order) => func((defer, _, _) => {
    slice<IPAddr> addrs = default;
    ...
    var lane = make_channel<result>(1);
    dnsmessage.Type qtypes = new slice<dnsmessage.Type>(new dnsmessage.Type[] { dnsmessage.TypeA, dnsmessage.TypeAAAA });
    switch (ipVersion(network)) {
        case '4': 
            qtypes = new slice<dnsmessage.Type>(new dnsmessage.Type[] { dnsmessage.TypeA });
            break;
        case '6': 
            qtypes = new slice<dnsmessage.Type>(new dnsmessage.Type[] { dnsmessage.TypeAAAA });
            break;
    }
...
private static (@string, error) goLookupCNAME(this ptr<Resolver> _addr_r, context.Context ctx, @string host) {
    ...
    var order = systemConf().hostLookupOrder(r, host);
    var (_, cname, err) = r.goLookupIPCNAMEOrder(ctx, "ip", host, order);
