...
private static (ptr<Interface>, error) readInterface(nint i) => func((defer, _, _) => {
    ptr<Interface> _p0 = default!;
    error _p0 = default!;

    ptr<Interface> ifc = addr(new Interface(Index:i+1,Name:netdir+"/ipifc/"+itoa.Itoa(i),));
    ...
    if (stringsHasPrefix(device, netdir + "/")) {
        ...
        if (len(line) > 0 && len(line) % 2 == 0) {
            ifc.HardwareAddr = make_slice<byte>(len(line) / 2);
            bool ok = default;
            foreach (var (i) in ifc.HardwareAddr) {
                var j = (i + 1) * 2;
                ifc.HardwareAddr[i], ok = xtoi2(line[(int)i * 2..(int)j], 0);
                if (!ok) {
                    ifc.HardwareAddr = ifc.HardwareAddr[..(int)i];
                    break;
                }
            }
        }
...
private static (nint, error) interfaceCount() => func((defer, _, _) => {
    nint _p0 = default;
    error _p0 = default!;
    ...
    var (names, err) = d.Readdirnames(0);
    if (err != null) {
        return (-1, error.As(err)!);
    }
    nint c = 0;
...
// interface.
private static (slice<Addr>, error) interfaceAddrTable(ptr<Interface> _addr_ifi) => func((defer, _, _) => {
    ...
    foreach (var (_, ifc) in ifcs) {
...

        {
            ...
            while (ok) {
                ...
                var maskfld = fields[1];
                maskfld = maskfld[(int)1..];
                ...
                IPMask mask = default;
                if (ip.To4() != null) { // IPv4 or IPv6 IPv4-mapped address
                    mask = CIDRMask(pfxlen - 8 * len(v4InV6Prefix), 8 * IPv4len);

                }

                if (ip.To16() != null && ip.To4() == null) { // IPv6 address
