...
namespace net
{
    public static partial class textproto_package
    {
        ...
        private static ulong Next(this ref Pipeline p)
        {
            p.mu.Lock();
            var id = p.id;
            p.id++;
        ...
        private static void StartRequest(this ref Pipeline p, ulong id)
        {
            p.request.Start(id);
        }
        ...
        private static void EndRequest(this ref Pipeline p, ulong id)
        {
            p.request.End(id);
        }
        ...
        // the request with the given id.
        private static void StartResponse(this ref Pipeline p, ulong id)
        {
            p.response.Start(id);
        }
        ...
        private static void EndResponse(this ref Pipeline p, ulong id)
        {
            p.response.End(id);
        }
        ...
        private partial struct sequencer
        {
            public sync.Mutex mu;
            public ulong id;
            public map<ulong, channel<ulong>> wait;
        }

        // Start waits until it is time for the event numbered id to begin.
        // That is, except for the first event, it waits until End(id-1) has
        // been called.
        private static void Start(this ref sequencer s, ulong id)
        {
            s.mu.Lock();
            if (s.id == id)
            {
            ...
            var c = make_channel<ulong>();
            if (s.wait == null)
            {
                s.wait = make_map<ulong, channel<ulong>>();
            }
            s.wait[id] = c;
            s.mu.Unlock().Send(c);
        }
        ...
        private static void End(this ref sequencer _s, ulong id) => func(_s, (ref sequencer s, Defer _, Panic panic, Recover __) =>
        {
            s.mu.Lock();
            if (s.id != id)
            {
