...
private static (slice<ptr<windows.IpAdapterAddresses>>, error) adapterAddresses() {
    ...
    ref var l = ref heap(uint32(15000), out ptr<var> _addr_l); // recommended initial size
    while (true) {
        b = make_slice<byte>(l);
        var err = windows.GetAdaptersAddresses(syscall.AF_UNSPEC, windows.GAA_FLAG_INCLUDE_PREFIX, 0, (windows.IpAdapterAddresses.val)(@unsafe.Pointer(_addr_b[0])), _addr_l);
        if (err == null) {
            if (l == 0) {
    ...
    {
        var aa = (windows.IpAdapterAddresses.val)(@unsafe.Pointer(_addr_b[0]));
