...
namespace @internal
{
    public static partial class checker_package
    {
        ...
        public static long Run(slice<@string> args, slice<ptr<analysis.Analyzer>> analyzers) => func((defer, _, __) =>
        {
...
;
            ...
            // Now we've got a set of valid edits for each file. Get the new file contents.
            {
                ...
                foreach (var (__f, __tree) in editsForFile)
                {
                    ...
                    long cur = 0L; // current position in the file
...
;
                    ...
                    ioutil.WriteFile(f.Name(), @out.Bytes(), 0644L);
        ...
        private static long printDiagnostics(slice<ptr<action>> roots)
        {
            long exitcode = default;
...
;
                ...
                print = act =>
                {
                    if (act.err != null)
                    {
                        ...
                        exitcode = 1L; // analysis failed, at least partially
                        return ;
...
;
                ...
                if (exitcode == 0L && len(seen) > 0L)
                {
                    exitcode = 3L; // successfully produced diagnostics
                }
...
;
        ...
        private static void execOnce(this ptr<action> _addr_act) => func((defer, _, __) =>
        {
            ref action act = ref _addr_act.val;
            ...
            // suggested fixes for compiler/type errors.
            {
                ...
                foreach (var (_, __err) in act.pkg.Errors)
                {
                    ...
                    act.pkg.Fset.Iterate(f =>
                    {
                        ...
                        offset = int(f.LineStart(line)) + col - 1L;
                        return false;
        ...
        private static bool importObjectFact(this ptr<action> _addr_act, types.Object obj, analysis.Fact ptr) => func((_, panic, __) =>
        {
            ref action act = ref _addr_act.val;
        ...
        private static void exportObjectFact(this ptr<action> _addr_act, types.Object obj, analysis.Fact fact)
        {
            ...
            if (dbg('f'))
            {
                var objstr = types.ObjectString(obj, ptr<types.Package>);
        ...
        private static slice<analysis.ObjectFact> allObjectFacts(this ptr<action> _addr_act)
        {
            ...
            var facts = make_slice<analysis.ObjectFact>(0L, len(act.objectFacts));
        ...
        private static bool importPackageFact(this ptr<action> _addr_act, ptr<types.Package> _addr_pkg, analysis.Fact ptr) => func((_, panic, __) =>
        {
            ref action act = ref _addr_act.val;
            ref types.Package pkg = ref _addr_pkg.val;
        ...
        private static void exportPackageFact(this ptr<action> _addr_act, analysis.Fact fact)
        {
            ...
            if (dbg('f'))
            {
                fmt.Fprintf(os.Stderr, "%s: package %s has fact %s\n", act.pkg.Fset.Position(act.pass.Files[0L].Pos()), act.pass.Pkg.Path(), fact);
            }
        ...
        private static slice<analysis.PackageFact> allPackageFacts(this ptr<action> _addr_act)
        {
            ...
            var facts = make_slice<analysis.PackageFact>(0L, len(act.packageFacts));
        ...
        {
            return strings.IndexByte(Debug, b) >= 0L;
        }
    }
}}}}}}}
