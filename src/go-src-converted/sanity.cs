...
// a negative result.
//
private static void mustSanityCheck(ptr<Function> _addr_fn, io.Writer reporter) => func((_, panic, _) => {
    ref Function fn = ref _addr_fn.val;
...
private static void checkInstr(this ptr<sanity> _addr_s, nint idx, Instruction instr) => func((_, panic, _) => {
    ref sanity s = ref _addr_s.val;
    ...
    // and a valid referrer list.
    {
        ...
        if (ok) {
            ...
            if (t == null) {
                ...
                else if (ok && b.Info() & types.IsUntyped != 0) {
...
private static void checkBlock(this ptr<sanity> _addr_s, ptr<BasicBlock> _addr_b, nint index) {
    ref sanity s = ref _addr_s.val;
    ...
    array<ptr<Value>> rands = new array<ptr<Value>>(10); // reuse storage
...
operands:
        foreach (var (i, op) in instr.Operands(rands[..(int)0])) {
            if (op == null) {
            ...
            // Check that "untyped" types only appear on constant operands.
            {
                ...
                if (!ok) {
                    {
                        ptr<types.Basic> (basic, ok) = (op.val).Type()._<ptr<types.Basic>>();
...
private static bool checkFunction(this ptr<sanity> _addr_s, ptr<Function> _addr_fn) {
        ...
        else
 {
    ...
    if (fn.Blocks != null && len(fn.Blocks) == 0) { 
...
private static void sanityCheckPackage(ptr<Package> _addr_pkg) => func((_, panic, _) => {
