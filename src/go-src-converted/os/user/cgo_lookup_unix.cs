...
// Original source: C:\Program Files\Go\src\os\user\cgo_lookup_unix.go
...
private static (ptr<User>, error) lookupUser(@string username) => func((defer, _, _) => {
    ...
    var nameC = make_slice<byte>(len(username) + 1);
    copy(nameC, username);
    ...
    var err = retryWithBuffer(_addr_buf, () => { 
        ...
        // Solaris.
        return _addr_syscall.Errno(C.mygetpwnam_r((C.@char.val)(@unsafe.Pointer(_addr_nameC[0])), _addr_pwd, (C.@char.val)(buf.ptr), C.size_t(buf.size), _addr_result))!;
...
private static (ptr<User>, error) lookupUnixUid(nint uid) => func((defer, _, _) => {
    ptr<User> _p0 = default!;
    error _p0 = default!;
...
private static (ptr<Group>, error) lookupGroup(@string groupname) => func((defer, _, _) => {
    ...
    defer(buf.free());
    var cname = make_slice<byte>(len(groupname) + 1);
    copy(cname, groupname);

    var err = retryWithBuffer(_addr_buf, () => {
        return _addr_syscall.Errno(C.mygetgrnam_r((C.@char.val)(@unsafe.Pointer(_addr_cname[0])), _addr_grp, (C.@char.val)(buf.ptr), C.size_t(buf.size), _addr_result))!;
    });
    if (err != null) {
...
private static (ptr<Group>, error) lookupUnixGid(nint gid) => func((defer, _, _) => {
    ptr<Group> _p0 = default!;
    error _p0 = default!;
...
private static readonly nint maxBufferSize = 1 << 20;
