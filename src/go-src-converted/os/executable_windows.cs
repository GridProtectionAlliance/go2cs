...
// package os -- go2cs converted at 2022 March 06 22:13:27 UTC
// import "os" ==> using os = go.os_package
// Original source: C:\Program Files\Go\src\os\executable_windows.go
...
private static (@string, error) getModuleFileName(syscall.Handle handle) {
    ...
    while (true) {
        ...
        var (r, err) = windows.GetModuleFileName(handle, _addr_buf[0], n);
        if (err != null) {
            return ("", error.As(err)!);
        }
        if (r < n) {
