...
// Original source: C:\Program Files\Go\src\os\types_windows.go
...
private static (ptr<fileStat>, error) newFileStatFromGetFileInformationByHandle(@string path, syscall.Handle h) {
    ...
    if (err != null) {
        return (_addr_null!, error.As(addr(new PathError(Op:"GetFileInformationByHandle",Path:path,Err:err))!)!);
    }
 ...
 {
                return (_addr_null!, error.As(addr(new PathError(Op:"GetFileInformationByHandleEx",Path:path,Err:err))!)!);
            }
...
private static error loadFileId(this ptr<fileStat> _addr_fs) => func((defer, _, _) => {
    ref fileStat fs = ref _addr_fs.val;
    ...
    else
 {
    ...
    var (h, err) = syscall.CreateFile(pathp, 0, 0, null, syscall.OPEN_EXISTING, attrs, 0);
    if (err != null) {
        return error.As(err)!;
    }
...
private static error saveInfoFromPath(this ptr<fileStat> _addr_fs, @string path) {
    ...
    if (!isAbs(fs.path)) {
        ...
        if (err != null) {
            return error.As(addr(new PathError(Op:"FullPath",Path:path,Err:err))!)!;
        }
    }
...
private static time.Time atime(FileInfo fi) {
    return time.Unix(0, fi.Sys()._<ptr<syscall.Win32FileAttributeData>>().LastAccessTime.Nanoseconds());
}
