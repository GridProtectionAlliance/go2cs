...
// package packagestest -- go2cs converted at 2022 March 06 23:32:26 UTC
...
private static error Finalize(this modules _p0, ptr<Exported> _addr_exported) {
    ...
    var primaryDir = primaryDir(_addr_exported);
    {
        ...
        var err = os.MkdirAll(primaryDir, 0755);
...

    {
        ...
        if (gomod != "") {
            ...
            }
            {
                ...
                err = ioutil.WriteFile(gomod + ".temp", contents, 0644);
    ...
    }    primaryGomod += ")\n";
    {
        ...
        err = ioutil.WriteFile(filepath.Join(primaryDir, "go.mod"), (slice<byte>)primaryGomod, 0644);
    ...
    // Create the mod cache so we can rename it later, even if we don't need it.
    {
        ...
        err = os.MkdirAll(modCache(_addr_exported), 0755);
