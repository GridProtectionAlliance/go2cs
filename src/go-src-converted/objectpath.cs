...
// but consistently.
// package objectpath -- go2cs converted at 2022 March 06 23:32:30 UTC
...
namespace go.golang.org.x.tools.go.types;
...
private static readonly char opType = '.'; // .Type()          (Object)

// type->type operators
private static readonly char opElem = 'E'; // .Elem()        (Pointer, Slice, Array, Chan, Map)
private static readonly char opKey = 'K'; // .Key()        (Map)
private static readonly char opParams = 'P'; // .Params()        (Signature)
private static readonly char opResults = 'R'; // .Results()    (Signature)
private static readonly char opUnderlying = 'U'; // .Underlying()    (Named)

// type->object operators
private static readonly char opAt = 'A'; // .At(i)        (Tuple)
private static readonly char opField = 'F'; // .Field(i)        (Struct)
private static readonly char opMethod = 'M'; // .Method(i)        (Named or Interface; not Struct: "promoted" names are ignored)
private static readonly char opObj = 'O'; // .Obj()        (Named)
...
public static (Path, error) For(types.Object obj) => func((_, panic, _) => {
    Path _p0 = default;
    error _p0 = default!;
    ...
    // the best paths because non-types may
    // refer to types, but not the reverse.
    var empty = make_slice<byte>(0, 48); // initial space
...
private static slice<byte> find(types.Object obj, types.Type T, slice<byte> path) => func((_, panic, _) => {
    switch (T.type()) {
        ...
        else
 {
    ...
    while (suffix != "") {
        ...
        if (code == opAt || code == opField || code == opMethod) 
            ...
            var numerals = suffix[..(int)len(suffix) - len(rest)];
            suffix = rest;
