...
// package deepequalerrors -- go2cs converted at 2022 March 06 23:33:57 UTC
...
public static readonly @string Doc = "check for calls of reflect.DeepEqual on error values\n\nThe deepequalerrors checker" +
...
private static (object, error) run(ptr<analysis.Pass> _addr_pass) {
    ...
    inspect.Preorder(nodeFilter, n => {
        ...
        if (fn.FullName() == "reflect.DeepEqual" && hasError(_addr_pass, call.Args[0]) && hasError(_addr_pass, call.Args[1])) {
