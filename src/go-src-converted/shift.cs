...
// package shift -- go2cs converted at 2020 October 09 06:04:13 UTC
// import "golang.org/x/tools/go/analysis/passes/shift" ==> using shift = go.golang.org.x.tools.go.analysis.passes.shift_package
// Original source: C:\Users\ritchie\go\src\golang.org\x\tools\go\analysis\passes\shift\shift.go
...
namespace passes
{
    ...
    {
        public static readonly @string Doc = (@string)"check for shifts that equal or exceed the width of the integer";
            ...
            inspect.Preorder(nodeFilter, node =>
            {
                ...
                switch (node.type())
                {
                    ...
                    case ptr<ast.AssignStmt> node:
                        ...
                        {
                            checkLongShift(_addr_pass, node, node.Lhs[0L], node.Rhs[0L]);
                        }
        ...
        private static void checkLongShift(ptr<analysis.Pass> _addr_pass, ast.Node node, ast.Expr x, ast.Expr y)
        {
            ...
            long size = 8L * pass.TypesSizes.Sizeof(t);
            if (amt >= size)
            {
