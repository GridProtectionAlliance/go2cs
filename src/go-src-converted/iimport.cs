...
namespace @internal
{
    public static partial class gcimporter_package
    {
        ...
        public static (long, ptr<types.Package>, error) IImportData(ptr<token.FileSet> _addr_fset, map<@string, ptr<types.Package>> imports, slice<byte> data, @string path) => func((defer, panic, _) =>
        {
            long _ = default;
            ...
            p.ipkg = pkgList[0L];
            var names = make_slice<@string>(0L, len(p.pkgIndex[p.ipkg]));
            {
