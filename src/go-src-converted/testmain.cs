...
// package ssa -- go2cs converted at 2020 October 09 06:03:31 UTC
...
namespace go
{
    public static partial class ssa_package
    {
        ...
        public static (slice<ptr<Function>>, slice<ptr<Function>>, slice<ptr<Function>>, ptr<Function>) FindTests(ptr<Package> _addr_pkg)
        {
            ...
            // Obtain the types from the parameters of testing.MainStart.
            {
                ...
                if (testingPkg != null)
                {
                    ...
                    testSig = funcField(@params.At(1L).Type());
                    benchmarkSig = funcField(@params.At(2L).Type()); 
                    ...
                    //   func TestMain(*testing.M)
                    {
                        ...
                        if (f != null)
                        {
                            ...
                            var starM = mainStart.Signature.Results().At(0L).Type(); // *testing.M
                            if (sig.Results().Len() == 0L && sig.Params().Len() == 1L && types.Identical(sig.Params().At(0L).Type(), starM))
                            {
                                main = f;
                            }
                        }
        ...
        {
            return slice._<ptr<types.Slice>>().Elem().Underlying()._<ptr<types.Struct>>().Field(1L).Type()._<ptr<types.Signature>>();
        }
        ...
        private static ptr<Package> CreateTestMainPackage(this ptr<Program> _addr_prog, ptr<Package> _addr_pkg)
        {
            ...
            var tmpl = testmainTmpl;
            {
                ...
                { 
                    ...
                    data.Go18 = types.IsInterface(testingPkg.Func("MainStart").Signature.Params().At(0L).Type());
            ...
            var (f, err) = parser.ParseFile(prog.Fset, path + ".go", _addr_buf, parser.Mode(0L));
            if (err != null)
            {
