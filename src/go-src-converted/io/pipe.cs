...
private static void Store(this ptr<onceError> _addr_a, error err) => func((defer, _, _) => {
...
private static (nint, error) Read(this ptr<pipe> _addr_p, slice<byte> b) {
    ...
    return (0, error.As(p.readCloseError())!);
    var nr = copy(b, bw);
    p.rdCh.Send(nr);
...
private static (nint, error) Write(this ptr<pipe> _addr_p, slice<byte> b) => func((defer, _, _) => {
    ...
    return (0, error.As(p.writeCloseError())!);
    p.wrMu.Lock();
    defer(p.wrMu.Unlock());
    {
        var once = true;
...
private static (nint, error) Read(this ptr<PipeReader> _addr_r, slice<byte> data) {
    nint n = default;
    error err = default!;
...
private static (nint, error) Write(this ptr<PipeWriter> _addr_w, slice<byte> data) {
    nint n = default;
    error err = default!;
