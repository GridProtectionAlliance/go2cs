...
// package ssa -- go2cs converted at 2022 March 06 23:33:29 UTC
...
public static partial class ssa_package {
    ...
    //
private static ptr<Function> makeWrapper(ptr<Program> _addr_prog, ptr<types.Selection> _addr_sel) => func((defer, _, _) => {
    ...
    nint start = default; // first regular param
    ...
    else
 {
    ...
    Value v = fn.Locals[0]; // spilled receiver
    ...
    v = emitImplicitSelections(fn, v, indices[..(int)len(indices) - 1]); 
    ...
    c = default;
    {
        ...
        if (!isInterface(r)) { // concrete method
            if (!isPointer(r)) {
                v = emitLoad(fn, v);
            }
        ...
        else
 {
    ...
    foreach (var (_, arg) in fn.Params[(int)1..]) {
...
//
private static void createParams(ptr<Function> _addr_fn, nint start) {
...
//
private static ptr<Function> makeBound(ptr<Program> _addr_prog, ptr<types.Func> _addr_obj) => func((defer, _, _) => {
    ref Program prog = ref _addr_prog.val;
