...
// package tests -- go2cs converted at 2020 October 09 06:04:18 UTC
...
namespace passes
{
    ...
    {
        public static readonly @string Doc = (@string)@"check for common mistaken usages of tests and examples
...
for the conventions that are enforced for Tests, Benchmarks, and Examples.";
        ...
        private static void checkExample(ptr<analysis.Pass> _addr_pass, ptr<ast.FuncDecl> _addr_fn)
        {
            ...
            var exName = strings.TrimPrefix(fnName, "Example");            var elems = strings.SplitN(exName, "_", 3L);            var ident = elems[0L];            var objs = lookup(_addr_pass.Pkg, ident);
            if (ident != "" && len(objs) == 0L)
            { 
            ...
            { 
                ...
                pass.Reportf(fn.Pos(), "%s has malformed example suffix: %s", fnName, elems[2L]);
