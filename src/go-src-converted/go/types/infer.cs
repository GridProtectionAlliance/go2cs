...
private static slice<Type> infer(this ptr<Checker> _addr_check, positioner posn, slice<ptr<TypeName>> tparams, slice<Type> targs, ptr<Tuple> _addr_@params, slice<ptr<operand>> args, bool report) => func((defer, _, _) => {
    ...
    if (@params.Len() > 0) {
        ...
        params = check.subst(token.NoPos, params, smap)._<ptr<Tuple>>();
    }
                    ...
                    else
 {
    ...
    }
    {
        ...
        foreach (var (_, __i) in indices) {
            ...
            {
                ptr<_TypeParam> tpar__prev1 = tpar;

                ptr<_TypeParam> (tpar, _) = par.typ._<ptr<_TypeParam>>();
    ...
    assert(index >= 0 && targs[index] == null);
...
private static (slice<Type>, nint) inferB(this ptr<Checker> _addr_check, slice<ptr<TypeName>> tparams, slice<Type> targs, bool report) {
    ...
    foreach (var (_, tpar) in tparams) {
        ptr<_TypeParam> typ = tpar.typ._<ptr<_TypeParam>>();
    ...
    while (len(dirty) > 0) { 
        ...
        foreach (var (_, index) in dirty) {
            var t0 = types[index];
            {
                var t1 = check.subst(token.NoPos, t0, smap);
...
private static Type structuralType(this ptr<Checker> _addr_check, Type constraint) {
...

    {
        ...
        if (iface != null) {
            ...
            var types = unpackType(iface.allTypes);
            if (len(types) == 1) {
                return types[0];
            }
            return null;
