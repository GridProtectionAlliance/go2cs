...
private partial struct context {
    ...
    public positioner errpos; // if set, identifier position of a constant with inherited initializer
...
public static ptr<Checker> NewChecker(ptr<Config> _addr_conf, ptr<token.FileSet> _addr_fset, ptr<Package> _addr_pkg, ptr<Info> _addr_info) => func((_, panic, _) => {
...
private static error checkFiles(this ptr<Checker> _addr_check, slice<ptr<ast.File>> files) => func((defer, _, _) => {
    error err = default!;
    ref Checker check = ref _addr_check.val;
