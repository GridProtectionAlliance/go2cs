...
// package types -- go2cs converted at 2022 March 06 22:41:41 UTC
// import "go/types" ==> using types = go.go.types_package
// Original source: C:\Program Files\Go\src\go\types\builtins.go
...
public static partial class types_package {
    ...
    //
private static bool builtin(this ptr<Checker> _addr_check, ptr<operand> _addr_x, ptr<ast.CallExpr> _addr_call, builtinId id) => func((defer, _, _) => {
    bool _ = default;
    ...
    if (call.Ellipsis.IsValid() && id != _Append) {
        check.invalidOp(atPos(call.Ellipsis), _InvalidDotDotDot, "invalid use of ... with built-in %s", bin.name);
        check.use(call.Args);
        return ;
    }
    if (id == _Len || id == _Cap) {
    ...
    if (id == _Make || id == _New || id == _Offsetof || id == _Trace)     else 
        ...
        var (xlist, _) = check.exprList(call.Args, false);
        arg = (x, i) => {
            x = xlist[i].val;
        ...
        else
 {
        ...
        if (dst == null || src == null) {
            check.invalidArg(x, _InvalidCopy, "copy expects slice arguments; found %s and %s", x, _addr_y);
            return ;
        }
        if (!check.identical(dst, src)) {
            check.invalidArg(x, _InvalidCopy, "arguments to copy %s and %s have different element types %s and %s", x, _addr_y, dst, src);
            return ;
        }
        if (check.Types != null) {
            ...
            else
 {
        ...
        slice<long> sizes = default; // constant integer arguments, if any
        {
            ...
            foreach (var (_, __arg) in call.Args[(int)1..]) {
                arg = __arg;
        ...
        y = default;
        arg(_addr_y, 1);
        if (!check.isValidIndex(_addr_y, _InvalidUnsafeAdd, "length", true)) {
            return ;
        }
        x.mode = value;
        ...
        y = default;
        arg(_addr_y, 1);
        if (!check.isValidIndex(_addr_y, _InvalidUnsafeSlice, "length", false)) {
            return ;
        }
        x.mode = value;
