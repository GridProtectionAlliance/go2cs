...
private static var @short = flag.Bool("test.short", false, "run smaller test suite to save time");private static var chatty = flag.Bool("test.v", false, "verbose: print additional output");private static var match = flag.String("test.run", "", "regular expression to select tests to run");private static var memProfile = flag.String("test.memprofile", "", "write a memory profile to the named file after execution");private static var memProfileRate = flag.Int("test.memprofilerate", 0, "if >=0, sets runtime.MemProfileRate");private static var cpuProfile = flag.String("test.cpuprofile", "", "write a cpu profile to the named file during execution");private static var timeout = flag.Duration("test.timeout", 0, "if positive, sets an aggregate time limit for all tests");private static var cpuListStr = flag.String("test.cpu", "", "comma-separated list of number of CPUs to use for each test");private static var parallel = flag.Int("test.parallel", runtime.GOMAXPROCS(0), "maximum test parallelism");private static slice<nint> cpuList = default;
...
private static @string decorate(@string s, bool addFileLine) {
    if (addFileLine) {
        var (_, file, line, ok) = runtime.Caller(3); // decorate + log + public function.
        if (ok) { 
        ...
        else
 {
    ...
    for (nint i = 0; i < n - 1; i++) { // -1 to avoid final newline
        if (s[i] == '\n') { 
...
private static void tRunner(ptr<T> _addr_t, ptr<InternalTest> _addr_test) => func((defer, _, _) => {
    ref T t = ref _addr_t.val;
...
public static bool RunTests(Func<@string, @string, (bool, error)> matchString, slice<InternalTest> tests) {
    ...
    foreach (var (_, procs) in cpuList) {
        ...
        nint running = 0;
        while (numParallel + running > 0) {
            if (running < parallel && numParallel > 0.val) {
...
private static void alarm() => func((_, panic, _) => {
    panic("test timed out");
});

private static void parseCpuList() {
    if (len(cpuListStr.val) == 0) {
