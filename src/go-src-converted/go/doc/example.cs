...
private static ptr<ast.File> playExample(ptr<ast.File> _addr_file, ptr<ast.FuncDecl> _addr_f) {
                ...
                else
 {
    ...
    var decls = make_slice<ast.Decl>(0, 2 + len(depDecls));
...
private static (ptr<ast.BlockStmt>, slice<ptr<ast.CommentGroup>>) stripOutputComment(ptr<ast.BlockStmt> _addr_body, slice<ptr<ast.CommentGroup>> comments) {
    ...
    var newComments = make_slice<ptr<ast.CommentGroup>>(len(comments) - 1);
...
private static (nint, ptr<ast.CommentGroup>) lastComment(ptr<ast.BlockStmt> _addr_b, slice<ptr<ast.CommentGroup>> c) {
    nint i = default;
...
private static (@string, @string, bool) splitExampleName(@string s, nint i) {
