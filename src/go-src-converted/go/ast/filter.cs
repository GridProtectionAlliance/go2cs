...
public partial struct MergeMode { // : nuint
}

 
// If set, duplicate function declarations are excluded.
public static readonly MergeMode FilterFuncDuplicates = 1 << (int)(iota); 
...
public static ptr<File> MergePackageFiles(ptr<Package> _addr_pkg, MergeMode mode) {
    ...
    if (ndocs > 0) {
        var list = make_slice<ptr<Comment>>(ndocs - 1); // -1: no separator before first group
        i = 0;
        {
            var filename__prev1 = filename;
    ...
    if (ndecls > 0) {
        ...
        var funcs = make_map<@string, nint>(); // map of func name -> decls index
        i = 0; // current index
    ...
    if (mode & FilterUnassociatedComments == 0) {
        ...
        i = 0;
        {
            ...
            foreach (var (_, __filename) in filenames) {
                filename = __filename;
                f = pkg.Files[filename];
                i += copy(comments[(int)i..], f.Comments);
            }
