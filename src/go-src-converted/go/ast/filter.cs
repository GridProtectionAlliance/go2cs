...
// package ast -- go2cs converted at 2020 August 29 08:48:31 UTC
...
namespace go
{
    public static partial class ast_package
    {
        ...
        //
        public static bool FileExports(ref File src)
        {
        ...
        //
        public static bool PackageExports(ref Package pkg)
        {
        ...
        private static slice<ref Ident> filterIdentList(slice<ref Ident> list, Filter f)
        {
            long j = 0L;
        ...
        //
        private static ref Ident fieldName(Expr x)
        {
            switch (x.type())
            {
                case ref Ident t:
                    return t;
                    break;
                case ref SelectorExpr t:
                    {
                        ref Ident (_, ok) = t.X._<ref Ident>();
        ...
        private static bool filterFieldList(ref FieldList fields, Filter filter, bool export)
        {
            if (fields == null)
            {
                return false;
        ...
        private static bool filterParamList(ref FieldList fields, Filter filter, bool export)
        {
            if (fields == null)
            {
                return false;
        ...
        private static bool filterType(Expr typ, Filter f, bool export)
        {
            switch (typ.type())
            {
                case ref Ident t:
                    return f(t.Name);
                    break;
                case ref ParenExpr t:
                    return filterType(t.X, f, export);
                    break;
                case ref ArrayType t:
                    return filterType(t.Elt, f, export);
                    break;
                case ref StructType t:
        ...
        //
        public static bool FilterFile(ref File src, Filter f)
        {
            return filterFile(src, f, false);
        }

        private static bool filterFile(ref File src, Filter f, bool export)
        {
            long j = 0L;
        ...
        // left after filtering.
        //
        public static bool FilterPackage(ref Package pkg, Filter f)
        {
            return filterPackage(pkg, f, false);
        }

        private static bool filterPackage(ref Package pkg, Filter f, bool export)
        {
            var hasDecls = false;
        ...
        public static readonly MergeMode FilterFuncDuplicates = 1L << (int)(iota); 
        ...
        //
        private static @string nameOf(ref FuncDecl f)
        {
            {
                var r = f.Recv;
                ...
                { 
                    ...
                    {
                        ref StarExpr p__prev2 = p;

                        ref StarExpr (p, _) = t._<ref StarExpr>();
        ...
        //
        private static Comment separator = ref new Comment(token.NoPos,"//");

        // MergePackageFiles creates a file AST by merging the ASTs of the
        // files belonging to a package. The mode flags control merging behavior.
        //
        public static ref File MergePackageFiles(ref Package pkg, MergeMode mode)
        { 
            ...
            ref CommentGroup doc = default;
            token.Pos pos = default;
            if (ndocs > 0L)
            {
                var list = make_slice<ref Comment>(ndocs - 1L); // -1: no separator before first group
                i = 0L;
                {
                    var filename__prev1 = filename;
                            ...
                            foreach (var (_, __d) in f.Decls)
                            {
                                ...
                                { 
                                    ...
                                    //            multiple declarations are common.
                                    {
                                        ...
                                        ref FuncDecl (f, isFun) = d._<ref FuncDecl>();
                                        ...
                                        if (isFun)
                                        {
                                            var name = nameOf(f);
                                            {
                                                ...
                                                { 
                                                    ...
                                                    if (decls[j] != null && decls[j]._<ref FuncDecl>().Doc == null)
                                                    { 
            ...
            slice<ref ImportSpec> imports = default;
            ...
            slice<ref CommentGroup> comments = default;
            if (mode & FilterUnassociatedComments == 0L)
            {
                comments = make_slice<ref CommentGroup>(ncomments);
                i = 0L;
                {
                    var f__prev1 = f;
            ...
            return ref new File(doc,pos,NewIdent(pkg.Name),decls,pkg.Scope,imports,nil,comments);
        }
    }
}}
