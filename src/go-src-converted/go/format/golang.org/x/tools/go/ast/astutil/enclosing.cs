...
// package astutil -- go2cs converted at 2020 October 09 05:39:52 UTC
// import "golang.org/x/tools/go/ast/astutil" ==> using astutil = go.golang.org.x.tools.go.ast.astutil_package
// Original source: C:\Users\ritchie\go\src\golang.org\x\tools\go\ast\astutil\enclosing.go
    ...
    public static partial class astutil_package
    {
        ...
        public static (slice<ast.Node>, bool) PathEnclosingInterval(ptr<ast.File> _addr_root, token.Pos start, token.Pos end)
        {
            ...
            visit = node =>
            {
                ...
                var l = len(children);
                {
                    ...
                    foreach (var (__i, __child) in children)
                    {
                        ...
                        if (i > 0L)
                        {
                            augPos = children[i - 1L].End(); // start of preceding whitespace
                        }
                        if (i < l - 1L)
                        {
                            var nextChildPos = children[i + 1L].Pos(); 
            ...
            if (start < root.End() && end > root.Pos())
            {
                if (start == end)
                {
                    end = start + 1L; // empty interval => interval of size 1
                }
                exact = visit(root); 
        ...
        private static ast.Node tok(token.Pos pos, long len)
        {
        ...
        private static slice<ast.Node> childrenOf(ast.Node n)
        {
            ...
            switch (n.type())
            {
                ...
                case ptr<ast.TypeAssertExpr> n:
                    children = append(children, tok(n.Lparen - 1L, len(".")), tok(n.Lparen, len("(")), tok(n.Rparen, len(")")));
                    break;
        ...
        private static long Len(this byPos sl)
        {
            return len(sl);
        }
        private static bool Less(this byPos sl, long i, long j)
        {
            return sl[i].Pos() < sl[j].Pos();
        }
        private static void Swap(this byPos sl, long i, long j)
        {
            sl[i] = sl[j];
            sl[j] = sl[i];
        ...
        //
        public static @string NodeDescription(ast.Node n) => func((_, panic, __) =>
        {
            switch (n.type())
            {
                case ptr<ast.ArrayType> n:
                ...
                case ptr<ast.BranchStmt> n:
                    ...
                    if (n.Tok == token.BREAK) 
                        ...
                        return "fall-through statement";
                                        break;
                case ptr<ast.CallExpr> n:
                    if (len(n.Args) == 1L && !n.Ellipsis.IsValid())
                    {
