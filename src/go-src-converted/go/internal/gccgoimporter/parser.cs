...
private partial struct parser {
    ...
    public map<nint, @string> aliases; // maps saved type number to alias name
}
...
private static void init(this ptr<parser> _addr_p, @string filename, io.Reader src, map<@string, ptr<types.Package>> imports) {
    ...
    p.imports = imports;
    p.aliases = make_map<nint, @string>();
    p.typeList = make_slice<types.Type>(1, 16);
}
...
private static @string parseUnquotedString(this ptr<parser> _addr_p) {
    ...
    // we need to let it be consumed by p.next().
    {
        ...
        while (ch != '\n' && ch != ';' && ch != scanner.EOF && p.scanner.Whitespace & (1 << (int)(uint(ch))) == 0) {
            buf.WriteRune(ch);
            p.scanner.Next();
            ch = p.scanner.Peek();
        }
...
private static (@string, @string) parseQualifiedNameStr(this ptr<parser> _addr_p, @string unquotedName) {
    ...
    switch (len(parts)) {
        ...
        default: 
            ...
            pkgpath = strings.Join(parts[(int)0..(int)len(parts) - 1], ".");
...
private static (constant.Value, types.Type) parseConstValue(this ptr<parser> _addr_p, ptr<types.Package> _addr_pkg) {
        ...
        else
 {
        ...
        var imval = constant.MakeFromLiteral(im + "i", token.IMAG, 0);
        if (imval == null) {
...
private static void reserve(this ptr<parser> _addr_p, nint n) {
    ref parser p = ref _addr_p.val;
...
private static readonly nint gccgoBuiltinINT8 = 1;
private static readonly nint gccgoBuiltinINT16 = 2;
private static readonly nint gccgoBuiltinINT32 = 3;
private static readonly nint gccgoBuiltinINT64 = 4;
private static readonly nint gccgoBuiltinUINT8 = 5;
private static readonly nint gccgoBuiltinUINT16 = 6;
private static readonly nint gccgoBuiltinUINT32 = 7;
private static readonly nint gccgoBuiltinUINT64 = 8;
private static readonly nint gccgoBuiltinFLOAT32 = 9;
private static readonly nint gccgoBuiltinFLOAT64 = 10;
private static readonly nint gccgoBuiltinINT = 11;
private static readonly nint gccgoBuiltinUINT = 12;
private static readonly nint gccgoBuiltinUINTPTR = 13;
private static readonly nint gccgoBuiltinBOOL = 15;
private static readonly nint gccgoBuiltinSTRING = 16;
private static readonly nint gccgoBuiltinCOMPLEX64 = 17;
private static readonly nint gccgoBuiltinCOMPLEX128 = 18;
...
private static (types.Type, nint) parseTypeAfterAngle(this ptr<parser> _addr_p, ptr<types.Package> _addr_pkg, params object[] n) {
    ...
    if (p.tok == scanner.Int) 
        n1 = p.parseInt();
        if (p.tok == '>') {
            if (len(p.typeData) > 0 && p.typeList[n1] == null) {
...
private static (types.Type, nint) parseTypeExtended(this ptr<parser> _addr_p, ptr<types.Package> _addr_pkg, params object[] n) {
    types.Type t = default;
    nint n1 = default;
    n = n.Clone();
    ref parser p = ref _addr_p.val;
...
private static void skipInlineBody(this ptr<parser> _addr_p) => func((defer, _, _) => {
    ref parser p = ref _addr_p.val;
...
private static void parseTypes(this ptr<parser> _addr_p, ptr<types.Package> _addr_pkg) => func((defer, _, _) => {
    ref parser p = ref _addr_p.val;
    ref types.Package pkg = ref _addr_pkg.val;
...
private static void parseSavedType(this ptr<parser> _addr_p, ptr<types.Package> _addr_pkg, nint i, slice<object> nlist) => func((defer, _, _) => {
    ref parser p = ref _addr_p.val;
    ref types.Package pkg = ref _addr_pkg.val;
...
private static void parseInitDataDirective(this ptr<parser> _addr_p) => func((defer, _, _) => {
    ref parser p = ref _addr_p.val;
