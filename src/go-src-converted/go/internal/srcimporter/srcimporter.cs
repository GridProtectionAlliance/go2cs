...
namespace @internal
{
    public static partial class srcimporter_package
    {
        ...
        private static (ptr<types.Package>, error) Import(this ptr<Importer> _addr_p, @string path)
        {
            ...
            return _addr_p.ImportFrom(path, ".", 0L)!; // use "." rather than "" (see issue #24441)
        }
        ...
        private static (slice<ptr<ast.File>>, error) parseFiles(this ptr<Importer> _addr_p, @string dir, slice<@string> filenames) => func((defer, _, __) =>
        {
        ...
        private static (ptr<ast.File>, error) cgo(this ptr<Importer> _addr_p, ptr<build.Package> _addr_bp) => func((defer, _, __) =>
        {
            ...
            var (tmpdir, err) = ioutil.TempDir("", "srcimporter");
            if (err != null)
            {
                return (_addr_null!, error.As(err)!);
            }
            ...
            cmd = exec.Command(args[0L], args[1L..]);
            cmd.Dir = bp.Dir;
            {
                var err = cmd.Run();
            ...
            return _addr_parser.ParseFile(p.fset, filepath.Join(tmpdir, "_cgo_gotypes.go"), null, 0L)!;
