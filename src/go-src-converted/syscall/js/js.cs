...
private static readonly nuint nanHead = 0x7FF80000;
...
private static Value floatValue(double f) {
    if (f == 0) {>>MARKER:FUNCTION_finalizeRef_BLOCK_PREFIX<<
        return valueZero;
    }
    if (f != f) {
        return valueNaN;
...
private static Value valueUndefined = new Value(ref:0);private static var valueNaN = predefValue(0, typeFlagNone);private static var valueZero = predefValue(1, typeFlagNone);private static var valueNull = predefValue(2, typeFlagNone);private static var valueTrue = predefValue(3, typeFlagNone);private static var valueFalse = predefValue(4, typeFlagNone);private static var valueGlobal = predefValue(5, typeFlagObject);private static var jsGo = predefValue(6, typeFlagObject);private static var objectConstructor = valueGlobal.Get("Object");private static var arrayConstructor = valueGlobal.Get("Array");
...
public static Value ValueOf(object x) => func((_, panic, _) => {
    switch (x.type()) {
        case Value x:
            return x;
            break;
...
public partial struct Type { // : nint
}

public static readonly Type TypeUndefined = iota;
...
public static Type Type(this Value v) => func((_, panic, _) => {
    ...
    if (v.@ref == valueUndefined.@ref) 
        ...
        return TypeBoolean;
        if (v.isNumber()) {>>MARKER:FUNCTION_stringVal_BLOCK_PREFIX<<
        return TypeNumber;
    }
    var typeFlag = (v.@ref >> 32) & 7;
...
public static Value Get(this Value v, @string p) => func((_, panic, _) => {
    {
        var vType = v.Type();
...
// It panics if v is not a JavaScript object.
public static void Set(this Value v, @string p, object x) => func((_, panic, _) => {
    {
        var vType = v.Type();

        if (!vType.isObject()) {>>MARKER:FUNCTION_valueGet_BLOCK_PREFIX<<
...
// It panics if v is not a JavaScript object.
public static void Delete(this Value v, @string p) => func((_, panic, _) => {
    {
        var vType = v.Type();

        if (!vType.isObject()) {>>MARKER:FUNCTION_valueSet_BLOCK_PREFIX<<
...
public static Value Index(this Value v, nint i) => func((_, panic, _) => {
    {
        ...
        if (!vType.isObject()) {>>MARKER:FUNCTION_valueDelete_BLOCK_PREFIX<<
...
// It panics if v is not a JavaScript object.
public static void SetIndex(this Value v, nint i, object x) => func((_, panic, _) => {
    {
        var vType = v.Type();

        if (!vType.isObject()) {>>MARKER:FUNCTION_valueIndex_BLOCK_PREFIX<<
...
// It panics if v is not a JavaScript object.
public static nint Length(this Value v) => func((_, panic, _) => {
    {
        var vType = v.Type();

        if (!vType.isObject()) {>>MARKER:FUNCTION_valueSetIndex_BLOCK_PREFIX<<
            panic(addr(new ValueError("Value.SetIndex",vType)));
        }
    }
...
public static Value Call(this Value v, @string m, params object[] args) => func((_, panic, _) => {
    args = args.Clone();
    ...
    if (!ok) {>>MARKER:FUNCTION_valueLength_BLOCK_PREFIX<<
        {
            var vType = v.Type();
...
// The arguments get mapped to JavaScript values according to the ValueOf function.
public static Value Invoke(this Value v, params object[] args) => func((_, panic, _) => {
    args = args.Clone();
    ...
    runtime.KeepAlive(v);
    runtime.KeepAlive(argVals);
    if (!ok) {>>MARKER:FUNCTION_valueCall_BLOCK_PREFIX<<
        {
            var vType = v.Type();
...
// It panics if v is not a JavaScript function.
// The arguments get mapped to JavaScript values according to the ValueOf function.
public static Value New(this Value v, params object[] args) => func((_, panic, _) => {
    args = args.Clone();
    ...
    runtime.KeepAlive(v);
    runtime.KeepAlive(argVals);
    if (!ok) {>>MARKER:FUNCTION_valueInvoke_BLOCK_PREFIX<<
        {
            var vType = v.Type();
...
public static bool isNumber(this Value v) {
    return v.@ref == valueZero.@ref || v.@ref == valueNaN.@ref || (v.@ref != valueUndefined.@ref && (v.@ref >> 32) & nanHead != nanHead);
}

public static double @float(this Value v, @string method) => func((_, panic, _) => {
    if (!v.isNumber()) {>>MARKER:FUNCTION_valueNew_BLOCK_PREFIX<<
...
// It panics if v is not a JavaScript number.
public static nint Int(this Value v) {
    return int(v.@float("Value.Int"));
}

// Bool returns the value v as a bool.
// It panics if v is not a JavaScript boolean.
public static bool Bool(this Value v) => func((_, panic, _) => {
...
public static bool Truthy(this Value v) => func((_, panic, _) => {
...
public static @string String(this Value v) => func((_, panic, _) => {
...
private static (ref, nint) valuePrepareString(ref v);
...
public static nint CopyBytesToGo(slice<byte> dst, Value src) => func((_, panic, _) => {
    var (n, ok) = copyBytesToGo(dst, src.@ref);
    runtime.KeepAlive(src);
    if (!ok) {>>MARKER:FUNCTION_valueInstanceOf_BLOCK_PREFIX<<
...
// It returns the number of bytes copied, which will be the minimum of the lengths of src and dst.
public static nint CopyBytesToJS(Value dst, slice<byte> src) => func((_, panic, _) => {
    var (n, ok) = copyBytesToJS(dst.@ref, src);
    runtime.KeepAlive(dst);
    if (!ok) {>>MARKER:FUNCTION_copyBytesToGo_BLOCK_PREFIX<<
