...
// package syscall -- go2cs converted at 2022 March 06 22:27:56 UTC
// import "syscall" ==> using syscall = go.syscall_package
// Original source: C:\Program Files\Go\src\syscall\zsyscall_freebsd_arm.go
...
private static error fstat_freebsd12(nint fd, ptr<Stat_t> _addr_stat) {
    ...
    var (_, _, e1) = Syscall(_SYS_FSTAT_FREEBSD12, uintptr(fd), uintptr(@unsafe.Pointer(stat)), 0);
    if (e1 != 0) {
        err = errnoErr(e1);
    }
    return ;
...
private static error fstatat_freebsd12(nint fd, @string path, ptr<Stat_t> _addr_stat, nint flags) {
    ...
    var (_, _, e1) = Syscall6(_SYS_FSTATAT_FREEBSD12, uintptr(fd), uintptr(@unsafe.Pointer(_p0)), uintptr(@unsafe.Pointer(stat)), uintptr(flags), 0, 0);
    if (e1 != 0) {
        err = errnoErr(e1);
    }
    return ;
...
private static error fstatfs_freebsd12(nint fd, ptr<Statfs_t> _addr_stat) {
    ...
    var (_, _, e1) = Syscall(_SYS_FSTATFS_FREEBSD12, uintptr(fd), uintptr(@unsafe.Pointer(stat)), 0);
    if (e1 != 0) {
        err = errnoErr(e1);
    }
    return ;
...
private static (nint, error) getdirentries_freebsd12(nint fd, slice<byte> buf, ptr<ulong> _addr_basep) {
    ...
    else
 {
    ...
    var (r0, _, e1) = Syscall6(_SYS_GETDIRENTRIES_FREEBSD12, uintptr(fd), uintptr(_p0), uintptr(len(buf)), uintptr(@unsafe.Pointer(basep)), 0, 0);
    n = int(r0);
    if (e1 != 0) {
        err = errnoErr(e1);
    }
...
private static error mknodat_freebsd12(nint fd, @string path, uint mode, ulong dev) {
    ...
    var (_, _, e1) = Syscall6(_SYS_MKNODAT_FREEBSD12, uintptr(fd), uintptr(@unsafe.Pointer(_p0)), uintptr(mode), uintptr(dev), 0, 0);
    if (e1 != 0) {
        err = errnoErr(e1);
    }
    return ;
...
public static error Select(nint n, ptr<FdSet> _addr_r, ptr<FdSet> _addr_w, ptr<FdSet> _addr_e, ptr<Timeval> _addr_timeout) {
    ...
    var (_, _, e1) = Syscall6(SYS_SELECT, uintptr(n), uintptr(@unsafe.Pointer(r)), uintptr(@unsafe.Pointer(w)), uintptr(@unsafe.Pointer(e)), uintptr(@unsafe.Pointer(timeout)), 0);
    if (e1 != 0) {
        err = errnoErr(e1);
    }
    return ;
...
private static error statfs_freebsd12(@string path, ptr<Statfs_t> _addr_stat) {
    ...
    var (_, _, e1) = Syscall(_SYS_STATFS_FREEBSD12, uintptr(@unsafe.Pointer(_p0)), uintptr(@unsafe.Pointer(stat)), 0);
    if (e1 != 0) {
        err = errnoErr(e1);
    }
    return ;
