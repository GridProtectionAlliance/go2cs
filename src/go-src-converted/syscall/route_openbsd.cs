...
// package syscall -- go2cs converted at 2022 March 06 22:26:45 UTC
// import "syscall" ==> using syscall = go.syscall_package
// Original source: C:\Program Files\Go\src\syscall\route_openbsd.go
...
private static RoutingMessage toRoutingMessage(this ptr<anyMessage> _addr_any, slice<byte> b) {
    ...
    if (any.Type == RTM_ADD || any.Type == RTM_DELETE || any.Type == RTM_CHANGE || any.Type == RTM_GET || any.Type == RTM_LOSING || any.Type == RTM_REDIRECT || any.Type == RTM_MISS || any.Type == RTM_LOCK || any.Type == RTM_RESOLVE) 
        ...
        return addr(new RouteMessage(Header:p.Header,Data:b[p.Header.Hdrlen:any.Msglen]));
    else if (any.Type == RTM_IFINFO) 
        p = (InterfaceMessage.val)(@unsafe.Pointer(any));
        return addr(new InterfaceMessage(Header:p.Header,Data:b[p.Header.Hdrlen:any.Msglen]));
        ...
        return addr(new InterfaceAddrMessage(Header:p.Header,Data:b[p.Header.Hdrlen:any.Msglen]));
        return null;
