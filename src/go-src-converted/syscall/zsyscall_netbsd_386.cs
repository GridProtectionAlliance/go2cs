...
// package syscall -- go2cs converted at 2022 March 06 22:29:04 UTC
// import "syscall" ==> using syscall = go.syscall_package
// Original source: C:\Program Files\Go\src\syscall\zsyscall_netbsd_386.go
...
private static (nint, error) paccept(nint fd, ptr<RawSockaddrAny> _addr_rsa, ptr<_Socklen> _addr_addrlen, ptr<sigset> _addr_sigmask, nint flags) {
    nint nfd = default;
    error err = default!;
    ref RawSockaddrAny rsa = ref _addr_rsa.val;
    ref _Socklen addrlen = ref _addr_addrlen.val;
    ref sigset sigmask = ref _addr_sigmask.val;

    var (r0, _, e1) = Syscall6(SYS_PACCEPT, uintptr(fd), uintptr(@unsafe.Pointer(rsa)), uintptr(@unsafe.Pointer(addrlen)), uintptr(@unsafe.Pointer(sigmask)), uintptr(flags), 0);
    nfd = int(r0);
    if (e1 != 0) {
        err = errnoErr(e1);
    }
