...
// package syscall -- go2cs converted at 2022 March 06 22:28:19 UTC
// import "syscall" ==> using syscall = go.syscall_package
// Original source: C:\Program Files\Go\src\syscall\zsyscall_linux_arm64.go
...
private static error fstatat(nint dirfd, @string path, ptr<Stat_t> _addr_stat, nint flags) {
    ...
    var (_, _, e1) = Syscall6(SYS_FSTATAT, uintptr(dirfd), uintptr(@unsafe.Pointer(_p0)), uintptr(@unsafe.Pointer(stat)), uintptr(flags), 0, 0);
    if (e1 != 0) {
        err = errnoErr(e1);
    }
    return ;
...
private static error getrlimit(nint resource, ptr<Rlimit> _addr_rlim) {
    error err = default!;
...
private static error setrlimit(nint resource, ptr<Rlimit> _addr_rlim) {
    error err = default!;
    ref Rlimit rlim = ref _addr_rlim.val;
...
public static error SyncFileRange(nint fd, long off, long n, nint flags) {
    ...
    var (_, _, e1) = Syscall6(SYS_SYNC_FILE_RANGE2, uintptr(fd), uintptr(off), uintptr(n), uintptr(flags), 0, 0);
    if (e1 != 0) {
        err = errnoErr(e1);
    }
    return ;
...
private static (nint, error) pselect(nint nfd, ptr<FdSet> _addr_r, ptr<FdSet> _addr_w, ptr<FdSet> _addr_e, ptr<Timespec> _addr_timeout, ptr<sigset_t> _addr_sigmask) {
    nint n = default;
    error err = default!;
