...
// Original source: C:\Program Files\Go\src\syscall\syscall_plan9.go
...
private static bool contains(@string s, @string sep) {
    var n = len(sep);
    var c = sep[0];
    for (nint i = 0; i + n <= len(s); i++) {
        if (s[i] == c && s[(int)i..(int)i + n] == sep) {
            return true;
        }
    }
    return false;
...
private static @string cstring(slice<byte> s) {
    foreach (var (i) in s) {
        if (s[i] == 0) {>>MARKER:FUNCTION_RawSyscall_BLOCK_PREFIX<<
            return string(s[(int)0..(int)i]);
        }
    }    return string(s);
...
public static (nint, error) Write(nint fd, slice<byte> p) {
    ...
    if (faketime && (fd == 1 || fd == 2)) {
        ...
        if (n < 0) {
            return (0, error.As(ErrorString("error"))!);
        }
...
public static error Mkdir(@string path, uint mode) {
    ...
    _, err = Stat(path[(int)0..(int)n], statbuf);
    if (err == null) {
