...
// package syscall -- go2cs converted at 2022 March 06 22:26:57 UTC
// import "syscall" ==> using syscall = go.syscall_package
// Original source: C:\Program Files\Go\src\syscall\syscall_freebsd.go
...
private static readonly nint _SYS_FSTAT_FREEBSD12 = 551; // { int fstat(int fd, _Out_ struct stat *sb); }
private static readonly nint _SYS_FSTATAT_FREEBSD12 = 552; // { int fstatat(int fd, _In_z_ char *path, _Out_ struct stat *buf, int flag); }
private static readonly nint _SYS_GETDIRENTRIES_FREEBSD12 = 554; // { ssize_t getdirentries(int fd, _Out_writes_bytes_(count) char *buf, size_t count, _Out_ off_t *basep); }
private static readonly nint _SYS_STATFS_FREEBSD12 = 555; // { int statfs(_In_z_ char *path, _Out_ struct statfs *buf); }
private static readonly nint _SYS_FSTATFS_FREEBSD12 = 556; // { int fstatfs(int fd, _Out_ struct statfs *buf); }
private static readonly nint _SYS_GETFSSTAT_FREEBSD12 = 557; // { int getfsstat(_Out_writes_bytes_opt_(bufsize) struct statfs *buf, long bufsize, int mode); }
private static readonly nint _SYS_MKNODAT_FREEBSD12 = 559; // { int mknodat(int fd, _In_z_ char *path, mode_t mode, dev_t dev); }
...
public static error Stat(@string path, ptr<Stat_t> _addr_st) {
    ...
    if (supportsABI(_ino64First)) {
        return error.As(fstatat_freebsd12(_AT_FDCWD, path, st, 0))!;
    }
