...
private static (nint, Errno) forkAndExecInChild(ptr<byte> _addr_argv0, slice<ptr<byte>> argv, slice<ptr<byte>> envv, ptr<byte> _addr_chroot, ptr<byte> _addr_dir, ptr<ProcAttr> _addr_attr, ptr<SysProcAttr> _addr_sys, nint pipe) {
    ...
    nextfd = len(attr.Files);
    {
        ...
        foreach (var (__i, __ufd) in attr.Files) {
            i = __i;
            ufd = __ufd;
            if (nextfd < int(ufd)) {>>MARKER:FUNCTION_write1_BLOCK_PREFIX<<
                nextfd = int(ufd);
            }
            fd[i] = int(ufd);
        }
    ...
    if (err1 != 0) {>>MARKER:FUNCTION_setpgid_BLOCK_PREFIX<<
        runtime_AfterFork();
        return (0, err1);
    }
    if (r1 != 0) {>>MARKER:FUNCTION_setuid_BLOCK_PREFIX<< 
        // parent; return PID
        runtime_AfterFork();
        return (int(r1), 0);
    ...
    if (sys.Setsid) {>>MARKER:FUNCTION_setsid_BLOCK_PREFIX<<
        _, err1 = setsid();
        if (err1 != 0) {>>MARKER:FUNCTION_setgroups1_BLOCK_PREFIX<<
            goto childerror;
        }
    }
    if (sys.Setpgid || sys.Foreground) {>>MARKER:FUNCTION_setgid_BLOCK_PREFIX<< 
        // Place child in process group.
        err1 = setpgid(0, uintptr(sys.Pgid));
        if (err1 != 0) {>>MARKER:FUNCTION_ioctl_BLOCK_PREFIX<<
            goto childerror;
        }
    }
    if (sys.Foreground) {>>MARKER:FUNCTION_getpid_BLOCK_PREFIX<<
        ref var pgrp = ref heap(_Pid_t(sys.Pgid), out ptr<var> _addr_pgrp);
        if (pgrp == 0) {>>MARKER:FUNCTION_forkx_BLOCK_PREFIX<<
            r1, err1 = getpid();
            if (err1 != 0) {>>MARKER:FUNCTION_fcntl1_BLOCK_PREFIX<<
                goto childerror;
            }
            pgrp = _Pid_t(r1);
        }
        err1 = ioctl(uintptr(sys.Ctty), uintptr(TIOCSPGRP), uintptr(@unsafe.Pointer(_addr_pgrp)));
        if (err1 != 0) {>>MARKER:FUNCTION_exit_BLOCK_PREFIX<<
            goto childerror;
        }
    }
    ...
    // Chroot
    if (chroot != null) {>>MARKER:FUNCTION_execve_BLOCK_PREFIX<<
        err1 = chroot1(uintptr(@unsafe.Pointer(chroot)));
        if (err1 != 0) {>>MARKER:FUNCTION_dup2child_BLOCK_PREFIX<<
            goto childerror;
        }
    }
    {
        ...
        if (cred != null) {>>MARKER:FUNCTION_close_BLOCK_PREFIX<<
            var ngroups = uintptr(len(cred.Groups));
            var groups = uintptr(0);
            if (ngroups > 0) {>>MARKER:FUNCTION_chroot1_BLOCK_PREFIX<<
                groups = uintptr(@unsafe.Pointer(_addr_cred.Groups[0]));
            }
            if (!cred.NoSetGroups) {>>MARKER:FUNCTION_chdir_BLOCK_PREFIX<<
                err1 = setgroups1(ngroups, groups);
                if (err1 != 0) {>>MARKER:FUNCTION_runtime_AfterForkInChild_BLOCK_PREFIX<<
                    goto childerror;
                }
            }
            ...
            if (err1 != 0) {>>MARKER:FUNCTION_runtime_BeforeFork_BLOCK_PREFIX<<
                goto childerror;
            }
        }
    } 
    ...
    err1 = execve(uintptr(@unsafe.Pointer(argv0)), uintptr(@unsafe.Pointer(_addr_argv[0])), uintptr(@unsafe.Pointer(_addr_envv[0])));
