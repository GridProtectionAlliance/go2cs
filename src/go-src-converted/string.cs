...
// package stringintconv -- go2cs converted at 2020 October 09 06:04:15 UTC
...
namespace passes
{
    ...
    {
        public static readonly @string Doc = (@string)@"check for string(int) conversions
...
";
        ...
        private static (object, error) run(ptr<analysis.Pass> _addr_pass)
        {
            ...
            inspect.Preorder(nodeFilter, n =>
            {
                ...
                if (V == null || V.Info() & types.IsInteger == 0L)
                {
                    return ;
                }
