...
// package httplex -- go2cs converted at 2020 August 29 10:12:13 UTC
// import "vendor/golang_org/x/net/lex/httplex" ==> using httplex = go.vendor.golang_org.x.net.lex.httplex_package
// Original source: C:\Go\src\vendor\golang_org\x\net\lex\httplex\httplex.go
...
namespace lex
{
    public static partial class httplex_package
    {
        ...
        { 
            ...
            // aware code.
            while (len(x) > 0L && isOWS(x[0L]))
            {
                x = x[1L..];
            }

            while (len(x) > 0L && isOWS(x[len(x) - 1L]))
            {
                x = x[..len(x) - 1L];
            }
        ...
        private static bool headerValueContainsToken(@string v, @string token)
        {
            v = trimOWS(v);
            {
                ...
                if (comma != -1L)
                {
                    return tokenEqual(trimOWS(v[..comma]), token) || headerValueContainsToken(v[comma + 1L..], token);
                }
 ...
 // a CTL
        ...
        public static (@string, error) PunycodeHostPort(@string v)
        {
            ...
            return (net.JoinHostPort(host, port), null);
        }
    }
}}}}}}
