...
namespace net
{
    ...
    {
        private static (Message, error) parseInterfaceAnnounceMessage(this ref wireFormat w, RIBType _, slice<byte> b)
        {
            if (len(b) < w.bodyOff)
            {
            ...
            InterfaceAnnounceMessage m = ref new InterfaceAnnounceMessage(Version:int(b[2]),Type:int(b[3]),Index:int(nativeEndian.Uint16(b[4:6])),What:int(nativeEndian.Uint16(b[22:24])),raw:b[:l],);
