...
// Original source: C:\Go\src\vendor\golang_org\x\crypto\poly1305\sum_arm.go
...
namespace crypto
{
    public static partial class poly1305_package
    {
        ...
        //go:noescape
        private static void poly1305_auth_armv6(ref array<byte> @out, ref byte m, uint mlen, ref array<byte> key)
;
        ...
        public static void Sum(ref array<byte> @out, slice<byte> m, ref array<byte> key)
        {
            ...
            if (len(m) > 0L)
            {>>MARKER:FUNCTION_poly1305_auth_armv6_BLOCK_PREFIX<<
                mPtr = ref m[0L];
            }
