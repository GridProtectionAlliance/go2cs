...
// package norm -- go2cs converted at 2020 August 29 10:13:11 UTC
...
namespace unicode
{
    public static partial class norm_package
    {
        ...
        private static (long, error) Write(this ref normWriter w, slice<byte> data)
        { 
            // Process data in pieces to keep w.buf size bounded.
            const long chunk = 4000L;
            ...
            { 
                ...
                w.rb.nsrc = m;
                w.buf = doAppend(ref w.rb, w.buf, 0L);
                data = data[m..];
                n += m; 

                // Write out complete prefix, save remainder.
                // Note that lastBoundary looks back at most 31 runes.
                var i = lastBoundary(ref w.rb.f, w.buf);
                if (i == -1L)
                {
                    i = 0L;
                }
        ...
        private static error Close(this ref normWriter w)
        {
            if (len(w.buf) > 0L)
            {
        ...
        {
            normWriter wr = ref new normWriter(rb:reorderBuffer{},w:w);
            wr.rb.init(f, null);
            return wr;
        }
        ...
        private static (long, error) Read(this ref normReader r, slice<byte> p)
        {
            while (true)
            {
                ...
                if (n > 0L)
                {
                    r.outbuf = doAppend(ref r.rb, r.outbuf, 0L);
                }
                if (err == io.EOF)
                {
                ...
                {
                    r.lastBoundary = lastBoundary(ref r.rb.f, r.outbuf);
        ...
        public static io.Reader Reader(this Form f, io.Reader r)
        {
            ...
            normReader rr = ref new normReader(rb:reorderBuffer{},r:r,inbuf:buf);
            rr.rb.init(f, buf);
            return rr;
        }
    }
