...
namespace net
{
    ...
    {
        private static sync.Once stackOnce = default;        private static bool ipv4Enabled = default;        private static bool ipv6Enabled = default;        private static bool unStrmDgramEnabled = default;        private static bool rawSocketSess = default;        private static var aLongTimeAgo = time.Unix(233431200L, 0L);        private static time.Time neverTimeout = new time.Time();        private static var errNoAvailableInterface = errors.New("no available interface");        private static var errNoAvailableAddress = errors.New("no available address");
            ...
            switch (runtime.GOOS)
            {
                case "aix": 
                    ...
                    { // AIX 7.2, Tech Level XX, Service Pack ZZ, date YYMM
                        var ver = string(out[..4L]);
                        var (tl, _) = strconv.Atoi(string(out[5L..7L]));
                        unStrmDgramEnabled = ver > "7200" || (ver == "7200" && tl >= 2L);
        ...
        {
            {
                ...
                switch (ss[0L])
                {
                    ...
                    case "unixpacket": 
                        // Abstract unix domain sockets, a Linux-ism.
                        if (address[0L] == '@' && runtime.GOOS != "linux")
                        {
                            return false;
                        }
        ...
        public static (ptr<net.Interface>, error) LoopbackInterface()
        {
            ...
            {
                if (ifi.Flags & net.FlagLoopback != 0L && ifi.Flags & net.FlagUp != 0L)
                {
