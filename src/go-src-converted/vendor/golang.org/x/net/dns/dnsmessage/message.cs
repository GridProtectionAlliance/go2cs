...
// Message.Rcode
public static readonly RCode RCodeSuccess = 0;
public static readonly RCode RCodeFormatError = 1;
public static readonly RCode RCodeServerFailure = 2;
public static readonly RCode RCodeNameError = 3;
...
private static @string printByteSlice(slice<byte> b) {
    ...
    var buf = make_slice<byte>(0, 5 * len(b));
    buf = printUint8Bytes(buf, uint8(b[0]));
    foreach (var (_, n) in b[(int)1..]) {
        buf = append(buf, ',', ' ');
...
private static @string printString(slice<byte> str) {
    var buf = make_slice<byte>(0, len(str));
...
private static @string printUint32(uint i) { 
    ...
    var buf = make_slice<byte>(10);
    {
        var b = buf;
        var d = uint32(1000000000);
...
private static readonly nint packStartingCap = 512; 

// uint16Len is the length (in bytes) of a uint16.
private static readonly nint uint16Len = 2; 

// uint32Len is the length (in bytes) of a uint32.
private static readonly nint uint32Len = 4; 
...
private static readonly section sectionNotStarted = iota;
...
private static readonly headerBitQR sectionDone = 1 << 15; // query/response (response=1)
private static readonly nint headerBitAA = 1 << 10; // authoritative
private static readonly nint headerBitTC = 1 << 9; // truncated
private static readonly nint headerBitRD = 1 << 8; // recursion desired
private static readonly nint headerBitRA = 1 << 7; // recursion available
...
private static (nint, error) unpack(this ptr<header> _addr_h, slice<byte> msg, nint off) {
    nint _p0 = default;
    error _p0 = default!;
    ref header h = ref _addr_h.val;
...
public partial interface ResourceBody {
    @string pack(slice<byte> msg, map<@string, nint> compression, nint compressionOff); // realType returns the actual type of the Resource. This is used to
...
private static (slice<byte>, error) pack(this ptr<Resource> _addr_r, slice<byte> msg, map<@string, nint> compression, nint compressionOff) {
...
private static (Header, error) Start(this ptr<Parser> _addr_p, slice<byte> msg) {
    ...
    p.msg = msg;
    error err = default!;
    p.off, err = p.header.unpack(msg, 0);
...
private static (slice<Resource>, error) AllAnswers(this ptr<Parser> _addr_p) {
    ...
    var @as = make_slice<Resource>(0, n);
    while (true) {
        var (a, err) = p.Answer();
        if (err == ErrSectionDone) {
...
// One of the XXXHeader methods must have been called before calling this
// method.
private static (UnknownResource, error) UnknownResource(this ptr<Parser> _addr_p) {
    UnknownResource _p0 = default;
    error _p0 = default!;
    ref Parser p = ref _addr_p.val;

    if (!p.resHeaderValid) {
        return (new UnknownResource(), error.As(ErrNotStarted)!);
    }
    var (r, err) = unpackUnknownResource(p.resHeader.Type, p.msg, p.off, p.resHeader.Length);
    if (err != null) {
...
private static (slice<byte>, error) Pack(this ptr<Message> _addr_m) {
    ...
    return m.AppendPack(make_slice<byte>(0, packStartingCap));
}
...
private static (slice<byte>, error) AppendPack(this ptr<Message> _addr_m, slice<byte> b) {
    ...
    map compression = /* TODO: Fix this in ScannerBase_Expression::ExitCompositeLit */ new map<@string, nint>{};
...
private static @string GoString(this ptr<Message> _addr_m) {
    ...
    if (len(m.Questions) > 0) {
        ...
        foreach (var (_, q) in m.Questions[(int)1..]) {
            s += ", " + q.GoString();
        }
    }
    ...
    if (len(m.Answers) > 0) {
        s += m.Answers[0].GoString();
        {
            ...
            foreach (var (_, __a) in m.Answers[(int)1..]) {
                a = __a;
                s += ", " + a.GoString();
            }
    ...
    if (len(m.Authorities) > 0) {
        s += m.Authorities[0].GoString();
        {
            ...
            foreach (var (_, __a) in m.Authorities[(int)1..]) {
                a = __a;
                s += ", " + a.GoString();
            }
    ...
    if (len(m.Additionals) > 0) {
        s += m.Additionals[0].GoString();
        {
            ...
            foreach (var (_, __a) in m.Additionals[(int)1..]) {
                a = __a;
                s += ", " + a.GoString();
            }
...
// called.
    ...
    public nint start; // compression is a mapping from name suffixes to their starting index
// in msg.
    public map<@string, nint> compression;
}
...
public static Builder NewBuilder(slice<byte> buf, Header h) {
    if (buf == null) {
        buf = make_slice<byte>(0, packStartingCap);
    }
...
private static void EnableCompression(this ptr<Builder> _addr_b) {
    ...
    b.compression = /* TODO: Fix this in ScannerBase_Expression::ExitCompositeLit */ new map<@string, nint>{};
}
...
private static error UnknownResource(this ptr<Builder> _addr_b, ResourceHeader h, UnknownResource r) {
    ref Builder b = ref _addr_b.val;
    ...
    if (err != null) {
        return error.As(addr(new nestedError("UnknownResource body",err))!)!;
    }
    {
        var err__prev1 = err;
...
private static (slice<byte>, nint, error) pack(this ptr<ResourceHeader> _addr_h, slice<byte> oldMsg, map<@string, nint> compression, nint compressionOff) {
    ...
    if (err != null) {
        return (oldMsg, 0, error.As(addr(new nestedError("Name",err))!)!);
    }
...
private static error fixLen(this ptr<ResourceHeader> _addr_h, slice<byte> msg, nint lenOff, nint preLen) {
    ref ResourceHeader h = ref _addr_h.val;
...
private static readonly nint edns0Version = 0;
...
private static error SetEDNS0(this ptr<ResourceHeader> _addr_h, nint udpPayloadLen, RCode extRCode, bool dnssecOK) {
    ref ResourceHeader h = ref _addr_h.val;
...
private static (ushort, nint, error) unpackUint16(slice<byte> msg, nint off) {
    ushort _p0 = default;
    nint _p0 = default;
    error _p0 = default!;
...
private static (Type, nint, error) unpackType(slice<byte> msg, nint off) {
    Type _p0 = default;
    nint _p0 = default;
    error _p0 = default!;
...
private static (nint, error) skipType(slice<byte> msg, nint off) {
    nint _p0 = default;
    error _p0 = default!;
...
private static (Class, nint, error) unpackClass(slice<byte> msg, nint off) {
    Class _p0 = default;
    nint _p0 = default;
    error _p0 = default!;
...
private static (nint, error) skipClass(slice<byte> msg, nint off) {
    nint _p0 = default;
    error _p0 = default!;
...
private static (uint, nint, error) unpackUint32(slice<byte> msg, nint off) {
    uint _p0 = default;
    nint _p0 = default;
    error _p0 = default!;
...
private static (nint, error) unpackBytes(slice<byte> msg, nint off, slice<byte> field) {
    nint _p0 = default;
    error _p0 = default!;
...
public static Name MustNewName(@string name) => func((_, panic, _) => {
...
private static @string GoString(this ptr<Name> _addr_n) {
    ...
    return "dnsmessage.MustNewName(\"" + printString(n.Data[..(int)n.Length]) + "\")";
}
...
private static (slice<byte>, error) pack(this ptr<Name> _addr_n, slice<byte> msg, map<@string, nint> compression, nint compressionOff) {
    slice<byte> _p0 = default;
    error _p0 = default!;
    ref Name n = ref _addr_n.val;
    ...
    nint begin = 0; i < int(n.Length); i++) { 
        ...
        if ((i == 0 || n.Data[i - 1] == '.') && compression != null) {
            {
                var (ptr, ok) = compression[string(n.Data[(int)i..])];
...
private static (nint, error) unpack(this ptr<Name> _addr_n, slice<byte> msg, nint off) {
    nint _p0 = default;
    error _p0 = default!;
    ref Name n = ref _addr_n.val;
...
private static (nint, error) unpackCompressed(this ptr<Name> _addr_n, slice<byte> msg, nint off, bool allowCompression) {
    nint _p0 = default;
    error _p0 = default!;
    ref Name n = ref _addr_n.val;
...
private static (slice<byte>, error) pack(this ptr<Question> _addr_q, slice<byte> msg, map<@string, nint> compression, nint compressionOff) {
    slice<byte> _p0 = default;
    error _p0 = default!;
...
private static (ResourceBody, nint, error) unpackResourceBody(slice<byte> msg, nint off, ResourceHeader hdr) {
...
private static (slice<byte>, error) pack(this ptr<CNAMEResource> _addr_r, slice<byte> msg, map<@string, nint> compression, nint compressionOff) {
    slice<byte> _p0 = default;
    error _p0 = default!;
    ref CNAMEResource r = ref _addr_r.val;
...
private static (CNAMEResource, error) unpackCNAMEResource(slice<byte> msg, nint off) {
    CNAMEResource _p0 = default;
    error _p0 = default!;
...
private static (slice<byte>, error) pack(this ptr<MXResource> _addr_r, slice<byte> msg, map<@string, nint> compression, nint compressionOff) {
    slice<byte> _p0 = default;
    error _p0 = default!;
    ref MXResource r = ref _addr_r.val;
...
private static (MXResource, error) unpackMXResource(slice<byte> msg, nint off) {
    MXResource _p0 = default;
    error _p0 = default!;
...
private static (slice<byte>, error) pack(this ptr<NSResource> _addr_r, slice<byte> msg, map<@string, nint> compression, nint compressionOff) {
    slice<byte> _p0 = default;
    error _p0 = default!;
    ref NSResource r = ref _addr_r.val;
...
private static (NSResource, error) unpackNSResource(slice<byte> msg, nint off) {
    NSResource _p0 = default;
    error _p0 = default!;
...
private static (slice<byte>, error) pack(this ptr<PTRResource> _addr_r, slice<byte> msg, map<@string, nint> compression, nint compressionOff) {
    slice<byte> _p0 = default;
    error _p0 = default!;
    ref PTRResource r = ref _addr_r.val;
...
private static (PTRResource, error) unpackPTRResource(slice<byte> msg, nint off) {
    PTRResource _p0 = default;
    error _p0 = default!;
...
private static (slice<byte>, error) pack(this ptr<SOAResource> _addr_r, slice<byte> msg, map<@string, nint> compression, nint compressionOff) {
    slice<byte> _p0 = default;
    error _p0 = default!;
    ref SOAResource r = ref _addr_r.val;
...
private static (SOAResource, error) unpackSOAResource(slice<byte> msg, nint off) {
    SOAResource _p0 = default;
    error _p0 = default!;
...
private static (slice<byte>, error) pack(this ptr<TXTResource> _addr_r, slice<byte> msg, map<@string, nint> compression, nint compressionOff) {
    slice<byte> _p0 = default;
    error _p0 = default!;
    ref TXTResource r = ref _addr_r.val;
...
private static @string GoString(this ptr<TXTResource> _addr_r) {
    ...
    s += "\"" + printString((slice<byte>)r.TXT[0]);
...
private static (TXTResource, error) unpackTXTResource(slice<byte> msg, nint off, ushort length) {
    ...
    var txts = make_slice<@string>(0, 1);
    {
        var n = uint16(0);
...
private static (slice<byte>, error) pack(this ptr<SRVResource> _addr_r, slice<byte> msg, map<@string, nint> compression, nint compressionOff) {
    slice<byte> _p0 = default;
    error _p0 = default!;
    ref SRVResource r = ref _addr_r.val;
...
private static (SRVResource, error) unpackSRVResource(slice<byte> msg, nint off) {
    SRVResource _p0 = default;
    error _p0 = default!;
...
private static (slice<byte>, error) pack(this ptr<AResource> _addr_r, slice<byte> msg, map<@string, nint> compression, nint compressionOff) {
    slice<byte> _p0 = default;
    error _p0 = default!;
    ref AResource r = ref _addr_r.val;
...
private static (AResource, error) unpackAResource(slice<byte> msg, nint off) {
    AResource _p0 = default;
    error _p0 = default!;
...
private static (slice<byte>, error) pack(this ptr<AAAAResource> _addr_r, slice<byte> msg, map<@string, nint> compression, nint compressionOff) {
    slice<byte> _p0 = default;
    error _p0 = default!;
    ref AAAAResource r = ref _addr_r.val;
...
private static (AAAAResource, error) unpackAAAAResource(slice<byte> msg, nint off) {
    AAAAResource _p0 = default;
    error _p0 = default!;

    array<byte> aaaa = new array<byte>(16);
    {
...
private static (slice<byte>, error) pack(this ptr<OPTResource> _addr_r, slice<byte> msg, map<@string, nint> compression, nint compressionOff) {
    slice<byte> _p0 = default;
    error _p0 = default!;
    ref OPTResource r = ref _addr_r.val;
...
private static (OPTResource, error) unpackOPTResource(slice<byte> msg, nint off, ushort length) {
    ...
    slice<Option> opts = default;
    {
        ...
        while (off < oldOff + int(length)) {
            ...
            if (copy(o.Data, msg[(int)off..]) != int(l)) {
...
private static Type realType(this ptr<UnknownResource> _addr_r) {
    ref UnknownResource r = ref _addr_r.val;

    return r.Type;
}

// pack appends the wire format of the UnknownResource to msg.
private static (slice<byte>, error) pack(this ptr<UnknownResource> _addr_r, slice<byte> msg, map<@string, nint> compression, nint compressionOff) {
    slice<byte> _p0 = default;
    error _p0 = default!;
    ref UnknownResource r = ref _addr_r.val;

    return (packBytes(msg, r.Data[..]), error.As(null!)!);
}

// GoString implements fmt.GoStringer.GoString.
private static @string GoString(this ptr<UnknownResource> _addr_r) {
    ref UnknownResource r = ref _addr_r.val;

    return "dnsmessage.UnknownResource{" + "Type: " + r.Type.GoString() + ", " + "Data: []byte{" + printByteSlice(r.Data) + "}}";
}

private static (UnknownResource, error) unpackUnknownResource(Type recordType, slice<byte> msg, nint off, ushort length) {
    UnknownResource _p0 = default;
    error _p0 = default!;

    UnknownResource parsed = new UnknownResource(Type:recordType,Data:make([]byte,length),);
    {
        var (_, err) = unpackBytes(msg, off, parsed.Data);
