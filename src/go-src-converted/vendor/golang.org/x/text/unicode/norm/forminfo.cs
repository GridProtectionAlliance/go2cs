...
namespace unicode
{
    public static partial class norm_package
    {
        ...
        private static readonly ulong qcInfoMask = (ulong)0x3FUL; // to clear all but the relevant bits in a qcInfo
        private static readonly ulong headerLenMask = (ulong)0x3FUL; // extract the length value from the header byte
        private static readonly ulong headerFlagsMask = (ulong)0xC0UL; // extract the qcInfo bits from the header byte
        ...
        public delegate  Properties lookupFunc(input,  long);
        ...
        {
            if (p.ccc == 0L && !p.combinesBackward())
            {
                return true;
            } 
        ...
        {
            return p.flags & qcInfoMask == 0L && p.ccc == 0L;
        }
        ...
        public static long Size(this Properties p)
        {
            return int(p.size);
        }
        ...
        {
            recompMap = make_map<uint, int>(len(recompMapPacked) / 8L);
            array<byte> buf = new array<byte>(8L);
            {
                ...
                {
                    copy(buf[..], recompMapPacked[i..i + 8L]);
                    var key = binary.BigEndian.Uint32(buf[..4L]);
                    var val = binary.BigEndian.Uint32(buf[4L..]);
        ...
        private static int combine(int a, int b) => func((_, panic, __) =>
        {
            var key = uint32(uint16(a)) << (int)(16L) + uint32(uint16(b));
            if (recompMap == null)
            {
        ...
        private static Properties lookupInfoNFC(input b, long i)
        {
            var (v, sz) = b.charinfoNFC(i);
            return compInfo(v, sz);
        }

        private static Properties lookupInfoNFKC(input b, long i)
        {
        ...
        private static Properties compInfo(ushort v, long sz)
        {
            if (v == 0L)
            {
            ...
            // has decomposition
            var h = decomps[v];
            var f = (qcInfo(h & headerFlagsMask) >> (int)(2L)) | 0x4UL;
            p = new Properties(size:uint8(sz),flags:f,index:v);
            if (v >= firstCCC)
            {
                v += uint16(h & headerLenMask) + 1L;
                var c = decomps[v];
                p.tccc = c >> (int)(2L);
                p.flags |= qcInfo(c & 0x3UL);
