...
namespace unicode
{
    public static partial class bidi_package
    {
        ...
        private static long Len(this bracketPairs b)
        {
            return len(b);
        }
        private static void Swap(this bracketPairs b, long i, long j)
        {
            b[i] = b[j];
            b[j] = b[i];
        }
        private static bool Less(this bracketPairs b, long i, long j)
        {
...
// bracket pair positions sorted by location of opening bracket
            ...
            public slice<long> indexes; // array of index values into the original string

        }

        // matchOpener reports whether characters at given positions form a matching
        // bracket pair.
        private static bool matchOpener(this ptr<bracketPairer> _addr_p, slice<int> pairValues, long opener, long closer)
        {
        ...
        private static readonly long maxPairingDepth = (long)63L;
        ...
        private static Class getStrongTypeN0(this ptr<bracketPairer> _addr_p, long index)
        {
            ref bracketPairer p = ref _addr_p.val;
        ...
        private static Class classifyPairContent(this ptr<bracketPairer> _addr_p, bracketPair loc, Class dirEmbed)
        {
            ...
            var dirOpposite = ON;
            for (var i = loc.opener + 1L; i < loc.closer; i++)
            {
        ...
        private static Class classBeforePair(this ptr<bracketPairer> _addr_p, bracketPair loc)
        {
            ...
            for (var i = loc.opener - 1L; i >= 0L; i--)
            {
                {
                    var dir = p.getStrongTypeN0(i);
        ...
        private static void setBracketsToType(this ptr<bracketPairer> _addr_p, bracketPair loc, Class dirPair, slice<Class> initialTypes)
        {
...

            {
                ...
                for (i = loc.closer + 1L; i < len(p.indexes); i++)
                {
                    index = p.indexes[i];
                    if (initialTypes[index] != NSM)
                    {
