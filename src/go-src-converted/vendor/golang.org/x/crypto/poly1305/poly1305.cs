...
public static bool Verify(ptr<array<byte>> _addr_mac, slice<byte> m, ptr<array<byte>> _addr_key) {
    ...
    ref array<byte> tmp = ref heap(new array<byte>(16), out ptr<array<byte>> _addr_tmp);
...
private static nint Size(this ptr<MAC> _addr_h) {
...
private static (nint, error) Write(this ptr<MAC> _addr_h, slice<byte> p) => func((_, panic, _) => {
    nint n = default;
    error err = default!;
    ref MAC h = ref _addr_h.val;
