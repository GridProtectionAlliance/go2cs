...
// package nilness -- go2cs converted at 2022 March 06 23:34:04 UTC
...
public static readonly @string Doc = @"check for redundant or impossible nil comparisons
...
private static void runFunc(ptr<analysis.Pass> _addr_pass, ptr<ssa.Function> _addr_fn) {
 ...
 { 
    ...
    if (fn.Blocks != null) {
        visit(fn.Blocks[0], make_slice<fact>(0, 20)); // 20 is plenty
    }
}
...
private partial struct nilness { // : nint
}

private static readonly nint isnonnil = -1;
private static readonly nilness unknown = 0;
private static readonly nint isnil = 1;
...
private static (ptr<ssa.BinOp>, ptr<ssa.BasicBlock>, ptr<ssa.BasicBlock>) eq(ptr<ssa.BasicBlock> _addr_b) {
    ...
    {
        ptr<ssa.If> (If, ok) = b.Instrs[len(b.Instrs) - 1]._<ptr<ssa.If>>();
        ...
        if (ok) {
            {
                ...
                if (ok) {
                    ...
                    if (binop.Op == token.EQL) 
                        return (_addr_binop!, _addr_b.Succs[0]!, _addr_b.Succs[1]!);
                    else if (binop.Op == token.NEQ) 
                        return (_addr_binop!, _addr_b.Succs[1]!, _addr_b.Succs[0]!);
