...
// package ssa -- go2cs converted at 2022 March 06 23:33:13 UTC
...
private static nint Len(this byDomPreorder a) {
    return len(a);
}
private static void Swap(this byDomPreorder a, nint i, nint j) {
    (a[i], a[j]) = (a[j], a[i]);
}
private static bool Less(this byDomPreorder a, nint i, nint j) {
...
private static void buildDomTree(ptr<Function> _addr_f) {
    ...
    var space = make_slice<ptr<BasicBlock>>(5 * n);
    ltState lt = new ltState(sdom:space[0:n],parent:space[n:2*n],ancestor:space[2*n:3*n],); 

    // Step 1.  Number vertices by depth-first preorder.
    var preorder = space[(int)3 * n..(int)4 * n];
    var root = f.Blocks[0];
    var prenum = lt.dfs(root, 0, preorder);
    var recover = f.Recover;
    if (recover != null) {
        lt.dfs(recover, prenum, preorder);
    }
    var buckets = space[(int)4 * n..(int)5 * n];
    copy(buckets, preorder); 

    // In reverse preorder...
    for (var i = int32(n) - 1; i > 0; i--) {
        var w = preorder[i]; 
        ...
        else
 {
    ...
    // node, in preorder.
    {
        ...
        foreach (var (_, __w) in preorder[(int)1..]) {
            w = __w;
            if (w == root || w == recover) {
                w.dom.idom = null;
            }
            else
 {
    ...
    var (pre, post) = numberDomTree(_addr_root, 0, 0);
    if (recover != null) {
...
//
private static void sanityCheckDomTree(ptr<Function> _addr_f) => func((_, panic, _) => {
    ref Function f = ref _addr_f.val;
        ...
        while (changed) {
            changed = false;
            {
                ...
                foreach (var (__i, __b) in f.Blocks) {
                    ...
                    x.SetBit(_addr_x, i, 1); // a block always dominates itself.
                    if (D[i].Cmp(_addr_x) != 0) {
                        D[i].Set(_addr_x);
                        changed = true;
                    }
...
private static void printDomTreeText(ptr<bytes.Buffer> _addr_buf, ptr<BasicBlock> _addr_v, nint indent) {
    ref bytes.Buffer buf = ref _addr_buf.val;
    ref BasicBlock v = ref _addr_v.val;

    fmt.Fprintf(buf, "%*s%s\n", 4 * indent, "", v);
    foreach (var (_, child) in v.dom.children) {
        printDomTreeText(_addr_buf, _addr_child, indent + 1);
    }
}
