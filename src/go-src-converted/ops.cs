...
// package interp -- go2cs converted at 2022 March 06 23:33:36 UTC
...
private partial struct exitPanic { // : nint
}

// constValue returns the value of the constant with the
// dynamic type tag appropriate for c.Type().
private static value constValue(ptr<ssa.Const> _addr_c) => func((_, panic, _) => {
...
private static nint asInt(value x) => func((_, panic, _) => {
    switch (x.type()) {
        case nint x:
            return x;
            break;
...
private static ulong asUint64(value x) => func((_, panic, _) => {
    switch (x.type()) {
        case nuint x:
            return uint64(x);
            break;
...
private static value zero(types.Type t) => func((_, panic, _) => {
    switch (t.type()) {
...
private static value slice(value x, value lo, value hi, value max) => func((_, panic, _) => {
    nint Len = default;    nint Cap = default;
...
private static value lookup(ptr<ssa.Lookup> _addr_instr, value x, value idx) => func((_, panic, _) => {
...
// dynamic type.
//
private static value binop(token.Token op, types.Type t, value x, value y) => func((_, panic, _) => {

    if (op == token.ADD) 
        switch (x.type()) {
            case nint _:
                return x._<nint>() + y._<nint>();
                break;
            case sbyte _:
...
// of that type.
//
private static bool eqnil(types.Type t, value x, value y) => func((_, panic, _) => {
...
private static value unop(ptr<ssa.UnOp> _addr_instr, value x) => func((_, panic, _) => {
...
//
private static value typeAssert(ptr<interpreter> _addr_i, ptr<ssa.TypeAssert> _addr_instr, iface itf) => func((_, panic, _) => {
...
private static (nint, error) print(slice<byte> b) {
    nint _p0 = default;
    error _p0 = default!;
...
private static value callBuiltin(ptr<frame> _addr_caller, token.Pos callpos, ptr<ssa.Builtin> _addr_fn, slice<value> args) => func((_, panic, _) => {
    ...
    switch (fn.Name()) {
        case "append": 
            ...
            {
                @string (s, ok) = args[1]._<@string>();

                if (ok) { 
                    // append([]byte, ...string) []byte
                    slice<value> arg0 = args[0]._<slice<value>>();
                    {
                        nint i__prev1 = i;
            ...
            // append([]T, ...[]T) []T
            return append(args[0]._<slice<value>>(), args[1]._<slice<value>>());
        ...
        case "copy": // copy([]T, []T) int or copy([]byte, string) int
            ...
            return copy(args[0]._<slice<value>>(), src._<slice<value>>());

            break;
        case "close": // close(chan T)
            close(args[0]._<channel<value>>());
            return null;
            break;
        ...
        case "complex": 
            switch (args[0].type()) {
                case float f:
                    return complex(f, args[1]._<float>());
                    break;
                case double f:
                    return complex(f, args[1]._<double>());
                    break;
                default:
                {
                    var f = args[0].type();
...
private static iter rangeIter(value x, types.Type t) => func((_, panic, _) => {
    switch (x.type()) {
        case map<value, value> x:
...
//
private static value widen(value x) => func((_, panic, _) => {
    switch (x.type()) {
        case bool y:
            return x;
            break;
...
//
private static value conv(types.Type t_dst, types.Type t_src, value x) => func((_, panic, _) => {
