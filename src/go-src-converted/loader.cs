...
// Created ones, and all imported dependencies.
        ...
        public static readonly @string FromArgsUsage = (@string)@"
...
";
        ...
        private static slice<ptr<PackageInfo>> InitialPackages(this ptr<Program> _addr_prog)
        {
            ...
            var infos = make_slice<ptr<PackageInfo>>(0L, len(prog.Created) + len(prog.Imported));
...
// packages.  Nodes are identified by their import paths.
        ...
        private static void Complete(this ptr<importInfo> _addr_ii, ptr<PackageInfo> _addr_info) => func((_, panic, __) =>
        {
            ref importInfo ii = ref _addr_ii.val;
            ref PackageInfo info = ref _addr_info.val;
        ...
        //
        private static (ptr<Program>, error) Load(this ptr<Config> _addr_conf) => func((_, panic, __) =>
        {
...
;
            ...
            {
                conf.FindPackage = ptr<build.Context>;
            }
...
; 
            ...
            foreach (var (_, cp) in conf.CreatePkgs)
            {
                ...
                var dir = conf.Cwd;
                if (len(files) > 0L && files[0L].Pos().IsValid())
                {
                    dir = filepath.Dir(conf.fset().File(files[0L].Pos()).Name());
                }
            ...
            if (len(prog.Imported) + len(prog.Created) == 0L)
            {
            ...
            { 
                ...
                if (errpkgs != null)
                {
                    ...
                    if (len(errpkgs) > 3L)
                    {
                        more = fmt.Sprintf(" and %d more", len(errpkgs) - 3L);
                        errpkgs = errpkgs[..3L];
                    }
        ...
        private static long Len(this byImportPath b)
        {
            return len(b);
        }
        private static bool Less(this byImportPath b, long i, long j)
        {
            return b[i].ImportPath < b[j].ImportPath;
        }
        private static void Swap(this byImportPath b, long i, long j)
        {
            b[i] = b[j];
            b[j] = b[i];
        }
...
;
        ...
        //
        private static (slice<ptr<ast.File>>, slice<error>) parsePackageFiles(this ptr<Config> _addr_conf, ptr<build.Package> _addr_bp, int which) => func((_, panic, __) =>
        {
        ...
        // Idempotent.
        //
        private static (ptr<types.Package>, error) doImport(this ptr<importer> _addr_imp, ptr<PackageInfo> _addr_from, @string to) => func((_, panic, __) =>
        {
            ...
            var (bp, err) = imp.findPackage(to, from.dir, 0L);
            if (err != null)
            {
                return (_addr_null!, error.As(err)!);
            } 
            ...
            var fromPath = from.Pkg.Path();
            {
                ...
                { 
                    ...
                    cycle = append(cycle, cycle[0L]); // add start node to end to show cycliness
        ...
        private static slice<@string> findPath(this ptr<importer> _addr_imp, @string from, @string to) => func((defer, _, __) =>
        {
            ref importer imp = ref _addr_imp.val;
...
;
            return search(make_slice<@string>(0L, 20L), from);
        ...
        //
        private static void addFiles(this ptr<importer> _addr_imp, ptr<PackageInfo> _addr_info, slice<ptr<ast.File>> files, bool cycleCheck) => func((_, panic, __) =>
        {
            ref importer imp = ref _addr_imp.val;
            ref PackageInfo info = ref _addr_info.val;
            ...
            imp.importAll(fromPath, info.dir, scanImports(files), 0L);
