...
// package sortslice -- go2cs converted at 2020 October 09 06:04:14 UTC
...
namespace passes
{
    ...
    {
        public static readonly @string Doc = (@string)"check the argument type of sort.Slice\n\nsort.Slice requires an argument of a slice" +
            ...
            inspect.Preorder(nodeFilter, n =>
            {
                ...
                switch (typ.Underlying().type())
                {
                    ...
                    case ptr<types.Signature> v:
                        if (v.Params().Len() != 0L || v.Results().Len() != 1L)
                        {
                            break;
                        }

                        {
                            (_, ok) = v.Results().At(0L).Type().Underlying()._<ptr<types.Slice>>();
