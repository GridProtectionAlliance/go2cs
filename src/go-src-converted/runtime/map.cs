...
namespace go
{
    public static partial class runtime_package
    {
        ...
        private static readonly long bucketCntBits = (long)3L;
        private static readonly long bucketCnt = (long)1L << (int)(bucketCntBits); 

        // Maximum average load of a bucket that triggers growth is 6.5.
        // Represent as loadFactorNum/loadFactorDen, to allow integer math.
        private static readonly long loadFactorNum = (long)13L;
        private static readonly long loadFactorDen = (long)2L; 

        // Maximum key or elem size to keep inline (instead of mallocing per element).
        // Must fit in a uint8.
        // Fast versions cannot handle big elems - the cutoff size for
        // fast versions in cmd/compile/internal/gc/walk.go must be at most this elem.
        private static readonly long maxKeySize = (long)128L;
        private static readonly long maxElemSize = (long)128L; 
        ...
        private static readonly long emptyRest = (long)0L; // this cell is empty, and there are no more non-empty cells at higher indexes or overflows.
        private static readonly long emptyOne = (long)1L; // this cell is empty
        private static readonly long evacuatedX = (long)2L; // key/elem is valid.  Entry has been evacuated to first half of larger table.
        private static readonly long evacuatedY = (long)3L; // same as above, but evacuated to second half of larger table.
        private static readonly long evacuatedEmpty = (long)4L; // cell is empty, bucket is evacuated.
        private static readonly long minTopHash = (long)5L; // minimum tophash for a normal filled cell.

        // flags
        private static readonly long iterator = (long)1L; // there may be an iterator using buckets
        private static readonly long oldIterator = (long)2L; // there may be an iterator using oldbuckets
        private static readonly long hashWriting = (long)4L; // a goroutine is writing to the map
        private static readonly long sameSizeGrow = (long)8L; // the current map growth is to a new map of the same size

        // sentinel bucket ID for iterator checks
        private static readonly long noCheck = (long)1L << (int)((8L * sys.PtrSize)) - 1L;
...
// Followed by an overflow pointer.
        ...
        private partial struct hiter
        {
            ...
            public unsafe.Pointer elem; // Must be in second position (see cmd/internal/gc/range.go).
        ...
        { 
            ...
            return uintptr(1L) << (int)((b & (sys.PtrSize * 8L - 1L)));
        ...
        private static ptr<hmap> makemap(ptr<maptype> _addr_t, long hint, ptr<hmap> _addr_h)
        {
            ref maptype t = ref _addr_t.val;
            ref hmap h = ref _addr_h.val;
        ...
        private static (unsafe.Pointer, ptr<bmap>) makeBucketArray(ptr<maptype> _addr_t, byte b, unsafe.Pointer dirtyalloc)
        {
            ...
            { 
                ...
                var last = (bmap.val)(add(buckets, (nbuckets - 1L) * uintptr(t.bucketsize)));
        ...
        private static unsafe.Pointer mapaccess1(ptr<maptype> _addr_t, ptr<hmap> _addr_h, unsafe.Pointer key)
        {
            ...
            if (h == null || h.count == 0L)
            {
                if (t.hashMightPanic())
                {
                    t.hasher(key, 0L); // see issue 23734
                }
...
bucketloop:
        ...
        private static (unsafe.Pointer, bool) mapaccess2(ptr<maptype> _addr_t, ptr<hmap> _addr_h, unsafe.Pointer key)
        {
            ...
            var hash = t.hasher(key, uintptr(h.hash0));
            var m = bucketMask(h.B);
            var b = (bmap.val)(@unsafe.Pointer(uintptr(h.buckets) + (hash & m) * uintptr(t.bucketsize)));
            {
                var c = h.oldbuckets;
                ...
                if (c != null)
                {
                    ...
                    var oldb = (bmap.val)(@unsafe.Pointer(uintptr(c) + (hash & m) * uintptr(t.bucketsize)));
                    if (!evacuated(_addr_oldb))
                    {
                        b = oldb;
                    }
...
bucketloop:
        ...
        private static unsafe.Pointer mapaccess1_fat(ptr<maptype> _addr_t, ptr<hmap> _addr_h, unsafe.Pointer key, unsafe.Pointer zero)
        {
            ...
            if (e == @unsafe.Pointer(_addr_zeroVal[0L]))
            {
                return zero;
            }
        ...
        private static unsafe.Pointer mapassign(ptr<maptype> _addr_t, ptr<hmap> _addr_h, unsafe.Pointer key) => func((_, panic, __) =>
        {
            ref maptype t = ref _addr_t.val;
            ref hmap h = ref _addr_h.val;
...
next:
        ...
        private partial struct evacDst
        {
            ...
            public long i; // key/elem index into b
                ...
                while (b != null)
                {
                    ...
                    b = b.overflow(t);
                            ...
                            { 
                                ...
                                if (h.flags & iterator != 0L && !t.reflexivekey() && !t.key.equal(k2, k2))
                                { 
                            ...
                            if (evacuatedX + 1L != evacuatedY || evacuatedX ^ 1L != evacuatedY)
                            {
        ...
        private static ptr<hmap> reflect_makemap(ptr<maptype> _addr_t, long cap)
        {
            ref maptype t = ref _addr_t.val;
        ...
        private static long reflect_maplen(ptr<hmap> _addr_h)
        {
            ref hmap h = ref _addr_h.val;
        ...
        private static long reflectlite_maplen(ptr<hmap> _addr_h)
        {
            ref hmap h = ref _addr_h.val;
        ...
        private static readonly long maxZero = (long)1024L; // must match value in cmd/compile/internal/gc/walk.go:zeroValSize
