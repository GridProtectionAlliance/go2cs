...
// Original source: C:\Program Files\Go\src\runtime\sigqueue_plan9.go
...
private static readonly nint qsize = 64;
...
private partial struct noteData {
    public array<byte> s;
    public nint n; // n bytes of s are valid
}
...
private static @string pop(this ptr<noteQueue> _addr_q) {
    ...
    var item = string(note.s[..(int)note.n]);
    q.ri++;
    if (q.ri == qsize) {
        q.ri = 0;
    }
