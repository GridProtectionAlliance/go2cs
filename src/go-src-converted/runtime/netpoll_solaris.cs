...
private static int netpollopen(System.UIntPtr fd, ptr<pollDesc> _addr_pd) {
    ...
    pd.user = 0;
    var r = port_associate(portfd, _PORT_SOURCE_FD, fd, 0, uintptr(@unsafe.Pointer(pd)));
...
private static void netpollarm(ptr<pollDesc> _addr_pd, nint mode) {
    ...
    switch (mode) {
        case 'r': 
            netpollupdate(_addr_pd, _POLLIN, 0);
            break;
        case 'w': 
            netpollupdate(_addr_pd, _POLLOUT, 0);
            break;
        default: 
...
private static void netpollBreak() {
    if (atomic.Cas(_addr_netpollWakeSig, 0, 1)) { 
        ...
        if (port_alert(portfd, _PORT_ALERT_UPDATE, _POLLHUP, uintptr(@unsafe.Pointer(_addr_portfd))) < 0) {
            {
                var e = errno();
...
private static gList netpoll(long delay) {
    ...
    else
 {
    ...
    array<portevent> events = new array<portevent>(128);
retry:
    ref uint n = ref heap(1, out ptr<uint> _addr_n);
    var r = port_getn(portfd, _addr_events[0], uint32(len(events)), _addr_n, wait);
    var e = errno();
    if (r < 0 && e == _ETIME && n > 0) { 
    ...
    for (nint i = 0; i < int(n); i++) {
        ...
        if (clear != 0) {
            lock(_addr_pd.@lock);
            netpollupdate(_addr_pd, 0, uint32(clear));
            unlock(_addr_pd.@lock);
        }
        if (mode != 0) { 
