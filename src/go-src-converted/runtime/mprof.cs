...
namespace go
{
    public static partial class runtime_package
    {
        ...
        private static readonly long maxStack = (long)32L;
...
// active.
        ...
        // newBucket allocates a bucket with the given type and number of stack entries.
        private static ptr<bucket> newBucket(bucketType typ, long nstk)
        {
            ...
            if (typ == memProfile) 
                ...
                throw("invalid profile bucket type");
                        var b = (bucket.val)(persistentalloc(size, 0L, _addr_memstats.buckhash_sys));
            bucketmem += size;
            b.typ = typ;
            b.nstk = uintptr(nstk);
            return _addr_b!;
        ...
        private static ptr<memRecord> mp(this ptr<bucket> _addr_b)
        {
            ...
            var data = add(@unsafe.Pointer(b), @unsafe.Sizeof(b.val) + b.nstk * @unsafe.Sizeof(uintptr(0L)));
        ...
        private static void mProf_PostSweep()
        {
            ...
            var c = mProf.cycle;
            {
                ...
                {
                    var mp = b.mp();
                    var mpc = _addr_mp.future[(c + 1L) % uint32(len(mp.future))];
                    mp.active.add(mpc);
                    mpc.val = new memRecordCycle();
                    b = b.allnext;
                }
        ...
        private static void mProf_Malloc(unsafe.Pointer p, System.UIntPtr size)
        {
            ...
            var c = mProf.cycle;
            var mp = b.mp();
            var mpc = _addr_mp.future[(c + 2L) % uint32(len(mp.future))];
            mpc.allocs++;
        ...
        private static void record(ptr<MemProfileRecord> _addr_r, ptr<bucket> _addr_b)
        {
            ...
            if (raceenabled)
            {
                racewriterangepc(@unsafe.Pointer(_addr_r.Stack0[0L]), @unsafe.Sizeof(r.Stack0), getcallerpc(), funcPC(MemProfile));
            }

            if (msanenabled)
            {
                msanwrite(@unsafe.Pointer(_addr_r.Stack0[0L]), @unsafe.Sizeof(r.Stack0));
            }
        ...
        private static void iterate_memprof(Action<ptr<bucket>, System.UIntPtr, ptr<System.UIntPtr>, System.UIntPtr, System.UIntPtr, System.UIntPtr> fn)
        {
            lock(_addr_proflock);
            {
                ...
                {
                    var mp = b.mp();
                    fn(b, b.nstk, _addr_b.stk()[0L], b.size, mp.active.allocs, mp.active.frees);
                    b = b.allnext;
                }
            ...
            if (n <= len(p))
            {
                ok = true;
                {
                    ...
                    while (b != null)
                    {
                        ...
                        if (raceenabled)
                        {
                            racewriterangepc(@unsafe.Pointer(_addr_r.Stack0[0L]), @unsafe.Sizeof(r.Stack0), getcallerpc(), funcPC(BlockProfile));
                        b = b.allnext;
                        }
        ...
        private static (long, bool) runtime_goroutineProfileWithLabels(slice<StackRecord> p, slice<unsafe.Pointer> labels)
        {
            long n = default;
            bool ok = default;
        ...
        private static (long, bool) goroutineProfileWithLabels(slice<StackRecord> p, slice<unsafe.Pointer> labels)
        {
            long n = default;
            bool ok = default;
...
;
            ...
            if (n <= len(p))
            {
                ...
                systemstack(() =>
                {
                    saveg(pc, sp, _addr_gp, _addr_r[0L]);
                });
                r = r[1L..]; 
                ...
                // Save other goroutines.
                {
                    ...
                    foreach (var (_, __gp1) in allgs)
                    {
                        ...
                        if (isOK(gp1))
                        {
                            ...
                            saveg(~uintptr(0L), ~uintptr(0L), _addr_gp1, _addr_r[0L]);
                            if (labels != null)
                            {
                                lbl[0L] = gp1.labels;
                                lbl = lbl[1L..];
        ...
        private static void saveg(System.UIntPtr pc, System.UIntPtr sp, ptr<g> _addr_gp, ptr<StackRecord> _addr_r)
        {
            ...
            var n = gentraceback(pc, sp, 0L, gp, 0L, _addr_r.Stack0[0L], len(r.Stack0), null, null, 0L);
            if (n < len(r.Stack0))
            {
                r.Stack0[n] = 0L;
            }
