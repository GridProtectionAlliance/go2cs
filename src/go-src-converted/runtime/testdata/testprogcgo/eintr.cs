...
// package main -- go2cs converted at 2022 March 06 22:26:13 UTC
...
private static (double, slice<byte>) spin() {
    ...
    float r1 = 0.0F;
    var r2 = make_slice<byte>(200);
    while (time.Now().Before(stop)) {
        for (nint i = 1; i < 1e6F; i++) {
            r1 += r1 / float64(i);
            r2 = append(r2, bytes.Repeat(new slice<byte>(new byte[] { byte(i) }), 100));
            r2 = r2[(int)100..];
        }
    }
    return (r1, r2);
...
private static void winch() => func((defer, _, _) => {
    var ticker = time.NewTicker(100 * time.Microsecond);
    defer(ticker.Stop());
...
private static void testPipe(ptr<sync.WaitGroup> _addr_wg) => func((defer, _, _) => {
    ...
    go_(() => () => {
        ...
        // pipe buffer and need multiple write system calls.
        {
            ...
            var (_, err) = w.Write(bytes.Repeat(new slice<byte>(new byte[] { 0 }), 2 << 20));
...
private static void testNet(ptr<sync.WaitGroup> _addr_wg) => func((defer, _, _) => {
    ref sync.WaitGroup wg = ref _addr_wg.val;
    ...
    go_(() => () => {
        ...
        sendSomeSignals();
        {
            ...
            var (_, err) = cf.Write(bytes.Repeat(new slice<byte>(new byte[] { 0 }), 2 << 20));
...
private static void testExec(ptr<sync.WaitGroup> _addr_wg) => func((defer, _, _) => {
    ref sync.WaitGroup wg = ref _addr_wg.val;

    wg.Add(1);
    go_(() => () => {
        defer(wg.Done());
        var cmd = exec.Command(os.Args[0], "Block");
