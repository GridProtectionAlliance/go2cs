...
// package runtime -- go2cs converted at 2022 March 06 22:09:09 UTC
// import "runtime" ==> using runtime = go.runtime_package
// Original source: C:\Program Files\Go\src\runtime\mbarrier.go
...
private static void reflect_typedmemmovepartial(ptr<_type> _addr_typ, unsafe.Pointer dst, unsafe.Pointer src, System.UIntPtr off, System.UIntPtr size) => func((_, panic, _) => {
    ref _type typ = ref _addr_typ.val;
...
//
//go:nosplit
private static void reflectcallmove(ptr<_type> _addr_typ, unsafe.Pointer dst, unsafe.Pointer src, System.UIntPtr size, ptr<abi.RegArgs> _addr_regs) {
    ref _type typ = ref _addr_typ.val;
    ref abi.RegArgs regs = ref _addr_regs.val;

    if (writeBarrier.needed && typ != null && typ.ptrdata != 0 && size >= sys.PtrSize) {
        bulkBarrierPreWrite(uintptr(dst), uintptr(src), size);
    }
    memmove(dst, src, size); 

    // Move pointers returned in registers to a place where the GC can see them.
    foreach (var (i) in regs.Ints) {
...
//go:nosplit
private static nint typedslicecopy(ptr<_type> _addr_typ, unsafe.Pointer dstPtr, nint dstLen, unsafe.Pointer srcPtr, nint srcLen) {
    ref _type typ = ref _addr_typ.val;
...
private static nint reflect_typedslicecopy(ptr<_type> _addr_elemType, slice dst, slice src) {
