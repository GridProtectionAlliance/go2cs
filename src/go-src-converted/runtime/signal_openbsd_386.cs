...
// Original source: C:\Program Files\Go\src\runtime\signal_openbsd_386.go
...
private static uint sigaddr(this ptr<sigctxt> _addr_c) {
    ...
    return new ptr<ptr<ptr<uint>>>(add(@unsafe.Pointer(c.info), 12));
}
...
private static void set_sigaddr(this ptr<sigctxt> _addr_c, uint x) {
    ...
    (uint32.val)(add(@unsafe.Pointer(c.info), 12)).val;
