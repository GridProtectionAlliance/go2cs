...
private static readonly nint semTabSize = 251;
...
private static void sync_runtime_Semacquire(ptr<uint> _addr_addr) {
    ...
    semacquire1(_addr_addr, false, semaBlockProfile, 0);
}
...
private static void sync_runtime_Semrelease(ptr<uint> _addr_addr, bool handoff, nint skipframes) {
    ref uint addr = ref _addr_addr.val;
...
private static void sync_runtime_SemacquireMutex(ptr<uint> _addr_addr, bool lifo, nint skipframes) {
    ref uint addr = ref _addr_addr.val;
...
private static void readyWithTime(ptr<sudog> _addr_s, nint traceskip) {
...
private static void semacquire(ptr<uint> _addr_addr) {
    ...
    semacquire1(_addr_addr, false, 0, 0);
}

private static void semacquire1(ptr<uint> _addr_addr, bool lifo, semaProfileFlags profile, nint skipframes) {
    ref uint addr = ref _addr_addr.val;
    ...
    while (true) {
        ...
        atomic.Xadd(_addr_root.nwait, 1); 
        ...
        goparkunlock(_addr_root.@lock, waitReasonSemacquire, traceEvGoBlockSync, 4 + skipframes);
        if (s.ticket != 0 || cansemacquire(_addr_addr)) {
            break;
        }
    }
    if (s.releasetime > 0) {
...
private static void semrelease(ptr<uint> _addr_addr) {
    ...
    semrelease1(_addr_addr, false, 0);
}

private static void semrelease1(ptr<uint> _addr_addr, bool handoff, nint skipframes) {
    ref uint addr = ref _addr_addr.val;
    ...
    if (atomic.Load(_addr_root.nwait) == 0) {
        return ;
    }
    lockWithRank(_addr_root.@lock, lockRankRoot);
    if (atomic.Load(_addr_root.nwait) == 0) { 
    ...
    unlock(_addr_root.@lock);
    if (s != null) { // May be slow or even yield, so unlock first
        ...
        readyWithTime(_addr_s, 5 + skipframes);
...
private static ptr<semaRoot> semroot(ptr<uint> _addr_addr) {
    ...
    return _addr__addr_semtable[(uintptr(@unsafe.Pointer(addr)) >> 3) % semTabSize].root!;
}
...
private static void queue(this ptr<semaRoot> _addr_root, ptr<uint> _addr_addr, ptr<sudog> _addr_s, bool lifo) => func((_, panic, _) => {
...
private static void notifyListWait(ptr<notifyList> _addr_l, uint t) {
    ...
    else
 {
    ...
    l.tail = s;
    goparkunlock(_addr_l.@lock, waitReasonSyncCondWait, traceEvGoBlockCond, 3);
    if (t0 != 0) {
...
private static void notifyListNotifyOne(ptr<notifyList> _addr_l) {
    ...
    atomic.Store(_addr_l.notify, t + 1); 
