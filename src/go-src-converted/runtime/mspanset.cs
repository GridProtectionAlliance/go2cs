...
// smallest heap which would be unrepresentable is 32 TiB in size.
        ...
        private static readonly long spanSetBlockEntries = (long)512L; // 4KB on 64-bit
        private static readonly long spanSetInitSpineCap = (long)256L; // Enough for 1GB heap on 64-bit
...
// may be safely recycled.
        ...
        private static void push(this ptr<spanSet> _addr_b, ptr<mspan> _addr_s)
        {
            ...
            // Obtain our slot.
            var cursor = uintptr(b.index.incTail().tail() - 1L);
...
claimLoop:
            while (true)
            {
                ...
                while (want == head)
                {
                    if (b.index.cas(headtail, makeHeadTailIndex(want + 1L, tail)))
                    {
            ...
            // to finish popping.
            if (atomic.Xadd(_addr_block.popped, 1L) == spanSetBlockEntries)
            { 
        ...
        private static void reset(this ptr<spanSet> _addr_b)
        {
            ...
            b.index.reset();
            atomic.Storeuintptr(_addr_b.spineLen, 0L);
        ...
        private static ptr<spanSetBlock> alloc(this ptr<spanSetBlockAlloc> _addr_p)
        {
            ...
            return _addr_(spanSetBlock.val)(persistentalloc(@unsafe.Sizeof(new spanSetBlock()), cpu.CacheLineSize, _addr_memstats.gc_sys))!;
        ...
        private static void free(this ptr<spanSetBlockAlloc> _addr_p, ptr<spanSetBlock> _addr_block)
        {
            ...
            atomic.Store(_addr_block.popped, 0L);
        ...
        private static headTailIndex incHead(this ptr<headTailIndex> _addr_h)
        {
            ...
            return headTailIndex(atomic.Xadd64((uint64.val)(h), (1L << (int)(32L))));
        }
        ...
        private static headTailIndex incTail(this ptr<headTailIndex> _addr_h)
        {
            ...
            var ht = headTailIndex(atomic.Xadd64((uint64.val)(h), +1L)); 
            // Check for overflow.
            if (ht.tail() == 0L)
            {
        ...
        private static void reset(this ptr<headTailIndex> _addr_h)
        {
            ...
            atomic.Store64((uint64.val)(h), 0L);
        }
    }
}
