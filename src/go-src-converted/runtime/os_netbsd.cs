...
// package runtime -- go2cs converted at 2022 March 06 22:10:31 UTC
// import "runtime" ==> using runtime = go.runtime_package
// Original source: C:\Program Files\Go\src\runtime\os_netbsd.go
...
private static readonly nuint _UC_SIGMASK = 0x01;
private static readonly nuint _UC_CPU = 0x04; 

// From <sys/lwp.h>
private static readonly nuint _LWP_DETACHED = 0x00000040;
...
private static void lwp_kill(int tid, nint sig);
...
private static readonly nint _ETIMEDOUT = 60; 
...
private static readonly nint _TIMER_RELTIME = 0;
private static readonly nint _TIMER_ABSTIME = 1;
...
private static readonly nint _HW_NCPUONLINE = 16;
...
private static (int, bool) sysctlInt(slice<uint> mib) {
    ...
    var ret = sysctl(_addr_mib[0], uint32(len(mib)), _addr_(byte.val)(@unsafe.Pointer(_addr_out)), _addr_nout, _addr_null, 0);
...
private static int getncpu() {
    {
        ...
        if (ok) {>>MARKER:FUNCTION_closeonexec_BLOCK_PREFIX<<
            return int32(n);
        }
        n = n__prev1;
...

    {
        ...
        if (ok) {>>MARKER:FUNCTION_pipe2_BLOCK_PREFIX<<
            return int32(n);
        }
        n = n__prev1;
...
private static System.UIntPtr getPageSize() {
    ...
    if (ret >= 0) {>>MARKER:FUNCTION_pipe_BLOCK_PREFIX<<
        return uintptr(out);
    }
    return 0;
...
private static int semasleep(long ns) {
    ...
    if (ns >= 0) {>>MARKER:FUNCTION_kevent_BLOCK_PREFIX<<
        deadline = nanotime() + ns;
    }
    while (true) {>>MARKER:FUNCTION_kqueue_BLOCK_PREFIX<<
        var v = atomic.Load(_addr__g_.m.waitsemacount);
        if (v > 0) {>>MARKER:FUNCTION_osyield_BLOCK_PREFIX<<
            if (atomic.Cas(_addr__g_.m.waitsemacount, v, v - 1)) {>>MARKER:FUNCTION_lwp_self_BLOCK_PREFIX<<
        ...
        if (ns >= 0) {>>MARKER:FUNCTION_lwp_unpark_BLOCK_PREFIX<<
            var wait = deadline - nanotime();
            if (wait <= 0) {>>MARKER:FUNCTION_lwp_park_BLOCK_PREFIX<<
                return -1;
            }
            ts.setNsec(wait);
            tsp = _addr_ts;
        }
        var ret = lwp_park(_CLOCK_MONOTONIC, _TIMER_RELTIME, tsp, 0, @unsafe.Pointer(_addr__g_.m.waitsemacount), null);
        if (ret == _ETIMEDOUT) {>>MARKER:FUNCTION_lwp_create_BLOCK_PREFIX<<
            return -1;
        }
    }
...
private static void semawakeup(ptr<m> _addr_mp) {
    ...
    if (ret != 0 && ret != _ESRCH) {>>MARKER:FUNCTION_getcontext_BLOCK_PREFIX<< 
        // semawakeup can be called on signal stack.
        systemstack(() => {>>MARKER:FUNCTION_lwp_kill_BLOCK_PREFIX<<
...
private static void newosproc(ptr<m> _addr_mp) {
    ...
    if (false) {>>MARKER:FUNCTION_raiseproc_BLOCK_PREFIX<<
    ...
    if (ret < 0) {>>MARKER:FUNCTION_lwp_tramp_BLOCK_PREFIX<<
        print("runtime: failed to create new OS thread (have ", mcount() - 1, " already; errno=", -ret, ")\n");
        if (ret == -_EAGAIN) {>>MARKER:FUNCTION_sysctl_BLOCK_PREFIX<<
...
//go:nosplit
private static void netbsdMstart0() {
...
private static void osinit() {
    ncpu = getncpu();
    if (physPageSize == 0) {>>MARKER:FUNCTION_netbsdMstart_BLOCK_PREFIX<<
...
private static void sysargs(int argc, ptr<ptr<byte>> _addr_argv) {
    ...
    while (argv_index(argv, n) != null) {>>MARKER:FUNCTION_sigprocmask_BLOCK_PREFIX<<
        n++;
    } 
...
private static void sysauxv(slice<System.UIntPtr> auxv) {
    {
        ...
        while (auxv[i] != _AT_NULL) {>>MARKER:FUNCTION_sigaltstack_BLOCK_PREFIX<<
            var tag = auxv[i];
            var val = auxv[i + 1];
