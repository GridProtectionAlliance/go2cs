...
// Original source: C:\Program Files\Go\src\runtime\vdso_freebsd.go
...
private static readonly nint _VDSO_TH_NUM = 4; // defined in <sys/vdso.h> #ifdef _KERNEL
...
private static long nanotime1() {
    ...
    if (bt == zeroBintime) {>>MARKER:FUNCTION_fallback_walltime_BLOCK_PREFIX<<
        return fallback_nanotime();
    }
    return int64((1e9F * uint64(bt.sec)) + ((1e9F * uint64(bt.frac >> 32)) >> 32));
...
private static (long, int) walltime() {
    ...
    if (bt == zeroBintime) {>>MARKER:FUNCTION_fallback_nanotime_BLOCK_PREFIX<<
        return fallback_walltime();
    }
    return (int64(bt.sec), int32((1e9F * uint64(bt.frac >> 32)) >> 32));
