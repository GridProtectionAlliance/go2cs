...
private static readonly tflag tflagRegularMemory = 1 << 3; // equal and hash can treat values of this type as a single region of t.size bytes
...
private static ptr<byte> data(this name n, nint off) {
...
private static @string pkgPath(this name n) {
    if (n.bytes == null || new ptr<ptr<n.data>>(0) & (1 << 2) == 0) {
        return "";
    }
    var (i, l) = n.readvarint(1);
    nint off = 1 + i + l;
