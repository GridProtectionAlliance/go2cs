...
namespace go
{
    public static partial class runtime_package
    {
        ...
        */
        ...
        // space at zero.
        private static readonly long _StackBig = (long)4096L; 

        // The stack guard is a pointer this many bytes above the
        // bottom of the stack.
        private static readonly long _StackGuard = (long)928L * sys.StackGuardMultiplier + _StackSystem; 

        // After a stack split check the SP is allowed to be this
        // many bytes below the stack guard. This saves an instruction
        // in the checking sequence for tiny frames.
        private static readonly long _StackSmall = (long)128L; 
        ...
        private static readonly long stackDebug = (long)0L;
        private static readonly long stackFromSystem = (long)0L; // allocate stacks from system memory instead of the heap
        private static readonly long stackFaultOnFree = (long)0L; // old stacks are mapped noaccess to detect use after free
        private static readonly long stackPoisonCopy = (long)0L; // fill stack that should not be accessed with garbage, to detect bad dereferences during copy
        ...
        private static gclinkptr stackpoolalloc(byte order)
        {
            ...
            { 
                ...
                s = mheap_.allocManual(_StackCacheSize >> (int)(_PageShift), _addr_memstats.stacks_inuse);
                if (s == null)
                {
                    throw("out of memory");
                }
        ...
        private static void stackpoolfree(gclinkptr x, byte order)
        {
            ...
            { 
                ...
                s.manualFreeList = 0L;
                osStackFree(s);
                mheap_.freeManual(s, _addr_memstats.stacks_inuse);
        ...
        { 
            ...
            if (n < _FixedStack << (int)(_NumStackOrders) && n < _StackCacheSize)
            {
                ...
                gclinkptr x = default;
                if (stackNoCache != 0L || thisg.m.p == 0L || thisg.m.preemptoff != "")
                { 
                ...
                { 
                    ...
                    s = mheap_.allocManual(npage, _addr_memstats.stacks_inuse);
                    if (s == null)
                    {
                        throw("out of memory");
                    }
        ...
        private static void stackfree(stack stk)
        {
            ...
            if (n < _FixedStack << (int)(_NumStackOrders) && n < _StackCacheSize)
            {
                ...
                var x = gclinkptr(v);
                if (stackNoCache != 0L || gp.m.p == 0L || gp.m.preemptoff != "")
                {
                    lock(_addr_stackpool[order].item.mu);
                    stackpoolfree(x, order);
                    unlock(_addr_stackpool[order].item.mu);
        ...
        private static byte ptrbit(this ptr<bitvector> _addr_bv, System.UIntPtr i)
        {
            ...
            var b = (addb(bv.bytedata, i / 8L)).val;
            return (b >> (int)((i % 8L))) & 1L;
        }
                    ...
                    {
                        {
                            ...
                            for (var j = uintptr(0L); j < 8L; j++)
                            {
                                print("        ", add(scanp, (i + j) * sys.PtrSize), ":", ptrnames[bv.ptrbit(i + j)], ":", hex(new ptr<ptr<ptr<System.UIntPtr>>>(add(scanp, (i + j) * sys.PtrSize))), " # ", i, " ", addb(bv.bytedata, i / 8L).val, "\n");
                            }
...
retry:
            ...
            if (frame.varp != 0L)
            {
                foreach (var (_, obj) in objs)
                {
...

                    {
                        ...
                        while (i < t.ptrdata)
                        {
                            if (addb(gcdata, i / (8L * sys.PtrSize)) >> (int)((i / sys.PtrSize & 7L)) & 1L != 0L.val)
                            {
        ...
        private static void copystack(ptr<g> _addr_gp, System.UIntPtr newsize)
        {
            ...
            gentraceback(~uintptr(0L), ~uintptr(0L), 0L, gp, 0L, null, 0x7fffffffUL, adjustframe, noescape(@unsafe.Pointer(_addr_adjinfo)), 0L); 
        ...
        private static void newstack()
        {
            ...
            // recheck the bounds on return.)
            {
                ...
                if (f.valid())
                {
                    ...
                    while (newsize - oldsize < max + _StackGuard)
                    {
                        newsize *= 2L;
                    }
        ...
        private static bool isShrinkStackSafe(ptr<g> _addr_gp)
        {
            ...
            // all frames.
            return gp.syscallsp == 0L && !gp.asyncSafePoint;
        ...
        private static void shrinkstack(ptr<g> _addr_gp)
        {
            ...
            if (gp == getg().m.curg && gp.m.libcallsp != 0L)
            {
        ...
        private static (bitvector, bitvector, slice<stackObjectRecord>) getStackMap(ptr<stkframe> _addr_frame, ptr<pcvalueCache> _addr_cache, bool debug)
        {
                        ...
                        if (size > minsize)
            {
                ...
                if (stkmap.nbit > 0L)
                {
                    if (stackid < 0L || stackid >= stkmap.n)
                    { 
                    ...
                    if (stackDebug >= 3L && debug)
                    {
                        print("      locals ", stackid, "/", stkmap.n, " ", locals.n, " words ", locals.bytedata, "\n");
                    }

                }
                else if (stackDebug >= 3L && debug)
                {
