...
// Original source: C:\Program Files\Go\src\runtime\signal_windows.go
...
private static void disableWER() { 
    ...
    const nuint SEM_FAILCRITICALERRORS = 0x0001;
    const nuint SEM_NOGPFAULTERRORBOX = 0x0002;
    const nuint SEM_NOALIGNMENTFAULTEXCEPT = 0x0004;
    const nuint SEM_NOOPENFILEERRORBOX = 0x8000;
...
private static void initExceptionHandler() {
    ...
    if (_AddVectoredContinueHandler == null || GOARCH == "386") {>>MARKER:FUNCTION_lastcontinuetramp_BLOCK_PREFIX<< 
...
private static bool isAbort(ptr<context> _addr_r) {
    ...
    var pc = r.ip();
    if (GOARCH == "386" || GOARCH == "amd64" || GOARCH == "arm") {>>MARKER:FUNCTION_exceptiontramp_BLOCK_PREFIX<< 
...
//
//go:nosplit
private static int exceptionhandler(ptr<exceptionrecord> _addr_info, ptr<context> _addr_r, ptr<g> _addr_gp) {
    ref exceptionrecord info = ref _addr_info.val;
    ref context r = ref _addr_r.val;
    ...
    if (gp.throwsplit || isAbort(_addr_r)) { 
        ...
        // Crash now.
        winthrow(_addr_info, _addr_r, _addr_gp);
...
private static int lastcontinuehandler(ptr<exceptionrecord> _addr_info, ptr<context> _addr_r, ptr<g> _addr_gp) {
    ...
    if (GOARCH == "arm64" && info.exceptioncode == _EXCEPTION_ILLEGAL_INSTRUCTION && (r.ip() < firstmoduledata.text || firstmoduledata.etext < r.ip())) {
        return _EXCEPTION_CONTINUE_SEARCH;
    }
    winthrow(_addr_info, _addr_r, _addr_gp);
...
//go:nosplit
private static void winthrow(ptr<exceptionrecord> _addr_info, ptr<context> _addr_r, ptr<g> _addr_gp) {
    ref exceptionrecord info = ref _addr_info.val;
    ref context r = ref _addr_r.val;
    ref g gp = ref _addr_gp.val;
...
private static array<byte> badsignalmsg = new array<byte>(100);private static int badsignallen = default;
