...
// readying it when done.
        ...
        private static suspendGState suspendG(ptr<g> _addr_gp)
        {
            ...
            // See https://golang.org/cl/21503 for justification of the yield delay.
            const long yieldDelay = (long)10L * 1000L;
            ...
            {
                {
                    ...
                    if (s == _Grunning) 
                    {
                        ...
                        // its status.
                        if (preemptMSupported && debug.asyncpreemptoff == 0L && needAsync)
                        { 
                            ...
                            if (now >= nextPreemptM)
                            {
                                nextPreemptM = now + yieldDelay / 2L;
                                preemptM(asyncM);
                            }
        ...
        private static bool canPreemptM(ptr<m> _addr_mp)
        {
            ...
            return mp.locks == 0L && mp.mallocing == 0L && mp.preemptoff == "" && mp.p.ptr().status == _Prunning;
        }
...
;
        ...
        private static void asyncPreempt2()
        {
            ...
            if (gp.preemptStop)
            {>>MARKER:FUNCTION_asyncPreempt_BLOCK_PREFIX<<
                mcall(preemptPark);
            }
            else
            {
        ...
        // asyncPreempt call.
        private static var asyncPreemptStack = ~uintptr(0L);
        ...
        private static void init()
        {
            ...
            asyncPreemptStack = uintptr(total) + 8L * sys.PtrSize;
        ...
        private static bool wantAsyncPreempt(ptr<g> _addr_gp)
        {
            ...
            return (gp.preempt || gp.m.p != 0L && gp.m.p.ptr().preempt) && readgstatus(gp) & ~_Gscan == _Grunning;
        ...
        private static (bool, System.UIntPtr) isAsyncSafePoint(ptr<g> _addr_gp, System.UIntPtr pc, System.UIntPtr sp, System.UIntPtr lr)
        {
            ...
            // Check M state.
            if (mp.p == 0L || !canPreemptM(_addr_mp))
            {
                return (false, 0L);
            } 
            ...
            if ((GOARCH == "mips" || GOARCH == "mipsle" || GOARCH == "mips64" || GOARCH == "mips64le") && lr == pc + 8L && funcspdelta(f, pc, null) == 0L)
            { 
            ...
            int up = default;
            System.UIntPtr startpc = default;
            if (!go115ReduceLiveness)
            {
                var smi = pcdatavalue(f, _PCDATA_RegMapIndex, pc, null);
                if (smi == -2L)
                { 
            ...
            if (go115RestartSeq)
            {
                ...
                if (up == _PCDATA_Restart1 || up == _PCDATA_Restart2) 
                    ...
                    // the start PC.
                    if (startpc == 0L || startpc > pc || pc - startpc > 20L)
                    {
            ...
            else
            {
                ...
                if (up == _PCDATA_Restart1 || up == _PCDATA_Restart2 || up == _PCDATA_RestartAtEntry) 
