...
// package ssa -- go2cs converted at 2020 October 08 04:56:51 UTC
    ...
    public static partial class ssa_package
    {
        ...
        private static ptr<Const> zeroConst(types.Type t) => func((_, panic, __) =>
        {
            switch (t.type())
            {
                case ptr<types.Basic> t:
                    ...
                    if (t.Info() & types.IsBoolean != 0L)
                    {
                        return _addr_NewConst(constant.MakeBool(false), t)!;
                        goto __switch_break0;
                    }
                    if (t.Info() & types.IsNumeric != 0L)
                    {
                        return _addr_NewConst(constant.MakeInt64(0L), t)!;
                        goto __switch_break0;
                    }
                    if (t.Info() & types.IsString != 0L)
                    {
        ...
        private static @string RelString(this ptr<Const> _addr_c, ptr<types.Package> _addr_from)
        {
            ...
            else if (c.Value.Kind() == constant.String)
            {
                ...
                const long max = (long)20L; 
                // TODO(adonovan): don't cut a rune in half.
 
                // TODO(adonovan): don't cut a rune in half.
                if (len(s) > max)
                {
                    s = s[..max - 3L] + "..."; // abbreviate
                }
        ...
        //
        private static long Int64(this ptr<Const> _addr_c) => func((_, panic, __) =>
        {
            ref Const c = ref _addr_c.val;
        ...
        //
        private static ulong Uint64(this ptr<Const> _addr_c) => func((_, panic, __) =>
        {
            ref Const c = ref _addr_c.val;
