...
nextPackage: 
    ...
    Func<@string, @string, (@string, error)> toPkgPath = (sourceDir, id) => {
        {
            ...
            if (i >= 0) {
                return state.resolveImport(sourceDir, id[..(int)i]);
            }
        }
        ...
        foreach (var (_, __pkg) in response.dr.Packages) {
            pkg = __pkg;
            {
                ...
                foreach (var (_, __imp) in pkg.Imports) {
                    ...
                    var (pkgPath, err) = toPkgPath(filepath.Dir(pkg.GoFiles[0]), imp.ID);
                    if (err != null) {
                        return (null, null, error.As(err)!);
                    }
                    {
    ...
    if (overlayAddsImports) {
        needPkgs = make_slice<@string>(0, len(needPkgsSet));
        {
            var pkg__prev1 = pkg;
    ...
    modifiedPkgs = make_slice<@string>(0, len(modifiedPkgsSet));
    {
        var pkg__prev1 = pkg;
