...
// package regexp -- go2cs converted at 2020 October 08 03:41:13 UTC
...
namespace go
{
    public static partial class regexp_package
    {
        ...
        private static (@string, bool, uint) onePassPrefix(ptr<syntax.Prog> _addr_p)
        {
            ...
            // Avoid allocation of buffer if prefix is empty.
            if (iop(_addr_i) != syntax.InstRune || len(i.Rune) != 1L)
            {
                return ("", i.Op == syntax.InstMatch, uint32(p.Start));
            } 
            ...
            while (iop(_addr_i) == syntax.InstRune && len(i.Rune) == 1L && syntax.Flags(i.Arg) & syntax.FoldCase == 0L)
            {
        ...
        private static ptr<queueOnePass> newQueue(long size)
        {
        ...
        private static readonly var mergeFailed = (var)uint32(0xffffffffUL);
        ...
        private static (slice<int>, slice<uint>) mergeRuneSets(ptr<slice<int>> _addr_leftRunes, ptr<slice<int>> _addr_rightRunes, uint leftPC, uint rightPC) => func((defer, panic, _) =>
        {
            ...
            ref long lx = ref heap(out ptr<long> _addr_lx);            ref long rx = ref heap(out ptr<long> _addr_rx);
            ...
            long ix = -1L;
            Func<ptr<long>, ptr<slice<int>>, uint, bool> extend = (newLow, newArray, pc) =>
            {
                if (ix > 0L && (newArray.val)[newLow.val] <= merged[ix])
                {
                    return false;
                }

                merged = append(merged, (newArray.val)[newLow.val], (newArray.val)[newLow + 1L.val]);
                newLow.val += 2L;
                ix += 2L;
                next = append(next, pc);
                return true;
