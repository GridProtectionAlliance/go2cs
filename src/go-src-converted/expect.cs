...
// package packagestest -- go2cs converted at 2022 March 06 23:32:23 UTC
...
namespace go.golang.org.x.tools.go.packages;

public static partial class packagestest_package {

private static readonly @string markMethod = "mark";
private static readonly @string eofIdentifier = "EOF";
...
private static error Expect(this ptr<Exported> _addr_e, object methods) {
    ...
    foreach (var (name, f) in methods) {
        ...
        mi.converters = make_slice<converter>(mi.f.Type().NumIn());
        {
            ...
            for (nint i = 0; i < len(mi.converters); i++) {
...
private static (slice<ptr<expect.Note>>, error) goModMarkers(ptr<Exported> _addr_e, @string gomod) {
    ...
    {
        var err = ioutil.WriteFile(gomod, content, 0644);
...
private static (converter, error) buildConverter(this ptr<Exported> _addr_e, reflect.Type pt) {
    ...
    if (pt == noteType) 
        ...
        return ((n, args) => {
            ...
            var result = reflect.MakeSlice(reflect.SliceOf(pt.Elem()), 0, len(args));
            foreach (>>MARKER:FORRANGEEXPRESSIONS_LEVEL_1<< in args) {>>MARKER:FORRANGEMUTABLEEXPRESSIONS_LEVEL_1<<
        ...
        if (pt.Kind() == reflect.Interface && pt.NumMethod() == 0) {
            return ((n, args) => {
                ...
                return (reflect.ValueOf(args[0]), error.As(args[(int)1..])!, null);
            }, error.As(null!)!);
        }
