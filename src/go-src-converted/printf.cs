...
// package printf -- go2cs converted at 2022 March 06 23:34:08 UTC
...
public static readonly @string Doc = @"check consistency of Printf format strings and arguments
...
public static readonly Kind KindNone = iota; // not a fmt wrapper function
...
private static void checkPrintfFwd(ptr<analysis.Pass> _addr_pass, ptr<printfWrapper> _addr_w, ptr<ast.CallExpr> _addr_call, Kind kind, ptr<Result> _addr_res) {
    ...
    var matched = kind == KindPrint || kind != KindNone && len(call.Args) >= 2 && match(_addr_pass.TypesInfo, call.Args[len(call.Args) - 2], _addr_w.format);
    if (!matched) {
        return ;
    }
...
private static (@string, nint) formatString(ptr<analysis.Pass> _addr_pass, ptr<ast.CallExpr> _addr_call) {
...
// constant.
private static (@string, bool) stringConstantArg(ptr<analysis.Pass> _addr_pass, ptr<ast.CallExpr> _addr_call, nint idx) {
    @string _p0 = default;
    bool _p0 = default;
    ref analysis.Pass pass = ref _addr_pass.val;
    ref ast.CallExpr call = ref _addr_call.val;
...
private partial struct formatState {
    ...
    public slice<nint> argNums; // the successive argument numbers that are consumed, adjusted to refer to actual arg in call
    public nint firstArg; // Index of first argument after the format in the Printf call.
// Used only during parse.
    public ptr<analysis.Pass> pass;
    public ptr<ast.CallExpr> call;
    public nint argNum; // Which argument we're expecting to format now.
    ...
    public nint nbytes; // number of bytes of the format string consumed.
}
...
private static bool parseIndex(this ptr<formatState> _addr_s) {
    ...
    var (arg32, err) = strconv.ParseInt(s.format[(int)start..(int)s.nbytes], 10, 32);
    if (err != null || !ok || arg32 <= 0 || arg32 > int64(len(s.call.Args) - s.firstArg)) {
        s.pass.ReportRangef(s.call, "%s format has invalid argument index [%s]", s.name, s.format[(int)start..(int)s.nbytes]);
        return false;
    }
    s.nbytes++; // skip ']'
    var arg = int(arg32);
...
private static ptr<formatState> parsePrintfVerb(ptr<analysis.Pass> _addr_pass, ptr<ast.CallExpr> _addr_call, @string name, @string format, nint firstArg, nint argNum) {
    ref analysis.Pass pass = ref _addr_pass.val;
    ref ast.CallExpr call = ref _addr_call.val;
    ...
    var (verb, w) = utf8.DecodeRuneInString(state.format[(int)state.nbytes..]);
    state.verb = verb;
    state.nbytes += w;
    if (verb != '%') {
...
private partial struct printfArgType { // : nint
}

private static readonly printfArgType argBool = 1 << (int)(iota);
...
private static bool okPrintfArg(ptr<analysis.Pass> _addr_pass, ptr<ast.CallExpr> _addr_call, ptr<formatState> _addr_state) {
    ...
    for (nint i = 0; i < nargs - trueArgs; i++) {
    ...
    if (!argCanBeChecked(_addr_pass, _addr_call, len(state.argNums) - 1, _addr_state)) {
        return false;
    }
...
private static bool isStringer(ptr<types.Signature> _addr_sig) {
    ...
    return sig.Params().Len() == 0 && sig.Results().Len() == 1 && sig.Results().At(0).Type() == types.Typ[types.String];
}
...
private static bool argCanBeChecked(ptr<analysis.Pass> _addr_pass, ptr<ast.CallExpr> _addr_call, nint formatArg, ptr<formatState> _addr_state) => func((_, panic, _) => {
    ref analysis.Pass pass = ref _addr_pass.val;
    ref ast.CallExpr call = ref _addr_call.val;
    ref formatState state = ref _addr_state.val;
...
private static void checkPrint(ptr<analysis.Pass> _addr_pass, ptr<ast.CallExpr> _addr_call, ptr<types.Func> _addr_fn) {
    ...
    if (firstArg == 0) {
        {
            ptr<ast.SelectorExpr> (sel, ok) = call.Args[0]._<ptr<ast.SelectorExpr>>();
...
private static @string count(nint n, @string what) {
    if (n == 1) {
        return "1 " + what;
    }
