...
// package ocagent -- go2cs converted at 2022 March 06 23:31:44 UTC
...
// its telemetry.
public static ptr<Exporter> Connect(ptr<Config> _addr_config) => func((defer, _, _) => {
    ...
    go_(() => () => {
        foreach (>>MARKER:FORRANGEEXPRESSIONS_LEVEL_1<< in time.Tick(exporter.config.Rate)) {>>MARKER:FORRANGEMUTABLEEXPRESSIONS_LEVEL_1<<
            exporter.Flush();
        }
    }());
    return _addr_exporter!;
...
private static context.Context ProcessEvent(this ptr<Exporter> _addr_e, context.Context ctx, core.Event ev, label.Map lm) => func((defer, _, _) => {
...
private static void Flush(this ptr<Exporter> _addr_e) => func((defer, _, _) => {
    ref Exporter e = ref _addr_e.val;
...
private static (nint, label.Label) skipToValidLabel(label.List list, nint index) {
    nint _p0 = default;
    ...
    return (-1, new label.Label());

}

private static ptr<wire.Attributes> convertAttributes(label.List list, nint index) {
...
private static (@string, nint) getAnnotationDescription(core.Event ev) {
    @string _p0 = default;
    nint _p0 = default;
