...
namespace hash
{
    ...
    {
        private static readonly long vecMinLen = 16L;
        private static readonly long vecAlignMask = 15L; // align to 16 bytes
        private static readonly long crcIEEE = 1L;
        private static readonly long crcCast = 2L;

        //go:noescape
        private static uint ppc64SlicingUpdateBy8(uint crc, ref slicing8Table table8, slice<byte> p)
;
...
;
        ...
        private static uint archUpdateCastagnoli(uint crc, slice<byte> p)
        {
            ...
            {>>MARKER:FUNCTION_vectorCrc32_BLOCK_PREFIX<< 
                ...
                if (uint64(uintptr(@unsafe.Pointer(ref p[0L]))) & uint64(vecAlignMask) != 0L)
                {>>MARKER:FUNCTION_ppc64SlicingUpdateBy8_BLOCK_PREFIX<<
                    var align = uint64(uintptr(@unsafe.Pointer(ref p[0L]))) & uint64(vecAlignMask);
