...
// package httpresponse -- go2cs converted at 2022 March 06 23:33:59 UTC
// import "golang.org/x/tools/go/analysis/passes/httpresponse" ==> using httpresponse = go.golang.org.x.tools.go.analysis.passes.httpresponse_package
// Original source: C:\Users\ritchie\go\src\golang.org\x\tools\go\analysis\passes\httpresponse\httpresponse.go
...
public static readonly @string Doc = @"check for mistakes using HTTP responses
...
private static (object, error) run(ptr<analysis.Pass> _addr_pass) {
    ...
    inspect.WithStack(nodeFilter, (n, push, stack) => {
        ...
        ptr<ast.AssignStmt> (asg, ok) = stmts[0]._<ptr<ast.AssignStmt>>();
        if (!ok) {
            return true; // the first statement is not assignment.
        }
        var resp = rootIdent(asg.Lhs[0]);
        if (resp == null) {
            return true; // could not find the http.Response in the assignment.
        }
        ptr<ast.DeferStmt> (def, ok) = stmts[1]._<ptr<ast.DeferStmt>>();
        if (!ok) {
...
private static bool isHTTPFuncOrMethodOnClient(ptr<types.Info> _addr_info, ptr<ast.CallExpr> _addr_expr) {
    ...
    }
    {
        ...
        ptr<types.Pointer> (ptr, ok) = res.At(0).Type()._<ptr<types.Pointer>>();
