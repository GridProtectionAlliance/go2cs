...
// package ssautil -- go2cs converted at 2022 March 06 23:33:51 UTC
...
private static (ssa.Value, ptr<ssa.Const>) isComparisonBlock(ptr<ssa.BasicBlock> _addr_b) {
...

    {
        ...
        if (n >= 2) {
            {
                ptr<ssa.If> (i, ok) = b.Instrs[n - 1]._<ptr<ssa.If>>();
                ...
                if (ok) {
                    {
                        ...
                        if (ok && ext1.Block() == b && ext1.Index == 1) {
                            {
                                ...
                                if (ok && ta.Block() == b) { 
                                    ...
                                    {
                                        ptr<ssa.Extract> (ext0, ok) = b.Instrs[n - 3]._<ptr<ssa.Extract>>();
