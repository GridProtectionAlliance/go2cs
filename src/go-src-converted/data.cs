...
// package metric -- go2cs converted at 2022 March 06 23:31:40 UTC
...
private static (nint, bool) getGroup(label.Map lm, ptr<slice<slice<label.Label>>> _addr_g, slice<label.Key> keys) {
    nint _p0 = default;
    bool _p0 = default;
    ...
    g = make_slice<slice<label.Label>>(len(old) + 1);
    copy(g, old[..(int)index]);
    copy((g)[(int)index + 1..], old[(int)index..]);
    (g)[index] = group;
    return (index, true);
...
private static Data modify(this ptr<Int64Data> _addr_data, time.Time at, label.Map lm, Func<long, long> f) {
    ...
    var old = data.Rows;
    if (insert) {
        data.Rows = make_slice<long>(len(old) + 1);
        copy(data.Rows, old[..(int)index]);
        copy(data.Rows[(int)index + 1..], old[(int)index..]);
    }
    else
 {
...
private static Data modify(this ptr<Float64Data> _addr_data, time.Time at, label.Map lm, Func<double, double> f) {
    ...
    var old = data.Rows;
    if (insert) {
        data.Rows = make_slice<double>(len(old) + 1);
        copy(data.Rows, old[..(int)index]);
        copy(data.Rows[(int)index + 1..], old[(int)index..]);
    }
    else
...
private static Data modify(this ptr<HistogramInt64Data> _addr_data, time.Time at, label.Map lm, Action<ptr<HistogramInt64Row>> f) {
    ...
    if (insert) {
        data.Rows = make_slice<ptr<HistogramInt64Row>>(len(old) + 1);
        copy(data.Rows, old[..(int)index]);
        copy(data.Rows[(int)index + 1..], old[(int)index..]);
    }
    else
...
private static Data modify(this ptr<HistogramFloat64Data> _addr_data, time.Time at, label.Map lm, Action<ptr<HistogramFloat64Row>> f) {
    ...
    if (insert) {
        data.Rows = make_slice<ptr<HistogramFloat64Row>>(len(old) + 1);
        copy(data.Rows, old[..(int)index]);
        copy(data.Rows[(int)index + 1..], old[(int)index..]);
    }
    else
