...
namespace @internal
{
 ...
 // default: false
        ...
        // 1: header format change (more regular), export package for _ struct fields
        // 0: Go1.7 encoding
        private static readonly long exportVersion = (long)4L;
        ...
        private partial struct exporter
        {
            ...
            public map<ptr<types.Package>, long> pkgIndex;
            public map<types.Type, long> typIndex; // position encoding
            public bool posInfoFormat;
            public @string prevFile;
            public long prevLine; // debugging support
            public long written; // bytes written
            public long indent; // for trace
        }
        ...
        private static void obj(this ptr<exporter> _addr_p, types.Object obj) => func((_, panic, __) =>
        {
            ref exporter p = ref _addr_p.val;
        ...
        private static (@string, long) fileLine(this ptr<exporter> _addr_p, types.Object obj)
        {
            @string file = default;
            long line = default;
            ref exporter p = ref _addr_p.val;
        ...
        private static void assocMethods(this ptr<exporter> _addr_p, ptr<types.Named> _addr_named)
        {
            ...
            slice<ptr<types.Func>> methods = default;
            {
                ...
                for (long i = 0L; i < named.NumMethods(); i++)
                {
...

            {
                ...
                foreach (var (__i, __m) in methods)
                {
                    ...
                    p.paramList(sig.Params(), sig.Variadic());
                    p.paramList(sig.Results(), false);
                    p.@int(0L); // dummy value for go:nointerface pragma - ignored by importer
                }
        ...
        private static long Len(this methodsByName x)
        {
            return len(x);
        }
        private static void Swap(this methodsByName x, long i, long j)
        {
            x[i] = x[j];
            x[j] = x[i];
        }
        private static bool Less(this methodsByName x, long i, long j)
        {
            return x[i].Name() < x[j].Name();
        }

        private static void fieldList(this ptr<exporter> _addr_p, ptr<types.Struct> _addr_t) => func((defer, _, __) =>
        {
            ref exporter p = ref _addr_p.val;
            ref types.Struct t = ref _addr_t.val;

            if (trace && t.NumFields() > 0L)
            {
                p.tracef("fields {>\n");
                defer(p.tracef("<\n} "));
            }
        ...
        private static void field(this ptr<exporter> _addr_p, ptr<types.Var> _addr_f) => func((_, panic, __) =>
        {
            ref exporter p = ref _addr_p.val;
        ...
        private static void iface(this ptr<exporter> _addr_p, ptr<types.Interface> _addr_t) => func((defer, _, __) =>
        {
            ref exporter p = ref _addr_p.val;
        ...
        private static void method(this ptr<exporter> _addr_p, ptr<types.Func> _addr_m) => func((_, panic, __) =>
        {
            ref exporter p = ref _addr_p.val;
        ...
        private static void paramList(this ptr<exporter> _addr_p, ptr<types.Tuple> _addr_@params, bool variadic)
        {
            ...
            var n = @params.Len();
            if (n > 0L && @params.At(0L).Name() == "")
            {
                n = -n;
            }

            p.@int(n);
            for (long i = 0L; i < @params.Len(); i++)
            {
                var q = @params.At(i);
                var t = q.Type();
                if (variadic && i == @params.Len() - 1L)
                {
        ...
        private static void value(this ptr<exporter> _addr_p, constant.Value x) => func((_, panic, __) =>
        {
            ref exporter p = ref _addr_p.val;
        ...
        private static void @float(this ptr<exporter> _addr_p, constant.Value x) => func((_, panic, __) =>
        {
            ref exporter p = ref _addr_p.val;
        ...
        { 
            ...
            for (long i = 0L; i < len(bytes) / 2L; i++)
            {
                bytes[i] = bytes[len(bytes) - 1L - i];
                bytes[len(bytes) - 1L - i] = bytes[i];
        ...
        private static bool @bool(this ptr<exporter> _addr_p, bool b) => func((defer, _, __) =>
        {
            ref exporter p = ref _addr_p.val;
        ...
        private static void index(this ptr<exporter> _addr_p, byte marker, long index) => func((_, panic, __) =>
        {
            ref exporter p = ref _addr_p.val;
        ...
        private static void tag(this ptr<exporter> _addr_p, long tag) => func((_, panic, __) =>
        {
            ref exporter p = ref _addr_p.val;
        ...
        private static void @int(this ptr<exporter> _addr_p, long x)
        {
            ref exporter p = ref _addr_p.val;
