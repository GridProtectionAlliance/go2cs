...
namespace @internal
{
    public static partial class gcimporter_package
    {
        ...
        //
        public static (ptr<types.Package>, error) Import(map<@string, ptr<types.Package>> packages, @string path, @string srcDir, Func<@string, (io.ReadCloser, error)> lookup) => func((defer, _, __) =>
        {
            ptr<types.Package> pkg = default!;
            error err = default!;
...
;
        ...
        private static void expectSpecial(this ptr<parser> _addr_p, @string tok)
        {
            ...
            if (i < len(tok))
            {
                p.errorf("expected %q, got %q", tok, tok[0L..i]);
            }
        ...
        private static (ptr<types.Var>, @string) parseField(this ptr<parser> _addr_p, ptr<types.Package> _addr_parent)
        {
            ...
            { 
                ...
                const @string blankpkg = (@string)"<_>";
        ...
        private static types.Type parseStructType(this ptr<parser> _addr_p, ptr<types.Package> _addr_parent)
        {
            ...
            p.expect('{');
            for (long i = 0L; p.tok != '}' && p.tok != scanner.EOF; i++)
            {
                if (i > 0L)
                {
                    p.expect(';');
        ...
        //
        private static (ptr<types.Basic>, constant.Value) parseNumber(this ptr<parser> _addr_p) => func((_, panic, __) =>
        {
            ...
            // mantissa
            var mant = constant.MakeFromLiteral(p.parseInt(), token.INT, 0L);
            if (mant == null)
            {
            ...
            { 
                // exponent (base 2)
                p.next();
                var (exp, err) = strconv.ParseInt(p.parseInt(), 10L, 0L);
                if (err != null)
                {
                    p.error(err);
                }
                ...
                if (exp < 0L)
                {
                    var denom = constant.MakeInt64(1L);
        ...
        private static void parseConstDecl(this ptr<parser> _addr_p)
        {
            ...
            if (p.tok == scanner.Ident) 
                ...
                // rune_lit
                typ = types.Typ[types.UntypedRune];
                val = constant.MakeFromLiteral(p.lit, token.CHAR, 0L);
                p.next();
            else if (p.tok == scanner.String) 
                // string_lit
                typ = types.Typ[types.UntypedString];
                val = constant.MakeFromLiteral(p.lit, token.STRING, 0L);
                p.next();
            else 
