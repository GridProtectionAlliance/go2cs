...
// package packages -- go2cs converted at 2022 March 06 23:31:31 UTC
...
private static map<@string, @string> mustGetEnv(this ptr<golistState> _addr_state) => func((_, panic, _) => {
    ref golistState state = ref _addr_state.val;
...
private static (ptr<driverResponse>, error) goListDriver(ptr<Config> _addr_cfg, params @string[] patterns) => func((defer, _, _) => {
    ...
    var restPatterns = make_slice<@string>(0, len(patterns)); 
...
extractQueries: 
 ...
 {
            var query = pattern[..(int)eqidx];
            var value = pattern[(int)eqidx + len("=")..];
            switch (query) {
                case "file": 
...
private static error runContainsQueries(this ptr<golistState> _addr_state, ptr<responseDeduper> _addr_response, slice<@string> queries) {
    ...
    foreach (var (_, query) in queries) { 
        ...
        if (err != null || len(dirResponse.Packages) == 1 && len(dirResponse.Packages[0].GoFiles) == 0 && len(dirResponse.Packages[0].Errors) == 1) {
...
private static (ptr<driverResponse>, error) adhocPackage(this ptr<golistState> _addr_state, @string pattern, @string query) {
    ...
    if (len(response.Packages) == 1) { 
        ...
        if (response.Packages[0].ID == "command-line-arguments" || filepath.ToSlash(response.Packages[0].PkgPath) == filepath.ToSlash(query)) {
            if (len(response.Packages[0].GoFiles) == 0) {
                ...
                foreach (var (path) in state.cfg.Overlay) {
                    if (path == filename) {
                        response.Packages[0].Errors = null;
                        response.Packages[0].GoFiles = new slice<@string>(new @string[] { path });
                        response.Packages[0].CompiledGoFiles = new slice<@string>(new @string[] { path });
                    }
                }
...
private static (ptr<driverResponse>, error) createDriverResponse(this ptr<golistState> _addr_state, params @string[] words) {
                ...
                if (found) { 
                    ...
                    if (old.Error != null) {
                        ...
                        if (errkind != "") {
                            ...
                            var importingPkg = old.Error.ImportStack[len(old.Error.ImportStack) - 1];
 ...
 { 
            ...
            if (len(pkg.OtherFiles) > 0) {
                ...
                var @out = pkg.CompiledGoFiles[..(int)0];
                {
                    var f__prev2 = f;
...
private static (ptr<bytes.Buffer>, error) invokeGo(this ptr<golistState> _addr_state, @string verb, params @string[] args) {
    ...
    "rr\": %q}}", strings.Trim(stderr.String(), "\n"));
        ...
        const @string noSuchDirectory = "no such directory";

        if (len(stderr.String()) > 0 && strings.Contains(stderr.String(), noSuchDirectory)) {
            var errstr = stderr.String();
            var abspath = strings.TrimSpace(errstr[(int)strings.Index(errstr, noSuchDirectory) + len(noSuchDirectory)..]);
    ...
    "rr\": %q}}", strings.Trim(stderr.String(), "\n"));
        ...
        if (len(stderr.String()) > 0 && strings.Contains(stderr.String(), "no Go files in")) { 
            ...
            if (colon > 0 && strings.HasPrefix(stderrStr, "go build ")) {
                importPath = stderrStr[(int)len("go build ")..(int)colon];
            }
