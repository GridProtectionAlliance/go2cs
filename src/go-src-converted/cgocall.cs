...
// package cgocall -- go2cs converted at 2022 March 06 23:33:53 UTC
// import "golang.org/x/tools/go/analysis/passes/cgocall" ==> using cgocall = go.golang.org.x.tools.go.analysis.passes.cgocall_package
// Original source: C:\Users\ritchie\go\src\golang.org\x\tools\go\analysis\passes\cgocall\cgocall.go
...
public static readonly @string Doc = @"detect some violations of the cgo pointer passing rules
    ...
    ast.Inspect(f, n => {
        ...
        foreach (var (_, arg) in call.Args) {
            ...
            // Check for passing the address of a bad type.
            {
                ...
                if (ok && len(conv.Args) == 1 && isUnsafePointer(_addr_info, conv.Fun)) {
                    arg = conv.Args[0];
                }
...
private static (slice<ptr<ast.File>>, ptr<types.Info>, error) typeCheckCgoSourceFiles(ptr<token.FileSet> _addr_fset, ptr<types.Package> _addr_pkg, slice<ptr<ast.File>> files, ptr<types.Info> _addr_info, types.Sizes sizes) {
    ...
    foreach (var (_, raw) in files) { 
        ...
        var (f, err) = parser.ParseFile(fset, filename, null, parser.Mode(0));
        if (err != null) {
...
private static types.Type cgoBaseType(ptr<types.Info> _addr_info, ast.Expr arg) {
    ...
    switch (arg.type()) {
        case ptr<ast.CallExpr> arg:
            if (len(arg.Args) == 1 && isUnsafePointer(_addr_info, arg.Fun)) {
                return cgoBaseType(_addr_info, arg.Args[0]);
            }
            break;
        case ptr<ast.StarExpr> arg:
            ...
            call, ok = call.Args[0]._<ptr<ast.CallExpr>>();
            if (!ok || len(call.Args) != 1) {
                break;
            } 
            ...
            ptr<ast.UnaryExpr> (u, ok) = call.Args[0]._<ptr<ast.UnaryExpr>>();
            if (!ok || u.Op != token.AND) {
                break;
            } 
            // Here arg is *(*unsafe.Pointer)(unsafe.Pointer(&v))
