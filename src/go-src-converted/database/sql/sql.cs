...
// package sql -- go2cs converted at 2022 March 06 23:35:40 UTC
// import "database/sql" ==> using sql = go.database.sql_package
// Original source: C:\Program Files\Go\src\database\sql\sql.go
...
private static void unregisterAllDrivers() => func((defer, _, _) => {
    driversMu.Lock();
    defer(driversMu.Unlock()); 
    // For tests.
...
public static slice<@string> Drivers() => func((defer, _, _) => {
    driversMu.RLock();
    defer(driversMu.RUnlock());
    var list = make_slice<@string>(0, len(drivers));
...
public partial struct IsolationLevel { // : nint
}
...
private static error Scan(this ptr<NullString> _addr_ns, object value) {
    ...
    if (value == null) {
        (ns.String, ns.Valid) = ("", false);        return error.As(null!)!;
    }
    ns.Valid = true;
...
private static error Scan(this ptr<NullInt64> _addr_n, object value) {
    ...
    if (value == null) {
        (n.Int64, n.Valid) = (0, false);        return error.As(null!)!;
    }
    n.Valid = true;
...
private static error Scan(this ptr<NullInt32> _addr_n, object value) {
    ...
    if (value == null) {
        (n.Int32, n.Valid) = (0, false);        return error.As(null!)!;
    }
    n.Valid = true;
...
// Scan implements the Scanner interface.
private static error Scan(this ptr<NullInt16> _addr_n, object value) {
    ref NullInt16 n = ref _addr_n.val;

    if (value == null) {
        (n.Int16, n.Valid) = (0, false);        return error.As(null!)!;
    }
    var err = convertAssign(_addr_n.Int16, value);
    n.Valid = err == null;
    return error.As(err)!;
...
public static (driver.Value, error) Value(this NullInt16 n) {
    ...
    return (int64(n.Int16), error.As(null!)!);
...
// Scan implements the Scanner interface.
private static error Scan(this ptr<NullByte> _addr_n, object value) {
    ref NullByte n = ref _addr_n.val;

    if (value == null) {
        (n.Byte, n.Valid) = (0, false);        return error.As(null!)!;
    }
    var err = convertAssign(_addr_n.Byte, value);
    n.Valid = err == null;
    return error.As(err)!;
...
public static (driver.Value, error) Value(this NullByte n) {
    ...
    return (int64(n.Byte), error.As(null!)!);
...
private static error Scan(this ptr<NullFloat64> _addr_n, object value) {
    ...
    if (value == null) {
        (n.Float64, n.Valid) = (0, false);        return error.As(null!)!;
    }
    n.Valid = true;
...
private static error Scan(this ptr<NullBool> _addr_n, object value) {
    ...
    if (value == null) {
        (n.Bool, n.Valid) = (false, false);        return error.As(null!)!;
    }
    n.Valid = true;
...
private static error Scan(this ptr<NullTime> _addr_n, object value) {
    ...
    if (value == null) {
        (n.Time, n.Valid) = (new time.Time(), false);        return error.As(null!)!;
    }
    n.Valid = true;
...
// connections in Stmt.css.
    ...
    public nint numOpen; // number of opened and pending open connections
...
// goroutine to exit.
    ...
    public nint maxIdleCount; // zero means defaultMaxIdleConns; negative means 0
    public nint maxOpen; // <= 0 means unlimited
    ...
    public Action stop; // stop cancels the connection opener.
}
...
private static void removeOpenStmt(this ptr<driverConn> _addr_dc, ptr<driverStmt> _addr_ds) => func((defer, _, _) => {
    ref driverConn dc = ref _addr_dc.val;
...
private static error resetSession(this ptr<driverConn> _addr_dc, context.Context ctx) => func((defer, _, _) => {
    ref driverConn dc = ref _addr_dc.val;
...
private static bool validateConnection(this ptr<driverConn> _addr_dc, bool needsReset) => func((defer, _, _) => {
    ref driverConn dc = ref _addr_dc.val;
...
private static Func<error> closeDBLocked(this ptr<driverConn> _addr_dc) => func((defer, _, _) => {
    ref driverConn dc = ref _addr_dc.val;
...
private static error finalClose(this ptr<driverConn> _addr_dc) {
    ...
    withLock(dc, () => {
        openStmt = make_slice<ptr<driverStmt>>(0, len(dc.openStmt));
        {
            var ds__prev1 = ds;
    ...
    atomic.AddUint64(_addr_dc.db.numClosed, 1);
    return error.As(err)!;
...
// result.
private static error Close(this ptr<driverStmt> _addr_ds) => func((defer, _, _) => {
    ref driverStmt ds = ref _addr_ds.val;
...
private static void addDep(this ptr<DB> _addr_db, finalCloser x, object dep) => func((defer, _, _) => {
...
private static Func<error> removeDepLocked(this ptr<DB> _addr_db, finalCloser x, object dep) => func((_, panic, _) => {
    ref DB db = ref _addr_db.val;
...
private static nint connectionRequestQueueSize = 1000000;
...
private static error PingContext(this ptr<DB> _addr_db, context.Context ctx) {
    ...
    for (nint i = 0; i < maxBadConnRetries; i++) {
...
private static error Close(this ptr<DB> _addr_db) {
    ...
    error err = default!;
    var fns = make_slice<Func<error>>(0, len(db.freeConn));
    ...
    {
        io.Closer (c, ok) = db.connector._<io.Closer>();
...
private static readonly nint defaultMaxIdleConns = 2;



private static nint maxIdleConnsLocked(this ptr<DB> _addr_db) {
    ref DB db = ref _addr_db.val;
...
// a future release.
private static void SetMaxIdleConns(this ptr<DB> _addr_db, nint n) {
    ref DB db = ref _addr_db.val;
...
private static void SetMaxOpenConns(this ptr<DB> _addr_db, nint n) {
    ref DB db = ref _addr_db.val;
...
private static void SetConnMaxLifetime(this ptr<DB> _addr_db, time.Duration d) {
    ...
    if (d > 0 && d < db.maxLifetime && db.cleanerCh != null)     }
    db.maxLifetime = d;
...
private static void SetConnMaxIdleTime(this ptr<DB> _addr_db, time.Duration d) => func((defer, _, _) => {
    ref DB db = ref _addr_db.val;
    ...
    if (d > 0 && d < db.maxIdleTime && db.cleanerCh != null)     }
    db.maxIdleTime = d;
    db.startCleanerLocked();
...
private static void startCleanerLocked(this ptr<DB> _addr_db) {
    ...
    if ((db.maxLifetime > 0 || db.maxIdleTime > 0) && db.numOpen > 0 && db.cleanerCh == null) {
        db.cleanerCh = make_channel<object>(1);
...
private static DBStats Stats(this ptr<DB> _addr_db) => func((defer, _, _) => {
    ref DB db = ref _addr_db.val;
...
private static void openNewConnection(this ptr<DB> _addr_db, context.Context ctx) => func((defer, _, _) => {
    ref DB db = ref _addr_db.val;
...
private static (ptr<driverConn>, error) conn(this ptr<DB> _addr_db, context.Context ctx, connReuseStrategy strategy) {
    ...
    if (strategy == cachedOrNewConn && numFree > 0) {
        var conn = db.freeConn[0];
        copy(db.freeConn, db.freeConn[(int)1..]);
        db.freeConn = db.freeConn[..(int)numFree - 1];
        conn.inUse = true;
    ...
    if (db.maxOpen > 0 && db.numOpen >= db.maxOpen) { 
        ...
        var req = make_channel<connRequest>(1);
...
// already closed.
private static void noteUnusedDriverStatement(this ptr<DB> _addr_db, ptr<driverConn> _addr_c, ptr<driverStmt> _addr_ds) => func((defer, _, _) => {
    ref DB db = ref _addr_db.val;
...
// putConn adds a connection to the db's free pool.
// err is optionally the last error that occurred on this connection.
private static void putConn(this ptr<DB> _addr_db, ptr<driverConn> _addr_dc, error err, bool resetSession) => func((_, panic, _) => {
    ref DB db = ref _addr_db.val;
    ref driverConn dc = ref _addr_dc.val;
...
private static readonly nint maxBadConnRetries = 2;
...
private static (ptr<Stmt>, error) prepareDC(this ptr<DB> _addr_db, context.Context ctx, ptr<driverConn> _addr_dc, Action<error> release, stmtConnGrabber cg, @string query) => func((defer, _, _) => {
    ptr<Stmt> _p0 = default!;
    error _p0 = default!;
    ref DB db = ref _addr_db.val;
    ref driverConn dc = ref _addr_dc.val;
...
private static (Result, error) execDC(this ptr<DB> _addr_db, context.Context ctx, ptr<driverConn> _addr_dc, Action<error> release, @string query, slice<object> args) => func((defer, _, _) => {
    Result res = default;
    error err = default!;
    ref DB db = ref _addr_db.val;
    ref driverConn dc = ref _addr_dc.val;
...
private static (ptr<driverConn>, releaseConn, error) grabConn(this ptr<Conn> _addr_c, context.Context _p0) {
    ...
    if (atomic.LoadInt32(_addr_c.done) != 0) {
...
private static error Raw(this ptr<Conn> _addr_c, Func<object, error> f) => func((defer, _, _) => {
    error err = default!;
    ref Conn c = ref _addr_c.val;
...
private static error close(this ptr<Conn> _addr_c, error err) => func((defer, _, _) => {
    ref Conn c = ref _addr_c.val;

    if (!atomic.CompareAndSwapInt32(_addr_c.done, 0, 1)) {
...
private static bool isDone(this ptr<Tx> _addr_tx) {
    ...
    return atomic.LoadInt32(_addr_tx.done) != 0;
}
...
private static void close(this ptr<Tx> _addr_tx, error err) {
    ref Tx tx = ref _addr_tx.val;
...
private static (ptr<driverConn>, releaseConn, error) grabConn(this ptr<Tx> _addr_tx, context.Context ctx) {
    ...
    if (hookTxGrabConn != null) { // test hook
        hookTxGrabConn();
...
private static void closePrepared(this ptr<Tx> _addr_tx) => func((defer, _, _) => {
    ref Tx tx = ref _addr_tx.val;
...
private static error Commit(this ptr<Tx> _addr_tx) {
    ref Tx tx = ref _addr_tx.val;
 
    // Check context first to avoid transaction leak.
    // If put it behind tx.done CompareAndSwap statement, we can't ensure
    // the consistency between tx.done and the real COMMIT operation.
    if (atomic.LoadInt32(_addr_tx.done) == 1) {
        return error.As(ErrTxDone)!;
    }
    return error.As(tx.ctx.Err())!;
    if (!atomic.CompareAndSwapInt32(_addr_tx.done, 0, 1)) {
        return error.As(ErrTxDone)!;
    }
    tx.cancel();
    tx.closemu.Lock();
...
// the connection.
private static error rollback(this ptr<Tx> _addr_tx, bool discardConn) {
    ref Tx tx = ref _addr_tx.val;
...
// The returned statement operates within the transaction and will be closed
// when the transaction has been committed or rolled back.
private static ptr<Stmt> StmtContext(this ptr<Tx> _addr_tx, context.Context ctx, ptr<Stmt> _addr_stmt) => func((defer, _, _) => {
    ref Tx tx = ref _addr_tx.val;
...
private static (Result, error) ExecContext(this ptr<Stmt> _addr_s, context.Context ctx, params object[] args) => func((defer, _, _) => {
    Result _p0 = default;
    error _p0 = default!;
    args = args.Clone();
    ...
    for (nint i = 0; i < maxBadConnRetries + 1; i++) {
...
private static (Result, error) resultFromStatement(context.Context ctx, driver.Conn ci, ptr<driverStmt> _addr_ds, params object[] args) => func((defer, _, _) => {
    Result _p0 = default;
    error _p0 = default!;
    args = args.Clone();
    ref driverStmt ds = ref _addr_ds.val;
...
private static (ptr<Rows>, error) QueryContext(this ptr<Stmt> _addr_s, context.Context ctx, params object[] args) => func((defer, _, _) => {
    ptr<Rows> _p0 = default!;
    error _p0 = default!;
    args = args.Clone();
    ref Stmt s = ref _addr_s.val;
...
private static (driver.Rows, error) rowsiFromStatement(context.Context ctx, driver.Conn ci, ptr<driverStmt> _addr_ds, params object[] args) => func((defer, _, _) => {
...
private static error Close(this ptr<Stmt> _addr_s) => func((defer, _, _) => {
    ref Stmt s = ref _addr_s.val;
...
private static error finalClose(this ptr<Stmt> _addr_s) => func((defer, _, _) => {
    ref Stmt s = ref _addr_s.val;
...
private static (bool, bool) nextLocked(this ptr<Rows> _addr_rs) => func((defer, _, _) => {
...
// set.
private static bool NextResultSet(this ptr<Rows> _addr_rs) => func((defer, _, _) => {
    ref Rows rs = ref _addr_rs.val;
...
private static error Err(this ptr<Rows> _addr_rs) => func((defer, _, _) => {
    ref Rows rs = ref _addr_rs.val;
...
private static (slice<@string>, error) Columns(this ptr<Rows> _addr_rs) => func((defer, _, _) => {
...
private static (slice<ptr<ColumnType>>, error) ColumnTypes(this ptr<Rows> _addr_rs) => func((defer, _, _) => {
...
private static error Scan(this ptr<Rows> _addr_rs, params object[] dest) {
    ...
    foreach (var (i, sv) in rs.lastcols) {
        ...
        if (err != null) {
            return error.As(fmt.Errorf("sql: Scan error on column index %d, name %q: %w", i, rs.rowsi.Columns()[i], err))!;
        }
    }    return error.As(null!)!;
...
private static error close(this ptr<Rows> _addr_rs, error err) => func((defer, _, _) => {
    ref Rows rs = ref _addr_rs.val;
...
private static error Scan(this ptr<Row> _addr_r, params object[] dest) => func((defer, _, _) => {
    dest = dest.Clone();
...
private static (long, error) LastInsertId(this driverResult dr) => func((defer, _, _) => {
    long _p0 = default;
    error _p0 = default!;
...
private static (long, error) RowsAffected(this driverResult dr) => func((defer, _, _) => {
    long _p0 = default;
    error _p0 = default!;
...
private static @string stack() {
    array<byte> buf = new array<byte>(2 << 10);
    return string(buf[..(int)runtime.Stack(buf[..], false)]);
}

// withLock runs while holding lk.
private static void withLock(sync.Locker lk, Action fn) => func((defer, _, _) => {
    lk.Lock();
