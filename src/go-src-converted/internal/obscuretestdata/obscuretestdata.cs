...
namespace @internal
{
    public static partial class obscuretestdata_package
    {
        ...
        public static (@string, error) DecodeToTempFile(@string name) => func((defer, _, __) =>
        {
            @string path = default;
            error err = default!;
            ...
            var (tmp, err) = ioutil.TempFile("", "obscuretestdata-decoded-");
            if (err != null)
            {
                return ("", error.As(err)!);
            }
        ...
        public static (slice<byte>, error) ReadFile(@string name) => func((defer, _, __) =>
        {
