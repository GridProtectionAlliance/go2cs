...
// Original source: C:\Program Files\Go\src\internal\syscall\execenv\execenv_windows.go
...
public static (slice<@string>, error) Default(ptr<syscall.SysProcAttr> _addr_sys) => func((defer, _, _) => {
    slice<@string> env = default;
    error err = default!;
    ref syscall.SysProcAttr sys = ref _addr_sys.val;

    if (sys == null || sys.Token == 0) {
    ...
    while (true) {
        ...
        var n = (uintptr(end) - uintptr(@unsafe.Pointer(blockp))) / 2;
        if (n == 0) { 
