...
// package windows -- go2cs converted at 2020 October 08 03:32:32 UTC
...
namespace syscall
{
    public static partial class windows_package
    {
        ...
        private static var modiphlpapi = syscall.NewLazyDLL(sysdll.Add("iphlpapi.dll"));        private static var modkernel32 = syscall.NewLazyDLL(sysdll.Add("kernel32.dll"));        private static var modws2_32 = syscall.NewLazyDLL(sysdll.Add("ws2_32.dll"));        private static var modnetapi32 = syscall.NewLazyDLL(sysdll.Add("netapi32.dll"));        private static var modadvapi32 = syscall.NewLazyDLL(sysdll.Add("advapi32.dll"));        private static var moduserenv = syscall.NewLazyDLL(sysdll.Add("userenv.dll"));        private static var modpsapi = syscall.NewLazyDLL(sysdll.Add("psapi.dll"));        private static var procGetAdaptersAddresses = modiphlpapi.NewProc("GetAdaptersAddresses");        private static var procGetComputerNameExW = modkernel32.NewProc("GetComputerNameExW");        private static var procMoveFileExW = modkernel32.NewProc("MoveFileExW");        private static var procGetModuleFileNameW = modkernel32.NewProc("GetModuleFileNameW");        private static var procWSASocketW = modws2_32.NewProc("WSASocketW");        private static var procLockFileEx = modkernel32.NewProc("LockFileEx");        private static var procUnlockFileEx = modkernel32.NewProc("UnlockFileEx");        private static var procGetACP = modkernel32.NewProc("GetACP");        private static var procGetConsoleCP = modkernel32.NewProc("GetConsoleCP");        private static var procMultiByteToWideChar = modkernel32.NewProc("MultiByteToWideChar");        private static var procGetCurrentThread = modkernel32.NewProc("GetCurrentThread");        private static var procNetShareAdd = modnetapi32.NewProc("NetShareAdd");        private static var procNetShareDel = modnetapi32.NewProc("NetShareDel");        private static var procGetFinalPathNameByHandleW = modkernel32.NewProc("GetFinalPathNameByHandleW");        private static var procCreateEnvironmentBlock = moduserenv.NewProc("CreateEnvironmentBlock");        private static var procDestroyEnvironmentBlock = moduserenv.NewProc("DestroyEnvironmentBlock");        private static var procImpersonateSelf = modadvapi32.NewProc("ImpersonateSelf");        private static var procRevertToSelf = modadvapi32.NewProc("RevertToSelf");        private static var procOpenThreadToken = modadvapi32.NewProc("OpenThreadToken");        private static var procLookupPrivilegeValueW = modadvapi32.NewProc("LookupPrivilegeValueW");        private static var procAdjustTokenPrivileges = modadvapi32.NewProc("AdjustTokenPrivileges");        private static var procDuplicateTokenEx = modadvapi32.NewProc("DuplicateTokenEx");        private static var procSetTokenInformation = modadvapi32.NewProc("SetTokenInformation");        private static var procGetProfilesDirectoryW = moduserenv.NewProc("GetProfilesDirectoryW");        private static var procNetUserGetLocalGroups = modnetapi32.NewProc("NetUserGetLocalGroups");        private static var procGetProcessMemoryInfo = modpsapi.NewProc("GetProcessMemoryInfo");        private static var procGetFileInformationByHandleEx = modkernel32.NewProc("GetFileInformationByHandleEx");
        ...
        public static error GetProfilesDirectory(ptr<ushort> _addr_dir, ptr<uint> _addr_dirLen)
        {
            ...
            var (r1, _, e1) = syscall.Syscall(procGetProfilesDirectoryW.Addr(), 2L, uintptr(@unsafe.Pointer(dir)), uintptr(@unsafe.Pointer(dirLen)), 0L);
            if (r1 == 0L)
            {
                if (e1 != 0L)
                {
        ...
        public static error NetUserGetLocalGroups(ptr<ushort> _addr_serverName, ptr<ushort> _addr_userName, uint level, uint flags, ptr<ptr<byte>> _addr_buf, uint prefMaxLen, ptr<uint> _addr_entriesRead, ptr<uint> _addr_totalEntries)
        {
            ...
            var (r0, _, _) = syscall.Syscall9(procNetUserGetLocalGroups.Addr(), 8L, uintptr(@unsafe.Pointer(serverName)), uintptr(@unsafe.Pointer(userName)), uintptr(level), uintptr(flags), uintptr(@unsafe.Pointer(buf)), uintptr(prefMaxLen), uintptr(@unsafe.Pointer(entriesRead)), uintptr(@unsafe.Pointer(totalEntries)), 0L);
            if (r0 != 0L)
            {
                neterr = syscall.Errno(r0);
            }
        ...
        public static error GetFileInformationByHandleEx(syscall.Handle handle, uint @class, ptr<byte> _addr_info, uint bufsize)
        {
            ...
            var (r1, _, e1) = syscall.Syscall6(procGetFileInformationByHandleEx.Addr(), 4L, uintptr(handle), uintptr(class), uintptr(@unsafe.Pointer(info)), uintptr(bufsize), 0L, 0L);
            if (r1 == 0L)
            {
                if (e1 != 0L)
                {
