...
// an unexported field.
private static void mustBeExported(this flag f) => func((_, panic, _) => {
    if (f == 0) {
...
private static void mustBeAssignable(this flag f) => func((_, panic, _) => {
    if (f == 0) {
...
private static nint chanlen(unsafe.Pointer _p0);
private static nint maplen(unsafe.Pointer _p0);
...
public static nint numMethod(this Value v) => func((_, panic, _) => {
    if (v.typ == null) {>>MARKER:FUNCTION_maplen_BLOCK_PREFIX<<
...
public static void Set(this Value v, Value x) {
    ...
    if (v.kind() == Interface) {>>MARKER:FUNCTION_chanlen_BLOCK_PREFIX<<
        target = v.ptr;
    }
...
public static Value ValueOf(object i) {
    if (i == null) {>>MARKER:FUNCTION_unsafe_New_BLOCK_PREFIX<<
        return new Value();
    }
    escapes(i);
...
private static void escapes(object x) {
    if (dummy.b) {>>MARKER:FUNCTION_typedmemmove_BLOCK_PREFIX<<
        dummy.x = x;
    }
}
