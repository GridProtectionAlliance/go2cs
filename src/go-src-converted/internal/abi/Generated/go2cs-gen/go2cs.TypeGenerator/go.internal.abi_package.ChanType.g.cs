//---------------------------------------------------------
// <auto-generated>
//     This code was generated by a tool. Changes to this
//     file may cause incorrect behavior and will be lost
//     if the code is regenerated.
// </auto-generated>
//---------------------------------------------------------

using System;
using System.CodeDom.Compiler;
using System.Collections;
using System.Collections.Generic;
using go;

#nullable enable

namespace go.@internal;

public static partial class abi_package
{
    [GeneratedCode("go2cs-gen", "0.1.4")]
    public partial struct ChanType
    {
        // Promoted Struct References
        private readonly ж<global::go.@internal.abi_package.Type> ᏑʗType;

        // Promoted Struct Accessors
        public partial ref global::go.@internal.abi_package.Type Type => ref ᏑʗType.val;

        // Promoted Struct Field Accessors
        internal ref nuint Size_ => ref Type.Size_;
        internal ref nuint PtrBytes => ref Type.PtrBytes;
        internal ref uint Hash => ref Type.Hash;
        public ref global::go.@internal.abi_package.TFlag TFlag => ref Type.TFlag;
        internal ref byte Align_ => ref Type.Align_;
        internal ref byte FieldAlign_ => ref Type.FieldAlign_;
        public ref global::go.@internal.abi_package.ΔKind Kind_ => ref Type.Kind_;
        public ref global::System.Func<global::go.unsafe_package.Pointer, global::go.unsafe_package.Pointer, bool> Equal => ref Type.Equal;
        internal ref global::go.ж<byte> GCData => ref Type.GCData;
        public ref global::go.@internal.abi_package.NameOff Str => ref Type.Str;
        public ref global::go.@internal.abi_package.TypeOff PtrToThis => ref Type.PtrToThis;

        // Promoted Struct Field Accessor References
        internal static ref nuint ᏑSize_(ref ChanType instance) => ref instance.Type.Size_;
        internal static ref nuint ᏑPtrBytes(ref ChanType instance) => ref instance.Type.PtrBytes;
        internal static ref uint ᏑHash(ref ChanType instance) => ref instance.Type.Hash;
        public static ref global::go.@internal.abi_package.TFlag ᏑTFlag(ref ChanType instance) => ref instance.Type.TFlag;
        internal static ref byte ᏑAlign_(ref ChanType instance) => ref instance.Type.Align_;
        internal static ref byte ᏑFieldAlign_(ref ChanType instance) => ref instance.Type.FieldAlign_;
        public static ref global::go.@internal.abi_package.ΔKind ᏑKind_(ref ChanType instance) => ref instance.Type.Kind_;
        public static ref global::System.Func<global::go.unsafe_package.Pointer, global::go.unsafe_package.Pointer, bool> ᏑEqual(ref ChanType instance) => ref instance.Type.Equal;
        internal static ref global::go.ж<byte> ᏑGCData(ref ChanType instance) => ref instance.Type.GCData;
        public static ref global::go.@internal.abi_package.NameOff ᏑStr(ref ChanType instance) => ref instance.Type.Str;
        public static ref global::go.@internal.abi_package.TypeOff ᏑPtrToThis(ref ChanType instance) => ref instance.Type.PtrToThis;

        // Field References
        public static ref global::go.@internal.abi_package.Type ᏑType(ref ChanType instance) => ref instance.Type;
        public static ref global::go.ж<global::go.@internal.abi_package.Type> ᏑElem(ref ChanType instance) => ref instance.Elem;
        public static ref global::go.@internal.abi_package.ΔChanDir ᏑDir(ref ChanType instance) => ref instance.Dir;
        
        // Constructors
        public ChanType(NilType _)
        {
            ᏑʗType = new ж<global::go.@internal.abi_package.Type>(new global::go.@internal.abi_package.Type(nil));
            this.Elem = default!;
            this.Dir = default!;
        }

        public ChanType(global::go.@internal.abi_package.Type Type = default!, global::go.ж<global::go.@internal.abi_package.Type> Elem = default!, global::go.@internal.abi_package.ΔChanDir Dir = default!)
        {
            ᏑʗType = new ж<global::go.@internal.abi_package.Type>(Type);
            this.Elem = Elem;
            this.Dir = Dir;
        }
        
        // Handle comparisons between struct 'ChanType' instances
        public bool Equals(ChanType other) =>
            Type == other.Type &&
            Elem == other.Elem &&
            Dir == other.Dir;
        
        public override bool Equals(object? obj) => obj is ChanType other && Equals(other);
        
        public override int GetHashCode() => runtime.HashCode.Combine(
            Type,
            Elem,
            Dir);
        
        public static bool operator ==(ChanType left, ChanType right) => left.Equals(right);
        
        public static bool operator !=(ChanType left, ChanType right) => !(left == right);

        // Handle comparisons between 'nil' and struct 'ChanType'
        public static bool operator ==(ChanType value, NilType nil) => value.Equals(default(ChanType));

        public static bool operator !=(ChanType value, NilType nil) => !(value == nil);

        public static bool operator ==(NilType nil, ChanType value) => value == nil;

        public static bool operator !=(NilType nil, ChanType value) => value != nil;

        public static implicit operator ChanType(NilType nil) => default(ChanType);

        public override string ToString() => string.Concat("{", string.Join(" ",
        [
            Type.ToString(),
            Elem?.ToString() ?? "<nil>",
            Dir.ToString()
        ]), "}");
    }

    // Promoted Struct Receivers
    public static go.@internal.abi_package.ΔKind Kind(this ref ChanType target) => target.Type.Kind();
    public static go.@internal.abi_package.ΔKind Kind(this ж<ChanType> Ꮡtarget)
    {
        ref var target = ref Ꮡtarget.val;
        return target.Kind();
    }
    internal static bool HasName(this ref ChanType target) => target.Type.HasName();
    internal static bool HasName(this ж<ChanType> Ꮡtarget)
    {
        ref var target = ref Ꮡtarget.val;
        return target.HasName();
    }
    internal static bool Pointers(this ref ChanType target) => target.Type.Pointers();
    internal static bool Pointers(this ж<ChanType> Ꮡtarget)
    {
        ref var target = ref Ꮡtarget.val;
        return target.Pointers();
    }
    internal static bool IfaceIndir(this ref ChanType target) => target.Type.IfaceIndir();
    internal static bool IfaceIndir(this ж<ChanType> Ꮡtarget)
    {
        ref var target = ref Ꮡtarget.val;
        return target.IfaceIndir();
    }
    internal static bool IsDirectIface(this ref ChanType target) => target.Type.IsDirectIface();
    internal static bool IsDirectIface(this ж<ChanType> Ꮡtarget)
    {
        ref var target = ref Ꮡtarget.val;
        return target.IsDirectIface();
    }
    internal static go.slice<byte> GcSlice(this ref ChanType target, nuint begin, nuint end) => target.Type.GcSlice(begin, end);
    internal static go.slice<byte> GcSlice(this ж<ChanType> Ꮡtarget, nuint begin, nuint end)
    {
        ref var target = ref Ꮡtarget.val;
        return target.GcSlice(begin, end);
    }
    internal static nint Len(this ref ChanType target) => target.Type.Len();
    internal static nint Len(this ж<ChanType> Ꮡtarget)
    {
        ref var target = ref Ꮡtarget.val;
        return target.Len();
    }
    public static go.ж<go.@internal.abi_package.Type> Common(this ref ChanType target) => target.Type.Common();
    public static go.ж<go.@internal.abi_package.Type> Common(this ж<ChanType> Ꮡtarget)
    {
        ref var target = ref Ꮡtarget.val;
        return target.Common();
    }
    public static go.@internal.abi_package.ΔChanDir ChanDir(this ref ChanType target) => target.Type.ChanDir();
    public static go.@internal.abi_package.ΔChanDir ChanDir(this ж<ChanType> Ꮡtarget)
    {
        ref var target = ref Ꮡtarget.val;
        return target.ChanDir();
    }
    public static go.ж<go.@internal.abi_package.UncommonType> Uncommon(this ref ChanType target) => target.Type.Uncommon();
    public static go.ж<go.@internal.abi_package.UncommonType> Uncommon(this ж<ChanType> Ꮡtarget)
    {
        ref var target = ref Ꮡtarget.val;
        return target.Uncommon();
    }
    public static go.ж<go.@internal.abi_package.Type> Elem(this ref ChanType target) => target.Type.Elem();
    public static go.ж<go.@internal.abi_package.Type> Elem(this ж<ChanType> Ꮡtarget)
    {
        ref var target = ref Ꮡtarget.val;
        return target.Elem();
    }
    public static go.ж<go.@internal.abi_package.ΔStructType> StructType(this ref ChanType target) => target.Type.StructType();
    public static go.ж<go.@internal.abi_package.ΔStructType> StructType(this ж<ChanType> Ꮡtarget)
    {
        ref var target = ref Ꮡtarget.val;
        return target.StructType();
    }
    public static go.ж<go.@internal.abi_package.ΔMapType> MapType(this ref ChanType target) => target.Type.MapType();
    public static go.ж<go.@internal.abi_package.ΔMapType> MapType(this ж<ChanType> Ꮡtarget)
    {
        ref var target = ref Ꮡtarget.val;
        return target.MapType();
    }
    public static go.ж<go.@internal.abi_package.ΔArrayType> ArrayType(this ref ChanType target) => target.Type.ArrayType();
    public static go.ж<go.@internal.abi_package.ΔArrayType> ArrayType(this ж<ChanType> Ꮡtarget)
    {
        ref var target = ref Ꮡtarget.val;
        return target.ArrayType();
    }
    public static go.ж<go.@internal.abi_package.ΔFuncType> FuncType(this ref ChanType target) => target.Type.FuncType();
    public static go.ж<go.@internal.abi_package.ΔFuncType> FuncType(this ж<ChanType> Ꮡtarget)
    {
        ref var target = ref Ꮡtarget.val;
        return target.FuncType();
    }
    public static go.ж<go.@internal.abi_package.ΔInterfaceType> InterfaceType(this ref ChanType target) => target.Type.InterfaceType();
    public static go.ж<go.@internal.abi_package.ΔInterfaceType> InterfaceType(this ж<ChanType> Ꮡtarget)
    {
        ref var target = ref Ꮡtarget.val;
        return target.InterfaceType();
    }
    internal static nuint Size(this ref ChanType target) => target.Type.Size();
    internal static nuint Size(this ж<ChanType> Ꮡtarget)
    {
        ref var target = ref Ꮡtarget.val;
        return target.Size();
    }
    internal static nint Align(this ref ChanType target) => target.Type.Align();
    internal static nint Align(this ж<ChanType> Ꮡtarget)
    {
        ref var target = ref Ꮡtarget.val;
        return target.Align();
    }
    internal static nint FieldAlign(this ref ChanType target) => target.Type.FieldAlign();
    internal static nint FieldAlign(this ж<ChanType> Ꮡtarget)
    {
        ref var target = ref Ꮡtarget.val;
        return target.FieldAlign();
    }
    public static go.slice<go.@internal.abi_package.Method> ExportedMethods(this ref ChanType target) => target.Type.ExportedMethods();
    public static go.slice<go.@internal.abi_package.Method> ExportedMethods(this ж<ChanType> Ꮡtarget)
    {
        ref var target = ref Ꮡtarget.val;
        return target.ExportedMethods();
    }
    internal static nint NumMethod(this ref ChanType target) => target.Type.NumMethod();
    internal static nint NumMethod(this ж<ChanType> Ꮡtarget)
    {
        ref var target = ref Ꮡtarget.val;
        return target.NumMethod();
    }
    public static go.ж<go.@internal.abi_package.Type> Key(this ref ChanType target) => target.Type.Key();
    public static go.ж<go.@internal.abi_package.Type> Key(this ж<ChanType> Ꮡtarget)
    {
        ref var target = ref Ꮡtarget.val;
        return target.Key();
    }
}
