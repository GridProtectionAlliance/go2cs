...
namespace @internal
{
    public static partial class xcoff_package
    {
        ...
        {
            public long Length;
            public long StorageMappingClass;
            public long SymbolType;
        }
        ...
        private static ptr<Section> Section(this ptr<File> _addr_f, @string name)
        {
            ...
            {
                if (s.Name == name || (len(name) > 8L && s.Name == name[..8L]))
                {
                    return _addr_s!;
                }
        ...
        private static (@string, bool) getString(slice<byte> st, uint offset)
        {
            ...
            if (offset < 4L || int(offset) >= len(st))
            {
                return ("", false);
            }
        ...
        public static (ptr<File>, error) NewFile(io.ReaderAt r)
        {
            ...
            // Read XCOFF file header
            {
                ...
                var (_, err) = sr.Seek(0L, os.SEEK_SET);
            ...
            // Symbol map needed by relocation
            var idxToSym = make_map<long, ptr<Symbol>>(); 
            ...
            f.Symbols = make_slice<ptr<Symbol>>(0L);
            {
                long i__prev1 = i;

                for (i = 0L; i < int(nsyms); i++)
                {
                    long numaux = default;
                    ...
                    if (f.TargetMachine == U802TOCMAGIC) 
                        ...
                        needAuxFcn = se.Ntype & SYM_TYPE_FUNC != 0L && numaux > 1L;
                        var zeroes = binary.BigEndian.Uint32(se.Nname[..4L]);
                        if (zeroes != 0L)
                        {
                        ...
                        {
                            offset = binary.BigEndian.Uint32(se.Nname[4L..]);
                    ...
                    if (numaux < 1L || i + numaux >= int(nsyms))
                    {
                        goto skip;
                    }
                    ...
                    if (sym.SectionNumber == 0L)
                    {
                        sym.Value = 0L;
                    }
                    else
                    {
                        sym.Value -= f.Sections[sym.SectionNumber - 1L].VirtualAddress;
                    }
                    ...
                    {
                        {
                            ...
                            (_, err) = sr.Seek(int64(numaux - 1L) * SYMESZ, os.SEEK_CUR);
                    ...
                    if (f.TargetMachine == U802TOCMAGIC) 
                        ...
                        sym.AuxCSect.SymbolType = int(aux.Xsmtyp & 0x7UL);
                        ...
                        sym.AuxCSect.SymbolType = int(aux.Xsmtyp & 0x7UL);
                        sym.AuxCSect.StorageMappingClass = int(aux.Xsmclas);
                        sym.AuxCSect.Length = int64(aux.Xscnlenhi) << (int)(32L) | int64(aux.Xscnlenlo);
...
skip: // Skip auxiliary entries
            ...
            foreach (var (_, sect) in f.Sections)
            {
...

                {
                    ...
                    for (i = uint32(0L); i < sect.Nreloc; i++)
                    {
                        ...
                        if (f.TargetMachine == U802TOCMAGIC) 
                            ...
                            sect.Relocs[i].Length = rel.Rsize & 0x3FUL + 1L;

                            if (rel.Rsize & 0x80UL != 0L)
                            {
                                sect.Relocs[i].Signed = true;
                            }

                            if (rel.Rsize & 0x40UL != 0L)
                            {
        ...
        private static (long, error) ReadAt(this zeroReaderAt w, slice<byte> p, long off)
        {
            long n = default;
            error err = default!;
        ...
        private static (slice<byte>, error) Data(this ptr<Section> _addr_s)
        {
            ...
            var (n, err) = s.sr.ReadAt(dat, 0L);
            if (n == len(dat))
            {
                err = null;
            }

            return (dat[..n], error.As(err)!);
        ...
        private static (slice<@string>, error) readImportIDs(this ptr<File> _addr_f, ptr<Section> _addr_s)
        {
            ...
            // Read loader header
            {
                ...
                var (_, err) = s.sr.Seek(0L, os.SEEK_SET);
            ...
            var libpath = cstring(table[offset..]);
            f.LibraryPaths = strings.Split(libpath, ":");
            offset += len(libpath) + 3L; // 3 null bytes
            var all = make_slice<@string>(0L);
            for (long i = 1L; i < int(nimpid); i++)
            {
                var impidpath = cstring(table[offset..]);
                offset += len(impidpath) + 1L;
                var impidbase = cstring(table[offset..]);
                offset += len(impidbase) + 1L;
                var impidmem = cstring(table[offset..]);
                offset += len(impidmem) + 1L;
                @string path = default;
                if (len(impidpath) > 0L)
                {
        ...
        private static (slice<ImportedSymbol>, error) ImportedSymbols(this ptr<File> _addr_f)
        {
            ...
            var all = make_slice<ImportedSymbol>(0L);
            for (long i = 0L; i < int(nsyms); i++)
            {
                @string name = default;
                int ifile = default;
                bool ok = default;
                ...
                if (f.TargetMachine == U802TOCMAGIC) 
                    ...
                    if (ldsym.Lsmtype & 0x40UL == 0L)
                    {
                        continue; // Imported symbols only
                    }

                    var zeroes = binary.BigEndian.Uint32(ldsym.Lname[..4L]);
                    if (zeroes != 0L)
                    {
                    ...
                    {
                        var offset = binary.BigEndian.Uint32(ldsym.Lname[4L..]);
                ...
                sym.Name = name;
                if (ifile >= 1L && int(ifile) <= len(libs))
                {
                    sym.Library = libs[ifile - 1L];
                }
