...
// ensure that this error is sticky.
        ...
        {
            return ref new Reader(r:r,curr:&regFileReader{r,0});
        }
        ...
        private static (ref Header, error) Next(this ref Reader tr)
        {
            if (tr.err != null)
            {
                return (null, tr.err);
...
loop:
            ...
            { 
                ...
                if (hdr.Typeflag == TypeXHeader || hdr.Typeflag == TypeXGlobalHeader) 
                    ...
                    if (hdr.Typeflag == TypeXGlobalHeader)
                    {
                        ...
                        return (ref new Header(Name:hdr.Name,Typeflag:hdr.Typeflag,Xattrs:hdr.Xattrs,PAXRecords:hdr.PAXRecords,Format:format,), null);
                    }
                    _continueloop = true; // This is a meta header affecting the next header
                    break;
                    ...
                    hdr.Format = format;
                    return (hdr, null); // This is a file, so stop
                            }
        }
        ...
        private static error handleRegularFile(this ref Reader tr, ref Header hdr)
        {
            var nb = hdr.Size;
            ...
            tr.curr = ref new regFileReader(r:tr.r,nb:nb);
            return error.As(null);
        }

        // handleSparseFile checks if the current file is a sparse format of any type
        // and sets the curr reader appropriately.
        private static error handleSparseFile(this ref Reader tr, ref Header hdr, ref block rawHdr)
        {
            ...
            if (err == null && spd != null)
            {
                ...
                tr.curr = ref new sparseFileReader(tr.curr,sph,0);
            }
            return error.As(err);
        }
        ...
        private static (sparseDatas, error) readGNUSparsePAXHeaders(this ref Reader tr, ref Header hdr)
        { 
            ...
            if (major == "0" && (minor == "0" || minor == "1")) 
                ...
                return (null, null); // Unknown GNU sparse PAX version
            else if (hdr.PAXRecords[paxGNUSparseMap] != "") 
                is1x0 = false; // 0.0 and 0.1 did not have explicit version records, so guess
            else 
                return (null, null); // Not a PAX format GNU sparse file.
        ...
        private static error mergePAX(ref Header hdr, map<@string, @string> paxHdrs)
        {
        ...
        private static (ref Header, ref block, error) readHeader(this ref Reader tr)
        { 
            // Two blocks of zero bytes marks the end of the archive.
            {
                ...
                if (err != null)
                {
                    return (null, null, err); // EOF is okay here; exactly 0 bytes read
                }
            ...
            {
                {
                    ...
                    if (err != null)
                    {
                        return (null, null, err); // EOF is okay here; exactly 1 block of zeros read
                    }

                }
                if (bytes.Equal(tr.blk[..], zeroBlock[..]))
                {
                    return (null, null, io.EOF); // normal EOF; exactly 2 block of zeros read
                }
                return (null, null, ErrHeader); // Zero block and then non-zero block
            } 
            ...
            if (format > formatV7)
            {
                ...
                if (format.has(FormatUSTAR | FormatPAX)) 
                    ...
                    Func<int, bool> notASCII = r => r >= 0x80UL;
                    if (bytes.IndexFunc(tr.blk[..], notASCII) >= 0L)
                    {
                        hdr.Format = FormatUnknown; // Non-ASCII characters in block.
                    }
                    Func<slice<byte>, bool> nul = b => int(b[len(b) - 1L]) == 0L;
        ...
        private static (sparseDatas, error) readOldGNUSparseMap(this ref Reader tr, ref Header hdr, ref block blk)
        { 
...
; 
        ...
        private static (long, error) Read(this ref Reader tr, slice<byte> b)
        {
            if (tr.err != null)
            {
                return (0L, tr.err);
        ...
        private static (long, error) writeTo(this ref Reader tr, io.Writer w)
        {
            if (tr.err != null)
            {
                return (0L, tr.err);
        ...
        private static (long, error) Read(this ref regFileReader fr, slice<byte> b)
        {
        ...
        private static (long, error) WriteTo(this ref regFileReader fr, io.Writer w)
        {
        ...
        private static (long, error) Read(this ref sparseFileReader sr, slice<byte> b)
        {
        ...
        private static (long, error) WriteTo(this ref sparseFileReader sr, io.Writer w)
        {
