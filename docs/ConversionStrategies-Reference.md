...
literal falls back to the heap `slice<T>`. A variadic function using the defer/recover execution wrapper
also forces the heap form (`variadicHoistMode`): the wrapper lambda must capture the rebound local, and C#
forbids capturing a ref struct. Function literals use the same eligibility map and conservatism. Golib's
`cap<T>(in sslice<T>)` complements the existing `len` overload; `append` deliberately has no sslice grow
path, so any append remains a heap case. (Guarded by `VariadicPointerParam`: safe range/len/cap/index sites
emit `.sslice()`, a spread element replacement is observed through the caller's slice against `go run`,
and defer-wrapper plus nested-closure cases pin `.slice()` fallbacks that would not compile as ref structs.)
