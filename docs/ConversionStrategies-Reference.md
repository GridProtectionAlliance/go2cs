...
The model is **abstract, not a unicode patch**: a 2026-07-24 GOROOT scan counts **44 black-box-only stdlib packages** (unicode, unicode/utf8, path, cmp, errors, slices, io/fs, iter, embed, sync/atomic, text/tabwriter, runtime/metrics, internal/itoa, internal/testenv, …), each of which selects the reference model automatically. Already-validated black-box packages (unicode/utf8, path, cmp, internal/itoa) switch models on their next pipeline run and re-validate identically. Known limitation, by design: the test project's references are still exactly the direct-import set plus the alias scan (B2c) — if a production API *signature* surfaces a type from an assembly the test files never import or alias, the compile fails loudly (CS0012) rather than silently widening the reference set, because extra references contribute child namespaces that break the alias machinery (CS0576). Guarded by the `TestSelectTestProjectModelGatesOnBlackBoxOnly`, `TestRecordsRequireProductionAnchorGatesReferenceModel`, `TestWriteTestProjectReferenceModelBindsProductionProject`, and `TestReferenceModelSeedAnchorsTestClassOnly` converter unit tests; unicode (28 tests, incl. `TestSpecialCaseNoMapping` and the `testing.Benchmark`-driven `TestCalibrate`) is the first package it validates.
