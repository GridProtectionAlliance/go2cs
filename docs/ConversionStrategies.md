...
Still open in this family: the **untyped-constant → type-parameter conversion** `E(100)` emits `(E)100` (CS0030 — needs a golib-routed conversion), and **range-over-func on a generic Seq** (`for v := range seq` where seq is `iter.Seq[E]` — the last slices error).
