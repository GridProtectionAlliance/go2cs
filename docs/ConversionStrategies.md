...
Still open in this family (recorded for the campaign): the **untyped-constant → type-parameter conversion** `E(100)` emits `(E)100` (CS0030 — needs a golib-routed conversion), and the **S-preserving sub-slice** family (`s[i:j]` on a constrained `S` yields `ISlice<E>`, losing `S` — CS0266/CS0411 in pdqsort's recursion; candidate design: `ISupportMake<S>` reconstruction).
