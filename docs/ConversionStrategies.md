...
Still open in this family (recorded for the campaign): the **untyped-constant → type-parameter conversion** `E(100)` emits `(E)100` (CS0030 — needs a golib-routed conversion), and passing a constrained `S` where `[]E` is expected (Go assignability — the `rotateRight(s[m:i], …)` family).
