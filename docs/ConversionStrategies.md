...
**The `@unsafe` alias is emitted whenever a file references the `unsafe.Pointer` type, even without a usable `unsafe` import.** The type renders as `@unsafe.Pointer`, which resolves only through the file-local alias `using @unsafe = unsafe_package;`. That alias is normally generated from a *named* `import "unsafe"`, but a file can reference the type with no such import: through **type inference** (`fd := funcdata(f, i)`, where `funcdata` returns `unsafe.Pointer` — the file never writes `unsafe.` textually, so it need not import the package) or a **blank import** (`_ "unsafe"`, which binds the C# alias to `_`, not `@unsafe`). Both previously yielded CS0246 (runtime `preempt.go`, `symtabinl.go`). The converter now flags any emitted `unsafe.Pointer` type (`getTypeName`, on the `*types.Basic` of kind `UnsafePointer`) and, at file close, supplies `using @unsafe = unsafe_package;` when it was referenced but not already aliased by a named import — idempotent-safe, so a normal named import emits it exactly once. (Guarded by `UnsafePointerInferredNoImport` — a consumer file that infers an `unsafe.Pointer` local while importing only `fmt`, plus a blank-import (`_ "unsafe"`) variant.)
